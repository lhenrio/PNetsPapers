...
An example of opM$\mu$ is the formula for open automaton in Sec.~\ref{Sec2:usecase}.  The formula can be expressed as follows:
\begin{equation}\label{formula3}
	\nu X(\langle fail?M\rangle\mu Y(M/t)(\langle resume\rangle X \vee \langle tick\rangle Y(t-1/t)\vee \langle ask\wedge reset\rangle true))
\end{equation}
First, a $fail$ occurs with receiving a value $M$, and then a $tick$ happens with decreasing $t$ by one. Or the system returns true after an $ask$ and the subsequent $reset$ occur. Otherwise, the system restarts after a $resume$ happens.
