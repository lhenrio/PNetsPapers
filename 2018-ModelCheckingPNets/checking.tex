...
\begin{abstract}
We present open pNets (open parameterised networks of processes) to describe a composition of concurrent systems. In order to verify the open pNets, in the paper, we present a new property language named opM$\mu$ (open parameterised modal $\mu$-calculus).

\keywords{ open pNets\quad opM$\mu$\quad pMG \quad  }
\end{abstract}
...
%\end{array}\]
\item
The set of leaves of a pNet is the set of all pLTSs occurring in the structure, as an indexed family of the form $\Leaves(\pNet)= \mylangle \pNet_i \myrangle^{i \in L}$. is said to be \emph{open}.
\end{itemize}
A pNet $Q$ is \emph{closed} if it has no hole: $\Holes(Q)=\emptyset$; else it is said to be \emph{open}.
\end{definition}
...
This definition is not constructive but it is easy to build the predicate constructively by brute-force unification of the sub-pNets actions with the corresponding vector actions, possibly followed by a simplification step.
 ...
 A transition in a pLTS is consisted as a tuple with labelling $guard$, $parameterised\; action$, $assignment$ between two states. Formally, a transition is defined as $s\xrightarrow{\langle\alpha, e_{b}, (x_{j}:=e_{j})^{j\in J}\rangle}s'$. While an open transition in an open automaton is defined with labelling $Pred$ and $Post$ between a composition of states in pLTSs and holes. In the open transition, $Pred$ is a predicate over the different variables of the terms, labels, and states. Specifically, a predicate consists of a set of parameterised actions in pLTSs and actions in holes. \NOTE{In a pNet, some ports from each component have an immediate relationship, and these ports are connected each other. Also, one of these ports is treated as a action to trigger a transition in the pLTS or in a hole. A predicate is a synchronisation vector from the action in each pLTS and each hole.}


\subsection{opM$\mu$}

 The property language is called open parameterised modal $\mu$-calculus (opM$\mu$). We give the syntax in Tab.~\ref{opmu}:
...
\begin{table}
	...
	\begin{tabular}{>{\bfseries}c| c } 
		\hline 
	Action expression & $e ::= \underline{c} \mid \{c(x)\mid x\in X\} \mid \{\beta_{j}\mid j\in J\}$\\ \hline
	Action formula & $\alpha ::= e \mid \neg\alpha\ \mid\ \alpha_{1}\vee\alpha_{2}\ \mid\ \alpha_{1}\wedge\alpha_{2}$   \\ 	\hline 	
		
	 State formula & $\varphi ::= \top \mid \bot \mid e_1 = e_2 \mid \neg\varphi \mid \varphi_{1}\wedge\varphi_{2}\ \mid \varphi_{1}\vee\varphi_{2} \mid [\alpha]\varphi$\\
	& $\mid \langle\alpha\rangle\varphi \mid \exists x_{i}\varphi \mid \forall x_{i}\varphi \mid \mu X(e_{i}/x_{i})\varphi \mid \nu X(e_{i}/x_{i})\varphi$	\\ \hline 
	\end{tabular} 
\end{table} 


An opM$\mu$ is defined as a property language that is used to describe the concurrent systems. $\alpha$ is an action formula. $\tau$ is a silent action and $c$ is an action name. $c(x)$ is a parameterised action from holes. $\underline{c}$ is a global action from synchronisation vector, where $h_B$ is an action from a hole. 

We propose a formula with opM$\mu$ based on the Open Automaton of the Failure Monitor architecture in Sec.~\ref{Sec2:usecase}. The formula can be expressed as follows:
\begin{equation}\label{formula3}
	\langle \underline{fail}\rangle \mu X(t)(\langle ask\rangle\langle reset\rangle \top \vee \langle tick\rangle X(t-1))
\end{equation}

If a $fail$ occurs with a hole action $h_B=fail(true)$, and then a $ask$ happens followed by a $reset$ action after $tick$ happens with decreasing $t$ by one. The formula can be employed to express a liveness property that an action $reset$ eventually happens.

Another example for the property of the open automaton of the Failure Monitor architecture is shown as follows:
\begin{equation}\label{formula4}
	\nu Y. \langle \underline{fail}\rangle \mu X(t).\langle ask\rangle\langle reset\rangle \top \vee (\langle \underline{resume}\rangle Y \vee \langle tick\rangle X(t-1))
\end{equation}

On all the path, a $\underline{fail}[h_B=fail(true)]\rangle$ happens, and then a $ask$ happens followed by a $reset$ action after $tick$ happens with decreasing $t$ by one, until $\langle \underline{resume}[h_B=resume(true)]\rangle Y$ holds after $M$ times $tick$ happens with decreasing $t$ by one.  The formula can be employed to express that an action $reset$ always eventually happens.
...
\centering
	...
	\node[noname] (4) [node distance=0.5cm and 2cm, above right=of 3]    {$t_{4}$};
	\node[noname] (5) [right=of 4]                                					{$t_{5}$};
	\node[noname] (6) [right=of 5]                                					{$t_{6}$};
	\node[noname] (7) [node distance=0.4cm and 1.8cm, below right=of 3]     {$t_{7}$};
...
where:
\begin{itemize}
	\item $L_{N}(t_{1}) = (\{\}, \langle fail[h_B=fail(true)]\rangle, 1)$
	\item $L_{N}(t_{2}) = (\{t\}, t := M, 1)$
	\item $L_{N}(t_{3}) = (\{t\}, \vee, 1)$
	\item $L_{N}(t_{4}) = (\{t\}, \langle ask\rangle, 1)$
	\item $L_{N}(t_{5}) = (\{t\}, \langle reset\rangle, 1)$
	\item $L_{N}(t_{6}) = (\{\}, true, 1)$
	\item $L_{N}(t_{7}) = (\{t\}, \langle tick\rangle, 1)$
\end{itemize}


\subsection{The modified algorithm}

An open automaton is unfolded by a pNet with some pLTSs and holes. An open transition a defined as a hole action, a predicate, a postcondition between a composition of states from different pLTSs and holes. 

What we have to deal with in the algorithm is the $hole$ and $predicate$. \TODO{The detail}


\subsubsection{Processing Holes}
We change the algorithm to make it suitable for checking open pNets. From the syntax of opM$\mu$, we define the global action including holes in the action as $\underline{c}(h_B = c(x))$. We change the modality to $\langle c\rangle$, $[c]$, $\langle \underline{c}(h_B = c(x))\rangle$, $[\underline{c}(h_B = c(x))]$.
It was shown as follows:

		
$\langle c\rangle\Rightarrow$ $checkOr(\{ (p_{i}, (n',\rho))\,|, p \xrightarrow{c} p_{i}, n \rightarrow n'\})$

$[c]\Rightarrow$ $checkAnd(\{ (p_{i}, (n', \rho))\,|\, p \xrightarrow{c} p_{i}, n\rightarrow n' \})$

		
$\langle \underline{c}(h_B = c(x))\rangle\Rightarrow$
	$ checkAnd(\{ (p_{i}[v/y], (n',\rho\{x \rightarrow v\}))\,|\,  n \rightarrow n', p \xrightarrow{\langle \underline{c}(h_B = c(x))\rangle } p_{i},v\in Val\})$
		
{$[\underline{c}(h_B = c(x))]\Rightarrow$}
	 $ checkOr(\{ (p_{i}[v/y], (n', \rho\{x \rightarrow v\}))\,|\,  n \rightarrow n', p \xrightarrow{[\underline{c}(h_B = c(x))]} p_{i},v\in Val\})$
...
\begin{thebibliography}{4}
	\bibitem{xudong2017smt}Xudong Qin, Simon Bliudze, Eric Madelaine, Min Zhang. Using SMT engine to generate Symbolic Automata -Extended version. [Research Report] RR-9177, Inria \& Universit� Cote d'Azur, CNRS, I3S, Sophia Antipolis, France; inria. 2018. 	
	...
	\bibitem{henrio2016pnet}Henrio, Ludovic, Eric Madelaine, and Min Zhang. "A theory for the composition of concurrent processes." International Conference on Formal Techniques for Distributed Objects, Components, and Systems. Springer, Cham, 2016.
\end{thebibliography}
