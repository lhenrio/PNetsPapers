...
However with that solution we cannot remove holes so we cannot express composition corectness, hence the need of the special composition correctness for hole-superset simulation.
...
The first is in the state \(1S\), which corresponds to the clock trying to imppose a tick when the counter is not set.
