...
\begin{multline*}
	...
	\mpar{\everymath{\displaystyle}\begin{array}{c}
		\bigsymb{\exists} \mpar{\OTx{1}{}{1}{1}, \OTx{2}{}{2}{2}} \in T_1 \times T_2, \\
		\sigma \vdash g_1 \wedge g_2 \wedge \alpha_1 = \alpha_2 \wedge \bigwedge_\subbox{j \in J'_1 \cap J'_2} \beta_{1j} = \beta_{2j} \wedge R\mpar{s'_1, s'_2}\psubst{\psi_1 \uplus \psi_2}
	\end{array}} \\
	\vee \forall t_2 \in \fOT{s_2}, \sigma \nvdash \fguard{t_2}
\end{multline*}
\end{defi}
...
% Decide later to modify or remove it
...
\defitem \nmm{\bigsymb{\forall} \OTg \in T, \vdash \reach{A}\mpar{s} \wedge g \implies \reach{A}\mpar{s'}\psubst{\psi}}.
\end{defi}
...
The composition resulting in \(A_p\subst{A_c}{k} \defobject \OA{S}{s_0}{V}{\sigma_0}{J}{T}\) with \(A_c \defobject \OA{S_c}{s_{0c}}{V_c}{\sigma_{0c}}{J_c}{T_c}\), \(A_p \defobject \OA{S_p}{s_{0p}}{V_p}{\sigma_{0p}}{J_p}{T_p}\) and \(k \in J_p\), is a non-locking composition if:
...
\begin{multline*}
	...
	\mpar{\exists t \in T, \sigma \vdash \fguard{t}} \vee \forall t_p \in \fOT{\pi_{S_p}\mpar{s}}, \sigma \nvdash \fguard{t_p}
\end{multline*}
\end{defi}
...
A simulation \(\leq\) is deadlock reducing if in all the \(R: S_1 \times S_2 \to \rformulae[V_1 \uplus V_2]\) such that \(\wrel{A_1}{A_2}{R}\), there is one that also satisfies
\[ \forall \mpar{s_1, s_2} \in S_1 \times S_2, \sigma: V_1 \uplus V_2 \to \values, \sigma \vdash R\mpar{s_1, s_2} \wedge \bigvee_\subbox{t_2 \in \fOT{s_2}} \fguard{t_2} \implies \bigvee_\subbox{t_1 \in \fOT{s_1}} \fguard{t_1} \]
\end{defi}
\begin{lem}
The intuitive and working
\begin{proof} % Always introduce hypothesis and goals on a big math env, recall definitions (also simulation) and call hypothesis H1, H2, ...
