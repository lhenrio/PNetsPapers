...
\defitem The terms restricted to variables in \(V\) are \(\rterms{V} \defnotation \mset{t \in \terms}{\fvars{t} = \emptyset}\); \(\rterms{V} \subset \terms\).
\defitem The formulae restricted to variables in \(V\) are \(\rformulae{V} \defnotation \mset{f \in \formulae}{\fvars{f} = \emptyset}\); \(\rformulae{V} \subset \terms\).
\defitem The closed terms are terms restricted to variables in \(\emptyset\), \(\cterms\).
\defitem The closed formulae are formulae restricted to variables in \(\emptyset\), \(\cformulae\).
\end{defi}
...
The substitutions might give a nonsensical expression; for instance let the terms be integers and pairs with (pointwise) addition, \(\mpar{a + b}\psubst{a \mapsto 7, b \mapsto \mpar{4, 5}}\) is not a valid term.
...
\begin{align*}
	...
	\mathit{target}\mpar{\OTg} & \defnotation s' \\
	\mathit{Post}\mpar{\OTg} & \defnotation \psi &
...
A relation \(\leq\) is a simulation iff for any two related automata \(\OA{S_1}{i_1}{J_1}{V_1}{\varphi_1}{T_1} \leq \OA{S_2}{i_2}{J_2}{V_2}{\varphi_2}{T_2}\), there is a relation under predicate between their states \(R: S_1 \times S_2 \to \rformulae{V_1 \uplus V_2}\) that satisfies both
...
\begin{defi}[Deadlock reduction]
A simulation \(\leq\) is deadlock reducing if for all related automata \(A_1 \leq A_2\), where \(A_1 \defobject \OA{S_1}{i_1}{V_1}{\varphi_1}{J_1}{T_1}\) and \(A_2 \defobject \OA{S_2}{i_2}{V_2}{\varphi_2}{J_2}{T_2}\), there is a witness \(R: S_1 \times S_2 \to \rformulae{V_1 \uplus V_2}\) of the fact that it is a simulation that also satisfies
\begin{multline*}
	\forall \mpar{s_1, s_2} \in S_1 \times S_2, \sigma: V_1 \uplus V_2 \to \values, \\
...
A way to solve this issue is to caracterise a correct composition that will be used instead of the composition introduced earlier.
\begin{defi}[Correct composition]
The composition \(A_p\subst{A_c}{k} \defobject \OA{S}{i}{V}{\varphi}{J}{T}\) where \(A_c \defobject \OA{S_c}{i_c}{V_c}{\varphi_c}{J_c}{T_c}\), \(k \in J_p\) and \(A_p \defobject \OA{S_p}{i_p}{V_p}{\varphi_p}{J_p}{T_p}\), is a correct composition if there is a predicate on its states \(p: S \to \rformulae{V}\) that satisfies both
...
\[ \forall s \in S, \sigma: V \to \values, \mpar{\exists t_p \in \fOT{\pi_{S_p}\mpar{s}}, \sigma \vdash p\mpar{s} \wedge \fguard{t_p}} \implies \bigsymb{\exists} \OTg \in T, \sigma \vdash g \wedge p\mpar{s'}\psubst{\psi} \]
\end{defi}
...
\item[\(\impliedby\):] If \(R\) satisfies the equivalent formula, let \(\mpar{s_1, s_2} \in S_1 \times S_2\) and \(\sigma: V_1 \uplus V_2 \to \values\).
	...
	The left part of the implication is the left side of \hyp{g2} so we get \(t'_2 \in T_2\) and what's after as the hypothesis \hyp{t}.
	The witnesses for \goal{G1} are \(t'_2\) and \(t_1\), and the rest is proved with a combination of \hyp{t} and \hyp{g1}.
\end{proof}

\begin{lem}
The second part of the formula in the definition of correct composition is equivalent to the following:
\[ \forall s \in S, \vdash p\mpar{s} \wedge \bigvee_\subbox{t_p \in \fOT{\pi_{S_p}\mpar{s}}} \fguard{t_p} \implies \bigvee_\subbox{t \in \fOT{s}} \mpar{\fguard{t} \wedge p\mpar{\mathit{target}\mpar{t}}\psubst{\mathit{Post}\mpar{t}}} \]
\end{lem}
\begin{proof}
% TODO HERE
\end{proof}
...
\Quentin{``\(R\) is a hole-identical simulation of \(A_1\) in \(A_2\)"}{Any idea on how to note that? \(R \vDash A_1 \leq_= A_2\) for instance.} is defined as:
\begin{multline*}
