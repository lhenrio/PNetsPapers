...
This implies that there is no relation between their holes (\(J \setminus \mset{k_i}{i \in \mdbrk{1; n}} \bigcup_{i \in \mdbrk{1; n}} J_i\) is neither subset nor superset of \(J_p\)).
...
\begin{multline*}
	\forall \mpar{s_p, s_c} \in S, \forall \sigma: V \to \values, \mpar{\sigma \vdash \reach{A_p\subst{A_c}{k}}\mpar{s}} \implies \\
...
In the parallel case, the condition is checked after the parallel composition because there is no guarantee that any order of sequential composition is non-locking, even if the parallel composition is non-locking.
...
Suppose that actually the network manager is reliable and the integrity check is only used to generate a fingerprint.
Someone may in that case eliminate the blue transitions as an optimization.
Once the blue transition eliminated, we can debranch the network manager and the file integrity check, but always at the same time.
The composition with a deadlock automaton in both holes (debranching) is non-locking, but any individual composition is locking.
\end{exi}
...
\section{Proof of equivalent definitions}\label{apx:lemeqd} % TODO: fix the proofs about the switch \vee -> \implies
