...
% TODO: distinguish composition from "parallel composition (pure interleaving)"
...
This implies that there is no relation between their holes, for instance composing \(A_i, 1 \leq i \leq n\) in \(A_0\) gives the holes \(J \defobject \mpar{\biguplus_{0 \leq i \leq n} J_i} \setminus \mset{k_i}{1 \leq i \leq n} \) that is neither subset nor superset of \(J_0\).
...
Deadlocks are configurations reachable in a run and from which no further transition is possible.
...
For any open automata \(A \defobject \OAg\), a reachability predicate \(\reach{A}: S \to \rformulas[V]\) is a predicate on states such that
...
Let \(A_p\subst{A_c}{j} = \OAg\) with \(A_c \defobject \OAg[c]\), \(A_p \defobject \OAg[p]\) and \(j \in J_p\).
The composition is non-locking if there is a reachability predicate such that from reachable configurations, if there was a possible transition in \(A_p\) then there is a possible transition in \(A_p\subst{A_c}{j}\):
\begin{multline*}
...
A relation on configurations \(R: S_1 \times S_2 \to \rformulas[V_1 \uplus V_2]\) is deadlock reducing if it is a pre-simulation and:
\begin{multline*}
...
It is not true.
Actually even if the last composition of any order of composition is non-locking there is no guarantee that this order is non-locking at every step.
...
\section{Refinement relations in restricted cases}\label{sec:prelref} % TODO: Fold definitions and proofs
...
\end{proof}
\begin{prop}[Composition correctness"]
% TODO: Proof that filling added holes with a fully specified automaton is a refinement
\end{prop}
\begin{proof}
\end{proof}
...
\begin{proof}
\defitem By transitivity of \(=\), \(J_1 = J_3\)
...
\begin{multline}
	\bigsymb{\forall} t_1 \defobject \OTx{1}{}{1}{1} \in \fOT{s_1}, \bigsymb{\exists} \mpar{t_{3x} \defobject \OTx{3}{x}{3x}{3x} \in \fOT{s_3}}^{x \in X}, \mpar{\forall x \in X, J'_{3x} = J'_1} \\
	\wedge V_1 \uplus V_3 \uplus \fvars{t_1} \vdash R_{13}\mpar{s_1, s_3} \wedge g_1 \implies \operatorname*{\bigsymb{\bigvee}}_{x \in X} \mpar{\everymath{\displaystyle}\begin{array}{l}
		\alpha_1 = \alpha_{3x} \wedge \bigwedge_\subbox{j \in J'_{3x}} \beta_{1j} = \beta_{3xj} \\[12pt]
		\nwedge g_{3x} \wedge R_{13}\mpar{s'_1, s'_{3x}}\psubst{\psi_1 \uplus \psi_{3x}}
	\end{array}} \goal{1}
\end{multline}
\[ V_1 \uplus V_3 \uplus \biguplus_\subbox{t_3 \in \fOT{s_3}} \fvars{t_3} \vdash R_{13}\mpar{s_1, s_3} \wedge \bigvee_\subbox{t_3 \in \fOT{s_3}} \fguard{t_3} \implies \bigvee_\subbox{t_1 \in \fOT{s_1}} \fguard{t_1} \goal{2} \]
\begin{description}
	\item[\goal{2}:] Let \(\sigma_1: V_1 \to \values\) and \nmm{\sigma_3: V_3 \uplus \biguplus_\subbox{t_3 \in \fOT{s_3}} \fvars{t_3} \to \values} be such that
	\[ \sigma_1 \uplus \sigma_3 \vdash R_{13}\mpar{s_1, s_3} \wedge \bigvee_\subbox{t_3 \in \fOT{s_3}} \fguard{t_3} \implies \bigvee_\subbox{t_1 \in \fOT{s_1}} \fguard{t_1} \goal{2'} \]
	...
	\begin{align}
		\nu & : V_2 \to \values &
		\sigma_1 \uplus \sigma_3 \uplus \nu & \vdash R_{12}\mpar{s_1, s_2} \wedge R_{23}\mpar{s_2, s_3} \hyp{5'} \\
		t_3 & \in \fOT{s_3} &
		\sigma_1 \uplus \sigma_3 & \vdash \fguard{t_3} \hyp{5"}
	\end{align}
	% TODO HERE
\end{description}
\end{proof}
