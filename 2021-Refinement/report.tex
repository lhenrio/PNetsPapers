...
This implies that there is no relation between their holes (\(J \setminus \mset{k_i}{i \in \mdbrk{1; n}} \cup \bigcup_{i \in \mdbrk{1; n}} J_i\) is neither subset nor superset of \(J_p\)).
...
\[ \forall ss \defobject \mpar{s_p, s_c} \in S, \forall \sigma: V \uplus \bigcup_\subbox{t_p \in \fOT{s_p}} \fvars{t_p} \to \values, \sigma \vdash \reach{A}\mpar{s} \wedge \bigvee_\subbox{t_p \in \fOT{s_p}} \fguard{t_p} \implies \bigvee_\subbox{t \in \fOT{s}} \fguard{t} \]
\end{defi}
\begin{lem}
...
We want to prove
\[ \forall s \defobject \mpar{s_p, s_c} \in S, \forall \sigma: V \uplus \bigcup_\subbox{t_p \in \fOT{s_p}} \fvars{t_p} \to \values, \sigma \vdash \reach{A}\mpar{s} \wedge \bigvee_\subbox{t_p \in \fOT{s_p}} \fguard{t_p} \implies \bigvee_\subbox{t \in \fOT{s}} \fguard{t} \tag{WD}\label{eq:nlcWD} \]
\[ \iff \]
\begin{multline*}
...
\item[\(\eqref{eq:nlcWD}\Rightarrow\eqref{eq:nlcID}\):]
	...
	\[ \sigma \uplus \nu \vdash \fguard{t_p} \hyp{2'} \]
	\(R\) satisfies \eqref{eq:nlcWD} with the current value of \(s\) and \(\sigma \uplus \nu\) completed with dummy values for the other variables of \(\bigcup_{t_p \in \fOT{s_p}} \fguard{t_p}\) so we have:
...
\item[\(\eqref{eq:nlcWD}\Leftarrow\eqref{eq:nlcID}\):]
	Let \(s \defobject \mpar{s_p, s_c} \in S\), \(\sigma \in V \uplus \bigcup_{t_p \in \fOT{s_p}} \fvars{t_p} \to \values\).
	We want to prove:
