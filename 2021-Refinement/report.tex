...
The variables of a transition are all the free variables in the expressions minus the auomaton variables.
...
\begin{figure} % TODO: consistency on register and counter, make number appear in the full composed, Change C so that is it not both a state and a hole
\centering
...
% TODO: explain what you simplified

\Ludo{TODO: exlpain X}{} % TODO: generic = XXX stands for Red Green or Yellow
...
In the state \(RC\) the composition makes 2 transition \Ludo{from}{be more precise} the \(\tau\) transition but both have a false guard; \Ludo{Let's look}{avoid informal english} at one of them. % TODO: Insist on the fact that it shows many usefull states and mechanisms and why it is useless
...
This guard and \Ludo{value update}{what do you mean?} can be simplified to the one shown on the drawing. % TODO: Variable assignment
...
\begin{exi}
An example of non-locking parallel substitution where no individual substitution is non-locking is given % TODO HERE
\end{exi}
