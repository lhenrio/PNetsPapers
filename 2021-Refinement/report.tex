...
The transitions obtained by composition are the following (generic and simplified version). \Ludo{Also satisfiable guards were replaced with simplified equivalent}{moved but a rephrasing may be better}:
\begin{itemize}
...
\label{fig:tlf}
\end{figure}
Let's look at the \(\tau\) transition from the state \(R\) for a more in depth example on how composition and value passing work.
The transition in the specification is \nmm{\OT{R}{R}{\tau}{\mbrc{R \mapsto \act{set}\mpar{x}, C \mapsto \theta\mpar{x}}}{\top}{\mbrc{}}}.
The composition makes 12 states from the \(R\) state, these states are in \(\mbrc{R} \times \mdbrk{1; 6} \times \mbrc{S, C}\), however only \(\mpar{R, 1, S}\) and \(\mpar{R, 2, C}\) are reachable so we will only consider these two.
In the figure they are noted \(RS\) and \(RC\).
The holes \(R\) and \(C\) are filles at the same time so the \(\tau\) transition must synchronise with a transition from both the register and the controller.

In the state \(RC\) the composition makes 2 transition from the \(\tau\) transition but both have a false guard; let's look at one of them.
...
Obviously the guard is false so the it is equivalent to have no transition.
Also the guard is long and can be simplified.
...
We can see that the \(x\) variables from the specification, the controller and the register are in conlict.
