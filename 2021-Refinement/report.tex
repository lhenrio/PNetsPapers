...
In the state \(RC\) the composition makes 2 transition from the \(\tau\) transition with a false guard, let's look at one of them.
...
In the state \(RS\) the composition makes 1 transition from the \(\tau\) transition; this transition is described before but let's look at the details of the composition.
% TODO: explain value passing then say that in the register \set{t} wouldn't have worked
\end{exi}
