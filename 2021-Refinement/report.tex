...
\begin{align*}
	...
	A_1\subst{A_2}{j_1}\subst{A_3}{j_2} & = A_1\subst{A_2\subst{A_3}{j_2}}{j_1} & \mpar{j_1, j_2} & \in J_1 \times J_2
\end{align*}
Where the equality is modulo isomorphism (state renaming, associativity, commutativity).
\end{prop}
This propriety is induced by the composition on open pNets. % LATER: proof?
...
This allows us to define the parallel composition \(A\psubst{A_j^{j \in J'}}\) if there is any order of (sequential) composition where each \(A_j\) is composed in the hole \(j\).
...
Informally, a simulation relation relating two automata means that one automaton states and transitions can be simulated by the other automaton.
...
Note that here there are two relations:
The first is between states of two specific automata (\(R \subseteq S_1 \times S_2\)), the second is between automatas (\(A_1 \leq A_2\)).
...
A relation of second kind will be called ``relation on automata" whereas a relation of the first kind could be called ``relation on states" though valuations also play the same role as states.
...
\end{defi}
For a relation on automata \(\wrel{}{}{}\) which uses a relation under predicate to state that two element are related \(\forall x \, x', \wrel{x}{x'}{} \iff \exists R, P\mpar{R}\), there is no unicity on \(R\).
To specify which witness is used in \(\wrel{x}{x'}{}\), we note \(\wrel{x}{x'}{R}\) where \(R\) is that witness.
...
\begin{defi}[Pre-simulation on open automata]
\Quentin{}{Is the definition satisfying now?}
For two automata \(A_1 \defobject \OA{S_1}{s_{01}}{J_1}{V_1}{\sigma_{01}}{T_1}\) and \(A_2 \defobject \OA{S_2}{s_{02}}{J_2}{V_2}{\sigma_{02}}{T_2}\), a relation under predicate \(R: S_1 \times S_2 \to \rformulas[V_1 \uplus V_2]\) is a pre-simulation if it satisfies both
...
Apart from holes, this is a natural extension of the notion of simulation on LTS, which is the same definition without variables and holes and guards.
The treatment of holes in the FH-bisimulation is that holes with the same name will receive the same open automata when composed and the resulting open automata will still be equivalent.
...
This implies that there is no relation between their holes, \(J = \mpar{J_p \uplus \biguplus_{i \in \mdbrk{1; n}} J_i} \setminus \mset{k_i}{i \in \mdbrk{1; n}} \) is neither subset nor superset of \(J_p\).
...
A relation under predicate \(R: S_1 \times S_2 \to \rformulas[V_1 \uplus V_2]\) is deadlock reducing if:
...
\end{multline*}
\end{defi}
This requirement effectively prevents the presence of deadlock state in the first automaton if there is a related non-deadlock state in the second by contraposition.
...
In all these properties, ``Deadlock reduction" conflicts with ``composition correctness" by disallowing some unwanted cases where an automaton in a hole cannot produce any action that any out-transition expects.
...
A way to solve this conflict is to characterise a composition that do not introduce deadlocks, which will be used instead of the composition introduced earlier.
...
\begin{align*}
	f\mpar{p} = \mbrc{s' \mapsto p\mpar{s'} \vee \bigsymb{\bigvee_\subbox{\OTg \,\in\, \fIT{s'}}} p\mpar{s}\psubst{\psi} \wedge g} &&
	\sigma_0 \vdash \reach{A}\mpar{s_0} &&
	f\mpar{\reach{A}} = \reach{A}
\end{align*}

\begin{defi}[Non-locking composition, intuitive definition]
Let  \(A_p\subst{A_c}{k} = \OA{S}{s_0}{V}{\sigma_0}{J}{T}\) with \(A_c \defobject \OA{S_c}{s_{0c}}{V_c}{\sigma_{0c}}{J_c}{T_c}\), \(A_p \defobject \OA{S_p}{s_{0p}}{V_p}{\sigma_{0p}}{J_p}{T_p}\) and \(k \in J_p\).
The composition is non-locking if there is a reachability predicate such that from reachable configurations, if there was a possible transition in \(A_p\) then there is a possible transition in \(A_p\subst{A_c}{k}\):
\begin{multline*}
	\forall s \defobject \mpar{s_p, s_c} \in S, \forall \sigma: V \to \values, \mpar{\sigma \vdash \reach{A_p\subst{A_c}{k}}\mpar{s}} \implies \\
...
We can expand the aliases \(S, V\) into \(S_p \times S_c, V_p \uplus V_c\) and the last transition \(t\) into a pair \(\mpar{t_p, t}\) where the first transition is the one from \(A_p\) which generated \(t\).
Doing that makes the definition become mildly similar with the deadlock reduction one.
...
A relation under predicate \(R: S_1 \times S_2 \to \rformulas[V_1 \uplus V_2]\) is deadlock reducing if it is a pre-simulation and:
\begin{multline*}
...
\end{multline*}
\end{defi}
The requirement here is that from any configuration related configurations, if there is a possible transition in the second automaton (whatever the value of the free variables) then there is a possible transition in the first automaton (the existential quantifier is implicit in the notation \(\vdash\)).
The transition are not matched because the relation under predicate is a pre-simulation and it is sufficient to ensure it as will be shown in the proof.
...
\[ \forall ss \defobject \mpar{s_p, s_c} \in S, \forall \sigma: V \uplus \biguplus_\subbox{t_p \in \fOT{s_p}} \fvars{t_p} \to \values, \sigma \vdash \reach{A}\mpar{s} \wedge \bigvee_\subbox{t_p \in \fOT{s_p}} \fguard{t_p} \implies \bigvee_\subbox{t \in \fOT{s}} \fguard{t} \]
\end{defi}
\begin{lem}
...
This new definition of composition can replace the standard composition in the case where only one hole is filled, and also in the parallel case.
In the parallel case, we may think that if the condition is checked in the parallel case (at the final composition), then the composition is non-locking for every composition order.
It is not true.
Actually even if the last composition is non-locking there is no guarantee that there is an order of sequential composition that is non-locking at every step.
However the oppsite way is true even if uninformative:
...
A relation on open automata and its corresponding relation under predicate are simulations if the latter is a deadlock reducing pre-simulations and the former is a preorder.
...
In particular if we have a family of open automata \(A_i \defobject \OA{S_i}{s_{0i}}{J_i}{V_i}{\sigma_{0i}}{T_i}\), \(i \in \mdbrk{0; n}\), let \(A \defobject \OA{S}{s_0}{J}{V}{\sigma_0}{T}\) be such that \(A = A_0\psubst{j_i \mapsto A_i \middle| i \in \mdbrk{1; n}}\).
Recall that there is no relation between the holes of \(A\) and \(A_0\) because \(J = \mpar{J_0 \uplus \biguplus_{i \in \mdbrk{1; n}} J_i} \setminus \mset{j_i}{i \in \mdbrk{1; n}}\) is neither subset nor superset of \(J_0\).
...
In the following definitions, \(A_1 \defobject \OA{S_1}{s_{01}}{J_1}{V_1}{\sigma_{01}}{T_1}\) and \(A_2 \defobject \OA{S_2}{s_{02}}{J_2}{V_2}{\sigma_{02}}{T_2}\) are two open automata.

The following relation is the basis for all the other ones.
We restrict the relation to automata with the same holes so that we can explain the refinements independant from composition.
These refinements are ``specifying more initial variables" and ``adding determinism".
This corresponds to the classical simulation refinement on LTS \Ludo{TODO}{j imagine que rabea peut donner des refs ici}.
\begin{defi}[Hole-identical simulation]
\(R: \mpar{S_1 \times S_2} \to \rformulas[V_1 \uplus V_2]\) is a hole-identical simulation of \(A_1\) by \(A_2\) if:
\defitem \(J_1 = J_2\);
...
\noindent\begin{multline*}
	\mpar{\everymath{\displaystyle}\begin{array}{l}
		...
		\quad \mpar{\forall x \in X, J'_{2x} \subseteq J'_1} \\ % TODO: or = ?
		\quad \wedge V_1 \uplus V_2 \uplus \fvars{t_1} \vdash R\mpar{s_1, s_2} \wedge g_1 \implies \operatorname*{\bigsymb{\bigvee}}_{x \in X} \mpar{\begin{array}{l}
			\alpha_1 = \alpha_{2x} \wedge \bigwedge_{j \in J'_{2x}} \beta_{1j} = \beta_{2xj} \\[12pt]
			\nwedge g_{2x} \wedge R\mpar{s'_1, s'_{2x}}\psubst{\psi_1 \uplus \psi_{2x}}
		\end{array}} \\[12pt]
	\end{array}} \\
	\wedge V_1 \uplus V_2 \uplus \biguplus_{t_2 \in \fOT{s_2}} \fvars{t_2} \vdash R\mpar{s_1, s_2} \wedge \bigvee_\subbox{t_2 \in \fOT{s_2}} \fguard{t_2} \implies \bigvee_\subbox{t_1 \in \fOT{s_1}} \fguard{t_1}
\end{multline*}
\end{defi}
The hole-identical name is referring to the first part of the definition: holes indices are the same in related automata.
...
On top of that the holes involved in the family of transition can vary and only be a subset of the transition simulated.
We can force their equality (between all the \(J'\), not only the \(J'_{2x}\) because of transitivity issues), and get a perfectly valid alternative depending on the properties you expect from the refinement relation.
The subset version is strange in term of simulation since a hole can perform more actions in a simulated run.
...
% TODO: re-écriture
\begin{lem}[Hole-identical simulation is a pre-simulation]
\begin{gather*}
