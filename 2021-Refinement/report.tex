...
In particular filling a hole with the deadlock automaton (only one state without out-transitions) is not considered a refinement for a \Ludo{deadlock reducing simulation refinement}{define or use the name of the definition}.
...
\Quentin{Reply}{Any variable not defined by the valuation is under an implicit \(\exists\) quantifier (satisfiability). What it means is that for every valuation where states are related and a transition is possible (disjunction on the left), there is a value for the free variables of the disjunction on the right that makes a transition possible (the existential quantifier has been wisely displaced in this explaination to make it easier to understand).}
\end{defi}
