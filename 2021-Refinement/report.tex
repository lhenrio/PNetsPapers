...
An open automaton is a tuple \(\OAg\) with \(S\) the set of states, \(s_0 \in S\) the initial state, \(V\) the set of variable names unique to this automaton, \(\sigma_0: V \to \values\) the initial valuation of variables, \(J\) the set of hole names and \(T\) the set of open transitions.
...
The initial valuation is a valuation of the automaton variables.
We could have defined it as a partial valuation in order to allow some variable to be parameters of the automaton and to allow not defining some variables never read before being set.
However the first is unnecessary because it is already possible by having a variable set once in the first transition by an unbound and unconstrained variable in a transition as we will see after the next definition.
And for the second point we could also define such a variable to any value.
\begin{defi}[Open transition, Instanciated transition]
...
To illustrate how a variable can be used as a parameter we could imagine how to represent a \(n\)-bit buffer: \(n \in V\) is initially set to any value in the initial valuation, then the first transition, taken only once, could have guard \(\top\) (always true) and variable assignement \(\mbrc{n \gets k}\) with \(k \notin V\) an unbound variable.
...
Relevant initial variable values are indicated near a double lined arrow pointing to the initial state.
\begin{figure}
\centering
...
However here \(\tau\) is not always allowed from the holes, for instance in the state \(L\) the hole \(r\) cannot emit it because there is no transition allowing this action from this hole.
...
The composition may look like a handshake, with both automata running in parallel (product of states and joint variables) however it is asymmetric:
The first automaton decides when the second can evolve by involving it in a transition, while the first automaton can evolve independently as soon as the hole is not involved in an open transition.
\begin{exi}\label{exi:full_comp}
\begin{figure}
\centering
...
Two configurations \(\mpar{s_1, \sigma_1} \in S_1 \mpar{V_1 \to \values}, \mpar{s_2, \sigma_2} \in S_2 \times \mpar{V_2 \to \values}\) are related iff \(\sigma_1 \uplus \sigma_2 \vdash R\mpar{s_1, s_2}\).
\end{defi}
...
\defitem \textbf{complete w.r.t.\@ composition} iff \(\forall a\, b, a \leq b \implies \exists c_j^{j \in J}, a \cong b\mdbrk{c_j^{j \in J}}\);
...
We won't have it because some operation impossible with composition, like merging states, can applied to open automata and give FH-bisimilar ones and FH-bisimulation compatibility is a much more important property.
...
\begin{defi}[Deadlock reduction, intuitive definition]\label{def:dpid}
...
\begin{defi}[Deadlock reduction, working definition]\label{def:dpwd}
...
\begin{itemize}
\item In the state \(0S\), the initial valuation and valuations with \(0 \leq c = t\) are reachable for both automata.
	On top of that, every valuations where \(c \geq t\) are reachable in the right automaton.
...
% However the opposite way is true: % Would need a proof, non-trivial
...
Instead of matching an instanciated transition to another instanciated transition as in the pre-simulation definition, we match the open transition \(t_1\) to the family of covering open transitions \(t_{2x}^{x \in X}\) to account for all instanciations of \(t_1\).
...
It is designed to capture all the refinements other than composition: Duplicating and merging states and transitions, changing variables, removing transitions, adding requirements to guards, and many more.
...
The automata of Example \ref{exi:full_comp} can be reused here.
...
\end{proofsketch}
For transitivity we need to introduce three open automata.
...
\item[\goal{0'}:] By case on \(\triangle\):
	...
	\item Let \(\triangle\) be \(\supseteq\).
		...
		\end{align*}
	\item Let \(\triangle\) be \(=\).
		We use either one of the proof for the two above cases so \goal{0'} holds.
	\end{itemize}
\item[\goal{2}:] Let \(\sigma_1: V_1 \uplus \fvars{t_1} \to \values\) and \(\sigma_3: V_3 \to \values\).
	We use dummy values for variables in \nmm{\biguplus_\subbox{z \in Z} \fvars{t_{3z}}} to get the left side of this implication as an hypothesis (independent from these values):
	...
	We prove \goal{2d} with valuation \(\psi_{s2x}\psubst{\sigma_2 \uplus \nu_2}\) for the branch \(s_2\):
	\begin{align*}
		& \sigma_1 \uplus \sigma_3 \uplus \nu_3 \vdash R\mpar{s'_1, s'_{3z}}\psubst{\psi_1 \uplus \psi_{3z}} \\
		\iff & \mpar{R_{12}\mpar{s'_1, s'_2} \wedge R_{23}\mpar{s'_2, s'_3}}\psubst{\psi_1 \uplus \psi_{3z}}\psubst{\psi_{s2x}\psubst{\sigma_2 \uplus \nu_2}}\psubst{\sigma_1 \uplus \sigma_3 \uplus \nu_3} \\
		\iff & R_{12}\mpar{s'_1, s'_2}\psubst{\psi_1}\psubst{\psi_{s2x}\psubst{\sigma_2 \uplus \nu_2}}\psubst{\sigma_1} \wedge R_{23}\mpar{s'_2, s'_3}\psubst{\psi_{3z}}\psubst{\psi_{s2x}\psubst{\sigma_2 \uplus \nu_2}}\psubst{\sigma_3 \uplus \nu_3} \\
		\iff & R_{12}\mpar{s'_1, s'_2}\psubst{\psi_1 \uplus \psi_{s2x}}\psubst{\sigma_2 \uplus \nu_2 \uplus \sigma_1} \wedge R_{23}\mpar{s'_2, s'_3}\psubst{\psi_{3z} \uplus \psi_{s2x}}\psubst{\sigma_2 \uplus \nu_2 \uplus \sigma_3 \uplus \nu_3} \\
		\iff & \mpar{\sigma_2 \uplus \nu_2 \uplus \sigma_1 \vdash R_{12}\mpar{s'_1, s'_2}\psubst{\psi_1 \uplus \psi_{s2x}}} \wedge \mpar{\sigma_2 \uplus \nu_2 \uplus \sigma_3 \uplus \nu_3 \vdash R_{23}\mpar{s'_2, s'_3}\psubst{\psi_{3z} \uplus \psi_{s2x}}}
	\end{align*}
	Which is proved by \hyp{5d} and \hyp{6d}.
	The manipulations of substitutions are possible because their definition domain are disjoint; more details are available in the proof for transitivity of FH-bisimulation \cite{2007.10770}.
\item[\goal{1}:] Let \(\sigma_1: V_1 \to \values\) and \nmm{\sigma_3: V_3 \uplus \biguplus_\subbox{t_3 \in \fOT{s_3}} \fvars{t_3} \to \values}.
	...
	\end{gather}
	We use dummy values for variables in \nmm{\biguplus_\subbox{t_1 \in \fOT{s_1}} \fvars{t_1}} to get the left side of this implication as an hypothesis (independent from these values):
...
\end{proof}
\begin{prop}[Limited composition correctness]\label{prop:cc'}
Hole-subset simulation is correct w.r.t.\@ composition with automata without holes.
\end{prop}
...
Let \(v_1 \mapsto v_{1'}\) be the renaming for \(v_1 \in V_1\) in \(A\).
Let \(\reach{A}\) be a witness that the composition \(A_1\subst{A_2}{k}\) is non-locking.
...
\item[\goal{2}:] Let \(\sigma: V \uplus V_1 \uplus \fvars{t} \to \values\).
	We rename some variables in the definition domain of \(\sigma\) to get \(\nu: \fvars{t_1} \to \values\).
	...
	\goal{2c} is proved using \hyp{1c} (\(g \iff g_1\) or \(g \iff g_1 \wedge \dots\)).
	For \goal{2d} we first perform a transformation.
	\begin{align*}
		& \sigma \uplus \nu \vdash R\mpar{s', s'_1}\psubst{\psi \uplus \psi_1} \\
		\iff & \sigma \uplus \nu \vdash \mpar{\reach{A}\mpar{s'} \wedge \bigwedge_{v_1 \in V_1} v_{1'} = v_1}\psubst{\psi_{1'} \uplus \psi_2 \uplus \psi_1} \\
		\iff & \sigma \uplus \nu \vdash \reach{A}\mpar{s'}\psubst{\psi} \wedge \bigwedge_\subbox{v_1 \in V_1} \psi_{1'}\mpar{v_{1'}} = \psi_1\mpar{v_1} \\
		\iff & \mpar{\sigma \vdash \reach{A}\mpar{s'}\psubst{\psi}} \wedge \mpar{\sigma \uplus \nu \vdash \bigwedge_\subbox{v_1 \in V_1} \psi_{1'}\mpar{v_{1'}} = \psi_1\mpar{v_1}}
	\end{align*}
	For the left side we use the definition of reachability (reachability is preserved across transitions):
	\[ \forall t \defobject \OTg \in T, \fvars{t} \vdash \reach{A}\mpar{s} \wedge g \implies \reach{A}\mpar{s'}\psubst{\psi} \hyp{2} \]
	\hyp{2} is used for transition \(t\) with valuation \(\sigma\), the left side of the implication is \hyp{1a} and \hyp{1c}.
	For the right side, the variables of \(\psi\) are in \(V_1 \uplus \fvars{t_1}\) so proving the following is sufficient:
	\[ \sigma \uplus \nu \vdash \bigwedge_\subbox{v_1 \in V_1 \uplus \fvars{t_1}} v_{1'} = v_1 \goal{2d'} \]
	By renaming of the definition domain, \(\nu\) coincides with \(\sigma\) on \(\fvars{t_1}\).
	For the other variables we use \hyp{1b}.
\item[\goal{1}:] Let \nmm{\sigma: V \uplus V_1 \uplus \biguplus_\subbox{t_1 \in \fOT{s_1}} \fvars{t_1} \to \values}.
	We use dummy values for the variables in \nmm{\biguplus_\subbox{t \in \fOT{s}} \fvars{t}} to get the left side of the implication (independent from these values), immediately broken into
	\begin{gather}
		\sigma \vdash \reach{A}\mpar{s} \hyp{3a} \\
		\sigma \vdash \bigwedge_\subbox{v_1 \in V_1} v_{1'} = v_1 \hyp{3b} \\
		\sigma \vdash \bigvee_\subbox{t_1 \in \fOT{s_1}} \fguard{t_1} \hyp{3c}
	\end{gather}
	\(\reach{A}\) is a witness of the non-locking composition so we have the hypothesis:
	\[ V \uplus \biguplus_\subbox{t_1 \in \fOT{s_1}} \fvars{t_1} \vdash \reach{A}\mpar{s} \wedge \bigvee_\subbox{t_1 \in \fOT{s_1}} \fguard{t_1} \implies \bigvee_\subbox{t \in \fOT{s}} \fguard{t} \hyp{4} \]
	We use \hyp{4} with valuation \(\sigma\) to get \nmm{\nu: \biguplus_\subbox{t \in \fOT{s}} \fvars{t} \to \values}.
	The left side of the implication is proved with \hyp{3a} and \hyp{3c}, the right side gives the hypothesis:
	\[ \sigma \uplus \nu \vdash \bigvee_\subbox{t \in \fOT{s}} \fguard{t} \hyp{4'} \]
	We prove \goal{1} with valuation \(\nu\) by admitting the left side of the implication and using \hyp{4'}.
\end{proof}
...
\end{prop}
Let \(\OAg[3] \defobject A_3\) and \(A \defobject A_1\subst{A_3}{k}\), with \(\OAg \defobject A\).
...
\item[\goal{2}:] Let \nmm{\sigma: V \uplus V_2 \uplus \fvars{t} \to \values}.
	We use dummy values for the variables in \nmm{\biguplus_\subbox{t_{2x} \in \fOT{s_2}} \fvars{t_{2x}}} to get the left side of the implication (independent from these values), immediately broken into
	\begin{gather}
	...
	\end{align*}
	The left side is \hyp{4d}.
	For the right side we use the definition of reachability (reachability is preserved across transitions):
	\[ \forall t \defobject \OTg \in T, \fvars{t} \vdash \reach{A}\mpar{s} \wedge g \implies \reach{A}\mpar{s'}\psubst{\psi} \hyp{5} \]
	\hyp{5} is used for transition \(t\) with valuation \(\sigma\), the left side of the implication is \hyp{3b} and \hyp{3c}.
\item[\goal{1}:]
	Let \nmm{\sigma: V \uplus V_2 \uplus \biguplus_\subbox{t_2 \in \fOT{s_2}} \fvars{t_2} \to \values}.
	We use dummy values for the variables in \nmm{\biguplus_\subbox{t \in \fOT{s}} \fvars{t}} to get the left side of the implication (independent from these values), immediately broken into
	\begin{gather}
		\sigma \vdash R_{12}\mpar{s_1, s_2} \hyp{6a} \\
		\sigma \vdash \reach{A}\mpar{s} \hyp{6b} \\
		\sigma \vdash \bigvee_\subbox{t_2 \in \fOT{s_2}} \fguard{t_2} \hyp{6c}
	\end{gather}
	\(R_{12}\) is a simulation and \(\reach{A}\) is a witness of the non-locking composition so we have the hypotheses:
	\begin{gather}
		V_1 \uplus V_2 \uplus \biguplus_\subbox{t_2 \in \fOT{s_2}} \fvars{t_2} \vdash R_{12}\mpar{s_1, s_2} \wedge \bigvee_\subbox{t_2 \in \fOT{s_2}} \fguard{t_2} \implies \bigvee_\subbox{t_1 \in \fOT{s_1}} \fguard{t_1} \hyp{7} \\
		V \uplus \biguplus_\subbox{t_1 \in \fOT{s_1}} \fvars{t_1} \vdash \reach{A}\mpar{s} \wedge \bigvee_\subbox{t_1 \in \fOT{s_1}} \fguard{t_1} \implies \bigvee_\subbox{t \in \fOT{s}} \fguard{t} \hyp{8}
	\end{gather}
	We use \hyp{7} with valuation \(\sigma\) to get \nmm{\nu: \biguplus_\subbox{t_1 \in \fOT{s_1}} \fvars{t_1} \to \values}.
	The left side of the implication is proved with \hyp{6a} and \hyp{6c}, the right side gives the hypothesis:
	\[ \sigma \uplus \nu \vdash \bigvee_\subbox{t_2 \in \fOT{s_2}} \fguard{t_2} \hyp{7'} \]
	We then use \hyp{8} with valuation \(\sigma \uplus \nu\) to get \nmm{\nu': \biguplus_\subbox{t \in \fOT{s}} \fvars{t}}.
	The left side of the implication is proved using \hyp{6b} and \hyp{7'}, the right side gives:
	\[ \sigma \uplus \nu' \vdash \bigvee_\subbox{t \in \fOT{s}} \fguard{t} \hyp{8'} \]
	We prove \goal{1} with valuation \(\nu'\) by admitting the left side of the implication and using \hyp{8'}.
\end{proof}
...
\end{thm}
Let \(A_{13} \defobject A_1\subst{A_3}{k}\) and \(A_{23} \defobject A_2\subst{A_3}{k}\).
Let \(v_3 \mapsto v_{3x}\) be the renaming for \(v_3 \in V_3\) in \(A_{x3}\).
We pose \(\OAg[13] \defobject A_{13}\) and \(\OAg[23] \defobject A_{23}\).
We have \(\reach{A_{13}} = \reach{A_1\subst{A_3}{k}}\).
...
\item[\goal{2}:] Let \(\sigma: V_{13} \uplus V_{23} \uplus \fvars{t_{13}} \to \values\).
	We use dummy values for the variables in \nmm{\biguplus_{x \in X} \fvars{t_{23x}}} to get the left side of the implication as an hypothesis (independent from these values):
	...
	For \goal{3d} we first perform a transformation.
	\begin{align*}
		...
		\iff & \mpar{\sigma \uplus \nu \vdash R_{12}\mpar{s'_1, s'_{2x}}\psubst{\psi_1 \uplus \psi_{2x}}} \wedge \mpar{\sigma \vdash \reach{A_{13}}\mpar{s'_{13}}\psubst{\psi_{13}}} \wedge \mpar{\sigma \uplus \nu' \vdash \bigwedge_\subbox{v_3 \in V_3} \psi_{31}\mpar{v_{31}} = \psi_{32}\mpar{v_{32}}}
	\end{align*}
	The left side is \hyp{3d}.
	For the middle part we use the definition of reachability (reachability is preserved across transitions):
	\[ \forall t_{13} \defobject \OTx{13}{}{13}{13} \in T_{13}, \fvars{t_{13}} \vdash \reach{A_{13}}\mpar{s_{13}} \wedge g_{13} \implies \reach{A_{13}}\mpar{s'_{13}}\psubst{\psi_{13}} \hyp{4} \]
	\hyp{4} is used for transition \(t_{13}\) with valuation \(\sigma\), the left side of the implication is \hyp{2b} and \hyp{2d}.

	For the right side, the variables of \(\psi_{13}\) and \(\psi_{23}\) are renaming of \(\psi_3\) on variables in \(\fvars{t_3}\) or do not involve these variables at all, so either \hyp{2c} is sufficient, or additionnaly proving the following is sufficient:
	\[ \nu' \vdash \bigwedge_\subbox{v_3 \in \fvars{t_3}} v_{31} = v_{32} \]
	Which holds by definition of \(\nu'\).
\item[\goal{1}:] Let \nmm{\sigma: V_{13} \uplus V_{23} \uplus \biguplus_\subbox{t_{23} \in \fOT{s_{23}}} \fvars{t_{23}} \to \values}.
	We use dummy values for the variables in \nmm{\biguplus_\subbox{t_{13} \in \fOT{s_{13}}} \fvars{t_{13}}} to get the left side of the implication (independent from these values), immediately broken into
	\begin{gather}
...
\end{proof}
\begin{thm}[Composition congruence]
...
Let \(v_1 \mapsto v_{1x}\) be the renaming for \(v_1 \in V_1\) in \(A_{x1}\).
...
\item[\goal{2}:] Let \(\sigma: V_{21} \uplus V_{31} \uplus \fvars{t_{21}} \to \values\).
	We use dummy values for the variables in \nmm{\biguplus_{x \in X} \fvars{t_{31x}}} to get the left side of the implication as an hypothesis (independent from these values):
	...
	For \goal{3d} we first perform a tranfsformation.
	\begin{align*}
		...
		\iff & \mpar{\sigma \uplus \nu \vdash R_{23}\mpar{s'_2, s'_{3x}}\psubst{\psi_2 \uplus \psi_{3x}}} \wedge \mpar{\sigma \vdash \reach{A_{21}}\mpar{s'_{21}}\psubst{\psi_{21}}} \wedge \mpar{\sigma \uplus \nu' \vdash \bigwedge_\subbox{v_1 \in V_1} \psi_{12}\mpar{v_{12}} = \psi_{13}\mpar{v_{13}}}
	\end{align*}
	The left side is \hyp{3d}.
	For the middle part we use the definition of reachability (reachability is preserved across transitions):
	\[ \forall t_{21} \defobject \OTx{21}{}{21}{21} \in T_{21}, \fvars{t_{21}} \vdash \reach{A_{21}}\mpar{s_{21}} \wedge g_{21} \implies \reach{A_{21}}\mpar{s'_{21}}\psubst{\psi_{21}} \hyp{4} \]
	\hyp{4} is used for transition \(t_{21}\) with valuation \(\sigma\), the left side of the implication is \hyp{2b} and \hyp{2d}.

	For the right side, the variables of \(\psi_{12}\) and \(\psi_{13}\) are renaming of \(\psi_1\) on variables in \(\fvars{t_1}\) so proving the following is sufficient:
	\[ \sigma \vdash \bigwedge_\subbox{v_1 \in V_1 \uplus \fvars{t_1}} v_{12} = v_{13} \goal{2d'} \]
	Which holds by \hyp{2c} and by definition of \(\nu'\) for variables of the transition.
\item[\goal{1}:] Let \nmm{\sigma: V_{21} \uplus V_{31} \uplus \biguplus_\subbox{t_{31} \in \fOT{s_{31}}} \fvars{t_{31}} \to \values}.
	We use dummy values for the variables in \nmm{\biguplus_\subbox{t_{21} \in \fOT{s_{21}}} \fvars{t_{21}}} to get the left side of the implication (independent from these values), immediately broken into
	\begin{gather}
		\sigma \vdash R_{23}\mpar{s_2, s_3} \hyp{5a} \\
		\sigma \vdash \reach{A_{21}}\mpar{s_{21}} \hyp{5b} \\
		\sigma \vdash \bigvee_\subbox{t_{31} \in \fOT{s_{31}}} \fguard{t_{31}} \hyp{5c}
	\end{gather}
	...
	\begin{gather}
		...
		V_{21} \uplus \biguplus_\subbox{t_{12} \in \fOT{s_{12}}} \fvars{t_{12}} \vdash \reach{A_{21}}\mpar{s_{21}} \wedge \bigvee_\subbox{t_{12} \in \fOT{s_{12}}} \fguard{t_{12}} \implies \bigvee_\subbox{t_{21} \in \fOT{s_{21}}} \fguard{t_{21}} \hyp{7}
	\end{gather}
	We use \hyp{6} with valuation \(\sigma\) to get \nmm{\nu: \biguplus_\subbox{t_{12} \in \fOT{s_{12}}} \fvars{t_{12}} \to \values}.
	The left side of the implication is proved using \hyp{5a} and \hyp{5c} because the guards from transitions of \(A_3\) are still in transitions of \(A_{31}\), the right side gives:
	\[ \sigma \uplus \nu \vdash \bigvee_\subbox{t_2 \in \fOT{s_2}} \fguard{t_2} \hyp{6'} \]
	We then use \hyp{7} with valuation \(\sigma \uplus \nu\) to get \nmm{\nu': \biguplus_\subbox{t_{21} \in \fOT{s_{21}}} \fvars{t_{21}}}.
	The left side of the implication is proved using \hyp{5b} and \hyp{6'}, the right side gives:
	\[ \sigma \uplus \nu' \vdash \bigvee_\subbox{t_{21} \in \fOT{s_{21}}} \fguard{t_{21}} \hyp{7'} \]
	\goal{1} is proved for valuation \(\nu'\) by admitting the left side and using \hyp{7'}.
\end{proof}
