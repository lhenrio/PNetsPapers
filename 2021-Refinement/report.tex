...
The contributions of this article are the definition of composition for open automata without having to define open pNets, and the introduciton of several refinement relations for open automata. % TODO: expand
...
Finally Section \ref{sec:ccl} recapitulates the contribution and present the work left.
...
If we are trying to build an automaton \(A\) with several constraints \(A \leq B_i, i \in I\) -method B like- and each \(B_i\) is a refinement of some automaton \(C\) then we can have each \(B_i\) constrain a different subset of \(A\)'s holes.
On each individual path \(A \leq B_i \leq C\), some holes of \(A\) are unconstrained but when considering all path they are all constrained.
...
Let \(A_1 \defobject \OAg[1]\), \(A_2 \defobject \OA[2]\) and \(k \in J_1\) such that \(A_1\subst{A_2}{k}\) is non-locking.
We pose
\begin{gather*}
	A'_2 \defobject \OA{S_2}{s_{02}}{V_2}{\sigma_{02}}{\emptyset}{T'_2} \\
	T'_2 \defobject \mset{\OT{s}{s'}{\alpha}{\mbrc{}}{g}{\psi}}{\OTg \in T_2}
\end{gather*}
By limited composition correctness we have \(\wrel{J_1 \setminus \mbrc{k}}{A_1\subst{A'_2}{k}}{A_1}\).
...
Let \(A_1 \defobject \OAg[1]\), \(A_2 \defobject \OAg[2]\) and \(A_3 \defobject \OAg[3]\).
Let \(k \in J_1 \cap J_2\), \(\mpar{R_{12}, H_{12}}\) be a hole-tracking simulation of \(A_1\) by \(A_2\) and \(\reach{A_1\subst{A_3}{k}}\) be a witness that \(A_1\subst{A_3}{k}\) is non-locking.
...
\(\mpar{R, H}\) is a witness of \(\wrel{H}{A_1\subst{A_3}{k}}{A_2\subst{A_3}{k}}\), proof is given in Appendix \ref{apx:cr}.
...
\begin{gather*}
	R \defobject \mpar{\mpar{s_{12}, s_2}, \mpar{s_{13}, s_3}} \mapsto \choice{R_{23}\mpar{s_2, s_3} \wedge \reach{A_1\subst{A_2}{k}}\mpar{s_{12}, s_2} \wedge \bigwedge_\subbox{v_1 \in V_1} v_{13} = v_{23} & \text{if } s_{12} = s_{13} \\ \bot & \text{otherwise}} \\
	H \defobject H_{23} \uplus J_1 \setminus \mbrc{k}
\end{gather*}
Where \(y_{1x}\) is the renaming of \(y_1\) from \(A_1\) in \(A_1\subst{A_x}{k}\).
\(\mpar{R, H}\) is a witness of \(\wrel{H}{A_1\subst{A_2}{k}}{A_1\subst{A_3}{k}}\), proof is given in Appendix \ref{apx:cong}.
\begin{thm}[FH-bisimulation compatibility] Open automata refinement is compatible with FH-bisimulation \end{thm}
The requirements of FH-bisimulation are strictly stronger than the open automata refinement so the transitivity proof is sufficient.

% TODO: What happens on untracked holes (untracked holes equivalent behaviour => free variables; some sort of daisy equivalence)
...
\begin{proof}
\item[1)] \(J_{13} = J_3 \uplus J_1 \setminus \mbrc{k} = J_3 \uplus J_2 \setminus \mbrc{k} = J_{23}\) by \(J_1 = J_2\).
...
\item[\goal{0'}:] Let \(x \in X\), by case on \(k \in J'_1\):
	\begin{align*}
		\text{If } k \in J'_1: && J'_{23x} \cap J_{13} & = \mpar{J'_{31} \uplus J'_{2x} \setminus \mbrc{k}} \cap \mpar{J_3 \uplus J_1 \setminus \mbrc{k}} \\
		&&& = \mpar{J'_{31} \cap J_3} \uplus \mpar{J'_{2x} \cap J_1} \setminus \mbrc{k} \\
		&&& = \mpar{J'_{31} \cap J_3} \uplus \mpar{J'_1 \cap J_2} \setminus \mbrc{k} & \text{by \hyp{1'}} \\
		&&& = \mpar{J'_{31} \uplus J'_1 \setminus \mbrc{k}} \cap \mpar{J_3 \uplus J_2 \setminus \mbrc{k}} \\
		&&& = J'_{13} \cap J_{23} \\
