...
% TODO: vérifier tous les big([^_]+)_\subbox{[^}]+}[)}]
...
The contributions of this article \Ludo{report?} are the definition of composition for open automata without having to define open pNets, and the introduction of several refinement relations for open automata\marginpar{TODO: expand}.
...
\begin{defi}[Pre-simulation]\label{def:presim}
...
This implies that there is no relation between their holes, for instance composing \(A_i\) in \(j_i\) (\(1 \leq i \leq n\)) of \(A_0\) gives the set of holes \nmm{J \defobject \mpar{\biguplus_\subbox{0 \leq i \leq n} J_i} \setminus \mset{j_i}{1 \leq i \leq n}} that is neither subset nor superset of \(J_0\).
...
Recall that there is no relation between the holes of \(A\) and \(A_0\) because \nmm{J = \mpar{\biguplus_{0 \leq i \leq n} J_i} \setminus \mset{j_i}{1 \leq i \leq n}} is neither subset nor superset of \(J_0\).
...
% TODO: give intuition, then define R on one line then renamin on the line after
Reflexivity is immediate with \(R \defobject \mpar{s, s'} \mapsto \choice{\bigwedge_{v \in V} v_1 = v_2 & \text{if } s = s'\\ \bot & \text{otherwise}}\) where \(v_x\) is the renaming to distinguish conflicting automaton variables.
...
For two open automata \(A_1 \defobject \OAg[1]\) and \(A_2 \defobject \OAg[2]\) with \(J_2 = \emptyset\), let \(k \in J_1\) and \(A \defobject A_1\subst{A_2}{k}\), with \(A = \OAg\).
...
\item[\(\eqref{eq:drWD}\Rightarrow\eqref{eq:drID}\):]
	...
	In order to get a \(t_2\) matching \(t_1\) we will use the fact that \(R\) is a \hyperref[def:presim]{pre-simulation}:
	\begin{multline}
		\forall \mpar{s_1, s_2} \in S_1 \times S_2, \bigsymb{\forall} t_1 \defobject \OTx{1}{}{1}{1} \in \fOT{s_1}, \forall \sigma: V_1 \uplus V_2 \uplus \fvars{t_1} \to \values, \\
		\mpar{\sigma \vdash R\mpar{s_1, s_2} \wedge g_1} \implies \bigsymb{\exists} t_2 \defobject \OTx{2}{}{2}{2} \in \fOT{s_2}, \exists \nu: \fvars{t_2} \to \values, \\
...
% TODO: put that on several lines using a math env
...
Let \(A_1 \defobject \OAg[1]\), \(A_2 \defobject \OAg[2]\) and \(A_3 \defobject \OAg[3]\) be three pen automata.
...
\item[\goal{2}:] Let \(\sigma: V_{13} \uplus V_{23} \uplus \fvars{t_{13}} \to \values\).
	...
	We can assume \(s_{31} = s_{32}\) and the family given \hyperref[eq:t23x]{previously} works.
	We break \hyp{3} into
	\begin{gather}
...
\item[\goal{1}:] Let \nmm{\sigma: V_{13} \uplus V_{23} \uplus \biguplus_\subbox{t_{23} \in \fOT{s_{23}}} \fvars{t_{23}} \to \values}.
	...
	\begin{gather}
		...
		V_{13} \uplus \biguplus_\subbox{t_1 \in \fOT{s_1}} \fvars{t_1} \vdash \reach{A_{13}}\mpar{s} \wedge \bigvee_\subbox{t_1 \in \fOT{s_1}} \fguard{t_1} \implies \bigvee_\subbox{t_{13} \in \fOT{s_{13}}} \fguard{t_{13}} \hyp{7}
	\end{gather}
...
\begin{proof}
\defitem \(J = J_2 \cup J_1 \setminus \mbrc{k} = \emptyset \cup J_1 \setminus \mbrc{k} \subseteq J_1\)
\defitem We use the fact that \(R\) doesn't constrain the behaviour of \(J_2\):
	\begin{align*}
		...
		\iff & \top
	\end{align*}
\defitem Let \(s \defobject \mpar{s_{1'}, s_2} \in S\) and \(s_1 \in S_1\).
	We want to prove both
	\begin{align*}
