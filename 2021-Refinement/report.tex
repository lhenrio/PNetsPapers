...
% TODO: verify cup -> uplus, tell that uplus is disjoint union
...
Let \(A_1 \defobject \OAg[1]\), \(A_2 \defobject \OAg[2]\) and \(A_3 \defobject \OAg[3]\) with \(J_1 = J_2\).
Let \(k \in J_1\), \(R_{12}\) be a hole-equal simulation of \(A_1\) by \(A_2\) and \(\reach{A\subst{A_3}{k}}\) be a witness that \(A\subst{A_3}{k}\) is non-locking.
...
\(R\) is a witness of the context refinement, proof is given in Appendix \ref{apx:crhe}.
...
\subsection{Context refinement for hole-equal simulation}\label{apx:crhe}
...
\begin{proof}
\item[1)] \(J = J_3 \cup J_1 \setminus \mbrc{k} \supseteq J_3 \cup J_2 \setminus \mbrc{k} \supseteq J_2 \setminus \mbrc{k} = J_2\)
