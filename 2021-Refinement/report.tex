...
Here the main properties that we want are related to composition and action refinement. % LATER: explain here or remove if not tackled
...
This property is what we want to capture with a refinement relation for a structure with composition like the open automata.
...
We wont have it because setting more automaton variables in the initial configuration will be a refinement and it can't be expressed by composing.
Yet a completeness with respect to some base of operation is still a strong and interesting property.
...
This property is also a must have on compositionnal structures to be of any use in proofs and for model checking.
Finally compatibility with FH-bisimulation is the refinement not being able to distinguish two FH-bisimilar automata.
Since FH-bisimulation is the pre-existing practical equivalence relation, we should not introduce an incompatible proof strategy.
...
The non introduction of deadlocks/livelocks are important properties because deadlocks/livelocks can arise in unpredicable ways when parallelism is involved.
The deadlocks/livelocks break the intended behaviour of a system and by preventing them we could hope that the refinement preserves some liveness properties.
