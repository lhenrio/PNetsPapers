...
This means that composition is only defined on open pNets and not directly on open automata. \Quentin{}{Clunky transition}
...
% \cite{10.1007/3-540-46428-X_19} Bellegarde2000: simulation with action refinement and silent actions working with parallel composition for LTL properties on TS; here we do not do action refinement nor silent actions but it should be done in future work, also behaviour wrt LTL properties should be checked, also our composition is more expressive than parallel compositon as parallel composition
% \cite{10.1007/978-3-540-70881-0_26} RR5898: LTS, LTL, Deadlock reduction, non-divergence, weak-refinement, synchronised composition: Closer to what we do here, still open automata are more expressive than LTS and composition is more general than sychronised composition
% \cite{Zhang2014} 703098: 
% \cite{DIHEGO2020110598} :
% \cite{DBLP:journals/corr/abs-2007-10770} weak FH:
%  \cite{10.1145/3372884.3373161} Algorithm and str-FH: 
...
Intuitively the right automaton is a refinement of the left automaton because the left automaton is what happen when there was no bound \(n\) in the right automaton.
...
\(R\) is a witness of the transitivity, proof is given in \Ludo{Appendix \ref{apx:trans}}{are we supposed to read appendix D before C.2?}.
\end{proofsketch}
...
Where \(y_1 \mapsto y_{1'}\) is the renaming for object of \(A_1\) in \(A\).
...
One of the \Ludo{solutions}{il manque les paragraphes qui presentent le probleme} to keep transitivity and constraints on holes actions from same holes is to keep track of which holes are in common at each step of refinement.
...
\end{multline*}
\Ludo{major questions}{il me semble qu il y a plein de trucs qu on comprend pas ici: que se passe t il si H est empty? on peut creer un rafinement qui fait n importe quoi avec les trous? on ne perd aucune propriete? peut etrer qu il manque juste un paragraphe qui explique l approche ses points forts et ses limitations mais l j ai pas l intuition}
...
The transitivity property becomes \(\wrel{A}{B}{X} \wedge \wrel{B}{C}{Y} \implies \wrel{A}{C}{X \cap Y}\).
It behaves the same as if the holes outside the intersection were renamed to avoid conflicts in the first place.
This makes the combination of the transitivity proofs of hole-\(\triangle\) simulation sufficient to prove \Ludo{transitivity}{lu comme ca c est surprenant: ca semble dire que les restrictions sur les trous de la sec precedente ne sont pas vraiment utiles pour prouver la transitivite}.
...
The holes not looked at (or untracked) are equivalent to having the any action automaton (center of Figure \ref{fig:sevpathref}) composed.
...
\begin{proofsketch}
For three open automata \(A_1 \defobject \OAg[1]\), \(A_2 \defobject \OAg[2]\) and \(A_3 \defobject \OAg[3]\), let \(\mpar{R_{12}, H_{12}}\) be a hole-tracking simulation of \(A_1\) by \(A_2\), \(k \in H_{12}\) and \(\reach{A_1\subst{A_3}{k}}\) be a witness that \(A_1\subst{A_3}{k}\) is non-locking.
...
\end{align*}
Where \(y_3 \mapsto y_{3x}\) is the renaming for object of \(A_3\) in \(A_x\subst{A_3}{k}\).
...
\begin{proofsketch}
For three open automata \(A_1 \defobject \OAg[1]\), \(A_2 \defobject \OAg[2]\) and \(A_3 \defobject \OAg[3]\), let \(k \in J_1\), \(\mpar{R_{23}, H_{23}}\) be a hole-tracking simulation of \(A_2\) by \(A_3\) and \(\reach{A_1\subst{A_2}{k}}\) be a witness that \(A_1\subst{A_2}{k}\) is non-locking.
...
\end{align*}
Where \(y_1 \mapsto y_{1x}\) is the renaming for object of \(A_1\) in \(A_1\subst{A_x}{k}\).
...
\item[2)] We use the fact that \(R\) doesn't constrain the behaviour of \(J_2\) and initial configuration is \Ludo{reachable}{comment on construit la premiere disjoint union sachant que sigma 0 est l union de simga 01 et sigma 02???}:
	\begin{align*}
...
\begin{align*}
	...
	H \defobject & J_3 \uplus H_{12} \setminus \mbrc{k}
\end{align*}
Where \(y_3 \mapsto y_{3x}\) is the renaming for object of \(A_3\) in \(A_{x3}\).
...
\begin{align*}
	...
	H \defobject & H_{23} \uplus J_1 \setminus \mbrc{k}
\end{align*}
Where \(y_1 \mapsto y_{1x}\) is the renaming for object of \(A_1\) in \(A_{x1}\).
