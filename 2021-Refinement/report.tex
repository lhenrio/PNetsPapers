...
\Ludo{verification is more general than that (or you need to refine specification): checking typing or absence of deadlocks belong to verification}
Open pNets with the open automaton semantics and equivalence relation introduced in previous work \cite{2007.10770} were sucessfully used to model and verify some properties of applications based on BIP  and GCM \cite{qin:01823507, ameurboulifa:01526055} \Ludo{cite and expand acronym (x2)}.
However equivalence relations are not always sufficient to perform verification, refinement relations are also used. \Ludo{verification/refinement/equivalence needs to be refined and explained}
...
\Ludo{choose: either 1) you do not refer to open pnets below and explain informally open automata here (why are they useful, what makes them different from automata), or 2) you want to refer to pnets in the following and you explain informally pnets, uatomata and the relation}

The contributions of this article are \Ludo{report?} the definition of composition for open automata without having to define open pNets, and the introduction of several refinement relations for open automata. % TODO: expand


% TODO: Related work: thesis

\Ludo{RAPPEL: pour ecrire une intro: 1) on ecrit les contribs, 2) on explique avant les termeis techniques pour que le lecteur comprenne les contribs, 3) quand on a quasi fini le reste on ecrit le plan. ON peut commencer 1 et 2 un peu tot meme si il faut rediger le tout assez tard: on peut pas ecrire proprement un article sans avoir en tete les contribs de celui ci}
...
The latter can only be used if there is a generating expression\Ludo{c est vrai pour les exemples mais pas pour les defs generiques (OA, bisim, ...): reformuler}; for instance \(\mpar{ax}^{x \in \setR}\) represents a scaling function, \(c^{i \in I}\) is a constant function over \(I\).
...
It can be \Ludo{''can be'' is not much informative} a subset of the terms.
...
Otherwise it doesn't invalidates theorems because it can stand as a shorthand for substitution with any term which is interpreted the kept value \Ludo{comprends pas la fin de phrase, anglais?}.
...
When refining \Ludo{bizarre de parler de rafinement ici: phrase a deplacer ou introduire par: Later we will consider refinement of autoamata and ....} the automaton it can be set by the implementation, or it can be left unspecified as a parameter of the automaton (for instance a \(n\) bits register has \(n\) as parameter).
...
\Ludo{here the reader needs either an intuition of the FH bisim or a definition in any case, he needs to know that he will get one or the other and when. For example we give below an intuitive definition of the FH bisim based on examples.}

\Ludo{ If you need the notation but the def is not important you can explicitly say that the readers should refer to ?? for the details of the def, but here I do not believe it is wise OR you could say FH bisim is useless and you do not introduce the notation but you should not use it afterwards (again not adapted I believe)}
...
The goal of the extractor \(\fvars{t}\) when \(t\) is a transition is to get variables that are unique \Ludo{what do you mean by unique here?} to that transition.
Other variables like automaton variables are already known at this point so there is no use in getting them \Ludo{this is not clear because I do not know what variables you ``NEED'' to ``GET'' (need and get are strange and vague to me)}.
...
\Ludo{is it really variable shadowing?} 

\Ludo{you do not necessarily need to explain everything if this is what you need, but you miss an intuition of what this set is}
...
\end{align*}
It is an alternative implementation of the enable operator which is \Ludo{FH-bisimilar}{makes no sense: FH bisimlar is still undefined, neither formally nor informally} to the previous one \cite{henrio:01299562}. 
Note that in this example \(r\) and \(l\) are a hole names and also closed terms: this is a naming conflict, though it is not ambiguous because \(J \cap \rterms = \emptyset\) always holds \Ludo{}{the intersection is at the same time empty and non-empty, why not use a different value (or underline them) or just say that they are in fact different names though it is easier to read using r and l for both}. \Ludo{le probleme ici c est qu on a l impression que tu essayes a la fois d etre tres strict et formel et pas vraiment en meme temps, il faut choisir}
...
\Ludo{In a run,}{je comprends ps cette phrase:}  when taking effective transitions (without local variables), its value is not substituted with a closed term.
...
\Ludo{New}{je suggere un paragraphe du style}
...
Correctness w.r.t.\@ composition means that every composition is considered a refinement. \Ludo{problem}{ you have not yet said that your relation was a refinement!}
...
\Ludo{}{I do not understand this: what is the domain of the valuation?}
...
\Ludo{ }{I do not understand the english + too many important properties you should give a hierarchy or at least find a better transition}
...
This implies that there is no relation between \Ludo{their}{whose? refine} holes, for instance composing \(A_i\) in \(j_i\) (\(1 \leq i \leq n\)) of \(A_0\) gives the set of holes \(J \defobject \mpar{\biguplus_{0 \leq i \leq n} J_i} \setminus \mset{j_i}{1 \leq i \leq n}\) that is neither subset nor superset of \(J_0\).
...
By preventing them we \Ludo{could}{This seems to say that it is useless to prevent them} hope that the refinement preserves some liveness properties.
...
\defitem If from related states, there is a possible transition in the \Ludo{specification}{I do not believe specification has been defined nor implementation}, then there is \Ludo{a pair of matching transition}{A matching transition in the IMPLEMENTATION}:
\begin{multline*}
...
\end{multline*}
\end{defi}
Actually, this requirement prevents the presence of deadlock state in the first automaton if there is a related non-deadlock state in the second by \Ludo{contraposition}{The contraposition is too tricky for me: are you sure?}.
Additionally, the transition preventing \Ludo{the deadlock}{in the implementation? how can it be the same?why is there a single one? this sentence makes no sense to me} has to be an existing transition in the specification.
This is because a valid transition unmatched in the specification \Ludo{is not a deadlock}{how can a transition be a deadlock? I am completely lost}, yet \Ludo{we do not want it}{too informal}.
The \Ludo{direct formulation}{usually you say ``the direct'' when the property seems to be defined negatively which is syntactically not the case here: you should explain or remove the remark} of the property would also have required the unintuitive quantification on a possibly empty set of transition to state that they are impossible.
...
A way to solve \Ludo{these conflicts}{which conflict? can be clarified by answering the previous remark} is to characterise a composition that do not introduce deadlocks, which will be used instead of the composition introduced earlier.
...
Examples of application of the different refinement relations are given in Appendix \ref{apx:refrel}.% TODO: Maybe put them here, not in appendix
...
% TODO HERE: What happens on untracked holes (untracked holes equivalent behaviour => free variables; some sort of daisy equivalence)
