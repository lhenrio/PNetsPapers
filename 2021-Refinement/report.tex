...
\defitem The validity relation on closed formulae, \({\vdash} \subseteq \rformulae\).
...
% maybe Valuation as formula /\ var=val
...
The usual validity of a formula \(f \in \formulae\) with some fixed valuation \(\sigma: V \to \values\) is noted \({\sigma \vdash f} \defnotation {\vdash \forall \fvars{f\psubst{\sigma}}, f\psubst{\sigma}}\). % TODO: remettre en exists? faire des LHS plus compliques style forall LHS exists RHS
% maybe satisfiability with formulas on the LHS (and variabl sets)
...
% TODO: run the automaton conceptually
\end{exi}
...
\defitem \(\forall \OTg \in T, \vdash \reach{A}\mpar{s} \wedge g \implies \reach{A}\mpar{s'}\psubst{\psi}\).
\end{defi}
