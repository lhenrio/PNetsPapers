...
Let \(A_1 \defobject \OAg[1]\), \(A_2 \defobject \OAg[2]\) and \(A_3\) be open automata.
If \(R\) is a hole-superset simulation of \(A_1\) by \(A_2\) then let \(j \in J_1 \setminus J_2\), there is a hole-superset simulation of \(A_1\subst{A_3}\) by \(A_2\).
\end{prop}
\begin{proof}
% TODO
\end{proof}
...
\defitem We prove that initial configurations are related using \(\sigma_{02}\) as the values of the variables in \(V_2\), and \(s_2\) as the branch of the disjunction.
\begin{gather*}
...
\begin{description}
	...
	\begin{gather}
		...
		V_2 \uplus V_3 \uplus \biguplus_\subbox{t_3 \in \fOT{s_3}} \fvars{t_3} \vdash R_{23}\mpar{s_2, s_3} \wedge \bigvee_\subbox{t_3 \in \fOT{s_3}} \fguard{t_3} \implies \bigvee_\subbox{t_2 \in \fOT{s_2}} \fguard{t_2} \hyp{7} \\
	...
	\[ \sigma_1 \uplus \sigma_3 \vdash R_{13}\mpar{s_1, s_3} \wedge \bigvee_\subbox{t_3 \in \fOT{s_3}} \fguard{t_3} \hyp{5} \]
	Hypothesis that we immediately break down into:
	\begin{align}
		\sigma_2 & : V_2 \to \values &
		\sigma_1 \uplus \sigma_2 \uplus \sigma_3 & \vdash R_{12}\mpar{s_1, s_2} \wedge R_{23}\mpar{s_2, s_3} \hyp{5'} \\
		&& \sigma_1 \uplus \sigma_3 & \vdash \bigvee_\subbox{t_3 \in \fOT{s_3}} \fguard{t_3} \hyp{5"}
	\end{align}
	We can use immediately \hyp{7} with valuation \(\sigma_2 \uplus \sigma_3\) to get \(\nu_2: \biguplus_{t_2 \in \fOT{s_2}} \fvars{t_2} \to \values\).
	The left side of the implication is proved with the right side of \hyp{5'} and \hyp{5"}, so we get
	\[ \sigma_2 \uplus \sigma_3 \uplus \nu_2 \vdash \bigvee_\subbox{t_2 \in \fOT{s_2}} \fguard{t_2} \hyp{7'} \]
	We can now use \hyp{6} with valuation \(\sigma_1 \uplus \sigma_2 \uplus \nu_2\) to get \(\nu_1: \biguplus_{t_1 \in \fOT{s_1}} \fvars{t_1} \to \values\).
	The left side of the implication is proved with the left side of \hyp{5'} and \hyp{7'}, so we get
	\[ \sigma_1 \uplus \sigma_2 \uplus \nu_1 \vdash \bigvee_\subbox{t_1 \in \fOT{s_1}} \fguard{t_1} \hyp{6'} \]
	Finally we can prove \goal{1'} with valuation \(\nu_1\) fot the implicit \(\exists\) by admitting the left side of the implication and using \hyp{6'} for the right side.
\end{description}
\end{proof}
