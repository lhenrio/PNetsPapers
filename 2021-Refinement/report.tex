...
The transitivity property becomes \(\wrel{X}{A}{B} \wedge \wrel{Y}{B}{C} \implies \wrel{X \cap Y}{A}{C}\).
...
% TODO: Explain two refinement steps comositions so that Correctness wrt composition is trivial
\begin{thm}[Context refinement] Open automata refinement is context refining. \end{thm}
Let \(A_1 \defobject \OAg[1]\), \(A_2 \defobject \OAg[2]\) and \(A_3 \defobject \OAg[3]\).
Let \(k \in J_1 \cap J_2\), \(\mpar{R_{12}, H_{12}}\) be a hole-tracking simulation of \(A_1\) by \(A_2\) and \(\reach{A\subst{A_3}{k}}\) be a witness that \(A\subst{A_3}{k}\) is non-locking.
...
\(\mpar{R, H}\) is a witness of \(\wrel{H}{A_{13}}{A_{23}}\), proof is given in Appendix \ref{apx:cr}.
% TODO: Proof for Congruence \ref{apx:cong}
% TODO: Proof for FH-bisimulation compatibility \ref{apx:FHbcomp}
...
\subsection{compatibility with FH-bisimulation}\label{apx:FHbcomp}
