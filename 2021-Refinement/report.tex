...
% change deadlock reduction into deadlock prevention?
...
The automaton drawn on the right side of Figure \ref{fig:enable} is \(\OA{\mbrc{»}}{»}{\mbrc{v}}{v \mapsto l}{\mbrc{l, r}}{T}\) where transitions in \(T\) are:
\begin{align*}
...
\begin{prop}[Parallel composition]
...
This allows us to define the parallel composition \(A\psubst{A_j^{j \in J'}}\) as the result of any order of (sequential) composition where each \(A_j\) is composed in the hole \(j\). % OR pcomp [[j |-> A_j]]
...
In the work on FH-bisimulation\marginpar{re-cite FH-bisim as near as the previous citation?} such a relation \(R\) was defined as triples in \(S_1 \times S_2 \times \formulas\) where for \(\mpar{s_1, s_2} \in S_1 \times S_2\) there is a unique \(f \in \formulas\) such that \(\mpar{s_1, s_2, f} \in R\).
...
The important properties we consider for the relation on states are being a simulation and the reduction of deadlocks as defined in the following definitions.
...
Deadlocks are reachable configurations and from which no further transition is possible.
...
``Deadlock reduction" conflicts with all the properties where composition is involved (for instance ``composition correctness") by disallowing some unwanted cases where an automaton in a hole cannot produce any action that any out-transition expects.
...
The exact reachability may not be reprensentable in the formulas, hence the need of potentially over-approximating. % TODO: is representable the correct word
...
Doing that makes the definition become mildly similar with the deadlock reduction one. % OR slightly
...
Indeed the big disjunctions are not quantifiers because in practise the out-transitions are finitary, and so is the expansion of the disjunction. % WHAT IS THE ISSUE HERE?
\begin{lem}
...
\end{lem}
An illustration of the introduction of a deadlock by composition and the definition of non-locking composition is given in Appendix \ref{apx:lockcomp}.
Proof of both lemma are given in Appendix \ref{apx:lemeqd}.

This new definition of composition can replace the standard composition in the case where only one hole is filled, and also be extended to a parallel composition.
For the parallel composition, we may think that if the composition is non-locking, then every composition is non-locking for all composition order.
It is not true.
Actually even if the last composition of any order of composition is non-locking there is no guarantee that this order is non-locking at each step.
However the oppsite way is true even if uninformative: % WHAT IS THE ISSUE?
If there is an order of composition where every composition is non-locking then the last composition is non-locking, last composition resulting in the same automaton as the parallel composition.
So the condition is better checked after the parallel composition or the last individual composition.
\begin{exi}
\begin{figure}
\centering
\input{file_sync.tex}
...
The two holes are \(net\) a network manager used to perform operations on the network, and \(hash\) a file integrity check used to generate a fingerprint.
...
If the preferred version is the remote one, we go in state \(d1\) asking the netwotk manager to fetch the lastest version, then ask the integrity checker to validate the fingerprint.
If the preferred version is the local one, we go in state \(d2\) asking for the integrity checker to generate a fingerprint for the local files, then ask the network manager to push them.
In fallback we use the local version.

We can debranch the network manager and the file integrity check by composing a deadlock automaton in their holes, but always both at the same time.
The reason for that is the parallel composition is non-locking, but any individual composition is locking because the state \(d1\) or \(d2\) (depending on the order) becomes a deadlock.
\end{exi}
...
Recall that there is no relation between the holes of \(A\) and \(A_0\) because \(J = \mpar{\biguplus_{0 \leq i \leq n} J_i} \setminus \mset{j_i}{i \in \mdbrk{1; n}}\) is neither subset nor superset of \(J_0\).
...
\defitem \(J_1 \triangle J_2\)
\defitem \(\sigma_{01} \uplus \sigma_{02} \vdash R\mpar{s_{01}, s_{02}}\)
\defitem \(\forall \mpar{s_1, s_2} \in S_1 \times S_2,\)\vspace{-8pt}
...
% TODO: expand on first part, especially the impact of /_\ on the different refinement allowed and what it means, and the fact that = could be substituted with \subseteq.
...
We can get back to prove \goal{0'} with the transition \(t_{2x}\) and the valuation \(\nu\) by \hyp{4'}.
\end{proof}
...
The solution is to expand the existential quantifier as we did in deadlock reduction, which is possible because states are finite.
\nmm{R_{13} \defobject \mpar{s_1, s_3} \mapsto \exists V_2, \bigvee_\subbox{s_2 \in S_2} R_{12}\mpar{s_1, s_2} \wedge R_{23}\mpar{s_2, s_3}} is a witness of the transitivity, proof is given in appendix \ref{apx:trans}.
\begin{thm}[Context refinement] Hole-equal simulation is context refining \end{thm}
\begin{proof}
% TODO: assume a <= b & a[c] -> b[c] + proof
\end{proof}
% Rédiger un paragraphe sur le fait que composer dans l'implem sans deadlock implique qu'il n'y a pas de deadlock quand on compose avec la spec, on voudrais avoirs l'autre sens mais ce n'est pas possible
\begin{prop}[Composition correctness'] Hole-subset simulation is correct w.r.t.\@ composition. \end{prop}
\begin{proof}
% TODO
\end{proof}
\begin{prop}[Composition correctness"]
...
\begin{multline*}
	...
	\mpar{\everymath{\displaystyle}\begin{array}{l}
		...
		\quad \wedge V_1 \uplus V_2 \uplus \fvars{t_1} \vdash R\mpar{s_1, s_2} \wedge g_1 \implies \operatorname*{\bigsymb{\bigvee}}_{x \in X} \mpar{\begin{array}{l}
...
\section{Flat model}
% Goal: some interfaces refine in one way while other refine in the other way and others are equivalent and some are untracked.
...
A symetric version of composition on a flat model of open automata is presented in Section ???\marginpar{TODO LATER}.
\end{exi}
...
\item[\(\eqref{eq:drWD}\Rightarrow\eqref{eq:drID}\):]
	...
	\[ \sigma \uplus \nu \vdash \fguard{t_2} \hyp{2'} \]
	\(R\) satisfies \eqref{eq:drWD} with the current value of \(\mpar{s_1, s_2}\) and \(\sigma \uplus \nu\) completed with dummy values for the other variables of \(\biguplus_{t_2 \in \fOT{s_2}} \fguard{t_2}\) so we have:
	...
	The \(\vdash\) hides an implicit \(\exists\) that we use to get values to the variables in \(\biguplus_{t_1 \in \fOT{s_1}}\) in a valuation \(\mu\).
...
\item[\(\eqref{eq:drWD}\Leftarrow\eqref{eq:drID}\):]
	...
	To do that we use \eqref{eq:drID} with the current value of \(\mpar{s_1, s_2}\) and \(\sigma\), and prove the premisse of the implication with \hyp{4} to get:
	\begin{multline}
		\mpar{\exists t_2 \in \fOT{s_2}, \exists \nu: \fvars{t_2} \to \values, \sigma \uplus \nu \vdash \fguard{t_2}} \implies \\
		\bigsymb{\exists} t_1 \defobject \OTx{1}{}{1}{1} \in \fOT{s_1}, t_2 \defobject \OTx{2}{}{2}{2} \in \fOT{s_2}, \exists \nu: \fvars{t_1} \uplus \fvars{t_2} \to \values, \\
		\sigma \uplus \nu \vdash g_1 \wedge g_2 \wedge \alpha_1 = \alpha_2 \wedge \bigwedge_\subbox{j \in J'_1 \cap J'_2} \beta_{1j} = \beta_{2j} \wedge R\mpar{s'_1, s'_2}\psubst{\psi_1 \uplus \psi_2} \hyp{6}
	\end{multline}
...
% Hole-subset: Traffic lights
% Hole-superset: scheduler
...
\begin{proof}
\defitem By transitivity of \(\triangle\), \(J_1 \triangle J_3\)
\defitem We prove that initial configurations are related using \(\sigma_{02}\) as the values of the variables in \(V_2\), and \(s_{02}\) as the branch of the disjunction.
\begin{gather*}
...
\end{gather*}
\defitem Let \(\mpar{s_1, s_3} \in S_1 \times S_3\), we want to prove both
\begin{multline}
...
\begin{description}
	\item[\goal{0}:] Let \nmm{t_1 \defobject \OTx{1}{}{1}{1} \in \fOT{s_1}}. % TODO: quantify on s_2
	We have the hypotheses on \(R_{12}\) and \(R_{23}\), then the current goal:
	\[
		\everymath{\displaystyle}\begin{array}{l}
		\bigsymb{\forall} t_1 \defobject \OTx{1}{}{1}{1} \in \fOT{s_1}, \bigsymb{\exists} \mpar{t_{2x} \defobject \OTx{2}{x}{2x}{2x} \in \fOT{s_2}}^{x \in X}, \\
		\quad \mpar{\forall x \in X, J'_{2x} \cap J_1 = J'_1 \cap J_2} \\[-10pt]
		\nwedge V_1 \uplus V_2 \uplus \fvars{t_1} \vdash R_{12}\mpar{s_1, s_2} \wedge g_1 \Rightarrow \operatorname*{\bigsymb{\bigvee}}_{x \in X} \mpar{\!\!\begin{array}{l}
			\alpha_1 = \alpha_{2x} \wedge \bigwedge_\subbox{j \in J'_{2x} \cap J_1} \beta_{1j} = \beta_{2xj} \\[12pt]
	...
	\]\[
		...
		\nwedge V_2 \uplus V_3 \uplus \fvars{t_2} \vdash R_{23}\mpar{s_2, s_3} \wedge g_2 \Rightarrow \operatorname*{\bigsymb{\bigvee}}_{x \in X} \mpar{\!\!\begin{array}{l}
	...
	\]\[
		...
		\nwedge V_1 \uplus V_3 \uplus \fvars{t_1} \vdash R_{13}\mpar{s_1, s_3} \wedge g_1 \Rightarrow \!\operatorname*{\bigsymb{\bigvee}}_{x \in X} \mpar{\!\!\begin{array}{l}
			\alpha_1 = \alpha_{3x} \wedge \bigwedge_\subbox{j \in J'_{3x} \cap J_1} \beta_{1j} = \beta_{3xj} \\[12pt]
			\nwedge g_{3x} \wedge R_{13}\mpar{s'_1, s'_{3x}}\psubst{\psi_1 \uplus \psi_{3x}}
		\end{array}\!\!}
		\end{array} \goal{0'}
	\]
	We use hypothesis \hyp{0} with \(t_1\) to get
	\begin{gather*}
		\mpar{t_{2x} \defobject \OTx{2}{x}{2x}{2x} \in \fOT{s_2}}^{x \in X} \\
		\mpar{\forall x \in X, J'_{2x} \cap J_1 = J'_1 \cap J_2} \hyp{0'} \\
		V_1 \uplus V_2 \uplus \fvars{t_1} \vdash R_{12}\mpar{s_1, s_2} \wedge g_1 \Rightarrow \operatorname*{\bigsymb{\bigvee}}_{x \in X} \mpar{\everymath{\displaystyle}\begin{array}{l}
			\alpha_1 = \alpha_{2x} \wedge \bigwedge_\subbox{j \in J'_{2x} \cap J_1} \beta_{1j} = \beta_{2xj} \\[12pt]
			\nwedge g_{2x} \wedge R_{12}\mpar{s'_1, s'_{2x}}\psubst{\psi_1 \uplus \psi_{2x}}
		\end{array}} \hyp{0"}
	\end{gather*}
	Then use \hyp{1} with every \(t_{2x}\) to get
	\begin{gather*}
		\mpar{t_{3xy} \defobject \OTx{3}{xy}{3xy}{3xy} \in \fOT{s_3}}^{y \in Y_x} \\
		\mpar{\forall y \in Y_x, J'_{3xy} \cap J_2 = J'_{2x} \cap J_3} \hyp{1x} \\
		V_2 \uplus V_3 \uplus \fvars{t_{2x}} \vdash R_{23}\mpar{s_2, s_3} \wedge g_{2x} \Rightarrow \operatorname*{\bigsymb{\bigvee}}_{y \in Y_x} \mpar{\everymath{\displaystyle}\begin{array}{l}
			\alpha_{2x} = \alpha_{3xy} \wedge \bigwedge_\subbox{j \in J'_{3xy} \cap J_2} \beta_{2xj} = \beta_{3xyj} \\[12pt]
			\nwedge g_{3xy} \wedge R_{23}\mpar{s'_{2x}, s'_{3xy}}\psubst{\psi_{2x} \uplus \psi_{3xy}}
		\end{array}} \hyp{1x'}
	\end{gather*}
	We pose \(Z \defobject \mset{\mpar{x, y}}{x \in X, y \in Y_x}\).
	We will prove \goal{0'} for the family \(t_{3xy}^{\mpar{x, y} \in Z}\).
	The new goals are:
	\begin{gather}
		\forall \mpar{x, y} \in Z, J'_{3xy} \cap J_1 = J'_1 \cap J_3 \goal{2} \\
		V_1 \uplus V_3 \uplus \fvars{t_1} \vdash R_{13}\mpar{s_1, s_3} \wedge g_1 \Rightarrow \hspace{-3mm}\operatorname*{\bigsymb{\bigvee}}_{\mpar{x, y} \in Z}\!\!\mpar{\!\!\everymath{\displaystyle}\begin{array}{l}
			\alpha_1 = \alpha_{3xy} \wedge \bigwedge_\subbox{j \in J'_{3xy} \cap J_1} \beta_{1j} = \beta_{3xyj} \\[12pt]
			\nwedge g_{3xy} \wedge R_{13}\mpar{s'_1, s'_{3xy}}\psubst{\psi_1 \uplus \psi_{3xy}}
		\end{array}\!\!} \goal{3}
	\end{gather}
	For \goal{2} the proof depends on \(\triangle\).
	\begin{itemize}
	\item Let \(\triangle\) be \(\subseteq\).
		\begin{align*}
		J'_{3xy} \cap J_1 & \subseteq J'_{3xy} \cap J_2 & \text{by } & J_1 \subseteq J_2 \\
			& \subseteq J'_{2x} \cap J_3 & \text{by } & \text{\hyp{1x}} \\
			& \subseteq J'_{2x} \cap J_3 \cap J_1 & \text{by } & J'_{3xy} \cap J_1 \subseteq J_1 \\
		...
		J'_1 \cap J_3 & = J'_1 = J'_1 \cap J_2 & \text{by } & J'_1 \subseteq J_1 \subseteq J_2 \subseteq J_3 \\
			...
			& \subseteq J'_{3xy} \cap J_2 & \text{by } & \text{\hyp{1x}} \\
			& \subseteq J'_{3xy} \cap J_2 \cap J_1 & \text{by } & J'_1 \subseteq J_1 \\
			& \subseteq J'_{3xy} \cap J_1 &&
		\end{align*}
		Hence \goal{2} holds.
	\item Let \(\triangle\) be \(\supseteq\).
		...
		J'_1 \cap J_3 & \subseteq J'_1 \cap J_2 & \text{by } & J_3 \supseteq J_2 \\
			...
			& \subseteq J'_{3xy} \cap J_2 & \text{by } & \text{\hyp{1x}} \\
			& \subseteq J'_{3xy} \cap J_1 & \text{by } & J_1 \supseteq J_2
		\end{align*}
		\begin{align*}
		J'_{3xy} \cap J_1 & = J'_{3xy} = J'_{3xy} \cap J_2 & \text{by } & J_1 \supseteq J_2 \supseteq J_3 \supseteq J'_{3xy} \\
			& = J'_{2x} \cap J_3 & \text{by } & \text{\hyp{1x}} \\
			& \subseteq J'_{2x} \cap J_1 & \text{by } & J_1 \supseteq J_3 \\
			& \subseteq J'_1 \cap J_2 & \text{by } & \text{\hyp{0'}} \\
			& \subseteq J'_1 \cap J_2 \cap J_3 & \text{by } & J'_{3xy} \subseteq J_3 \\
			& \subseteq J'_1 \cap J_3 &&
		\end{align*}
		Hence \goal{2} holds.
	\item Let \(\triangle\) be \(=\).
		We combine the proofs for two above cases and get both inclusions so \goal{2} holds.
	\end{itemize}
	Now the proof for \goal{3}.
	Let \(\sigma_1: V_1 \uplus \fvars{t_1} \to \values\) and \(\sigma_3: V_3 \to \values\).
	We use dummy values for variables in \nmm{\biguplus_\subbox{\mpar{x, y} \in Z} \fvars{t_{3xy}}} to get the left side of this implication as an hypothesis (independant from these values):
	...
	The left side of the implication is proved with the right side of \hyp{2'} and \hyp{2"}, so we get \(x \in X\) and the hypotheses
	\begin{align}
		\sigma_1 \uplus \sigma_2 \uplus \nu_2 & \vdash \alpha_1 = \alpha_{2x} \hyp{3a} \\
		\sigma_1 \uplus \sigma_2 \uplus \nu_2 & \vdash \bigwedge_\subbox{j \in J'_1 \cap J'_{2x}} \beta_{1j} = \beta_{2xj} \hyp{3b} \\
		\sigma_2 \uplus \nu_2 & \vdash g_{2x} \hyp{3c} \\
		\sigma_1 \uplus \sigma_2 \uplus \nu_2 & \vdash R\mpar{s'_1, s'_{2x}}\psubst{\psi_1 \uplus \psi_{2x}} \hyp{3d}
	\end{align}
	We can now use \hyp{1x'} with valuation \(\sigma_2 \uplus \nu_2 \uplus \sigma_3\) to get \nmm{\nu_3: \biguplus_{y \in Y_x} \fvars{t_{3xy}} \to \values}.
	The left side of the implication is proved with the left side of \hyp{2'} and \hyp{2"}, so we get \(y \in Y_x\) and the hypotheses
	\begin{align}
		\sigma_2 \uplus \nu_2 \uplus \sigma_3 \uplus \nu_3 & \vdash \alpha_{2x} = \alpha_{3xy} \hyp{4a} \\
		\sigma_2 \uplus \nu_2 \uplus \sigma_3 \uplus \nu_3 & \vdash \bigwedge_\subbox{j \in J'_{2x} \cap J'_{3xy}} \beta_{2xj} = \beta_{3xyj} \hyp{4b} \\
		\sigma_3 \uplus \nu_3 & \vdash g_{3xy} \hyp{4c} \\
		\sigma_2 \uplus \nu_2 \uplus \sigma_3 \uplus \nu_3 & \vdash R\mpar{s'_{2x}, s'_{3xy}}\psubst{\psi_{2x} \uplus \psi_{3xy}} \hyp{4d}
	\end{align}
	Finally to prove \goal{3} with valuation \(\nu_3\) for the implicit \(\exists\), we admit the left side of the implication choose to prove the branch \(\mpar{x, y}\):
	\begin{align}
		\sigma_1 \uplus \sigma_3 \uplus \nu_3 & \vdash \alpha_1 = \alpha_{3xy} \goal{3a} \\
		\sigma_1 \uplus \sigma_3 \uplus \nu_3 & \vdash \bigwedge_\subbox{j \in J'_{3xy} \cap J_1} \beta_{1j} = \beta_{3xyj} \goal{3b} \\
		\sigma_3 \uplus \nu_3 & \vdash g_{3xy} \goal{3c} \\
		\sigma_1 \uplus \sigma_3 \uplus \nu_3 &\vdash R_{13}\mpar{s'_1, s'_{3xy}}\psubst{\psi_1 \uplus \psi_{3xy}} \goal{3d}
	\end{align}
	\goal{3a} is proved by transitivity using \hyp{3a} and \hyp{4a}.
	\goal{3b} depends on \(\triangle\).
	If \(\triangle\) is \(\subseteq\) % TODO
	If \(\triangle\) is \(\supseteq\) % TODO
	\goal{3c} is exactly \hyp{4c}.
	\goal{3d} % TODO: branch s_2
	\item[\goal{1}:] Let \(\sigma_1: V_1 \to \values\) and \nmm{\sigma_3: V_3 \uplus \biguplus_\subbox{t_3 \in \fOT{s_3}} \fvars{t_3} \to \values}.
	We have the hypotheses on \(R_{12}\) and \(R_{23}\), then the current goal: % TODO HERE: get s_2 by decomposing R_{13}
	\begin{gather}
