...
In the state \(0S\) any valuation is reachable (no initial valuation).
...
In the state \(0C\) % TODO: run the definition of non-locking composition
\end{exi}
...
\end{multline*}
\Ludo{Question}{on the composed side you might have more variables, eg communicated by the sub-automata or by a sub-hole or stored in the state, how are they quantified?}
\Quentin{Merge-conflict}{I edited the formula above before reading your message, is your question still relevant?}
\end{defi}
\begin{lem}
...
\label{fig:pnls}
\end{figure}
An example of non-locking parallel substitution where no individual substitution is non-locking is given in Figure \ref{fig:pnls}.% TODO
\end{exi}
