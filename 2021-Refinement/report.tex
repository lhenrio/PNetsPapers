...
In this article we introduce a refinement relation for open automata. % TODO: expand
...
\Ludo{Strange}{do we really need this interpretation of quantifiers? I find it disturbing}
...
\Ludo{Strange}{ you mean the last notation is forbidden? but isn't it introduced before?}
...
\Ludo{Strange}{not clear: it seems that $w$ is useless  I guess you mean $P$ depend on $w$ but it is not exactly what is written}
...
The formulae contain at least first order logic terms with an equality relation that is not necessarily \Ludo{?}{restricted to?} the equality on \(\terms\).
...
\Ludo{?}{I do not understand the meaning of the net paragraph: I must have missed something}
...
To illustrate these definitions we consider  two implementations (figure \ref{fig:enable}) of the LOTOS\marginpar{cite something here} operator enable in the open automata model.
...
\Ludo{Well}{avoid informal English in scientific writing} here \(\tau\) is not always allowed from the holes in the state \(L\) the hole \(r\) cannot emit it for the sake of simplifying the first example of open automaton.
\end{exi}
...
\(v\) is a variable name for the automaton and a variable in the guard, \Ludo{so its value is not substituted}{I do not understand this: it seems to mean that the substitution of v has no effect}.\Ludo{?}{I believe this needs to be explained more clearly, perhaps based on the def 6 below}
...
\Ludo{Figure \ref{fig:tls} shows}{rephrased: typo} the light controller with some synchronisation logic.
...
\label{fig:tlh}
\end{figure}
The component we choose to compose in the holes are in \Ludo{F}{}igure \ref{fig:tlh}.
...
The transitions obtained by composition are the following (generic and simplified version). \Ludo{Also satisfiable guards were replaced with simplified equivalent}{moved but a rephrasing may be better: simplified twice?}:

\Ludo{TODO: exlpain X}{}

\Ludo{TODO: express composition in terms of fililng holes like in def 7}{}


\begin{itemize}
\item \nmm{\OT{XS}{XC}{\tau}{\mbrc{}}{\top}{\mbrc{t \gets k, c \gets 0}}} which is the composition of \nmm{\OT{X}{X}{\tau}{\mbrc{C \mapsto \theta\mpar{x}, R \mapsto \act{set}\mpar{x}}}{\top}{\mbrc{}}} from the specification with \nmm{\OT{S}{C}{\act{set}\mpar{x}}{\mbrc{}}{\top}{\mbrc{t \gets x, c \gets 0}}} from the \Ludo{register}{what is the register? do you mean counter?} and \nmm{\OT{n}{n+1}{\theta\mpar{k}}{\mbrc{}}{\top}{\mbrc{}}} from \Ludo{the controller}{why is there no $n$ and $n+1$ in the global state?}.
...
\label{fig:tlf}
\end{figure}
\Ludo{Let's look}{avoid informal english} at the \(\tau\) transition from the state \(R\) for a more in depth example on how composition and value passing work.
The transition in the specification is \nmm{\OT{R}{R}{\tau}{\mbrc{R \mapsto \act{set}\mpar{x}, C \mapsto \theta\mpar{x}}}{\top}{\mbrc{}}}. \Ludo{confused}{it is not clear in the text when you write about transistions or states : clarify}
...
The holes \(R\) and \(C\) are \Ludo{filles}{active? required?} at the same time so the \(\tau\) transition must synchronise with a transition from both the \Ludo{register}{counter?} and the controller.

In the state \(RC\) the composition makes 2 transition \Ludo{from}{be more precise} the \(\tau\) transition but both have a false guard; \Ludo{Let's look}{avoid informal english} at one of them.
\nmm{\OT{RC}{\mpar{R, 3, C}}{\tau}{\mbrc{}}{\top \wedge \top \wedge c < t \wedge \act{set}\mpar{x} = \act{tick} \wedge \theta\mpar{x} = \delta\mpar{x_C}}{\mbrc{c \gets c + 1}}} is made with \nmm{\OT{2}{3}{\delta\mpar{x}}{\mbrc{}}{\top}{\mbrc{}}} from the controller and \nmm{\OT{C}{C}{\act{tick}}{\mbrc{}}{c < t}{\mbrc{c \gets c + 1}}} from the register.
Obviously the guard is false so  it is equivalent to have no transition.
\Ludo{rest of the paragraph}{why so much discussion on a useless transition? did I miss something?} Also the guard  can be simplified.
...
This transition is described before but \Ludo{Let's look}{avoid informal english} at the details of the composition.
...
We can see here how value passing works: the only possible value of \(x\) and \(x_R\) making the guard hold is \(17\), this value has been synchronised from the controller to \(x\) then to the second occurrence of \(x\) to the distinct \(x\) of the register.
This guard and \Ludo{value update}{what do you mean?} can be simplified to the one shown on the drawing.
Note that \nmm{\OT{S}{C}{\act{set}\mpar{t}}{\mbrc{}}{\top}{\mbrc{c \gets 0}}} wouldn't have worked as a transition for the controller because \(t\) is fixed before the transition and its value cannot be set outside of the variable update, hence the use of a local variable \(x\).
\end{exi}
...
\defitem \textbf{\Ludo{complete}{not sure whyy this is called completeness} w.r.t.\@ composition} iff \(\forall a\, b, a\mbrk{b} \leq a\);
\defitem \textbf{\Ludo{correct}{This eems very difficult to achieve so calling it correctness and implying that the other relations are not interesting is very strong} w.r.t.\@ composition} iff \(\forall a\, b, a \leq b \implies \exists c, a \overset{FH}= b\mbrk{c}\);
\defitem \textbf{context \Ludo{equivalent}{equivalent is strange here as there is no equivalence as far as I can see} for composition} iff \(\forall a\, b\, c, a \leq b \implies a\mbrk{c} \leq b\mbrk{c}\);
...
Compatibility with composition is the two latter at the same time \Ludo{?}{(plus transitivity?) it depends what you mean by ``at the same time''}.
Finally compatibility with FH-bisimulation is the refinement not being able to distinguish two FH-bisimilar automata.

The relations will have to be at least preorders, simulations and forbid the \Ludo{introduction of deadlocks}{define more precisely introduction of deadlocks or refer to a future definition} (states where no transition is possible) to be called refinement simulation.
On top of that weak refinements are refinement where some actions, typically one named \(\tau\), are considered not observable and do not have to be matched.
In  weak refinements, the relations have to forbid the introduction of livelocks (only a sequence of \(\tau\) possible) and respect \(\tau\)-stuttering \marginpar{cite many things arround here}.\Ludo{to add}{I believe we want more arguments on why we want some properties, at least deadlock and livelock related, perhaps also weak vs strong. How did you decide of the list of useful properties / when ais such or such property useful?}
...
\end{defi}
Now, we adapt the properties about states of automata to open automata.
...
\end{defi}
For the reader not used to simulations and bisimulations, this definition with \Ludo{a witness}{what is the witness in the definition above?} can seem strange.
...
The composition  \(A_p\subst{A_c}{k} = \OA{S}{s_0}{V}{\sigma_0}{J}{T}\) with \(A_c \defobject \OA{S_c}{s_{0c}}{V_c}{\sigma_{0c}}{J_c}{T_c}\), \(A_p \defobject \OA{S_p}{s_{0p}}{V_p}{\sigma_{0p}}{J_p}{T_p}\) and \(k \in J_p\), is a non-locking composition if:
...
\begin{thm}[Parallel non-locking substitution]
...
\end{gather}
By admitting and decomposing the left part of \goal{1} and \goal{2} we get the following:
\begin{align*}
...
\end{align*}
We use the fact that \(A\subst{C}{l}\subst{B}{k} \cong A\subst{B}{k}\subst{C}{l}\) on \hyp{3} to get: % TODO HERE: Add lemma about reachability implication for composition
...
\begin{proof}[Lemma 1] % TODO HERE: Always introduce hypothesis and goals on a big math env, recall definitions (also simulation) and call hypothesis H1, H2, ...
