...
\begin{proof}
	For three open automata \(A_1 \defobject \OAg[1]\), \(A_2 \defobject \OAg[2]\) and \(A_3 \defobject \OAg[3]\), let \(k \in J_1\), \(A_{13} \defobject A_1\subst{A_3}{k}\) and \(A_{23} \defobject A_2\subst{A_3}{k}\) with \(A_{13} = \OAg[13]\) and \(A_{23} = \OAg[23]\).
	Let \(R_{12}\) be a hole-equal simulation of \(A_1\) by \(A_2\) and \(\reach{A_{13}}\) be a witness that \(A_{13}\) is non-locking.
	The idea for the relation on configurations is that \(A_2\) simulates \(A_1\) while \(A_3\) does the same things in both compositions.
	We pose
	\[ R \defobject \mpar{\mpar{s_1, s_{31}}, \mpar{s_2, s_{32}}} \mapsto \choice{R_{12}\mpar{s_1, s_2} \wedge \reach{A_{13}}\mpar{s_1, s_{31}} \wedge \bigwedge_{v_3 \in V_3} v_{31} = v_{32} & \text{if } s_{31} = s_{32} \\ \bot & \text{otherwise}} \]
	Where \(y_{3x}\) is the renaming of \(y_3\) from \(A_3\) in \(A_{x3}\).
	We want to prove that \(R\) is a hole-equal simulation of \(A_{12}\) by \(A_{23}\):
\defitem \(J_{13} = J_3 \cup J_1 \setminus \mbrc{k} = J_3 \cup J_2 \setminus \mbrc{k} = J_{23}\) by \(J_1 = J_2\).
\defitem For \(\sigma_{013} \uplus \sigma_{023} \vdash R\mpar{s_{013}, s_{023}}\) we use the fact that \(R_{12}\) relates inital configurations and initial configuration is reachable:
	\begin{align*}
		...
		\iff & \mpar{\sigma_{01} \uplus \sigma_{02} \vdash R_{12}\mpar{s_{01}, s_{02}}} \wedge \mpar{\sigma_{013} \vdash \reach{A_{13}}\mpar{s_{013}}} \wedge \bigwedge_{v_3 \in V_3} \sigma_{031}\mpar{v_{31}} = \sigma_{032}\mpar{v_{32}} \\
	...
	\end{align*}
\defitem Let \(s_{13} \defobject \mpar{s_1, s_{31}} \in S_{13}\) and \(s_{23} \defobject \mpar{s_2, s_{32}} \in S_{23}\).
	...
	\begin{align*}
		...
		\end{array}\!\!} \goal{1}
	\end{align*}
	\[ V_{13} \uplus V_{23} \uplus \biguplus_\subbox{t_{23} \in \fOT{s_{23}}} \fvars{t_{23}} \vdash R\mpar{s_{13}, s_{23}} \wedge \bigvee_\subbox{t_{23} \in \fOT{s_{23}}} \fguard{t_{23}} \implies \bigvee_\subbox{t_{13} \in \fOT{s_{13}}} \fguard{t_{13}} \goal{2} \]
	...
	\begin{align*}
		...
		\end{array}} \hyp{5}
	\end{align*}
	Transition \(t_{13}\) is obtained in \(A_{13}\) by composition of two transitions \nmm{t_1 \defobject \OTx{1}{}{1}{1} \in \fOT{s_1}} and \nmm{t_3 \defobject \OTx{3}{}{3}{3} \in \fOT{s_3}} if \(k \in J'_1\) or by a the first transition and a fixed state \(s_3 \in S_3\) if \(k \notin J'_1\).
	We use \hyp{5} with the transition \(t_1\) to get
	\begin{gather*}
		\mpar{t_{2x} \defobject \OTx{2}{x}{2x}{2x} \in \fOT{s_2}}^{x \in X} \\
		\forall x \in X, J'_{2x} \cap J_1 = J'_1 \cap J_2 \hyp{5'} \\
		V_1 \uplus V_2 \uplus \fvars{t_1} \vdash R\mpar{s_1, s_2} \wedge g_1 \implies \operatorname*{\bigsymb{\bigvee}}_{x \in X} \mpar{\everymath{\displaystyle}\begin{array}{l}
			\alpha_1 = \alpha_{2x} \wedge \bigwedge_\subbox{j \in J'_{2x} \cap J_1} \beta_{1j} = \beta_{2xj} \\[12pt]
			\nwedge g_{2x} \wedge R\mpar{s'_1, s'_{2x}}\psubst{\psi_1 \uplus \psi_{2x}}
		\end{array}} \hyp{5"}
	...
	\begin{align*}
		...
		& = J'_{2x} \cap J_1 & \text{by \hyp{5'}} \\
		& = J'_{2x} \cap J_2 & \text{by } J_1 = J_2 \\
		& = J'_{2x} \hyp{6}
	\end{align*}
	Using \hyp{6} we can build the familly of transition generated by the family \(t_{2x}^{x \in X}\):
	\[ t_{23x}^{x \in X} \defobject x \mapsto \choice{
		\OT{\mpar{s_2, s_3}}{\mpar{s'_{2x}, s'_3}}{\alpha_{2x}}{\beta_{2xj}^{j \in J'_{2x} \setminus \mbrc{k}} \uplus \beta_{3j}^{j \in J'_3}}{g_{2x} \wedge g_3 \wedge \alpha_3 = \beta_{2xk}}{\psi_{2x} \uplus \psi_3} & \text{if } k \in J'_{2x} \\
		\OT{\mpar{s_2, s_3}}{\mpar{s'_{2x}, s_3}}{\alpha_{2x}}{\beta_{2xj}^{j \in J'_2}}{g_2}{\psi_2} & \text{if } k \notin J'_{2x}
	} \]
	We will prove \goal{1} for the family \(t_{23x}^{x \in X}\), the new goals are
	\begin{gather}
		\forall x \in X, J'_{23x} \cap J_{13} = J'_{13} \cap J_{23} \goal{1'} \\
		...
		\end{array}} \goal{3}
	\end{gather}
\item[\goal{1'}:] Let \(x \in X\), by case on \(k \in J'_1\):
	\begin{align*}
		\text{If } k \in J'_1: && J'_{23x} \cap J_{13} & = \mpar{J'_3 \cup J'_{2x} \setminus \mbrc{k}} \cap \mpar{J_3 \cup J_1 \setminus \mbrc{k}} \\
		&&& = \mpar{J'_3 \cap J_3} \cup \mpar{J'_{2x} \cap J_1} \setminus \mbrc{k} \\
		&&& = \mpar{J'_3 \cap J_3} \cup \mpar{J'_1 \cap J_2} \setminus \mbrc{k} & \text{by \hyp{5'}} \\
		&&& = \mpar{J'_3 \cup J'_1 \setminus \mbrc{k}} \cap \mpar{J_3 \cup J_2 \setminus \mbrc{k}} \\
		&&& = J'_{13} \cap J_{23} \\
		\text{If } k \notin J'_1: && J'_{23x} \cap J_{13} & = \mpar{J'_{2x} \setminus \mbrc{k}} \cap \mpar{J_1 \setminus \mbrc{k}} \\
		&&& = J'_{2x} \cap J_1 & \text{by } k \notin J'_1 = J'_{2x} \text{ \hyp{6}} \\
		&&& = J'_1 \cap J_2 & \text{by \hyp{5'}} \\
		&&& = \mpar{J'_1 \setminus \mbrc{k}} \cap \mpar{J_2 \setminus \mbrc{k}} & \text{by } k \notin J'_1 \\
		&&& = J'_{13} \cap J_{23}
	\end{align*}
\item[\goal{3}:] Let \(\sigma: V_{13} \uplus V_{23} \uplus \fvars{t_{13}} \to \values\).
	...
	\[ \sigma \vdash R\mpar{s_{13}, s_{23}} \wedge g_{13} \hyp{7} \]
	Hypothesis immediately breaken down into
	\begin{gather}
		\sigma \vdash R_{12}\mpar{s_1, s_2} \hyp{7a} \\
		\sigma \vdash \reach{A_{13}}\mpar{s_{13}} \hyp{7b} \\
		\sigma \vdash \bigwedge_{v_3 \in V_3} v_{31} = v_{32} \hyp{7c} \\
		\sigma \vdash g_{13} \hyp{7d}
	\end{gather}
	We use \hyp{5"} with valuation \(\sigma\) on variables \(V_1 \uplus V_2 \uplus \fvars{t_1}\), which is possible because the composition of transitions doesn't remove any variable (the simplification steps do, they give bisimilar automata).
	Then we get the values of the implicit \(\exists\) in \nmm{\nu: \biguplus_{x \in X} \fvars{t_{2x}}}.
	The left side of the implication is proved with \hyp{7a} and \hyp{7d}.
	So we get a \(x \in X\) such that
	\begin{gather}
		\sigma \uplus \nu \vdash \alpha_1 = \alpha_{2x} \hyp{8a} \\
		\sigma \uplus \nu \vdash \bigwedge_\subbox{j \in J'_{2x} \cap J_1} \beta_{1j} = \beta_{2xj} \hyp{8b} \\
		\sigma \uplus \nu \vdash g_{2x} \hyp{8c} \\
		\sigma \uplus \nu \vdash R_{12}\mpar{s'_1, s'_{2x}}\psubst{\psi_1 \uplus \psi_{2x}} \hyp{8d}
	\end{gather}
	If \(k \in J'_1\), we extend the valuation \(\nu\) to cover the variables of \(t_3\) in \(t_{23x}\) using their value in \(t_{13}\), \(\nu' \defobject \nu \uplus \mset{v_{32} \mapsto \sigma\mpar{v_{31}}}{v \in \fvars{t_3}}\)
	Otherwise \(\nu' \defobject \nu\).
	We progress on \goal{3} by giving the valuation \(\sigma \uplus \nu'\) to the implicit \(\exists\), admitting the left side of the implication and choosing to prove the branch \(x\) of the disjunction:
	\begin{gather}
	...
	\end{gather}
	Because composition doesn't change the produced actions nor its variables, \goal{3a} is proved by \hyp{8a}.
	\goal{3b} holds because the valuations for the actions of the holes in \(J_3\) coincide by definition of \(\nu'\) and by \hyp{8b} for the others.
	For \goal{3c} we have two cases, if \(k \notin J'_1\) then \(g_{23x} \equiv g_{2x}\) and \hyp{8c} is sufficient, if \(k \in J'_1\) then \(g_{23x} \equiv g_{2x} \wedge g_3 \wedge \alpha_3 = \beta_{2xk}\) so we also need \hyp{7d} (\(g_{13} \equiv g_1 \wedge g_3 \wedge \alpha_3 = \beta_{1k}\)) and \hyp{8b} for the value \(k\) (\(\beta_{1k} = \beta_{2xk}\)).
	\goal{3d} is proved using \hyp{8d}, \hyp{7b} with the preservation of reachability across transitions (+ \hyp{7d}), \hyp{7c} and the fact that \(A_3\) performs the same transition with same values in both composed automata:
	\begin{align*}
		& \sigma \uplus \nu' \vdash R\mpar{s'_{13}, s'_{23x}}\psubst{\psi_{13} \uplus \psi_{23x}} \\
		\iff & \sigma \uplus \nu' \vdash \mpar{R_{12}\mpar{s'_1, s'_{2x}} \wedge \reach{A_{13}}\mpar{s'_{13}} \wedge \bigwedge_{v_3 \in V_3} v_{31} = v_{32}}\psubst{\psi_1 \uplus \psi_{31} \uplus \psi_{2x} \uplus \psi_{32}} \\
		\iff & \sigma \uplus \nu' \vdash R_{12}\mpar{s'_1, s'_{2x}}\psubst{\psi_1 \uplus \psi_{2x}} \wedge \reach{A_{13}}\mpar{s'_{13}}\psubst{\psi_{13}} \wedge \bigwedge_{v_3 \in V_3} \psi_{31}\mpar{v_{31}} = \psi_{32}\mpar{v_{32}} \\
		\iff & \mpar{\sigma \uplus \nu \vdash R_{12}\mpar{s'_1, s'_{2x}}\psubst{\psi_1 \uplus \psi_{2x}}} \wedge \mpar{\sigma \vdash \reach{A_{13}}\mpar{s'_{13}}\psi_{13}} \wedge \mpar{\sigma \vdash \bigwedge_{v_3 \in V_3} v_{31} = v_{32}}
	\end{align*}
\item[\goal{2}:] Let \(\sigma: V_{13} \plus V_{23} \uplus \biguplus_\subbox{t_{23} \in \fOT{s_{23}}} \fvars{t_{23}} \to \values}.
	We use dummy values for the variables in \nmm{\biguplus_\subbox{t_{13} \in \fOT{s_{13}}} \fvars{t_{13}}} to get the left side of the implication (independant from dummy values), immediately broken into
	\begin{gather}
		\sigma \vdash R_{12}\mpar{s_1, s_2} \hyp{9a} \\
		\sigma \vdash \reach{A_{13}}\mpar{s_{13}} \hyp{9b} \\
		\sigma \vdash \bigvee_\subbox{t_{23} \in \fOT{s_{23}}} \fguard{t_{23}} \hyp{9c}
	\end{gather}
	\(R_{12}\) is a simulation and \(\reach{A_{13}}\) witnesses the non-locking simulation so we have the hypothesis:
	\begin{gather}
		V_1 \uplus V_2 \uplus \biguplus_\subbox{t_2 \in \fOT{s_2}} \fvars{t_2} \vdash R_{12}\mpar{s_1, s_2} \wedge \bigvee_\subbox{t_2 \in \fOT{s_2}} \fguard{t_2} \implies \bigvee_\subbox{t_1 \in \fOT{s_1}} \fguard{t_1} \hyp{10} \\
		V_{13} \uplus \biguplus_\subbox{t_1 \in \fOT{s_1}} \fvars{t_1} \vdash \reach{A_{13}}\mpar{s} \wedge \bigvee_\subbox{t_1 \in \fOT{s_1}} \fguard{t_1} \implies \bigvee_\subbox{t_{13} \in \fOT{s_{13}}} \fguard{t_{13}} \hyp{11}
	\end{gather}
	We use \hyp{10} with valuation \(\sigma\) projected on the variables from generating transitions in \(A_2\) to get a valuation \nmm{\nu: \biguplus_\subbox{t_1 \in \fOT{s_1}} \fvars{t_1}}.
	The left side of the implication is proved using \hyp{9a} and \hyp{9c} because the guards from transitions of \(A_2\) are still in transitions of \(A_{23}\), the right side gives:
	\[ \sigma \uplus \nu \vdash \bigvee_\subbox{t_1 \in \fOT{s_1}} \fguard{t_1} \hyp{10'} \]
	We then use \hyp{11} with valuation \(\sigma \uplus \nu\) to get \nmm{\nu': \biguplus_\subbox{t_{13} \in \fOT{s_{13}}} \fvars{t_13}}.
	The left side of the implication is proved using \hyp{9b} and \hyp{10'}, the right side gives:
	\[ \sigma \uplus \nu' \vdash \bigvee_\subbox{t_{13} \in \fOT{s_{13}}} \fguard{t_{13}} \hyp{11'} \]
	\goal{2} is proved for valuation \(\nu'\) by admitting the left side and using \hyp{11'}.
\end{proof}
\begin{prop}[Composition correctness'] Hole-subset simulation is correct w.r.t.\@ composition with automata without holes. \end{prop}
\begin{proof}
	...
	We pose
	\[ R \defobject \mpar{\mpar{s_{1'}, s_2}, s_1} \mapsto \choice{\bigwedge_{v_1 \in V_1} v_{1'} = v_1 & \text{if } s_{1'} = s_1 \\ \bot & \text{otherwise}} \]
	Where \(y_{1'}\) is the renaming of \(y_1\) from \(A_1\) in \(A\).
	We want to prove that \(R\) is a hole-subset simulation of \(A\) by \(A_1\):
	\begin{gather}
		J_2 \cup J_1 \setminus \mbrc{k} \subseteq J_1 \goal{5a} \\
		\sigma_0 \uplus \sigma_{01} \vdash R\mpar{s_0, s_{01}} \goal{5b}
	\end{gather}\vspace{-1cm}
	\begin{multline}
		\forall \mpar{s, s_1} \in S \times S_1, \\
		\mpar{\everymath{\displaystyle}\begin{array}{l}
			...
			\nwedge V \uplus V_1 \uplus \fvars{t} \vdash R\mpar{s, s_1} \wedge g \implies \operatorname*{\bigsymb{\bigvee}}_{x \in X} \mpar{\begin{array}{l}
			...
			\end{array}}
		\end{array}} \\
		\wedge V \uplus V_1 \uplus \biguplus_\subbox{t_1 \in \fOT{s_1}} \fvars{t_1} \vdash R\mpar{s, s_1} \wedge \bigvee_\subbox{t_1 \in \fOT{s_1}} \fguard{t_1} \implies \bigvee_\subbox{t \in \fOT{s}} \fguard{t} \goal{5c}
	\end{multline}
	\goal{5a} is easy because \(J_2 = \emptyset\).
	For \goal{5b} we use the fact that we don't constrain the behaviour of \(J_2\):
	\begin{align*}
		...
		\iff & \bigwedge_{v_1 \in V_1} \sigma_{01'}\mpar{v_{1'}} = \sigma_{01}\mpar{v_1} \\
		\iff & \top
	\end{align*}
	Let \(s \defobject \mpar{s_{1'}, s_2} \in S\) and \(s_1 \in S_1\).
	The goals left to prove are
	\begin{align*}
		& \bigsymb{\forall} t \defobject \OTg \in \fOT{s}, \bigsymb{\exists} \mpar{t_{1x} \defobject \OTx{1}{x}{1x}{1x} \in \fOT{s_1}}^{x \in X}, \\
		& \quad \mpar{\forall x \in X, J'_{1x} \cap J = J' \cap J_1} \\[-10pt]
		...
		\end{array}} \goal{6}
	\end{align*}
	\[ V \uplus V_1 \uplus \biguplus_\subbox{t_1 \in \fOT{s_1}} \fvars{t_1} \vdash R\mpar{s, s_1} \wedge \bigvee_\subbox{t_1 \in \fOT{s_1}} \fguard{t_1} \implies \bigvee_\subbox{t \in \fOT{s}} \fguard{t} \goal{7} \]
\item[\goal{6}:] % TODO: proove each goal
\item[\goal{7}:] % TODO: proove each goal
\end{proof}
...
\item[\(\eqref{eq:drWD}\Leftarrow\eqref{eq:drID}\):]
	Let \(\mpar{s_1, s_2} \in S_1 \times S_2\) and \nmm{\sigma: V_1 \uplus V_2 \uplus \biguplus_{t_2 \in \fOT{s_2}} \fvars{t_2} \to \values}.
	We want to prove:
...
\item[\(\eqref{eq:nlcWD}\Rightarrow\eqref{eq:nlcID}\):]
	...
	The \(\vdash\) hides an implicit \(\exists\) that we use to get values to the variables in \nmm{\biguplus_\subbox{t \in \fOT{s}} \fvars{t}} in a valuation \(\mu\).
...
\item[\(\eqref{eq:nlcWD}\Leftarrow\eqref{eq:nlcID}\):]
	Let \(s \defobject \mpar{s_p, s_c} \in S\), \nmm{\sigma \in V \uplus \biguplus_\subbox{t_p \in \fOT{s_p}} \fvars{t_p} \to \values}.
	We want to prove:
...
\defitem We prove that initial configurations are related using \(\sigma_{02}\) as the values of the variables in \(V_2\), and \(s_{02}\) to choose the branch of the disjunction.
\begin{align*}
	\sigma_{01} \uplus \sigma_{03} \vdash R_{13}\mpar{s_{01}, s_{03}} \iff & \sigma_{01} \uplus \sigma_{03} \vdash \exists V_2, \bigvee_\subbox{s_2 \in S_2} R_{12}\mpar{s_{01}, s_2} \wedge R_{23}\mpar{s_2, s_{03}} \\
...
\item[\goal{2}:] Let \(\sigma_1: V_1 \uplus \fvars{t_1} \to \values\) and \(\sigma_3: V_3 \to \values\).
	...
	\begin{align}
		...
		\sigma_1 \uplus \sigma_3 \uplus \nu_3 &\vdash R_{13}\mpar{s'_1, s'_{3z}}\psubst{\psi_1 \uplus \psi_{3z}} \goal{2d}
	\end{align}
	...
	\begin{align*}
		& \sigma_1 \uplus \sigma_3 \uplus \nu_3 \vdash R_{13}\mpar{s'_1, s'_{3z}}\psubst{\psi_1 \uplus \psi_{3z}} \\
