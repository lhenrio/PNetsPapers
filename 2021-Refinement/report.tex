...
\begin{defi}[Deadlock prevention, intuitive definition]\label{def:dpid}
...
\begin{defi}[Deadlock prevention, working definition]\label{def:dpwd}
...
The intuitive and working definitions of deadlock prevention are equivalent.
\end{lem}
...
\begin{proofsketch}
For three open autoamta \(A_1 \defobject \OAg[1]\), \(A_2 \defobject \OAg[2]\) and \(A_3 \defobject \OAg[3]\), let \(k \in J_1\), \(\mpar{R_{23}, H_{23}}\) be a hole-tracking simulation of \(A_2\) by \(A_3\) and \(\reach{A_1\subst{A_2}{k}}\) be a witness that \(A_1\subst{A_2}{k}\) is non-locking.
