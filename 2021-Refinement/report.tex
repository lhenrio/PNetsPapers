...
For instance an example of open pNet where equivalence is not sufficient was encountered in an article about algorithms fo weak bisimulation on open automata \cite{wang:hal-03126313}.
...
\section{Proof of equivalent definitions}\label{apx:lemeqd} % TODO: disjunction branch, say where we get the value
...
\item[\(\eqref{eq:drWD}\Rightarrow\eqref{eq:drID}\):]
	...
	At this point we have a certain \(t_1\) and \(t_2\) so we may be tempted to use them to prove the goal:
	\begin{multline}
...
We have the hypotheses on \(R_{12}\) and \(R_{23}\) for a specific \(s_2\), then the current goal:
\begin{gather}
...
\end{gather*}
Hypotheses \hyp{5} and \hyp{6} are considered with the obtained state \(s_2\).
