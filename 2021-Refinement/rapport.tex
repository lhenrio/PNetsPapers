...
\newcommand\sat{{\vdash}} % suppress spaces
\newcommand\fvars[1]{{\mathit{vars}\mpar{#1}}}
\newcommand\fterms[1]{{\mathcal{T}}}
\newcommand\fcterms[1]{{\mathcal{T}_{\makebox[1pt][r]{\(\scriptstyle 0\)}}}}
\newcommand\fformulae[1]{{\mathcal{F}}}
\newcommand\fcformulae[1]{{\mathcal{F}_{\makebox[1pt][r]{\(\scriptstyle 0\)}}}}
% \newcommand\fterms[1]{{\mathcal{T}\mpar{#1}}}
% \newcommand\fcterms[1]{{\mathcal{T}_{\makebox[1pt][r]{\(\scriptstyle 0\)}}\mpar{#1}}}
% \newcommand\fformulae[1]{{\mathcal{F}\mpar{#1}}}
% \newcommand\fcformulae[1]{{\mathcal{F}_{\makebox[1pt][r]{\(\scriptstyle 0\)}}\mpar{#1}}}
...
An expression algebra \(E\) is a disjoint union \(E \defobject \mathcal{T} \uplus \mathcal{F}\) where \(\mathcal{T}\) is a term algebra and \(\mathcal{F}\) the formulae over \(\mathcal{T}\).
% We also define convenience notations \(\fterms{E} \defnotation \mathcal{T}\) and \(\fformulae{E} \defnotation \mathcal{F}\).
\end{defi}
The terms algebra is arbitrary.
The formulae form at least a propositional logic over terms in \(\mathcal{T}\) and some relations, or equivalently they are at least first order logic formulae without quantifiers but with free variables allowed.
An example of term algebra can be Peano integers (zero, variable and successor function), the formulae associated can use the relations equality, \(\mathit{sum}\mpar{a, b, c} \defobject a = b + c\) and \(\mathit{prod}\mpar{a, b, c} \defobject a = b \times c\).
\begin{defi}[Free variables, Closed terms/formulae]
...
The closed formulae are \(\fcformulae{E} \defnotation \mset{f \in \fformulae{E}}{\fvars{f} = \emptyset}\); \(\fcformulae{E} \subset \fformulae{E}\).
\end{defi}
More generally a closed expression is an expression without unbound variable.
If the expression algebra has no binding construction then closed terms are terms without variables.
...
\item The satisfaction of closed formulae, \(\sat \subseteq \fcformulae{E} \to \mbrc{0, 1, ?}\);
...
% \item The formula interpretation of any variable assignement \(\psi \in \fterms{E}^V\), \nmm{\mdbrk{\psi} \defnotation \bigwedge_{v \in V} v = \psi\mpar{v}}; \(\mdbrk{\psi} \in \fformulae{E}\).
\end{itemize}
\end{defi}
...
The satisfaction of formulae is not supposed to be decidable, however it is better if it is ;).
...
\begin{itemize}
\item Let \nmm{f \in \fcformulae{E}}, \nmm{\vdash f_0 \defnotation \sat\mpar{f_0} = 1} and \nmm{\nvdash f_0 \defnotation \sat\mpar{f_0} = 0};
\item Let \nmm{f \in \fformulae{E}, \sigma \in \fvalues{E}^V}, \\
	\nmm{\sigma \vdash f \defnotation \exists \varphi \in \fvalues{E}^{\fvars{f} \setminus V}, f\psubst{\sigma \uplus \varphi} \in \fcformulae{E} \nwedge \vdash f\psubst{\sigma \uplus \varphi}}.
\end{itemize}
\end{defi}
The first notation is quite classical when satisfaction is defined as a relation rather than a function.
The second notation is the satisfiability of formulae under a (partial) valuation.
% \begin{defi}[Formulas combination, Expression algebra combination]
% The propositional logic built without existential quantifiers on top of two propositional logics \(\fformulae{E_1}\) and \(\fformulae{E_2}\) is \(\fformulae{E_1} + \fformulae{E_2}\).
% By extension \(E_1 + E_2 \defnotation \mpar{\fformulae{E_1} + \fformulae{E_2}} \uplus \mpar{\fterms{E_1} \cup \fterms{E_2}}\).
% \end{defi}
...
An open automaton is a tuple \(\OA{S}{i}{V}{\varphi}{J}{T}\) with \(S\) the set of states, \(i \in S\) the initial state, \(V\) the set of variable names unique to this automaton, \(\varphi \in \fvalues{E}^V\) the initial valuation of variables, \(J\) the set of hole names and \(T\) a set of open-transitions.

\(S, V, J\) are arbitrary sets, \Quentin{only \(J\) is required to be finite.}{Is \(S\) also required to be finite?}
\end{defi}
The variable names may clash when considering two automata, in that case we suppose that we can still distinguish the variables.
All the possible open transitions, from which transitions in \(T\) are selected, are defined below.
\begin{defi}[Open transition]
An open transition is a tuple \nmm{\OT{s}{s'}{\alpha\mpar{v_\alpha}}{\mpar{\beta_j\mpar{v_{\beta j}}}^{j \in J'}}{g}{\psi}} with \(s, s' \in S\) the source and target states, \(\alpha \in A\) an action label, \(J' \subseteq J\) the holes involved, \(\beta_j \in A\) the action labels of the holes, \(v_\alpha, v_{\beta j}^{j \in J'}\) fresh distinct variables, \(g \in \fformulae{E}\) the guard and \(\psi \in \fterms{E}^V\) the new value of the variables.

\Quentin{The set of action labels \(A\) is arbitrary and implicit.}{For the moment}
...
% TODO: Expand on why not to do that, using an example for instance
\begin{exi}
\end{exi}

% refinement on Open Automata
% Caveat (is blablabla a strict refinement) and what we may want (trace/run safety)
\begin{defi}[Refinement simulation correction] % Correction definition
\end{defi}
...
\begin{multline*}
%	\mathrm{OA}_1 \leq_= \mathrm{OA}_2 \defnotation \exists R \in \mpar{S_1 \times S_2} \to \fformulae{E}_1 + \fformulae{E}_2, \\
	\mathrm{OA}_1 \leq_= \mathrm{OA}_2 \defnotation \exists R \in \mpar{S_1 \times S_2} \to \fformulae{E}, \\
%	\mpar{\exists p \in \fformulae{E}_1 + \fformulae{E}_2, \quad \prel{R}{i_1}{i_2}{p} \qwedge \vdash p\psubst{\varphi_1 \uplus \varphi_2}} \\
...
\item 
\end{enumerate}
% TODO: How to prove a relation is a witness of a refinement
...
\item[Transitivity:] Let \(\mathrm{OA}_1, \mathrm{OA}_2, \mathrm{OA}_3\) be open automata with respectively variables in \(V_1, V_2, V_3\).
	...
	\begin{align*}
		...
		\mathit{merge}_{V_2}\mpar{p_{12}, p_{23}} \defobject & p_{12} \wedge p_{23} % TODO HERE
	\end{align*}
...
\begin{multline*}
%	\mathrm{OA}_1 \leq_\subseteq \mathrm{OA}_2 \defnotation \exists R \in \mpar{S_1 \times S_2} \to \fformulae{E}_1 + \fformulae{E}_2, \\
	\mathrm{OA}_1 \leq_\subseteq \mathrm{OA}_2 \defnotation \exists R \in \mpar{S_1 \times S_2} \to \fformulae{E}, \\
%	\mpar{\exists p \in \fformulae{E}_1 + \fformulae{E}_2, \prel{R}{i_1}{i_2}{p} \wedge \vdash p\psubst{\varphi_1 \uplus \varphi_2}} \\
...
\begin{multline*}
%	\mathrm{OA}_1 \leq_\# \mathrm{OA}_2 \defnotation \exists R \in \mpar{S_1 \times S_2} \to \fformulae{E}_1 + \fformulae{E}_2, \quad f \in J_1 \setminus J_2 \to J_2 \setminus J_1, \\
...
%	\mpar{\exists p \in \fformulae{E}_1 + \fformulae{E}_2, \prel{R}{i_1}{i_2}{p} \wedge \vdash p\psubst{\varphi_1 \uplus \varphi_2}} \qwedge f\mpar{J_1 \setminus J_2} = J_2 \setminus J_1 \\
