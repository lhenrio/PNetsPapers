...
behaviours of a modified program simulate those of to the source one,  and bisimulation relations  formalise the desired  relation between a specification and an implementation, 
...
Automata are convenient for specifying and verifying systems, but most automata definitions allow only the specifications of finite closed systems. Indeed those systems are the ones we can verify efficiently, but programming often consist in writing open systems that should be interfaced with others, and with potentially unbound behaviours (at least statically). We investigate in our works the possibility to define open symbolic systems, verify some of their properties, and use them to compose in a structured way more complex systems.


Refinement relations verify that and implementation satisfies some properties with respect to a specification. Refinement entails that one system can be considered as a more precise version of the specification, featuring all the specified behaviours with more concrete details. In this article we mostly rely on a simulation point of view, where all the specified behaviour must be followed by the refined system but additional behaviours may exist. However we also ensure that a while scenario, made of several steps, of the specification can also be simulated by the refined system, which is a bit richer than the traditional simulation relation.
...
In this article we mostly rely on a simulation point of view, where all the specified behaviour must be followed by the refined system but additional behaviours may exist. However we also ensure that a while scenario, made of several steps, of the specification can also be simulated by the refined system, which is a bit richer than the traditional simulation relation.
...
Families of values, or equivalently maps will be noted \(\mset{i \mapsto x_i}{i \in I}\), or \(\mset{i \gets x_i}{i \in I}\), or \(x_i^{i \in I}\) depending on what is more convenient. % TODO: introduire le fait que \exists c_j^{j \in J} défini J et {j \mapsto c_j}
...
\begin{figure}[h]
 \centering
   ...
   \caption{Open Automaton of \texttt{counter} process (left).  Its role is to get a target, then count ticks until that target is reached, then emits an action with the elapsed time and restart. And Open Automation of  the composition \texttt{database}[\texttt{counter}] (right).\label{Fig:counter}}
\end{figure}
...
The idea is that we rely on a classical notion of simulation inspired from~\cite{fh-bisim}. The idea is that two states related by a given relation and states that if one state can do a transition, then the other can do a transition too. But to limit the creation of deadlock when simulating an automaton, we first define when a relation is preserved along identical transition  before characterising a first simple refinement relation  to compare automata with identical holes. 

%\subsection{Deadlock Reduction, Reachability, and Composition}

Let us first explain the reasons why simulation can introduce deadlocks and how we want too limit them. 
The automaton that simulates a specification features any behaviour that contains the behaviour of the specification, or more precisely, for each state that simulates a state of the specification, the simulating automaton must at least feature the behaviour of the specification.
 In this definition, nothing prevents the implementation from adding non-determinism and non-deterministically reaching a state that is unrelated to the specification, this state could be a deadlock even though we only followed transitions that exist in the specification.
...
Let's consider any $s_{2p} \in (s_{2p})^{p\in P}$. According to the refinement relation between  $A_1$ and $A_2$ and Definition \ref{def:preservation} we have:
...
Furthermore, according to the refinement relation between  $A_2$ and $A_3$ and Definition \ref{Def:OA-Refinement} we have  for any open transition $t_{2px}$ in $T_2$ originating from $s_{2p}$ the following:
\begin{align*}
\OTx{2p}{x}{2px}{2px} \in \fOT{s_{2p}}
\end{align*}
there exists an indexed family of OTs originating from $s_3$: 
\begin{align*}
\mpar{\OTx{3}{py}{3py}{3py} \in \fOT{s_3}}^{y \in Y} 
\end{align*}
such that $\forall y \in Y, J'_{2p} \cap H = J'_{3py} \cap H$ and
\begin{multline*}
%\everymath{\displaystyle}
V_2 \uplus V_3 \uplus \fvars{t_{2p}} \vdash\\
R_{23}\mpar{s_{2p}, s_{3}} \wedge g_{2p} \implies  \operatorname*{\bigsymb{\bigvee}}_{y \in Y} \mpar{\everymath{\displaystyle}
\begin{array}{l}
