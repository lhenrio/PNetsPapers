...
\begin{figure}[h]
 \centering
   ...
   \caption{Open Automaton of \texttt{counter} process (left).  Its role is to get a target, then count ticks until that target is reached, then emits an action with the elapsed time and restart. And Open Automaton of  the composition \texttt{database}[\texttt{counter/cnt}] (right).\label{Fig:counter}}
\end{figure}
