...
\begin{itemize}
\item if $k \in J'_1$:\\
Because composition doesn't change the resulting actions, nor their variables, we can extend the valuation context of the variables to cover the variables of the transition $t_3$.   By building  $t_{13}$ and $t_{23}$ we have $\alpha_{13}=
\alpha_{1}$ and $\alpha_{23x}=\alpha_{2x}$. So we deduce: 
$\alpha_{1} = \alpha_{2x} \Rightarrow \alpha_{13} = \alpha_{23x}$.\\
We have also:\\ $\beta_{13j}^{J'_{13}}= \beta_{1j}^{j \in J'_{1}  \setminus \{k\}} \uplus   {(\beta^1_{3j})^{j \in J^{1 \prime}_3} }$
and $\beta_{23xj}^{J'_{23}}= \beta_{2xj}^{j \in J'_{2x}  \setminus \{k\}} \uplus   {(\beta^2_{3j})^{j \in J^{2 \prime}_3} }$\\
So we have:  $\beta_{13j} = \beta_{23xj} \Rightarrow (\beta_{1j} = \beta_{2xj}) \vee (\beta^1_{3j}= \beta^2_{3j})$\\
Considering $\beta^1_{3j}$ and  $\beta^2_{3j}$  are the same (up  renaming) hence we have: $V^1_{3} \uplus V^2_{3}  \vdash {\displaystyle \bigwedge_\subbox{j \in J^{2 \prime}_3} \beta^1_{3j}= \beta^2_{3j}}$\\

Assuming we have $V_{1} \uplus V_{2} \uplus \fvars{t_1}  \vdash {\displaystyle \bigwedge_\subbox{j \in J'_{2x} \cap H} \beta_{1j} = \beta_{2xj}}$ then we can deduce:
\begin{align*}
...
\end{align*}
On another side,   the composition gives $g_{13} \Leftrightarrow g_1 \wedge g^1_3 \wedge \alpha^1_3=\beta_{1k}$ and
$g_{23x}  \Leftrightarrow g_{2x} \wedge g^2_3 \wedge \alpha^2_3=\beta_{2xk}$. \\
In addition as $k \in H$ we have   
...
\begin{multline*}
V_{13} \uplus V_{23} \uplus \fvars{t_1}  \uplus \fvars{t_{13}} \vdash\\
...
Considering that ate each step the valuations $\sigma_{3}^1$ and $\sigma_{3}^2$ associate the same values to the ``same'' variables modulo renaming, we have the tautology:\\ $\left({\displaystyle \bigwedge_\subbox{v_3\in V_3} v^1_3=v^2_3\wedge s^1_3 = s^2_3}\right)\psubst{\sigma_{3}^1 \uplus \sigma_{3}^2 }$.
...
\end{multline*}
By hypothesis we also have \(A_1\subst{A_3}{k}\) is non-blocking,  thus we can write the following:
\[ \fvars{t_{13}} \vdash \left(\reach{A_{13}}\mpar{s_{13}} \wedge g_{13} \implies \reach{A_{13}}\mpar{s_{13}'}\psubst{\psi_{13}}\right) \]	
\end{enumerate}
