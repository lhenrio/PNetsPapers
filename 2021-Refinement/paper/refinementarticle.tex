...
Families of values, or equivalently maps will be noted \(x_i^{i \in I}\), \(\mset{i \mapsto x_i}{i \in I}\), or \(\mset{i \gets x_i}{i \in I}\), depending on what is more convenient. % TODO: introduire le fait que \exists c_j^{j \in J} défini J et {j \mapsto c_j}
...
\begin{figure}[h]
 \centering
   ...
   \caption{Open Automaton of \texttt{counter} process (left).  Its role is to get a target, then count ticks until that target is reached, then emits an action with the elapsed time and restart. And Open Automation of  the composition \texttt{database}[\texttt{counter/cnt}] (right).\label{Fig:counter}}
\end{figure}
...
This definition leads to a notion of simulation as refinement the same as the one used in LOTOS??? and presented under the name extension refinement.
