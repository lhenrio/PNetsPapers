...
% TODO: This version of simulation/trace inclusion doesn't fit our needs for another reason,
% in our case all behaviours of the implementation do actually belong to the behaviours of the specification with our refinement (it seems counter-intuitive so it may be worth explaining):
% Our specifications says "you must do that the way you choose to do it" (more precisely "the way the automaton in this hole chooses to do it as long as the guard is satisfied"), the implementation says "I do that this specific way"
% so the behaviour of the implementation is a behaviour allowed by the specification, in particular it is included in the behaviours (of/allowed by) the specification
...
\TODO{The first and second conditions coincide with the natural way to prove inductively that an automaton simulates another by starting with the initial state. The third condition ensures that the refinement relation prevents the introduction of deadlocks.}
