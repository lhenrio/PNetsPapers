...
\newpage
\TODO{[RABEA] proof}
Let's consider any $s_{2p} \in (s_{2p})^{p\in P}$. According to the refinement relation between  $A_1$ and $A_2$, we have  the following formula that we denote ($*$):\\
for any open transition $t_1$ in $T_1$  originating from $s_1$:
\begin{align*}
\OTx{1}{}{1}{1} \in \fOT{s_1}
\end{align*}
there exists an indexed family of OTs originating from $s_{2p}$: 
...
\begin{array}{l}
		...
		\end{array}}} 
\end{multline*}	
On the other hand, according to the refinement relation between   $A_2$ and $A_3$, we know condition 3 is satisfied, $\bigforall \mpar{s_{2p}, s_3} \in S_1 \times S_3$ and $\bigsymb{\forall} \mpar{t_{2p}, t_3} \in \fOT{s_{2p}} \times \fOT{s_3}$ and in particular for  the set $(t_{2px})^{x\in X_p}$ so we  have the following that we denote ($**$):
\begin{multline*}
