...
\TODO{Notion of Extension refinement. May be we need to introduce the notion of  refusal of
a trace $t \in \mathcal{T}$ is a set of actions such that all actions in the set can be refused after the automaton
has executed the trace $t$. \[\bot_{A}(t) \Leftrightarrow \{\exists s. s \xRightarrow {t} s' \wedge \forall a \in \bot_A(t). s \not {\xrightarrow{a}}    \}\] and replace deadlock reducing, under (3) by
\[\bot_{A_2}(t) \subseteq  \bot_{A_1}(t) \text{ for all trace} \] } 
