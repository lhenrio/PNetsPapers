...
By combining  formula $(**)$ in $(*)$, we get: 
\begin{multline*}
...
\end{array}}
\end{multline*}
Since $\psi_{2px}$ has no effect on variables of $A_1$ and $A_3$, it then implies:
\begin{align*}
...
\end{multline*}	
\medskip
\item Finally, the satisfaction of the condition of deadlock reducing. According to the relation between   $A_2$ and $A_3$ we have for all $\mpar{s_{2p}, s_3} \in S_2 \times S_3$:
\begin{multline*}
...
* or by  the transition $t_1=\OTx{1}{}{1}{1}$ alone with the state $s^1_3$ unchanged, if $k \not\in J_1'$
...
& = (J'_1\cap (J_3 \uplus H)) \uplus ((J^{1 \prime}_{3} \cap (J_3 \uplus H)) \setminus \mbrc{k} \text{ since } J_3\cap J'_{1} = \emptyset \text{ and } H\cap J^{1 \prime}_{3} = \emptyset\\
		...
		& = J'_{13} \cap H'\\
\text{if } k \notin J'_1:\quad &	J'_{23x} \cap H'  = J'_{2x}  \cap \mpar{J_3 \uplus H \setminus \mbrc{k}} \\
...
Third, considering $\beta^1_{3j}$ and  $\beta^2_{3j}$  are the same (up to renaming)  we have: 
...
We can deduce for any transition $t_3$ in $A_3$ relying $s_3$ and $s^{\prime}_3$ we have:
\begin{multline*}
V_{13} \uplus V_{23}  \uplus \fvars{t_{13}} \vdash\\
...
\begin{array}{l}
			\alpha_{13} = \alpha_{23x} \wedge \bigwedge_\subbox{j \in J'_{23x} \cap H'} \beta_{13j} = \beta_{23xj} \nwedge g_{23x}\land\\
			 \wedge R\mpar{s'_1, s'_{2x}}\psubst{\psi_1 \uplus \psi_{2x}}
 \bigwedge_\subbox{v_3\in V_3}\!\! \psi_{13}(v^1_3)=\psi_{13}(v^2_3) \wedge s^{1\prime}_3 = s^{2\prime}_3
		\end{array}}   
\end{multline*}
And finally By hypothesis we have  \(A_1\subst{A_3}{k}\) is non-blocking,  then according to the Definition \ref{Def:Reach} we have $ \sigma_{013} \vdash \reach{A_{13}}\mpar{s_{013}}$ and for all $t_{13} \in T_{13}$:
...
\wedge \bigwedge_\subbox{v_3\in V_3}  \psi_{13}(v^1_3)=\psi_{13}(v^2_3)\wedge s^{1\prime}_3=s^{2\prime}_3
		\end{array}}   
\end{multline*}	
Because of the independence of the substitution domains, we get the expected formula:
	\begin{multline*}
...
\item if $k \notin J'_1$:\\
The proof of this case follows the same steps as the previous case the only argument that changes is that  $g_{13} \Leftrightarrow g_1$ and
...
\end{multline*}
We have a second hypothesis stating that the result of the composition \(A_1\subst{A_3}{k}\) is non-blocking, which leads to:
\begin{multline*}
...
\end{multline*}
We use the the first, we extend the valuation to cover the variables of  $t^2_3$ in $t_{23}$ using their value in $t_3$ and because $\fvars{t_{13}} \vdash \reach{A_{13}}\mpar{s_{13}}$, we get:
\begin{multline*}
...
\end{multline*}
By transitivity with the second hypothesis we obtain:
\begin{multline*}
...
$\mpar{\sigma_{02} \uplus \sigma_{03} \vdash R\mpar{s_{02}, s_{03}}}$,  and relying on the  fact that initial states are reachable $\sigma_{012} \vdash \reach{A_{12}}\mpar{s_{012}}$ we have:
\[ \mpar{\sigma_{02} \uplus \sigma_{03} \vdash R\mpar{s_{02}, s_{03}}} \wedge 
\mpar{\sigma_{012} \vdash \reach{A_{12}}\mpar{s_{012}}}\]
Because initial valuations $\sigma_{01}^1$ and $\sigma_{01}^2$ associate the same values to the ``same'' variables modulo renaming, we have the tautology:\\ $\left(\,\,{\displaystyle \bigwedge_\subbox{v_1\in V_1} v^1_1=v^2_1\wedge s^1_1 = s^2_1}\right)\psubst{\sigma_{01}^1 \uplus \sigma_{01}^2 }$.
...
* either by the composition of a transition $t_2=\OTx{2}{}{2}{2}$ and a transition \[t_1^1=
...
* or by an the transition $t_1^2=
\OT{ s^2_1}{{s^2_1}'}{\alpha_1^2}{{(\beta^2_{1j})^{j \in J^{2 \prime}_1} }}{ g^2_1 }{ \psi^2_1}$  alone with the state $s_2$ unchanged, if $k \not\in {J^1_{1}}^{\prime}$
\[t_{12} = \OT{\mpar{s^2_1, s_2}}{\mpar{{s^2_1}', s'_2}}{\alpha_1^2}{ {(\beta^2_{1j})^{j \in J^{1 \prime}_1} }}{g^2_1}{\psi^2_1}
\]
We then obtain a family of OTs by the simulation of $A_2$ by $A_3$ (stated above).
As  $k \in H$ we have $k \not\in J'_{3x}$ and we build:
\[t_{13x} = \left(
\OT{\mpar{s^1_1, s_3}}{\mpar{s^2_1,s'_{2x}}}{\alpha_{3x}}{\beta_{3xj}^{j \in J'_{3x}}}{g_{3x}}{\psi_{3x}} \right)^{x\in X}\]

According to how the transition $t_{12}$ is built we have to consider both cases to prove the following:
 $\mpar{\forall x \in X, J'_{13x} \cap H' = J'_{12} \cap H'}$ and
\begin{multline*}	
V_{12} \uplus V_{13} \uplus \fvars{t_{12}} \vdash\\ R'\mpar{s_{12}, s_{13}} \wedge g_{12} \implies \operatorname*{\bigsymb{\bigvee}}_{x \in X} \mpar{\everymath{\displaystyle}\begin{array}{l}
			\alpha_{12} = \alpha_{13x} \wedge \bigwedge_\subbox{j \in J'_{13x} \cap H'} \beta_{12j} = \beta_{13xj} \wedge\\[12pt]
			 g_{13x} \wedge R'\mpar{s'_{12}, s'_{13x}}\psubst{\psi_{12} \uplus \psi_{13x}}
...
For all $x \in X$ we have to consider both  cases:
\begin{align*}
	\text{if } k \in {J^1_{1}}^{\prime}:\quad &  J'_{13} \cap H'  
...
& = J'_{12}\cap H'\\
\text{if } k \notin {J^1_{1}}^{\prime}:\quad &	J'_{13x} \cap H'  = J'_{3x}  \cap \mpar{J_1 \uplus H \setminus \mbrc{k}} \\
...
For the rest of the formula we will also consider the two cases separately. 
\begin{itemize}
\item if $k \in {J^1_{1}}^{\prime}$:\\
As before, the proof  is based  on the fact that the composition doesn't change the resulting actions, nor their variables, we can extend the valuation context of the variables to cover the variables of the transition $t_3$.   By construction of  $t_{12}$ and $t_{13}$ we have $\alpha_{12}=
\alpha^1_{1}$ and $\alpha_{13x}=\alpha^2_{1}$, so we deduce: 
$\alpha_{12} = \alpha_{13x}$. In addition we have:\\
 $\beta_{12j}^{j\in J'_{12}}= {{\beta^1_{1j}}^{j \in J^{1 \prime}_1 \setminus \{k\}} \uplus  {(\beta_{2j})^{j \in J^{\prime}_2} }}$ and $\beta_{13xj}^{j\in J'_{13}}= {{(\beta^2_{1j})^{j \in J^{2 \prime}_1}}\uplus \beta_{3xj}^{j \in J'_{3x}  \setminus \{k\}} }$\\

Therefore, we have for all $j\in J'_{12}$ (recall that $J'_{12}=J'_{13}$):  \[\beta_{12j} = \beta_{13xj} \Rightarrow (j\in J^{1 \prime}_1 \land \beta^1_{1j}= \beta^2_{1j}) \vee  (j\in J_2 \land \beta_{2j} = \beta_{3xj})  \]\\
Third, considering $\beta^1_{3j}$ and  $\beta^2_{3j}$  are the same (up to renaming)  we have: 
${\displaystyle V^1_{1} \uplus V^2_{1} \uplus \fvars{t_{12}} \vdash \bigwedge_\subbox{v_1\in V_1}\!\! v^1_1=v^2_1\implies \bigwedge_\subbox{j \in J^{\prime}_1} \beta^1_{1j}= \beta^2_{1j}}$
\end{itemize}
