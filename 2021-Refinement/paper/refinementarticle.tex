...
perhaps say that one of the automaton is expected to be the reinement of the other BUT the def is not dependent on the notion of refinement

Beware implicit existential quantification on vars of t1

Next definition is not intuitive, it does not say much because it says that if t2 is not in a deldlock position then t1 can do a transition but not necessarily with the same input values. This is very weak but sufficient when composed with the def  of simulation.
...
\proof We consider different conditions:
\begin{enumerate}
...
R_{12}\mpar{s_{01},s_{02}}\psubst{\sigma_{01} \uplus \sigma_{02} \uplus \sigma_{03}} \wedge R_{23}\mpar{s_{02}, s_{03}}\psubst{\sigma_{01} \uplus \sigma_{02} \uplus \sigma_{03}}\implies\\R_{12}\mpar{s_{01},s_{02}}\wedge R_{23}\mpar{s_{02}, s_{03}}\psubst{\sigma_{01} \uplus \sigma_{02} \uplus \sigma_{03}}
\end{multline*}
\TODO{I will continue???}
\item For any open transition $t_1$ in $T_1$  originating from $s_1$:
\begin{align*}
...
\end{align*}
such that $\mpar{\forall x \in X, J'_{2x} \cap H = J'_1 \cap H}$ and
\begin{multline*}
...
For any open transition $t_{2x}$ in $T_2$, since  originating from $s_2$:
\begin{align*}
...
\end{align*}
such that $\mpar{\forall y \in Y, J'_{2x} \cap H' = J'_{3xy} \cap H'}$ and
\begin{multline*}
\everymath{\displaystyle}
V_2 \uplus V_3 \uplus \fvars{t_{2x}} \vdash\\\hspace{2em} \mpar{R_{23}\mpar{s_1, s_{2x}} \wedge g_{2x} \implies \operatorname*{\bigsymb{\bigvee}}_{y \in Y} \mpar{\everymath{\displaystyle}
\begin{array}{l}
			 \alpha_{2x}=\alpha_{3xy} \wedge \bigwedge_\subbox{j \in J'_{3xy} \cap H'} \beta_{2xj}=\beta_{3xyj} \\
			\nwedge g_{3xy} \wedge R_{23}\mpar{s'_{2x}, s'_{3xy}}\psubst{\psi_{2x} \uplus \psi_{3xy}}
		\end{array}}} 
\end{multline*}
\item For the third condition. We have: 
...
We get:
\begin{multline*}
 V_2 \uplus V_3 \uplus \biguplus_\subbox{t_3 \in \fOT{s_3}} \fvars{t_3} \vdash\\ \left(R_{12}\mpar{s_1, s_2} \wedge R_{23}\mpar{s_2, s_3} \wedge \bigvee_\subbox{t_3 \in \fOT{s_3}} \fguard{t_3} \implies R_{12}\mpar{s_1, s_2} \wedge \bigvee_\subbox{t_2 \in \fOT{s_2}} \fguard{t_2} \right)
 \end{multline*}
 But we have already:
\begin{multline*}
