...
\cite{GayH:2005,BravettiZ:2021}. Such refinement is  instead based  on interface-oriented approach, it allows the expression more internal choices and less external choices. The refinement of open automata is defined in terms of  alternating simulation \cite{Alur:1998,deAlfaro:2021}, which is based on a process-oriented approach. Alternating simulation that is originating from the game theory \cite{deAlfaro:2003} allows  the study of relation between individual components by viewing them as alternating transition systems. In particular,  a refinement
of automata expresses that the refined component can offer more services (input actions) and fewer service demands (output actions).
The result of the composition of automata may
contain illegal states, where one automaton issues an output that is not acceptable as input in the other one. An optimistic approach to compute compatibility between automata based on the fact that each automaton expects the other to provide  legal inputs, i.e, they can be composed together. Although our approach to design refinement differs from that of the above mentioned \cite{deAlfaro:2021},  because it is based on the notion of simulation, it has some commonalities:  our formalism is based on transition systems and on  optimistic approach to composition. 
...
\subsection{Deadlock Preservation, Reachability, and Composition}

\begin{definition}[Deadlock reduction]\label{def:dpwd}
Let \(\OAg[1]\), \(\OAg[2]\) be two OAs.
...
\TODO{explain in particular why guards do the trick}
...
For any open automata \(A = \OAg\), a reachability predicate \(\reach{A}: S \to \rformulas[V]\) is any predicate on states that is preserved across transitions, and valdi on initial state:
