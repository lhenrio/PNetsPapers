...
We generalise simulation between labelled transition systems to Hole-equal simulation  between  open automata that have exactly the same holes.
...
states $(s_1,s_2)$  of the two automata, and for every each transition of the first automaton from $s_1$, there is a matching transitions  of the second automaton  from $s_2$ such that the produced action match, the actions of the same holes and the successors are related after variable update. 
