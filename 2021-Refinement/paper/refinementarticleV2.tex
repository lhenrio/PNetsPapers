...
The refinement concept also plays  an important role in software engineering. In addition to helping to cope with the complexity of requirements and design \LUDO{\st{of what?}}, refinement provides a foundation for ensuring  system (software) correctness. The correctness of a system can
...
Refinement entails that one system  can be considered as a more precise version of another one that is considered to be the specification. The refined model features all the specified behaviours with more concrete details. From a formal point of view,  refinement can be defined as a mathematical relations between a specification and its implementation, one frequently used relation is trace inclusion or simulation  \cite{Milner:1980,Kouchnarenko:2007}. Each semantic treatment \LUDO{\st{each what?}} offers specific guarantees on the  behavioural properties of the specification, and it \LUDO{what is ``it''?} ensures that all behaviours of the specification must belong to the behaviours of the implementation. This refinement semantics is  used to study the behaviour of  closed
systems \LUDO{\st{which definition?}}, i.e.  systems whose behaviour is fully defined. It is not well-suited for open systems,  as it requires to reason also about unspecified behaviours.
...
The origins of the simulation are in the  approach of programming that aims to provide solid foundations for building correct programs \cite{Dijkstra76}.  Many work  contributed to the  development of the refinement calculus \LUDO{TODO: define refinement calculus} in various area (e.g. \cite{BACK1990133,Abrialbooks96,soton250550,Bellegarde:2000}).
\RAB{This development approach is primarily concerned with  whether a program is correct with respect to given properties, and how to improve (refine) a program while preserving its correctness.}
 However, these works focused on the analysis and applicability of refinement to systems that are inherently closed, i.e., systems which are bounded and without environment,
...
\cite{GayH:2005,BravettiZ:2021}. This kind of refinement is  based  on an interface-oriented approach, it allows the expression of more\LUDO{more and lesss than what???} internal choices and less external choices. The refinement of open systems is also defined in terms of  alternating simulation \cite{Alur:1998,deAlfaro:2021}.
...
all linear temporal logic properties \LUDO{add ref!}.  Our focus is about the properties of simulation, in particular closure properties such as preservation under composition. Some works   have addressed the problem of non-introduction of blocking by composition. For example,  \cite{DIHEGO2020110598}  presented a refinement relation on process algebra based on failure semantics that is basically different from the (bi)simulation \cite{10.5555/640428.640430}.   \LUDO{the last sentence is not really informative} \LUDO{how do we compare our work?}
