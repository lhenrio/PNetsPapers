...
We define refinement relations between systems including either equal or different sets of holes, and 
...
The refinement concept also plays  an important role in software engineering. In addition to helping to cope with the complexity \LUDO{of what?}, refinement provides a foundation for ensuring  system (software) correctness. The correctness of a system can
...
Refinement entails that one system  can be considered as a more precise version of another one that is considered to be the specification. The refined model features all the specified behaviours with more concrete details. From a formal point of view,  refinement can be defined as a mathematical relations between a specification and its implementation, one frequently used relation is trace inclusion or simulation  \cite{Milner:1980,Kouchnarenko:2007}. Each \LUDO{each what?} offers specific guarantees on the  behavioural properties of the specification, and it \LUDO{what is ``it''?} ensures that all behaviours of the specification must belong to the behaviours of the implementation. This definition \LUDO{which definition?}, which is based on systems whose behaviour is fully defined, is not well-suited for open systems,  as it requires to reason also about unspecified behaviours.



Open automata (that we abbreviate OA) were defined as a way to provide a semantics for open parameterized hierarchical labelled transition systems (abbreviated LTS). They were proposed as a theoretical foundation for parametrised automatas used in verification tools and called \emph{pNets}.
...
\section{Simulation Relations for automata with the same holes}\label{sec:refinement}
...
For example,
With such a relation we are able to check the simulation between two OA that differ by duplicated states, removed duplicated transitions,  reinforced  guards, different variables, etc.
...
This section  extends the preceding relation to automata where the set of holes is not the same. Indeed, when composing the set of  holes changes, and for example we want to state whether the automaton after composition is a simulation of the original automaton or not.
...
In general, one would not want to only consider non-blocking composition as it may reveal a bit restrictive, but it is the best necessary condition that can be identified for compositionality of simulation. It will be used to prove composition theorems in  Section~\ref{sec:prop}. 
Proving simulation relation in absence of non-blocking composition may also checked specifically for the composed automaton under study.
...
Appendix~\ref{sec:proof-transitivity} presents the proof of transitivity. It is done classically by identifying the relation between $A_1$ and $A_3$ that is a simulation. What is less classical is the definition of this relation because it is a boolean formula. For each couple of states  $s_1$ and $s_3$ of $A_1$ and $A_3$ we build a  formula that defines the simulation. To do this, we take the disjunction of formulas relating $s_1$ and $s_3$, and passing by all states $s_2$ of $A_2$. More precisely, we define a relation of the following form:
  \[\mathcal{R}_{13}(s_1,s_3)=\bigvee_{s_2\in S_2}\mpar{\mathcal{R}_{12}(s_1,s_2) \land R_{23}(s_2,s_3) } \]
...
The origins of the simulation are in the  approach of programming that aims to provide solid foundations for building correct programs \cite{Dijkstra76}.  There many work that contributed to the further development of the refinement calculus in various area (e.g. \cite{BACK1990133,Abrialbooks96,soton250550,Bellegarde:2000}). However, the work focused on the analysis and applicability of refinement to systems that are inherently closed, i.e., systems which is bounded and without environment,
 
There is not a lot of works that has focused on  the refinement of open systems. Defining refinement of open systems as trace inclusion  is  addressed  as a notion of subtyping in type theory 
...
our approach to design simulation  has some commonalities with that of the above mentioned \cite{deAlfaro:2021}: both are process-oriented approach even if they are not based on the same notion of simulation and they are based on optimistic approach to composition. 
...
In  \cite{10.1145/3372884.3373161}, a computable bisimulation is introduced, while in \cite{AMHEEMA:2023} is introduced a weak version of the bisimulation on OA. 
In this paper we tackle the refinement relation in the form of simulation, as is the case for the corresponding relations on labelled transition systems \cite{Bellegarde:2000}. But unlike the standard simulation we deal with symbolic and open models. In \cite{Zhang2014}, the authors exploit transition systems to reason about the systems  that are partially specified by using variables, making  the state space potentially infinite.
...
& \qquad\qquad\qquad \text{ since } J^{1 \prime}_{1}=J^{2 \prime}_{1} \text{ and } J'_2 \cap H = J'_{3x} \cap H\\
...
Thus we have, as $\mathcal{R}$ contains the reachability of state $s_{12}$:\TODO{faire une jolie equation}
\begin{multline*}
V_{12} \uplus V_{13}  \vdash  \mathcal{R}'\mpar{s_{12}, s_{13}} \nwedge  \neg(\bigvee_\subbox{t_{12} \in \fOT{s_{12}}} \fguard{t_{12}}) \\
