...
The open automaton  modelling the behaviour of such a system  using an  unbounded circular/ring buffer is depicted in Fig. \ref{Fig:RefineOA}.  The automaton has a single state with  two holes: $\texttt{P}$ and  $\texttt{Q}$ that are the two interacting processes.  $\texttt{l}$   indicates the next available position for  enqueuing an element  and  $\texttt{f}$ is the position that contains the next element to be dequeued. %\footnote{The  arrow  without any state of origin is a graphical convention to indicate that the pointers are initialized at the beginning}. 
...
To illustrate the refinement simulation of open automata, we consider a variation on the \texttt{prod-cons} example. Namely,  we suppose that  the two processes  \texttt{P} and  \texttt{Q} communicate through a one-place  buffer. The  corresponding behaviour of this simpler version of the system is specified by the    open automaton  depicted in Fig. \ref{Fig:SpecOA}, referred to as \texttt{Simprod-cons}. 
\begin{figure}[!t]
 \centering
...
\texttt{prod-cons[P/producer]} $\leq_{\{\texttt{Q}\}}$  \texttt{Simprod-cons[P/producer]}. This can be verified by comparing the behaviour of the automaton of the figure Fig.~\ref{Fig:ComposeOA} and the automaton resulting  from the composition of the \texttt{Simprod} and  \texttt{producer} automata.\qed
\end{example}
...
As an example of the use of this theorem, consider a refined version of the producer process of Example ~\ref{Example:prodandcons} that we  refer to as $\texttt{producer}^{*}$. According to  Theorem \ref{thm:CongurenceRefinement}, we have 
\texttt{prod-cons[P/producer]} $\leq_{\{\texttt{Q}\}}$  \texttt{prod-cons[P/$\texttt{producer}^{*}$]}; as well as \texttt{Simprod-cons[P/producer]} $\leq_{\{\texttt{Q}\}}$  \texttt{Simprod-cons[P/$\texttt{producer}^{*}$]}.
