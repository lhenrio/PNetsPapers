...
\RAB{The composition of the family of OTs $t_{3x}^{x\in X}$ originating from $s_3$ which are related $s_2$ with the same transitions of $A_1$ (modulo renaming) as one used to build $t_{12}$ produces a single transition $t_{13}$.} This is,
because the hole is not involved we can define $t_{13}$ with the same transition $t_1$ of $A_1$ (where elements of $t_1$ are the same as above modulo renaming of variables):
\[t_{13} = \OT{\mpar{s^1_1, s_3}}{\mpar{{s^2_1}', s_3}}{\alpha_1^2}{ {(\beta^2_{1j})^{j \in J^{2 \prime}_1} }}{g^2_1}{\psi^2_1}
\]

\RAB{Pourquoi masquer partie en dessous dans le latex?\\ we need to prove  $\mpar{\forall x \in X, J'_{13x} \cap H' = J'_{12} \cap H'}$}
% 
%$\forall x \in X$ we have
%\begin{align*}		
...
\RAB{Because $\alpha_{12}$ and $\alpha_{13}$ are the same, as well as  $\beta_{12}$ and $\beta_{13}$ are the same (modulo renaming of variables), and by considering all the family of weak open transitions indexed over X starting from $s_{3}$ which are related with $s_{2}$, we deduce from the above the expected formula:}
