...
\RAB{Many parallel programs  such as skeleton-based parallel programs,  can be characterized  by a collection of high-level patterns with parallel semantics,  but also provide a systematic way of developing practical programs. Skeleton specification is refined by applying a sequence of transformation rules until implementation.  In this high-level parallel design  approach, each skeleton is a parametric process, it accepts as a parameter the kind of computation to be performed according to the behaviour it models.
}
...
\RAB{Most  refinement work focused on  the study  of  systems whose behaviour is fully defined.} 

\LUDO{revoir la fin a mon avis: si on veut se distinguer des travaux composants, il faut expliquer} 
...
based on Communicating Finite-State Machines, that are used for  modelling processes communicating through FIFO channels.   \LUDO{\st{more and lesss than what???}}. The refinement of open systems is also defined in terms of  alternating simulation \cite{Alur:1998,deAlfaro:2021}.
