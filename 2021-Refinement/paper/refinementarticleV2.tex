...
\TODO{La section 4.1 reintroduit la notion de reachability et non introduction de blocage}
\subsection{Deadlock Reduction, Reachability, and Composition}
...
\end{theorem}
\RAB{A t-on des conditions sur H?}
...
Let $A_1$, $A_2$ and $A_3$ be three open automata with $\wrel{A_2}{A_3}{H}$. 
 $\wrel{A_1}{A_2}{H}$.  Let $J_3$ be the set of holes of $A_3$ and suppose that \(k \in H\).
We have: \[\wrel{A_1\subst{A_2}{k}}{A_1\subst{A_3}{k}}{J_1 \uplus H \setminus \mbrc{k}}\]
\end{theorem}
...
\begin{example} \RAB{cet exemple illustre plutot  que la composition est un raffinement, i.e.,  $\wrel{A_1\subst{A_2}{k}}{A_1}{J_1   \setminus \mbrc{k}}$ doit-on ajouter ce resultat comme un lemme?}
...
\begin{enumerate}
\item  Because composition doesn't change the resulting actions, nor their variables, we can extend the valuation context of the variables to cover the variables of the transitions $t_3$ and .   By construction of  $t_{13}$ and $t_{23x}$ we have $\alpha_{13}=
...
\RAB{ I think there is no need to the no blocking condition} Furthermore, by hypothesis we have  \(A_1\subst{A_3}{k}\) is non-blocking,  then according to the Definition \ref{Def:Reach} we have $ \sigma_{013} \vdash \reach{A_{13}}\mpar{s_{013}}$ and for all $t_{13} \in T_{13}$:
...
R\mpar{s_1, s_{2}} \wedge g_{13}  \cancel{\wedge \reach{A_{13}}\mpar{s_{13}}}
\wedge \bigwedge_\subbox{v_3\in V_3}  v^1_3=v^2_3\wedge s^1_3=s^2_3
...
\begin{array}{l}
		  ...
		  \wedge \cancel{\reach{A_{13}}\mpar{s_{13}'}\psubst{\psi_{13}} 
...
\begin{multline*}
V_{13} \uplus V_{23} \uplus \fvars{t_{13}}  \vdash\\
R\mpar{s_1, s_{2}} \wedge g_{13} \land \cancel{\reach{A_{13}}\mpar{s_{13}} \land} \bigwedge_\subbox{v_3\in V_3} v^1_3=v^2_3 \wedge s^1_3 = s^2_3
\implies \hspace{7em} \\ \operatorname*{\bigsymb{\bigvee}}_{x \in X} \mpar{\everymath{\displaystyle}
\begin{array}{l}
			\alpha_{13} = \alpha_{23x} \wedge \bigwedge_\subbox{j \in J'_{23x} \cap H'} \beta_{13j} = \beta_{23xj} \nwedge g_{23x}\land  \cancel{\reach{A_{13}}\mpar{s_{13}'}\psubst{\psi_{13}}}		\nwedge\\
...
\item \TODO{To check from this point:} Lastly, we have to prove the satisfaction of the deadlock reduction condition, i.e., for all $\mpar{s_{13}, s_{23}} \in S_{13} \times S_{23}$
\begin{multline*}
...
By hypothesis  the second  \(A_1\subst{A_3}{k}\) is non-blocking,  then according to the Definition \ref{Def:Non-block} we have for all state $s_{13}$ which is reachable, i.e. $ \fvars{t_{13}}\vdash \reach{A_{13}}\mpar{s_{13}}$,  the following: 


\begin{multline*}
V_{13} \uplus \biguplus_\subbox{t_1 \in \fOT{s_1}} \fvars{t_1}  \vdash \bigvee_\subbox{t_1 \in \fOT{s_1}} \fguard{t_1} \implies \bigvee_\subbox{t_{13} \in \fOT{s_{13}}} \fguard{t_{13}}
\end{multline*}
...
Because the guards from transitions of $A_2$ are still transitions of $A_{23}$,  thus we have:
\begin{multline*}
...
\begin{multline*}
V_{13} \uplus V_{23}  \vdash  \underbrace{R\mpar{s_1, s_2} \wedge  \mpar{\,\,\bigwedge_\subbox{v_3\in V_3}  v^1_3=v^2_3 \wedge s^1_3=s^2_3}}_{R^{\prime}(s_{13},s_{23})} \wedge \neg(\bigvee_\subbox{t_{13} \in \fOT{s_{13}}} \fguard{t_{13}})   \\ \implies \neg(\bigvee_\subbox{t_{23} \in \fOT{s_{23}}} \fguard{t_{23}})
\end{multline*}
...
Suppose $\wrel{A_2}{A_3}{H}$ and  $k \in H$. We have: \[\wrel{A_1\subst{A_2}{k}}{A_1\subst{A_3}{k}}{J_1 \uplus H \setminus \mbrc{k}}\]
\proof
...
Consider any two states $s_{12} = \mpar{s_1,s^1_2}$ and $s_{13} = \mpar{s^2_1,s_3}$ ($s^1_1$ and $s^2_1$  are the same with renaming). We define a relation $R'$ relating states of $s_{12}$ and $s_{13}$ as follows:
\[ R' \mpar{s_{12}, s_{13}} = R\mpar{s_{2}, s_{2}} \wedge \bigwedge_\subbox{v_1\in V_1}
...
The proof of the rest  is based on the following facts: 
\begin{enumerate}
\item  Because composition doesn't change the resulting actions, nor their variables, we can extend the valuation context of the variables to cover the variables of the transitions $t_2$ and $t_{3x}$.   By construction of  $t_{12}$ and $t_{13x}$ we have $\alpha_{12}=
\alpha^1_{1}$ and $\alpha_{13x}=\alpha^2_{1}$. Since  $\alpha^1_{1}$ and  $\alpha^2_{1}$ are the same (up the renaming. So we deduce: 
...
The disjunction with the following hypothesis:\\ $V_{2} \uplus V_{3} \uplus \fvars{t_2}  \vdash {\displaystyle \bigwedge_\subbox{j \in J'_{3x} \cap H} \beta_{2j} = \beta_{3xj}}$  will give:
\begin{align*}
...
Moreover, we have  for any transition $t_1 $ in $A_1$ relying $s_3$ and $s^{\prime}_1$ the following:
\begin{multline*}
V_{12} \uplus V_{13}  \uplus \fvars{t_{12}} \vdash\\
\bigwedge_\subbox{v_1\in V_1}\!\! v^1_1=v^2_1  \wedge s^1_1 = s^2_1 \implies \bigwedge_\subbox{v_1\in V_1}\!\! \psi_{12}(v^1_1)=\psi_{12}(v^2_1)  \wedge s^{1\prime}_1 = s^{2\prime}_1 
\end{multline*}
...
\begin{array}{l}
			\alpha_{12} = \alpha_{13x} \wedge \bigwedge_\subbox{j \in J'_{13x} \cap H'} \beta_{12j} = \beta_{13xj} \nwedge g_{13x} \wedge \\
			 R\mpar{s'_2, s'_{3x}}\psubst{\psi_2 \uplus \psi_{3x}} \wedge \bigwedge_\subbox{v_1\in V_1}\!\! \psi_{12}(v^1_1)=\psi_{12}(v^2_1)  \wedge s^{1\prime}_1 = s^{2\prime}_1
		\end{array}}   
\end{multline*}	
...
As  $k \in H$ we have $k \not\in J^{1\prime}_1$ and we build:
\[t_{13x} = \left(
\OT{\mpar{s^2_1, s_{3x}}}{\mpar{s^{2\prime}_1,s_{3x}}}{\alpha^2_{1}}{\beta_{3xj}^{j \in J'_{3x}}}{g_{3x}}{\psi_{3x}} \right)^{x\in X}\]
 

These two cases define (depending on how $t_{12}$ is built) the family $t_{13x}$ of OTs we are looking for.
Thus, for both cases we have to prove the following:
\begin{align*}		
		&\mpar{\forall x \in X, J'_{13x} \cap H' =
 J'_{12} \cap H'} \text{ and }\\
		&  V_{12} \uplus V_{13} \uplus \fvars{t_{13}} \vdash\\ & R'\mpar{s_{12}, s_{13}} \wedge g_{12} \implies \operatorname*{\bigsymb{\bigvee}}_{x \in X} \mpar{\everymath{\displaystyle}\begin{array}{l}
			\alpha_{12} = \alpha_{13x} \wedge \bigwedge_\subbox{j \in J'_{13x} \cap H'} \beta_{12j} = \beta_{13xj} \nwedge\\[12pt]
			g_{13x} \wedge R'\mpar{s'_{12}, s'_{13x}}\psubst{\psi_{12} \uplus \psi_{13x}}
		\end{array}} 
	\end{align*}
	
Recall in this second case, i.e. where only $A_1$ performs a transition $t_{12}$,  $k \notin J^{\prime}_1$, $\forall x \in X$ we have
\begin{align*}		
J'_{13x} \cap H' & = J'_{3x}  \cap \mpar{J'_1 \uplus H \setminus \mbrc{k}} \\
& = \mpar{J'_{3x}\cap H } \text{ since } J'_{3x}\cap J'_1=\emptyset \land k\not\in J'_1 \\
& = \mpar{J'_2\cap H } \text{  since } J'_2 \cap H = J'_{3x} \cap H\\
& = \mpar{J'_{12}\cap H' } \text{ since } J_3\cap J'_1=\emptyset \land k\not\in J_1'\\
& \text{\RAB{Pas sure cette derniere etape}}
\end{align*}

The proof of the rest of the formula follows the same steps as the previous case the only argument that changes is that by composition we obtain:  $g_{12} \Leftrightarrow g_1$ and
$g_{13x}  \Leftrightarrow g_{1}$.\\
...
By adding the same terms to both sides of the implication, we get:
\begin{multline*}
V_2 \uplus V_3  \vdash R\mpar{s_2, s_3} \nwedge \neg (\bigvee_\subbox{t_2 \in \fOT{s_2}} \fguard{t_2}) \vee \neg (\bigvee_\subbox{t_1 \in \fOT{s_1}} \fguard{t_1}) \vee \neg \bigcirc \\ \vspace{4em} \implies   \neg(\bigvee_\subbox{t_3 \in \fOT{s_3}} \fguard{t_3})\vee \neg (\bigvee_\subbox{t_1 \in \fOT{s_1}} \fguard{t_1})\vee \neg \bigcirc
\end{multline*}

\RAB{On aurait pas envie d'ecrire  l'egalite des actions de trous}
