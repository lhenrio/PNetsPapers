...
An \emph{open transition} is a tuple \nmm{\OTg} with \(s, s' \in S\) the source and target states, \(\alpha \in \actions\) the produced action, \(J' \subseteq J\) the holes involved in the transition, \(g \in \formulas\) the guard and \(\psi: V \to \terms\) the variable  assignments. Each \(\beta_j \in \actions\) is an action of the holes $j$,
