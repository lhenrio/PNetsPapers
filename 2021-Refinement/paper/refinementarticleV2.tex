...
In other words, for each pair of states is attached a boolean formula that may refer to the  variables of each of the two OA states, stating whether the two states are related or not.
...
\RAB{Pourquoi on parle bisimulation??}
...
Let $A_1$, $A_2$, $A_3$ and $A_4$ be four OA with $\wrel{A_1}{A_2}{H}$ and $\wrel{A_3}{A_4}{H'}$.   Let $J_1$ and $J_2$ be the set of holes of $A_1$ and $A_2$ resp. and suppose that \(k \in J_1 \cap J_2\). We have: \[\wrel{A_1\subst{A_3}{k}}{A_2\subst{A_4}{k}}{J_1 \uplus J_2 \uplus J_3 \uplus J_4 \uplus H \uplus H'\setminus \mbrc{k}}\]
\end{theorem}

\begin{example}
As an example of the use of this theorem, if we design a refined version of the producer process of Example~\ref{Example:prodandcons} called $\texttt{producer}^{\flat}$. According to  Theorem \ref{thm:Composability}, we have 
\texttt{prod-cons[producer/P]} $\leq_{\{\texttt{Q}\}}$  \texttt{Simprod-cons[$\texttt{producer}^{\flat}$/P]}. 
