...
\item Good properties relatively to composition: we prove that composition preserves the refinement relation.
...
We  then  define a refinement relation for OA, first only considering two automata with the same set of holes in Section~\ref{sec:refinement} and generalize it to automata with a different set of holes in Section~\ref{sec:holes}.  Section~\ref{sec:prop} is dedicated to formalize and prove basic properties of the refinement defined  ; here we prove that refinement is a preorder and has nice composability properties. 
...
Similarly to FH-bisimulation~\cite{AMHEEMA:2023} we are interested  in finding refinement relations between configurations of two OA that contain variables and holes. When dealing with open systems it is common to define refinement in terms of a simulation relation.
...
OA are convenient for component-based software design. Indeed, OA allow the designer to model parallel systems that are parameterised both by the use of variables and by the possibility to compose automata. The formalism supports compositional specification through the refinement paradigm.
The contribution of this paper, we introduce refinement  between open automata. The refinement relation  relies on a simulation relation between the specification and its implementation;  it  specifies that the refined process must follow the behaviour of the specification, not only at every compatible state but also along any trace. We finally showed that refinement is a preorder that is preserved when filling a hole.
