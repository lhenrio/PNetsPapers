...
\medskip
\TODO{To check form this point}

We have for all $\mpar{s_{2}, s_{3}} \in S_{2} \times S_{3}$ such that $\reach{A_{12}}(s_{12})$ is satisfiable:
\begin{align*}
 R\mpar{s_{2}, s_{3}} &\implies  R\mpar{s_{2}, s_{3}} \land \reach{A_{12}}(s_{12}) \wedge \bigwedge_\subbox{v_1\in V_1} v^1_1=v^2_1\wedge s^1_1 = s^2_1 \\
 & \implies
R' \mpar{s_{12}, s_{13}}
\end{align*} 

Since in this case the state $s'_{12}$ is the same as $s_{12}$ and $s'_{13x}$ is the same as $s_{13}$, we get:
\begin{align*}
 R\mpar{s_{2}, s_{3}}  & \implies
R' \mpar{s_{12}, s_{13}}\\
& \implies
R' \mpar{s'_{12}, s'_{13x}}
\end{align*}
Furthermore for this case, the  composition doesn't change the produced actions nor its variables, we have:  $g_{12}$ is the same as $ g^1_1$, $g_{13x}$    as $g^2_1$, $\alpha_{12}$ as $\alpha^1_1$ and $\alpha_{23x}$ as  $\alpha^2_1$.  Similarly, 
$\beta_{13j}$ and $\beta_{23xj}$ are the same as $\beta_{1j}$ and $\beta_{2xj}$ respectively.\\

Moreover, we have  for any transition $t_1$ in $A_1$ that is used to build $t_{12}$ and $t_{13x}$ the following:
\begin{multline*}
V_{12} \uplus V_{13}  \uplus \fvars{t_{12}} \vdash\\
\bigwedge_\subbox{v_1\in V_1}\!\! v^1_1=v^2_1  \wedge s^1_1 = s^2_1 \implies \bigwedge_\subbox{v_1\in V_1}\!\! \psi_{12}(v^1_1)=\psi_{13x}(v^2_1)  \wedge s^{1\prime}_1 = s^{2\prime}_1 
\end{multline*}
...
From the above we can deduce the expected formula:
\begin{multline*}
V_{12} \uplus V_{13}  \uplus \fvars{t_{12}} \vdash
R'\mpar{s_{12}, s_{13}} \wedge g_{12} 
\implies \hspace{10em}\\ \operatorname*{\bigsymb{\bigvee}}_{x \in X} \mpar{\everymath{\displaystyle}
