...
\TODO{Debut Exemple-1}
\newpage
\begin{example} As a running example, we consider a classical producer-consumer pair  with FIFO buffer,  referred to as \texttt{prod-cons}. The  structure shown in Fig. \ref{Fig:Architect} reflects the different processes involved in the system and all the actions performed.


\begin{figure}[h]
 \centering
   \includegraphics[width=.70\textwidth]{Figures/Architecture.pdf}
   \caption{The structure of the example \label{Fig:Architect}} 
\end{figure}


The open automaton  modelling the behaviour of the \texttt{prod-cons}  system is depicted in Fig. \ref{Fig:SpecOA}. The \texttt{prod-cons} can be fed using an operation \texttt{push} and discharged  using an operation \texttt{pop}. The automata has two holes: $\texttt{P}$ and  $\texttt{Q}$  depicting  the behaviour of the producer and consumer resp. When the \texttt{prod-cons} agrees with $\texttt{P}$ on a data admission, it   stores the data in a local variable and  exposes to the environment an observable action \texttt{push}. At the opposite side, when it agrees with $\texttt{Q}$ on a data retrieval, it exposes  a \texttt{pop} action. The system can also agree with $\texttt{Q}$ on the admission of data but this one is rather printed not stored.  Besides, producing and  consuming, it can agree on the computation action of both processes but this action is exposed as unobservable actions $\tau$.


\begin{figure}[h]
 \centering
   \includegraphics[width=.95\textwidth]{Figures/FIFOpen2.pdf}
   \caption{Open Automaton of the \texttt{prod-cons} system. It exposes two unspecified processes in holes
   $\texttt{P}$ and  $\texttt{Q}$. \label{Fig:SpecOA}} \TODO{refaire l exemple pour l instant il a pas vraiment de sens}
\end{figure}
...
Two processes that are composable with the open automaton in  Fig. \ref{Fig:SpecOA} are those given in  Fig. \ref{Fig:procandcons}. 
\begin{figure}[h]
 \centering
   \includegraphics[width=.35\textwidth]{Figures/P-proc.pdf}\hfill 
   \includegraphics[width=.45\textwidth]{Figures/Q-proc.pdf}
   \caption{(Left) Open Automaton of the \texttt{producer}.  It  transmits data calculated, sending one item at a time to the FIFO. (Right) The \texttt{consumer} gets one item at a time from the FIFO and sends it after a prior processing. \label{Fig:procandcons}}
\end{figure}
~~\\
\smallskip
\RAB{Pbme: L'automate de la fig \ref{Fig:ComposeOA} n'est pas le resultat de la composition du prod et cons avec la sepc, i.e l'automate de la fifure \ref{Fig:SpecOA} mais avec celui du raffine (qui vient apres) i.e fig \ref{Fig:RefineOA}. Solutions: 1) deplacer la composition 2) refaire la figure l'automate sera un peu plus gros? }
\begin{figure}[h]
 \centering
   \includegraphics[width=.95\textwidth]{Figures/compos-FIFO-Pro.pdf}
   \caption{Open Automaton of the \texttt{prod-cons}[\texttt{P}/\texttt{producer}]. 
   $\texttt{P}$ and  $\texttt{Q}$. \label{Fig:ComposeOA}} 
\end{figure}
\end{example}
...
\begin{example} 
To illustrate the refinement simulation of open automata, consider a variation on the \texttt{prod-cons}.  We suppose that on more concrete level,
FIFO circular buffer
  The behaviour of the concrete database is given by 
an open automaton  depicted in Fig. \ref{Fig:RefineOA}. 
\end{example}
\begin{figure}[!tb]
 \centering
   \includegraphics[width=.99\textwidth]{Figures/FIFORing.pdf}
   \caption{Open Automation of the \texttt{prod-cons} system communicating through a FIFO circular buffer.  \label{Fig:RefineOA}}
\end{figure}
