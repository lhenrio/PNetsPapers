...
* case 2 of the proofs for both theorems
...
\begin{figure}[h]
 ...
 to count external tick actions. The system sets a counter when a query is enabled and updates  when  the time is over.\label{Fig:SpecOA}} \TODO{refaire l exemple pour l instant il a pas vraiment de sens}
\end{figure}
...
\end{multline*}
 ...
 Furthermore, by hypothesis we have  \(A_1\subst{A_3}{k}\) is non-blocking,  then according to the Definition \ref{Def:Reach} we have, for all $t_{13} \in T_{13}$:
...
The proof of the rest of the formula follows the same steps as the previous case the only argument that change is that by composition we obtain:  $g_{13} \Leftrightarrow g_1$ and $g_{23x}  \Leftrightarrow g_{2x}$ and the actions of $\alpha_{13}$ and $\alpha_{23x}$ are renaming of  $\alpha_{1}$ and $\alpha_{2x}$ resp. The same for $\beta_{13j}$ and $\beta_{23xj}$ that are renaming of $\beta_{1j}$ and $\beta_{2xj}$ resp.
 
\TODO{Redone! To check}
...
By hypothesis we have $k \in H$, so in the case where $k \in {J^1_{1}}^{\prime}$, we deduce that $k \in J'_{3x}$ we can  then build  a family of OTs $t_{13x}^{x\in X}$ with the same transitions as $A_1$ (up to renaming) as those used to build $t_{12}$.
\[
...
\end{multline*}	
 ...
 Furthermore, by hypothesis we have  \(A_1\subst{A_2}{k}\) is non-blocking,  then according to the Definition \ref{Def:Reach} we have, for all $t_{12} \in T_{12}$:
...
\wedge \bigwedge_\subbox{v_1\in V_1}\!\! \psi_{12}(v^1_1)=\psi_{12}(v^2_1)  \wedge s^{1\prime}_1 = s^{2\prime}_1
		\end{array}}   
\end{multline*}	
...
\[t_{13x} = \left(\OT{\mpar{s^2_1, s_{3}}}  {\mpar{s^{2\prime}_1, s_{3x}}}  {\alpha_1^2}{{(\beta^2_{1j})^{j \in J^{2 \prime}_1} }}{ g^2_1 }{\psi^2_1} \right) \]
...
For this case, the  composition doesn't change the produced actions nor its variables, we have:  $g_{12} \Leftrightarrow g^1_1$, $g_{13x}  \Leftrightarrow g^2_1$, $\alpha_{12}= \alpha^1_1$ and $\alpha_{23x} =  \alpha^2_1$.  Furthermore, we have the actions of the holes in $J_{13}$ coincide by the composition, $\beta_{13j}$ and $\beta_{23xj}$  are renaming of $\beta_{1j}$ and $\beta_{2xj}$ resp.


\TODO{D'apres l'hypothese de depart, la formule (*) nous obtenons une formule sous cette forme: $R\mpar{s_2, s_3} \wedge g_2 \implies R\mpar{s'_2, s'_{3x}}$  l'idee est de faire disparaitre g2. On a la formule suivante:}

According to the relation between $A_2$ and $A_3$ open automata,  for all $\mpar{s_2, s_{3}} \in S_2\times S_3$  we have:
\begin{multline*}
 V_2 \uplus V_3  \vdash R\mpar{s_2, s_3} \wedge \lnot( \bigvee_\subbox{t_2 \in \fOT{s_2}} \fguard{t_2} )\implies \lnot( \bigvee_\subbox{t_3 \in \fOT{s_3}} \fguard{t_3}  )
\end{multline*}

\TODO{La conjonction des deux donne $False \implies R\mpar{s'_2, s'_{3x}}$ qui true???}
