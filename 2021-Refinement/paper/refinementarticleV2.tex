...
In previous works \cite{fhbisim,wang:03126313} a bisimulation relation was defined for OA and open parameterized hierarchical LTSs. It has already good properties relatively to bisimulation, but refinement relations were not studied.

This article first introduces OA that were defined in  \cite{henrio:01299562}. %,  but compared to previous works, we 
and additionaly defines the composition of OA.
Then we define  a refinement relation for OA that has the following characteristics:
\begin{itemize}
...
\begin{example} \label{Example:prodandcons} Fig. \ref{Fig:procandcons} shows a producer automaton  and a consumer  automaton that can be used  to fill the holes $\texttt{P}$ and $\texttt{Q}$ of \texttt{prod-cons} defined in Example~\ref{ex:fifo-system}.
 \begin{figure}[!ht]
 \centering
...
Similarly to FH-bisimulation~\cite{fhbisim} we are interested  in finding relations between configurations of two OA that contain variables and holes. 
...
To illustrate the refinement simulation of OA, we consider a variation on the \texttt{prod-cons} example. Namely,  we suppose that  the two processes  \texttt{P} and  \texttt{Q} communicate through a one-place  buffer. The  corresponding behaviour of this simpler version of the system is specified by the    OA  depicted in Fig. \ref{Fig:SpecOA}, referred to as \texttt{Simprod-cons}. 
\begin{figure}[!t]
 \centering
   \includegraphics[width=.90\textwidth]{Figures/FIFOpen2.pdf}
   \caption{  The \texttt{Simprod-cons} Open Automaton: \texttt{prod-cons} system using one-place  buffer.  \label{Fig:SpecOA}}
\end{figure}
We can easily check that this automaton simulates the one on Fig.~\ref{Fig:RefineOA}.
The simulation relation is $R=\{(r_0,s_0) \mapsto \texttt{l}=\texttt{f}, (r_0,s_1) \mapsto \texttt{f}=\texttt{l}+1\% \texttt{N} \}$ is a simulation. \qed
\end{example}
...
We call \emph{non-blocking composition} a composition that can be safely be used to compose OA that are involved in a deadlock reducing relation.
...
%\end{definition}
\begin{definition}[Non-blocking composition] \label{Def:Non-block}
Let \(A_1 \defobject \OAg[1]\), \( A_2 \defobject \OAg[1]\) be two OA.
...
\begin{example}  Consider again the \texttt{prod-cons} and \texttt{Simprod-cons}  automata given in the examples above.  Since we have \texttt{prod-cons} $\leq_{\{\texttt{P,Q}\}}$ \texttt{simprod-cons},  hence according to Theorem \ref{thm:ContextRefinement}, we get 
\texttt{prod-cons[P/producer]} $\leq_{\{\texttt{Q}\}}$  \texttt{Simprod-cons[P/producer]}. 
The behaviour of \texttt{prod-cons[P/producer]} is shown Fig.~\ref{Fig:ComposeOA}.
...
\begin{example}
As an example of the use of this theorem, if we design a refined version of the producer process of Example~\ref{Example:prodandcons} and call it $\texttt{producer}^{*}$. According to  Theorem \ref{thm:CongurenceRefinement}, we have 
\texttt{prod-cons[P/producer]} $\leq_{\{\texttt{Q}\}}$  \texttt{prod-cons[P/$\texttt{producer}^{*}$]}; as well as \texttt{prod-cons[P/producer]} $\leq_{\{\texttt{Q}\}}$  \texttt{Simprod-cons[P/$\texttt{producer}^{*}$]}.
