...
\end{figure}
	...
	composing two occurences of the Enable1 pNet. For simplicity,
	we only represent one instance of the synchronisation vector set,
...
\begin{example}[An open transition]\TODO{change for Enable example}
  ...
  trivially true. The global state shows that controler $C_3$ has
...
The semantics of pNets in term of open automata has been defined in
...
(before applying the predicate and the  OT construction). Matching and
...
Algorithm~\ref{alg3} implements this new stategy: \TODO{Explain
  significant detials}
...
either \emph{trigger} (triangles in Figure~\ref{schema:ArchFailure:BIP})
or \emph{synchron} (bullets in Figure~\ref{schema:ArchFailure:BIP}).
%
...
units (more details in Section~\ref{section:resultOA}).
...
$\true$ (\eg $\cstate{0} \goesto{\PortFail(\true)} \cstate{1}$) and the
...
transitions, is shown in Figure~\ref{schema:resultOA}.
...
To improve the readability of this figure, we use the following conventions:
we omit the transitions of the two PLTSs, and the set of ``working''
holes; and we directly write the resulting action as the first element of
...
algorithm. For reasons of readability, we do not show the full
details of the open transitions in this figure. Below we list the
first three of them (full details available in \ref{TimerRR}). 
This automaton has 12 transitions, including those encoding various
possible firing of some interactions (e.g. OTs 2 and 3 for
{\PortFail}). It appears that the rough behaviour is
reasonable, although a detailed study shows that the property stated
above does not hold, because of the
{\PortFail} loops on states $11$ and $12$. The reason is that we do not
...
%                \textsl{for any sort} & & $=,\ \ne$
 ...
 \medskip
        This shows a significant benefit of the ``smart'' strategy, in the considered use-cases, between 30\% and 60\% gain depending on the examples.
...
parameterised actions named FUN, which can accept any number of
...
%%   \texttt{assert} in Z3. A constant action or a parameterised
...
\QIN{
We apply several structural rules to generate the predicates restricting the composition of the subnets by synchronisation vectors.
...
\begin{itemize}
    ...
    \item $k$ is the indices of the synchronisation vectors.
    \item $SV$ is the set of the synchronisation vectors.
    \item $Vars()$ is the function that gets all the variables in the object.
    \item $\alpha$ and $\alpha^{'}$ is the element of the  the synchronisation vector.
\end{itemize}
\end{definition}
...
%    \item (2) The chosen synchronisation vector: If the result is also one of the elements, it is also the intermediate variable. Otherwise, all of the parameters are ground.
