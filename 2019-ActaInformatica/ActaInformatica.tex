...
transition systems to formalize value-passing or parameterised systems, using
various symbolic encodings of the
transitions~\cite{deSimone85,Larsen87,HennessyLin:TCS95,Linconcur96}. 
In \cite{Linconcur96}, Lin considered value-passing CCS, for which he
...
Later on, symbolic semantics were extended to the $\pi$-calculus and decision algorithms for various symbolic bisimulations were proposed \cite{Li99,Deng2001}. However, no
...
each component of a distributed system and their composition.
...
global behaviour of the system. In previous work,
...
%% internal actions is
are given by  \emph{synchronisation vectors}, which %% : a
...
\end{figure}
	...
	In Figure \ref{schema:enable-pnets}, we show 2 possible encodings of the  
	``Enable'' operator.
	...
	composing 2 occurences of the Enable1 pNet. For simplicity,
...
Suppose $sv = \left((a'_i)^{i\in I} (b'_j)^{j\in J}\rightarrow v'\right) \in SV$ is a synchronisation vector, $G_k$ is its guard, 
...
transition of Figure~\ref{schema:unsat-ot}
