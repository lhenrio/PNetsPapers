...
\openrule
    {
       \set{(\beta^1_i)},\Pred^1_i,\Post^1_i   }
         {\ostate{s^{l\in L}_{il}} \OTarrow {\tau} \ostate{s^{l\in L}_{(i+1)l}}}$
~and~ $\forall i \in [0\ldots m_2] \,\, P[Q]_{j_0}\!\models \openrule
         {
           \set{(\beta^3_i)},\Pred\,^3_i, \Post_i^3   }
         {\ostate{u^{l\in L}_{il}} \OTarrow {\tau} \ostate{u^{l\in L}_{(i+1)l}}}
$\\
...
$
s_j=s_{0 l} \wedge s_{(m_1+1) l}=t_l \wedge t'_l = u_{0 l} \wedge u_{(m_2+1) l}=s'_l,
...
$P[Q]_{j_0}	\models		\openrule
    {
       \set{\beta}_p,\Pred_p,\Post_p   }
         {\ostate{s_p^1} \OTarrow {\tau} \ostate{s^1_{p+1}}}$ \\
