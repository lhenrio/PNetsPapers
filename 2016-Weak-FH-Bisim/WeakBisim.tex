...
%       
       	...
       	\[\mathcal{R}'' = 
       	\{(s,u|\Pred\,'')\,\,\Big|\,\Pred\,''=\bigvee_{\begin{array}{c}       		
       		(s,t|\Pred)\in\mathcal{R}\\ (t,u|\Pred\,')\in\mathcal{R}' 	
       		\end{array}
       	}\,\Pred\land\Pred\,'\}\]
...
satisfiability of the predicate relating the first state to the last one.
       	...
       	The relation is built as follows: for each pair of state $s$, $u$, for each 
       	state 
       	...
       	We will show
       	that $\mathcal{R}''$ is an FH-bisimulation. Consider 
       	$(s,u|\Pred\,'')\in\mathcal{R}''$. Then there is a set of states of 
       	$\mathcal{T}_2$ relating $s$ and $u$, let $(t_p)^{p\in P}$ be this family.  
       	
       	For any $p\in P$ by definition of $\mathcal{R}''$,
       	$(s,t_p|\Pred_p)\in\mathcal{R}$,  and 
       	$(t_p,u|\Pred\,'_p)\in\mathcal{R}'$. 
       	We have 
       	the 
       	following by definition of bisimulation.
       	...
       	%    \left( fresh \ \overline{a_i}, \overline{b_j}, v_x.\ \
       	\inferrule*[myfraction=\reddottedrule]
       	{\{t_{p i}\xrightarrow{a_{p i x}}_i t_{p i x}\}^{i\in I_{2 p x}},
       		\{\xrightarrow{b_{j p x}}_j\}^{j\in J_{2px}}, \Pred_{OT_{px}},\Post_{OT_{px}}}
       	{t_p \OTarrow {v_{px}} t_{p x}}
       	%         \right)
       	\end{mathpar}
       	
       	such that  $\forall x, J_1=J_{2px}, (s',t_{px}|\Pred_{{px}})\in 
       	\mathcal{R}$;
       	and  \\
       	...
       	for any open transition $OT_{px}$, since
       	$(t_p,u|\Pred\,^\prime_p)\in\mathcal{R}'$ there exist open transitions
       	...
       	%    \left( fresh \ \overline{a_i}, \overline{b_j}, v_x.\ \
       	\inferrule*[myfraction=\reddottedrule]
       	{\{u_i\xrightarrow{a_{i p x y}}_i u_{i p x y}\}^{i\in I_{3 p x y}},
       	...
       	First, $\forall y, \forall x, \forall p, J_1=J_{2px}=J_{3pxy}, 
       	({s}',s_{pxy}|\Pred\,'_{{pxy}})\in \mathcal{R}''$. 
       	Indeed for any 
       	$p$, 
       	$x$, and 
       	$y$, $t_{px}$
       	...
       	and $(t_{px},u_{pxy}|\Pred{{pxy}})\in \mathcal{R}'$. 
       	More precisely, $t_{px} \in ({t_p}')^{p\in P'}$ where $({t_p}')^{p\in 
       		P'}$ 
       	is 
...
variable in $\mathcal{T}_1$.
       	...
       	\begin{small}
       		...
       		\left( \forall j. b_j=b_{jpx}  \Rightarrow \Pred_{OT_{px}}
       		\land v\!=\!v_{px} \land
       		\Pred_{{px}}\subst{\Post_{OT}}\subst{\Post_{OT_{px}}}\right)\land\Pred'_p\\
       		%
       		\hspace{1cm} \implies\!\!\! \bigvee_{x\in X}
       		\left( \forall j. b_j=b_{jpx}  \Rightarrow (\Pred_{OT_{px}}\land\Pred'_p)
       		\land v\!=\!v_{px} \land
