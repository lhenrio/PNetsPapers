...
$ where\\
$
s=s^1_0 \wedge s^1_{m_1}=s^2 \wedge s'^2 = s^3_0 \wedge s^3_{m_2}=s',
...
\Post=\biguplus_{i=0}^{m_2}(\Post^3_i\subst{\Post^2})\bigotimes_{i=0}^{m_1}\subst{\Post^1_i}
$
...
we obtain by Lemma \ref{lem-decompose}:\\
$P\models{\openrule
			{
  ...
  \begin{array}{l}
		...
		\openrule
         {
           (j\mapsto\tau)^{j\in L\cap\Holes(Q)},\True,\Id(s_{pl}^{l\in L_Q})}
         {s_{pl}^{l\in L_Q} \OTarrow {\tau} s_{pl}^{l\in L_Q}}
...
\item Similarly, for the open transition $\symb{OT}$ in the form: $P[Q]_{j_0}	\models \openrule
         {
           \set{\beta},\Pred,\Post }
         {\ostate{s} \OTarrow {\alpha} \ostate{s'}}$\\
We obtain by Lemma \ref{lem-decompose}:\\
...
\end{mathpar}
Considering each  $\symb{OT}_p$ of $Q$ such that $p \in S$ and $S \subseteq [0\ldots (m_1\!+\!m_2)\!+\!1]$, the set of open transition that moves at each time, i.e,  the open transition $\symb{OT}_p$ in the form:
\begin{mathpar}
Q\models{\openrule
			{
...
By rule (\WTTrois)  we can conclude that for each $p\in S$ we have:
\begin{mathpar}
Q\models{\openrule
			{
...
\TODO{maybe it is better to use $\gamma_p=[\beta_p]$}\\
