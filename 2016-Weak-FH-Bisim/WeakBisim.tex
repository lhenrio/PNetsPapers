...
Ludo
\begin{itemize}
\item rewrite Tr2 and open transition with actions
\item $v'\to \alpha'$ dans pred sv et enlever l exists
\item $\beta$ ou $b$
\item propager Tr2 dans l annexe
\item add simple equivalence on OT
...
Rabea
\begin{itemize}
\item uniformiser p et P pour les pnets
\item relectures
\end{itemize}
=== en parallele
...
\[\begin{array}{l}\Pred_{\symb{sv}}(\SV{{(\alpha'_i)}^{i\in I}, {(\beta'_j)}^{j\in J}} 
{\alpha'} 
{e_b}, \alpha_i^{i\in I}, \beta_j^{j\in J}, \alpha)\Leftrightarrow \\~\qquad\qquad%\bigg(
%
...
%                       1 \xrightarrow{send(m,sb)}_{Sender} 2 ~~
   %                           \xrightarrow{in(?m,?b)}_{M1}
      \emptyset; Pred=[m=s_1\_m, b=s_1\_sb, v=\tau];  Post=\{m_1\_m:=m, m_1\_b:=b\}
                      }
  %  {\ostate{00} \xrightarrow{\underline{\delta(x1)}} \ostate{10}}
    {\ostate{s_1,m_0,m_0,r_0} \OTarrow{v} \ostate{s_2,m_1,m_0,r_0}}
    $
...
%                       1 \xrightarrow{send(m,sb)}_{Sender} 2 ~~
%                            \xrightarrow{in(?m,?b)}_{M1}
      \{\xrightarrow{b_P}_P\}; Pred=[b_P=send(m)\land v=\underline{send(m)}]; Post=\{s1\_m:=m\}
                      }
  %  {\ostate{00} \xrightarrow{\underline{\delta(x1)}} \ostate{10}}
    {\ostate{s_0,m_0,m_0,r_0} \OTarrow{v} \ostate{s_1,m_0,m_0,r_0}}
    $
...
\forall m\!\!\in\!\! I_2.		{ \pNet_m 
     ...
     J' = \biguplus_{m\in I_1}\!\! J_m \uplus J 	\\
    	...
    	\Pred_{\symb{sv}}(SV_k,v_m^{m\in (I_1\uplus I_2)},\beta_j^{j\in J},v)\\ 
...
WOT is:
 ...
 $$\ostate{1000}\OTWeakarrow{v,
   [v=out(s_1\_m)\land r_0\_b=s_1\_b],\{r_2\_b:=r_0\_b\}} \ostate{2002}$$
%% \\
...
\begin{lemma}[OT decomposition]\label{lem-decompose} 
	...
	\vspace{-2.2ex}\\\text{and~~}
		...
		Q\models{\openrule
			{
				\{\xrightarrow{\beta_j}\}^{j\in J\cap\Holes(Q)}, \Pred\,'',  
				\Post\,''}
