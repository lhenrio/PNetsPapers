...
Let $\uplus$ be the disjoint union operator on indexed sets. 
The elements
of the union are then accessed by using an index of one of the two
joined families.
...
We denote $y\gets x$ a substitution. Application of the substitution is denoted
...
$\pNet\triangleq pLTS~|~\mylangle \pNet_i^{i\in I}, Sort_j^{j\in J}, \symb{SV}_k^{k\in K}\myrangle$
where
\begin{itemize}
...
\item[$\bullet$] $Sort_j \subseteq \AlgAS$ is a set of action terms, denoting the $\Sort$ of
hole $j$.
...
x\in\symb{iv}(\alpha)}|s \xrightarrow{\langle \alpha,~e_b,~(x_j\!:= {e}_j)^{j\in
    J}\rangle} s'\in \to \} \\
\Sort(\mylangle \pNet_i^{i\in I}\!,Sort_j^{j\in J}\!,
\overline{\symb{SV}}\myrangle)
=\{\alpha'_k |\, \alpha_j^{j\in J_k}\to\alpha'_k\in\set{\symb{SV}}\}
\end{array}
\]
...
\Holes(\mylangle \pNet_i^{i\in I}\!,Sort_j^{j\in J}\!, \overline{\symb{SV}}\myrangle) =J\cup{\displaystyle \bigcup_{i\in I}\Holes(\pNet_i)}\\
...
\Leaves(\mylangle \pNet_i^{i\in I}\!,Sort_j^{j\in J}\!, \overline{\symb{SV}}\myrangle) = {\displaystyle \bigcup_{i\in I}\Leaves(\pNet_i)}
\end{array}\]
\end{itemize}
\end{definition}

\RAB{La notation overline n'a pas ete introduite auparavant? Aussi, je n'ai pas trop bien saisi quand on utilise overline et la la notation k in K}
...
\RAB{les notations des figures pretes a confusion: carre/carre arrondi et rond/triangle plein}
...
\[\Pred_{\symb{sv}}(SV_k, a_i^{i\in I}, b_j^{j\in J}, v)\Leftrightarrow
\begin{array}{l}
\exists {(a'_i)}^{i\in I},
...
\TODO{say that This predicate entails a verification of satisfiability}
...
%	equations of a predicate.
	...
	\noindent
    $\inferrule
    {k\!\in\! K \\ SV\!\!= \!{\tt clone}(SV_k) \!=\! \alpha_m^{m \in I_k\uplus J_k} \!\to\! 
    \alpha'_k \\
    	...
    	\Pred_{\symb{sv}}(SV,a_i^{i\in I_k},b_j^{j\in J_k},v)\\
    	\forall j\!\!\in\!\! J_k. {\tt 
    		fresh}(b_j) \\ {\tt fresh}(v) \\ 
    	\forall i\in
    	L\backslash I'.\,s'_i=s_i 
    }
    {\mylangle \pNet_i^{i\in I}, Sort_j^{j\in J}, \symb{SV}_k^{k\in K}\myrangle
    	\models
    	{\inferrule*[myfraction=\reddottedrule]
...
\RAB{Exemple a revoir. Les regles ne passent...}
...
(symbolic) transitions, has a finite automaton (see \ref{Forte}).
  ...
  \TODO{Eric: what is ``elimination of intermediate variables ?'' it
    transforms the predicates, so I think it is not any kind of
    ``equivalence'' transformation, rather some sort of
    abstraction... Define... Does it change the satisfiability ?}
...
\TODO{To check:}
Let \ERIC{$\mathcal{R}=\{(s^1,s^2|\Pred_{s^1,s^2})\}$} be a relation over the set $\mathcal{S}_1$ and $\mathcal{S}_2$ constrained by a predicate
More precisely, for any pair $(s^1,s^2)$, there is a 
   single
      $(s^1,s^2|\Pred_{s^1,s^2})\in\mathcal{R}$  stating that $s^1$ and $s^2$ are related if $\Pred_{s^1,s^2}$\ 
      is 
      true.
      
\RAB{ I propose:
Let $\mathcal{R}=\{(s^1,s^2|\Pred_{s^1,s^2})\}$ be a relation over the set $\mathcal{S}_1$ and $\mathcal{S}_2$ constrained by a predicate: if $(s^1,s^2) \in \mathcal{R}$ then $\Pred_{s^1,s^2}$ is  true.}     
...
%         \right)
\end{mathpar}
 such that  $\forall x, J_1=J_{2x}, (s^{1'},s^{2}_x|\Pred_{s^{1'},s^{2}_x})\in 
 \mathcal{R}$; 
 and  \\
 $\Pred \land \Pred_{OT}\\
\hspace{1cm} \implies\!\!\! \bigvee_{x\in X}
   \left( \forall j. b_j=b_{jx}  \Rightarrow \Pred_{OT_x}
     \land v\!=\!v_x \land  
     \Pred_{s^{1'},s^{2}_x}\subst{\Post_{OT}}\subst{\Post_{OT_x}}\right)$
...
automata that is an FH-bisimulation and their initial states are equivalent.
...
\medskip
\ERIC{If we assume that everything is finite (states and transitions in the
...
\LUDO{added congruence theorem below}
\begin{theorem}[Congruence]\label{thm-congr-eq}
	...
	$\Sort(Q)=\Sort(Q')=Sort_{j_0}$\footnote{Note that $\Sort(Q)=\Sort(Q')$ is ensured by 
...
\begin{theorem}[Context equivalence]\label{thm-ctxt-eq}
	...
	Let $j_0\in J$ be a hole, and $Q$ be a pNet such that $\Sort(Q)=Sort_{j_0}$. Then $\pNet[Q]_{j_0}$ and 
	$\pNet'[Q]_{j_0}$ are FH-bisimilar.
\end{theorem}
...
algebra. Naturely, an internal action cannot be synchronised with
...
Classicaly, we start by defining a derived transition relation based
on sequences of invisible actions:

\begin{definition}[Weak open transitions]
  Let $A = <L,J,\mathcal{S},s_0,\mathcal{T}>$ be an open automaton.
  We define $\mathcal{T^*}$ the \emph{Weak open transition relation}
  with respect to a set of invisible actions $\mathcal{Inv}$ as:
...
$S_1$ and $S_2$, as in Definition \ref{def-FH-bisim}.

Then 
   $\mathcal{R}$ is an Weak FH-bisimulation iff for any  states
$s^1\in\mathcal{S}_1$ and\\
$s^2\in\mathcal{S}_2$, $(s^1,s^2|\Pred)\in\mathcal{R}$, we 
   have the following:
 ...
 \begin{mathpar}
     \inferrule*[myfraction=\reddottedrule]
         {\{s^1_i\xrightarrow{a_i}_i s^{1'}_i\}^{i\in I_1},
           \{\xrightarrow{b_j}_j\}^{j\in J_1},\Pred_{OT},\Post_{OT}}
         {s^1 \xrightarrow {v} s^{1'}}
...
\end{mathpar}
 ...
 \begin{mathpar}
    \inferrule*[myfraction=\reddottedrule]
         ...
         {s^2 \xRightarrow {v_x} s^{2'}_x}
\end{mathpar}
 such that  $\forall x, J_1=J_{2x}, (s^{1'},s^{2'}_x|\Pred_{s^{1'},s^{2'}_x})\in \mathcal{R}$; 
 and  \\
 $\Pred \land \Pred_{OT}\\
\hspace{1cm} \implies\!\!\! \bigvee_{x\in X}
   \left( \forall j. b_j=b_{jx}  \Rightarrow \Pred_{OT_x}
 ...
 \item  and symmetrically any open transition from $s^2$ in $\mathcal{T}_2$ can be 
      covered by a set of weak transitions from $t^1$ in $\mathcal{T}_1^*$.
 \end{itemize}

Two pNets are Weak FH-bisimilar if there exist a relation between their associated 
automata that is an Weak FH-bisimulation and their initial sstates are equivalent.

 \end{definition}

\TODO{Here we should include:
  \\  - A definition for Weak FH-Bisimulation
...
\TODO{Eric: ask Elie, Hubert or others about interesting weak
  equationnal theories for Lotos -- DONE, no answer..., so I got some examples from \cite{Lotos:ISO}}

\TODO{Ludo: ask Simon for weak properties of Bip composition
  ``operators'' -- DONE!}
    ...
    $$ \delta(x_1)=\delta(y_1) \land acc(x_1)=acc(y_1)
       ...
       \subsection{Equivalence relation (Theorem~\ref{thm-equiv})}
        \emph{Suppose $\mathcal{R}$ 
       	is an FH-bisimualtion. Then $\mathcal{R}$ is an equivalence, that is, 
       	$\mathcal{R}$ is 
       	reflexive, symmetric and transitive.
       	}
...
%       
       	...
       	$\mathcal{T}_1$, $\mathcal{T}_2$, $\mathcal{T}_3$ and states $s^1$, $s^2$, $s^3$ 
       	in those 
       	automata\footnote{We omit the constraints stating that each $s^i_x$ is in the 
       	states of 
       		$\mathcal{T}_i$ for the sake of readability}.
       	Suppose we have $\mathcal{R}$ an FH-bisimulation relation between states of 
       	$\mathcal{T}_1$ and of  $\mathcal{T}_2$; members of $\mathcal{R}$ are of the form 
       	...
       	of 
       	$\mathcal{T}_2$ and of  $\mathcal{T}_3$; members of $\mathcal{R}2$ are of the 
       	form 
       	$(s^2,s^3|\Pred\,')$.
       	
       	Let $\mathcal{R}''$ be the relation: 
       	\[\mathcal{R}'' = \{(s^1,s^3|\Pred\,'')\,\,\Big|\,\Pred\,''=\bigvee_{\begin{array}{c}       		
       		(s^1,s^2|\Pred)\in\mathcal{R}\\ (s^2,s^3|\Pred\,')\in\mathcal{R}' 	
       		\end{array}
       	}\,\Pred\land\Pred\,'\}\]
...
satisfiability of the predicate relating the first state to the last one..
       	...
       	The relation is built as follows: for each pair of state $s^1$, $s^3$, for each 
       	state 
       	$s^2$ such that $\mathcal{R}$ relates $s^1$ and $s^2$, and $\mathcal{R}'$ relates 
       	$s^2$ 
       	and $s^3$, we take the conjunction of the two predicates. The predicates for 
       	different 
       	values of $s^2$ are collected by a disjunction. 
       	...
       	following by definition of bisimulation.
       	For any open transition $OT$ in $\mathcal{T}_1$ originating from $s^1$:
       	Additionally we have $\Pred\,'' = \bigvee_{p\in P} \Pred_p\land\Pred_p\,'$.
       	\begin{mathpar}
       	...
       	\inferrule*[myfraction=\reddottedrule]
       	{\{s^{3}_i\xrightarrow{a_{i p x y}}_i s^{3}_{i p x y}\}^{i\in I_{3 p x y}},
       		\{\xrightarrow{b_{j p x y}}_j\}^{j\in J_{3pxy}}, 
		...
		name clash. In practice,
       	we can suppose that, as $\Pred\,'_{target_{pxy}}$ is used to relate states of 
       	$\mathcal{T}_1$, and $\mathcal{T}_3$, there exists a predicate obtained by 
       	renaming variables, that is equivalent to $\Pred\,'_{target_{pxy}}$ and does not 
       	contain the variables of 	$\mathcal{T}_2$. This predicate with ``fresh'' 
       	variables is called   $\Pred\,'_{target_{pxy}}$ in the following.
       	Similarly, we can suppose that $\Pred_{target_{px}}$ contains no 
       	variable in $\mathcal{T}_3$, and $\Pred_{target_{pxy}}$ contains no 
       	...
       	is 
       	the set of states relating ${s^{1}}'$ and $s^{3}_{pxy}$.
       	Consequently, for all $p$, $x$, $y$, $\Pred_{target_{px}}\land 
       	\Pred_{target_{pxy}}\implies 
       	\Pred\,'_{target_{pxy}}$ (this is one element of the  disjunction defining the 
       	predicate 
       	relating ${s^{1}}'$ and $s^{3}_{pxy}$ in the definition of $\mathcal{R}''$).
...
By construction, the Post substitutions only have an effect on the  
...
substitutions  and of the predicates, we have:\\
       	...
       	\Pred_{target_{px}}\subst{\Post_{OT}}\subst{\Post_{OT_{px}}}\subst{\Post_{OT_{pxy}}}\land
       	\Pred_{target_{pxy}}\subst{\Post_{OT}}\subst{\Post_{OT_{px}}}\subst{\Post_{OT_{pxy}}}\\
       	\implies\Pred\,'_{target_{pxy}}\subst{\Post_{OT}}\subst{\Post_{OT_{px}}}\subst{\Post_{OT_{pxy}}}\\
       	= \Pred\,'_{target_{pxy}}\subst{\Post_{OT}}\subst{\Post_{OT_{pxy}}} $
       	
       	This allows us to conclude, with $\Pred\,'' = \bigvee_{p\in P} 
       	\Pred_p\land\Pred_p\,'$:
         	\begin{small}     	
$\Pred\,'' \land \Pred_{OT}\\
\hspace{1cm} \implies\!\!\! \bigvee_{p\in P} (
\Pred_p\land\Pred_p\,' \land \Pred_{OT})\\
\hspace{1cm} \implies\!\!\!\bigvee_{p\in P}
...
\Pred_{OT_{pxy}}
\land v\!=v_{px}\!=\!v_{pxy}\\~\hspace{3em}~ \land
\Pred\,'_{target_{pxy}}\subst{\Post_{OT}}\subst{\Post_{OT_{pxy}}}\big)\big)
\\
\hspace{1cm} \implies\!\!\! \bigvee_{x\in X}\bigvee_{y\in Y}\bigvee_{p\in P}
\left( \forall j. b_j=b_{jpxy}  \Rightarrow \Pred_{OT_{pxy}}
\land v\!=\!v_{pxy} \land
\Pred\,'_{target_{pxy}}\subst{\Post_{OT}}\subst{\Post_{OT_{pxy}}}\right)$.
  \end{small}
       	...
       	the role 
       	of $s^1$ and $s^3$ in the definition of $\mathcal{R}''$ is symmetrical, and thus 
       	the 
       	proof is similar.
...
\end{small}
         ...
         \newpage
 \section{Proof of Theorem ~\ref{thm-congr-eq}}
 ...
 s_2=s'_2\uplus s \land s\in \mathcal{S}_P \land (s_1',s_2'|\Pred)\in\mathcal{R}\}$ 
 ...
 states: $(\ostate{s_{1 i}^{i \in L_P\uplus L}},\ostate{{s}_{2 i}^{i \in L'}\uplus 
 	{s}_{1 i}^{i \in L_P}}|\Pred)\in\mathcal{R}'$. %Those are states of the open 
 %	automata of 
 ...
 { \ostate{s_{1 i}^{i \in L_P\uplus L}}\xrightarrow {v} \ostate{{s'}_{1 i}^{i \in 
 			L_P\uplus 
 			L}}}\]
 ...
 P\models{\inferrule*[myfraction=\reddottedrule]
 	{\{s_i\xrightarrow{a_i}_i s_i'\}^{i\in I\setminus L},
 		...
 		\Pred\,',  
 		\Post'}
 	{\ostate{s_{1 i}^{i\in L_P}} \xrightarrow {v}
 		\ostate{s_{1 i}'^{\, i\in L_P}}}
 }\\
 Q\models{\inferrule*[myfraction=\reddottedrule]
 	{\{s_i\xrightarrow{a_i}_i s_i'\}^{i\in I\cap L},
 		\{\xrightarrow{b_j}_j\}^{j\in J\cap\Holes(Q)}, \Pred\,'',  
 		\Post''}
 	{\ostate{s_{1 i}^{i\in L}} \xrightarrow {v_Q}
 ...
 $\Leaves(Q)$). As $Q$ is FH-bisimilar to $Q'$ and $(\ostate{s_{1 i}^{i \in 
 ...
 $i\in L_P$, $s_{2 i}=s_{1 i}$ and $s_{2 i x}=s'_{1 i}$, and for $j\in Holes(P)$, 
 $b_{j x}=b_j$):\\[-2ex]
 \[ P[Q']_{j_0}  
 \models
 {\inferrule*[myfraction=\reddottedrule]
 	...
 	{\ostate{s_{2 i }^{i\in L'\uplus L_Q}} \xrightarrow {v_x}
 		\ostate{{s}_{2 i x}^{i\in L'\uplus L_Q}}}
 }
 \]
 ...
 $\forall x, (\ostate{{s'}_{1 i}^{i\in L_P\uplus L}},\ostate{s_{2 i 
 		x}^{i\in L_P\uplus L'}}|\Pred_{tgt_x})\in 
 \mathcal{R}'$ (by definition of
 $\mathcal{R}'$) and in four steps we get:
 
 \noindent                        
 \begin{small} $\Pred \land \Pred_{OT} \implies
 	\Pred\land(\Pred\,'\land \Pred\,'')\subst{b_{j_0}\leftarrow v_Q}\\ % reversed 
 	%substitution 
 	\implies 
 	(\Pred\land\Pred\,'')\land \Pred\,'\subst{b_{j_0}\leftarrow v_Q}\\ % $b_{j_0}$ only 
 	%appears 
 	%in $\Pred\,'$
 	\implies  \hspace{-2ex}
 ...
 \begin{itemize}
 	...
 	$\Pred_{tgt_x}\subst{\Post_{OT}}\subst{\Post'\uplus \Post_{OT_x}}$ 
 	, i.e.:
 	$\Pred_{tgt_x}\subst{\Post'\uplus 
 		\Post''}\subst{\Post'\uplus \Post_{OT_x}}$. We 
 	can conclude by observing that	$\Pred_{tgt_x}$ does not use any variable of $P$ 
 ...
 similar.\qed
        ...
        \newpage
        \section{Proof of Theorem ~\ref{thm-ctxt-eq}}
