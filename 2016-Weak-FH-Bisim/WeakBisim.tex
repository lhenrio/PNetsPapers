...
$P\models{\openrule
			{
				(\beta^1_{p})_j^{j\in (J\setminus \Holes(Q)) \cup \{j_0\}}, 
				{\Pred^1_p}\!',  
				{\Post^1_p}\!'}
	...
	\,\text{~~and~~} 
		Q\models{\openrule
			{
				(\beta^1_{p})_j^{j\in J\cap\Holes(Q)}, {{\Pred^1_p}\!''},  
				{{\Post^1_p}\!''}}
...
$P\models{\openrule
			{
				(\beta^1_{p})_j^{j\in (J\setminus \Holes(Q)) }, 
				{\Pred^1_p}\!',  
				{\Post^1_p}\!'}
			{\ostate{(s_{p l})^{l\in L\setminus L_Q}} \OTarrow {\tau}
...
$
		P\models{\openrule
			{
				(\beta^2)_j^{j\in (J\setminus \Holes(Q)) \cup \{j_0\}}, 
				{\Pred^2}',  
				{\Post^2}'}
	...
	Q\models{\openrule
			{
				(\beta^2)_j^{j\in J\cap\Holes(Q)}, {\Pred^2}'',  
				{\Post^2}''}
...
$\forall j\in (J\setminus \Holes(Q))\cup \{j_0\},\quad
\displaystyle{\gamma_j}=\mybigdotcup_{i=0}^{m_1}  (\beta^1_{ij}) \dotcup  (\set{\beta^2}_i) \dotcup \mybigdotcup_{i=0}^{m_2} (\beta^3_{ij})
\\
\Pred\,'=\bigwedge_{i=0}^{m_1}
{\Pred_i^1}'\land{\Pred^2}'\bigotimes_{i=0}^{m_1}\subst{{\Post_i^1}'}\land (\bigwedge_{i=0}^{m_2}{\Pred\,^3_i}'\subst{{\Post^2}'})\bigotimes_{i=0}^{m_1}\subst{{\Post^1_i}'}\\
\Post'=(\bigotimes_{i=0}^{m_2}{\Post^3_i}')\subst{{\Post^2}'}\bigotimes_{i=0}^{m_1}\subst{{\Post^1_i}'}
$\\
\TODO{To check: ce n'est pas $m_1, m_2$ mais $m_1', m_2'$ such that 
 $m_1'= m_1\setminus\Holes(Q)$???}\\
...
$\forall p \in [0\ldots n_2] Q\models{\openrule
			{
				(\beta^3_{p})_j^{j\in J\cap\Holes(Q)}, {{\Pred^3_p}''},  
				{{\Post^3_p}''}}
...
$
and
$\forall p \in [(n'+1)\ldots (len-1)]\,\, Q\models{\openrule
			{
				(\beta^3_{p})_j^{j\in J\cap\Holes(Q)}, {{\Pred^3_p}''},  
				{{\Post^3_p}''}}
				{\ostate{v_{pl}^{l\in L_Q}} \OTarrow {\tau}
...
$\\ 
\TODO{ Attention peut dire que ${{\Pred^1_p}''}, {{\Pred^2}''}$ are diffrent from previous one}\\ 
Thereafter, by Lemma \ref{lem-rel-OT-WOT} we obtain the following weak open transition:
\begin{mathpar}
Q\models{\openrule
			{
			(\gamma_k)_j^{j\in J\cap\Holes(Q)},  {{\Pred_k}},  
				{{\Post_k}}}
			{\ostate{v_{l}^{l\in L_Q}} \OTWeakarrow {\alpha_{k}}
				\ostate{v_{l}'^{l\in L_Q}}}
		}%\]
\end{mathpar}
$v_l=v_{0l} \wedge v_l'=v_{len}$\\
$\forall j \in {j\in (J\cap\Holes(Q))},\quad
\displaystyle{\gamma_{kj} }=\mybigdotcup_{i=0}^{(n'_1-1)}\!\!(\beta^1_{ij})\,\dotcup\,(\set{\beta^2})_j\,\dotcup\!\!\!\mybigdotcup_{i=(n'+1)}^{(len-1)}\!\!\!(\beta^3_{ij})
\\
\Pred_k=\!\!\bigwedge_{i=0}^{(n'_1-1)}\!\!
{\Pred_i^1}''\land{\Pred^2}''\bigotimes_{i=0}^{(n'_1-1)}\subst{{\Post_i^1}''}\land \big(\!\!\!\!\!\bigwedge_{i=(n'+1)}^{(len-1)}\!\!{\Pred^3_i}''\subst{{\Post^2}''}\big)\bigotimes_{i=0}^{(n'_1-1)}\subst{{\Post^1_i}''}\\
\Post_k=\big(\!\!\!\!\bigotimes_{i=(n'+1)}^{(len-1)}{\Post^3_i}''\big)\subst{{\Post^2}''}\bigotimes_{i=0}^{(n'_1-1)}\subst{{\Post^1_i}''}
$\\
...
For  $\gamma_{j_0}=[\beta_0..\beta_n]$ we have $n$ weak open transitions  $\symb{WOT}_p$ such that $p\in [0\ldots n]$.
Therefore, we can conclude that we have forall $p\in [0\ldots n]$:  
\begin{mathpar}
Q\models{\openrule
			{
