...
Figure \ref{SimpleProtCounter:ImplOA} shows the full open automaton of the implementation $SimpleSystem$. Here, in the open automaton drawing, we use a user-friendly notation for the OTs, in the form ``$\{actions of holes\} [guard] action \{assigns\}$'', rather than the full OT form with the red-dotted rule. Note that the resulting action of the OTs appear here betwen the guard and the set of assignments.
Note that we have also simplified the form of the variable dependencies in the OTs, again for readability, replacing e.g. in OT1 $\{P\mapsto p\_send(m)\} \qquad [m=m']$ and $\alpha= \underline{in(m')}$ by the shorter \texttt{\{P->p\_send(m)\} in(m)}.
