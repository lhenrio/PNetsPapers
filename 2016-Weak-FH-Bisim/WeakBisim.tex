...
Relying on open automata, it then defines bisimulation relations  for the comparison of systems specified as pNets.
 We first present a strong bisimulation for open pNets.
...
\begin{itemize}
\item The definition of open automata: an algebra of parameterised automata with holes, and a strong bisimulation equivalence. This is an adaptation of~\cite{henrio:Forte2016} with an additional result stating that strong bisimulation equivalence is indeed an equivalence relation.
...
A pNet $\pNet$ is a hierarchical structure where leaves are pLTSs and holes \qquad
...
Two open automata are FH-bisimilar if there exist an FH-bisimulation that relates their initial states.
...
It is well-known that weak bisimulation is not a congruence in CCS, this corresponds to the fact that the $+$ operator can observe the $\tau$ transitions. Thus, even if we can define a weak bisimulation for CCS with $+$ it does not verify the necessary requirements for being a congruence.
