...
Consider rule \TrDeux\ in 
       Definition~\ref{def:operationalSemantics}, applied to the pNet $P$. 	
...
$P$.  $j_0\in I_1$ because $Q$ is not a pLTS.
...
\begin{lemma}[Weak OT decomposition]\label{lem-decompose} 
	...
	\]
		...
		$\Post\,'$,   $\gamma_{j_0}=[\beta_0..\beta_n]$, and for all $p\in[0..n]$ there exist $\alpha_p$, $\Pred_p$, $\Post_p$ and a family $\gamma_{p j}^{j\in J_p}$ and for all $p\in[0..n+1]$ $s_{p i}^{\,i\in L_Q}$. s.t.:\\[-2ex]
		%\[
		\begin{mathpar}
		P\models{\openrule
			{
...
%p\in[0..n]
%\end{array}} 
\bigcup_{i=0}^{n}J_{p} = J\cap\Holes(Q) \qquad
 ...
 \text{and\quad} \forall i \in L_Q.\, s_{(n+1) i} = s_i'\land s_{0 i} = s_i\\{\hspace{-3ex}}\text{and~~} \Pred \iff \Pred\,'
		\land \!\!\bigwedge_{p=0}^n([\alpha_p]=\gamma_{pj_0}\land \Pred_p) \qquad\text{and~}
...
where $\Post_p$ only act upon variables of 
		$\Leaves(Q)$.
\end{lemma}
\LUDO{A V2 by Ludo} \TODO{We will check notation  $\alpha_p=...$}

\begin{proof}
By lemma \ref{lem-rel-OT-WOT} we have: 
\begin{mathpar}
 P[Q]_{j_0} \models \openrule
			{
				\gamma_j^{j\in J}, \Pred,  
...
$			
	P[Q]_{j_0}	\models		
\Big(\forall i \in [0\ldots n].\openrule
    {
       \set{\gamma^1_i},\Pred^1_i,\Post^1_i   }
         {\ostate{s^1_i} \OTarrow {\tau} \ostate{s^1_{i+1}}}\, \wedge\,
\openrule
         {
           \set{\gamma^2},\Pred\,^2,\Post^2 }
         {\ostate{s^2} \OTarrow {\alpha} \ostate{s'^2}}
\,\wedge\,
\forall i \in [0\ldots m].\openrule
         {
           \set{\gamma^3_i},\Pred\,^3_i, \Post^3   }
...
On other hand by Theorem \ref{thm-composability} we have:


\TODO{La preuve page 27 est difficile a suivre...}
