...
For this property to be valid when the substitution does not operate on all variables,
...
More precisely, for any pair $(s,t)\in \mathcal{S}_1\times \mathcal{S}_2$, there is a 
   single
      ...
      if $\Pred_{s,t}$       is 
      True, i.e. the states are related when the value of the variables in $s$ and $t$  verify the predicate $\Pred_{s,t}$. Note that this implies that the free variables of  $\Pred_{s,t}$ belong to $V_1$ and $V_2$. Each variable used in $\Pred_{s,t}$ should refer to a state variable of $s$ or $t$ and there must be no ambiguity on the referenced variable. Consequently, the considered relation is well-defined if the variables of the two open automata are disjoint, i.e.  $V_1\cap V_2=\emptyset$.
...
\item	The second rule deals with pNet nodes: for each possible
	...
	I_2$}\footnote{It is of course possible to replace $I_1$ and $I_2$ by their formal definition but the rule would be more difficult to read.}.                                                                    
\begin{mathpar}
    \mprset {vskip=.45ex}
\inferrule
    {
...
We define weak open transitions similarly to open transitions except that holes can perform \emph{sequences of observable actions} instead of single actions (observable or not).
