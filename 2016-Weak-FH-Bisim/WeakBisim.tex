...
%\begin{minipage}{\linewidth}
 ...
 \includegraphics[width=4.5cm]{XFIG/TransportSpec}
  \caption{Schema and specification of the protocol }  \label{schema:ABP-pnets}
%\end{minipage}
  \vspace{1cm}
%\begin{minipage}{6cm}
  \centerline{\includegraphics[width=11cm]{XFIG/ABP3-Schema}}
...
\begin{example}\emph{An open-transition.}
  \label{OT:ABP}
  The \texttt{ABP} pNet of Fig. \ref{schema:ABP-composed} has 2 controllers and 2 holes. One of its possible open-transition is:
...
%                       1 \xrightarrow{send(m,sb)}_{Sender} 2 ~~
   ...
   Pred=[$m=s_1\_m, b=s_1\_sb]$] Post=$\{M1\_m:=m, M1\_b:=b\}
                      }
  %  {\ostate{00} \xrightarrow{\underline{\delta(x1)}} \ostate{10}}
    {\ostate{1,0,0,0} \OTarrow{\underline{send(m,b)}} \ostate{2,1,0,0}}
  $

Or at $SV_{Trnasport}$ level:
 \smallskip
 $  OT_2  = \openrule{
                       1 \xrightarrow{send(m,sb)}_{Sender} 2 ~~
                            \xrightarrow{in(?m,?b)}_{M1}
  ...
  %  {\ostate{00} \xrightarrow{\underline{\delta(x1)}} \ostate{10}}
    { \OTarrow{\ostate{1,0,0,0}\underline{send(m,b)}} \ostate{2,1,0,0}}
  $
\end{example}
...
\begin{array}{l}
\exists {(a'_i)}^{i\in I},
{(b'_j)}^{j\in J},v'.\, SV=\SV{{(a'_i)}^{i\in I}, {(b'_j)}^{j\in J}} {v'} {e_b}
\\~~\land
...
%	equations of a predicate.
	...
	\in M}}{\alpha'}{e_b}$ is a synchronisation vector  of \pNet  then $K=M\cap 
	\Holes(\pNet)$, $I_2=M\cap \Leaves(\pNet)$,  $I_1=M\setminus K \setminus 
	I_2$}.                                                                               
    ...
    $\inferrule
    {
	\forall m\!\!\in\!\! I_1. {\pNet_m \models\inferrule*[myfraction=\reddottedrule]
    	{
    	\{{b_{j}}\}^{j\in J_m}, \Pred_m, \Post_m}
...
k\!\in\! K \qquad SV_k \!=\! \SV{\alpha_m^{m \in I_1\uplus I_2\uplus J}}{\alpha'}{e_b} \\
\\     	
     J' = \biguplus_{m\in I_1}\!\! J_m \uplus J 	\\
    	\Pred = \bigwedge_{m\in (I_1\uplus I_2)}\!\! \Pred_m \land
    	\Pred_{\symb{sv}}(SV_k,v_m^{m\in (I_1\uplus I_2)},b_j^{j\in J},v)\\ 
    ...
    {\mylangle \overline{\pNet}, \overline{\Sort}, \symb{SV}_k^{k\in K}\myrangle
    	...
    	{\inferrule*[myfraction=\reddottedrule]
    		{
    		\{{b_j}\}^{j\in J^\prime}, \Pred, \uplus_{m\in I_1\uplus I_2} 
    		\Post_m}
...
\end{minipage}
 ...
 \begin{mathpar}
     \inferrule*[myfraction=\reddottedrule]
         {
           \{{b_j}\}^{j\in J_1},\Pred_{OT},\Post_{OT}}
         {s \OTarrow {v} s'}
...
%    \left( fresh \ \overline{a_i}, \overline{b_j}, v_x.\ \
    \inferrule*[myfraction=\reddottedrule]
         {
           \{{b_{j x}}\}^{j\in J_{x}}, \Pred_{OT_x},\Post_{OT_x}}
         {t \OTarrow {v_x} t_x}
%         \right)
\end{mathpar}
...
\medskip
       	...
       	\inferrule*[myfraction=\reddottedrule]
       	{
       		\{{b_{j p x}}\}^{j\in J_{px}}, \Pred_{OT_{px}},\Post_{OT_{px}}}
...
\begin{lemma}[OT decomposition]\label{lem-decompose} 
	...
	\]
		...
		P\models{\inferrule*[myfraction=\reddottedrule]
			{
				\{{b_j}\}^{j\in (J\setminus \Holes(Q)) \cup \{j_0\}}, 
				\Pred\,',  
				\Post\,'}
	...
	\vspace{-2.2ex}\\\text{and~~}
		...
		Q\models{\inferrule*[myfraction=\reddottedrule]
			{
				\{\xrightarrow{b_j}\}^{j\in J\cap\Holes(Q)}, \Pred\,'',  
				\Post\,''}
...
$L_{j_0}=L_Q$, and $J_{j_0}=	J\cap\Holes(Q)$):\\[-2ex]
	\[Q\models{\inferrule*[myfraction=\reddottedrule]
		{
			\{{b_j}\}^{j\in J\cap\Holes(Q)}, \Pred_{j_0},  
			\Post\,''}
...
Then, we have\\[-2ex]
	...
	{\inferrule*[myfraction=\reddottedrule]
		{
			\{{b_j}\}^{(j\in J\setminus\{j_0\}) \uplus J_Q}, 
...
\[
    	...
    	\models\inferrule*[myfraction=\reddottedrule]
    	{
    	\{{b_{j}}\}^{j\in J_m}, \Pred_m, \Post_m}
    	{\ostate{s_{i}^{i \in L_m}} \OTarrow {v_m}
    		\ostate{(s_i^\prime)^{i\in L_m}}} }	
...
\end{small}
 ...
 Q'\models\inferrule*[myfraction=\reddottedrule]
 {
 	\{\xrightarrow{b_{j x}}\}^{j\in J\cap\Holes(Q)}, 
