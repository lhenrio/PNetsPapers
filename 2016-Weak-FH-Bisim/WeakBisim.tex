...
\wedge\\
\forall p \in [0..n].\openrule
         {
           \set{\beta^3_p},\Pred^3_p, \Post^3_p}
         { \ostate{(s^3_{pi})^{i\in L}} \OTarrow {\tau} \ostate{(s^3_{(p+1)i})^{i\in L}}} \in \mathcal{T}$ 
\\
where for all $l\in L$:\\
$s_i=s^1_{0 i} \wedge s^1_{(m_1+1) i}=s^2_{0i} \wedge  s^2_{1i}= s^3_{0 i} \wedge s^3_{(m_2+1) l}=s'_i$\\
...
\[Q\models{\openrule
			{
				...
				{{\Post'^{2}}}}
				{\ostate{s_{i}^{i\in L_Q}} \OTarrow {\alpha'_p}
				\ostate{s_{i}'^{i\in L_Q}}}
				}%\]
\]
...
\openrule
    {
       {\beta_p^{p\in J}},\Pred_p,\Post_p   }
         {\ostate{s^{i\in L}_{pi}} \OTarrow {\alpha} \ostate{s^{i\in L}_{(p+1)i}}}$ 
\begin{enumerate}
\item If $Q$ moves meaning that we have $Q\!\models		
\openrule
    {
