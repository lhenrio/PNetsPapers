...
\item \TODO{uniformiser pNet \pNet pNet s \pNet s (dans le texte et les formules!)}

\item \TODO{proposition Ludo \& Rabea: \pNet -> P,Q dans les formules pour que ce soit moins long}
\end{itemize}
Ludo:
\begin{itemize}
...
\begin{theorem}[Context equivalencefor  weak bisimulation]\label{weak-thm-ctxt-eq}
...
\begin{lemma} Let $A = <J,\mathcal{S}, s_0,
    ...
    \WT>$ be the
weak open automata derived from $A$. 
...
\begin{proof} ($\Rightarrow$) We proceed by induction the number of transitions $l$ with which we  build  a weak transition (introduced in Definition \ref{def:buildweakOT}). We have  $l=n+m+1$.
\begin{itemize}
\item The base case $l=1$ ($n=0$ and $m=0$) then obtain
\begin{mathpar}
 % \mprset {vskip=.5ex}
%\inferrule{
 \openrule
...
\end{mathpar} 
\item For the inductive step, first we have by induction hypothesis that the formula holds for some length $l$.   Induction step is to infer that formula holds for transitions of length $l + 1$. In particular, for $n+1$ or $m+1$.


First by using rule (2) (and by replacing $\alpha$ by $\tau$)  we can directly conclude :
\begin{mathpar}
 % \mprset {vskip=.5ex}
\inferrule{
 \openrule
...
\end{itemize}
\noindent ($\Leftarrow$) We proceed also  by induction the number of transitions $l$ with which we  build  a weak transition.
...
W have two cases:
\begin{itemize}
\item either $\alpha = \tau$, $s=s'$, $\overline{\beta}=\emptyset$, $\Pred = \symb{True}$ and $\Post=\symb{Id}(s)$. \TODO{Conclusion?}

\item Or we can directly conclude by rule (2) that we have
\begin{mathpar}
\openrule
         {
           \overline{\beta},\Pred,\Post}
         {s \OTarrow {\alpha} s'} \in \mathcal{T}
\end{mathpar}
such that $\overline{\gamma} = \vis{\overline{\beta}}$
\end{itemize}
...
$\Pred=\Pred_1\land\Pred_2\subst{\Post_1}\land (\Pred_3\subst{\Post_2})\subst{\Post_1}$, and $\Post=$ \\

By induction hypothesis and simplification we have: \\
...
\forall i \in [0\ldots n_1].\openrule
    {
       \overline{\beta^1_i},\Pred^1_i,\Post^1_i   }
         {s^1_i \OTarrow {\tau} s^1_{i+1}} \in \mathcal{T}         
\end{mathpar}
...
\\\\
\overline{\gamma}=\biguplus_{i=0}^{n_2} \vis{\overline{\beta^1_i}} \dotcup  \vis{\overline{\beta^2}} \dotcup \biguplus_{i=0}^{m_2} \vis{\overline{\beta^3_i}}
\\
...
First, using rule (1) we can directly conclude \\


Second by using rule (2),  
\begin{mathpar}
{ \openrule
         {
          \overline{\gamma} \!,\Pred,\Post
...
implies that 
.\\
Finally by using rule (3) we have
\begin{mathpar}
\inferrule{
\openrule
         {
...
\end{mathpar}
and by induction hypothesis this means that we have first,
\begin{mathpar}
%\inferrule{
\openrule
         {
...
implies
\begin{mathpar}
\forall i \in [0\ldots n_2].\openrule
    {
       \overline{\beta^1_i},\Pred^1_i,\Post^1_i   }
         {s^1_i \OTarrow {\tau} s^1_{i+1}} \in \mathcal{T} \quad \wedge
\quad
