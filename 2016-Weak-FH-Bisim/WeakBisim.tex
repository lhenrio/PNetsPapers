...
We consider the case $n'=n+1$, $m'=m+1$ is similar. Side conditions are omitted for readability. We first focus on the left part of the disjunction. Suppose we have:
\begin{mathpar}
...
\openrule
         {
          \vis{\overline{\beta}} \dotcup \overline{\gamma},
		\Pred,
				({x_k\gets (e_k\subst {\Post_2})\subst{\Post_1}})^{k\in K} } 
         {s \OTWeakarrow {\alpha} s''} \in\WT
} \WTTrois
...
} \implies \Big(\alpha = \tau \wedge  \overline{\gamma}=\emptyset \wedge \Pred =\True \wedge \Post =\Id(s)\Big)
\end{mathpar}
...
\item By induction hypothesis this means each tau weak transition can be written as a series of tau transitions, hence by simplification we have: 
\begin{mathpar}
%\inferrule{
\bigg( \openrule
         {
...
where\\
$
s_2=s^3_0 \wedge s^1_{m_1}=s',
\overline{\gamma_3}=\biguplus_{i=0}^{m_1} \vis{\overline{\beta^3_i}},
\Pred_3\!=\!\bigwedge_{i=0}^{m_1}\Pred^3_i,\Post_3\!=\! \biguplus_{i=0}^{m_1} \Post_i^3\\
$

\item On the other hand, by induction hypothesis we have:
\begin{mathpar}
\bigg(\openrule
         {
           \overline{\gamma_2},\Pred_2,\Post_2  }
