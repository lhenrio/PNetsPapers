...
\begin{example}
  ...
  $\openrule
         ...
         {\text{b1} \OTWeakarrow \tau \text{b1}} \forall n \geq 0$.  Finally,  applying again \WTTrois, and using the central open transition having \texttt{\underline{out(m,ec)}}  as $\alpha$, we get the resulting weak open transition between b1 and b0 (as shown in Figure \ref{WOT2}).
 %     $\openrule
...
$ WI_{8c}(n) = \openrule
         ...
         {210 \OTWeakarrow {\underline{\texttt{out(r1\_msg,r1\_ec)}\subst{(\texttt{r1\_msg}\gets \texttt{s1\_msg}, \texttt{r1\_ec}\gets \texttt{s2\_msg}+n)}}} 202}
        $($\forall n\ge 0$)
