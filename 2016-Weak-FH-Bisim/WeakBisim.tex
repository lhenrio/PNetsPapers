...
Additionally, we have
$[\alpha'_0.. \alpha'_n]$ is a a list of $\alpha_p$ such that $p\in [0..(n_1\!+\!n_2\!+\!2)] \land \alpha_p\neq \tau$ where for all $k\in [0\ldots n_1+n_2+2]$, $\beta_{kj_0}=\alpha_k$. In other words, $\mybigdotcup_{i=0}^{m_1}  \vis{\beta^1_{i{j_0}}} \dotcup  \vis{\set{\beta^2_i}} \dotcup \mybigdotcup_{i=0}^{m_2} \vis{\beta^3_{i{j_0}}} = [\alpha'_0.. \alpha'_n]$. This ensures that $\gamma_{j_0}=[\alpha'_0.. \alpha'_n]$.
...
$ \forall i \in [0..m].\openrule
    {
       \set{\beta^1_i},\Pred^1_i,\Post^1_i   }
         {s_{il}^{\, l\in L} \OTarrow {\tau} s_{(i+1)l}^{\, l\in L}} \in \mathcal{T} \quad \wedge
\quad
\openrule
         {
           \set{\beta^2},\Pred^2,\Post^2 }
         {t_{l}^{\, l\in L} \OTarrow {\alpha} {t'}_{l}^{\,l\in L}} \in \mathcal{T}
\quad \wedge\\
\forall i \in [0..n].\openrule
         {
           \set{\beta^3_i},\Pred^3_i, \Post^3_i}
         {v_{il}^{\, l\in L} \OTarrow {\tau} v_{(i+1)l}^{\, l\in L}} \in \mathcal{T}$ 
\\
where for all $l\in L$:\\
$s_l=s_{0 l} \wedge s_{(m_1+1) l}=t_l \wedge t'_l = u_{0 l} \wedge u_{(m_2+1) l}=s'_l$\\
$\displaystyle{\gamma_j^{j\in J}=\mybigdotcup_{i=0}^{m_1} \vis{\set{\beta^1_i}} \dotcup  \vis{\set{\beta^2}} \dotcup \mybigdotcup_{i=0}^{m_2} \vis{\set{\beta^3_i}}}\\
...
\end{mathpar}
Also, by Lemma \ref{lem-rel-OT-WOT} this implies the following:\\
$\forall k\! \in\! [0..m']\,\, Q\models{\openrule
			{
				(\beta^1_{kj})^{j\in J\cap\Holes(Q)}, {{\Pred_k^{1b}}},  
				{{\Post_k^{1b}}}}
				{\ostate{s_{kl}^{l\in L_Q}} \OTarrow {\tau}
				\ostate{s_{(k+1)l}^{l\in L_Q}}}
				}%\]
,$ 
$Q\models{\openrule
			{
				(\beta^2_j)^{j\in J\cap\Holes(Q)}, {{\Pred^{2b}}},  
				{{\Post^{2b}}}}
				{\ostate{s_{l}^{l\in L_Q}} \OTarrow {\alpha'_p}
				\ostate{s_{l}'^{l\in L_Q}}}
				}%\]
$
and
$\forall k \in [0..n']\,\, Q\models{\openrule
			{
				(\beta^3_{kj})^{j\in J\cap\Holes(Q)}, {{\Pred_p^{3b}}},  
				{{\Post_p^{3b}}}}
				{\ostate{s_{kl}^{l\in L_Q}} \OTarrow {\tau}
				\ostate{s_{(k+1)l}^{l\in L_Q}}}
				}%\]
$\\ 
Knowing that we have $j_0\in J$ and $\gamma_{j_0}=[\beta_0..\beta_n]$:
Suppose that $Q$ moves in transition $\alpha$, then there is  a matching $p_0$ transition for some $p_0\in[0..n]$. 
\[ P[Q]_{j_0}  
	\models
	{\openrule
		{
			(\beta^2_j)^{(j\in J\setminus\{j_0\}) \uplus J_Q}, 
			\Pred^2\land\Pred^{2b}\land \alpha'_{p_0}=\beta^2_{j_0},  
			\Post^2\uplus \Post^{2b}}
		{\ostate{s_i^{i\in L\uplus L_Q}} \OTarrow {\alpha}
			\ostate{s_i'^{\, i\in L\uplus L_Q}}}
	}
	\]
NOTE: And $\beta^2_{j_0}= \beta_{p_0}$.

Moreover, for each:
$\openrule
         {
           \set{\beta_i},\Pred_i, \Post_i}
         {s_i \OTarrow {\tau} s_{i+1}} \in \mathcal{T}$
\qed
\end{proof}
