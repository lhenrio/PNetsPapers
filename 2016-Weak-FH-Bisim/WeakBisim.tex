...
$receiver$. The underlying network is modelled by a medium entity transporting messages from the sender to the receiver, and that is able to detect some transport error, and signal it to the sender. The return $ack$ message from $receiver$ to $sender$ is supposed to be safe.

This is only a graphical syntax,
with an obvious semantics for building the corresponding pNets.

Fig. \ref{SimpleProt:Spec} shows the Schema of this system, and on the right side, its global specification, expressed as a pNet: it should be equivalent to a simple system where the holes would be connected by a perfect reliable medium.
...
\subsection{Open Automata}
 Open automata are not composition structures but they are made of transitions that are dependent of the actions of the holes, and they can reason on variables (potentially with only symbolic values).
...
\TODO{rertavailler le paragraphe ci dessous}
In
...
\begin{theorem}[Congruence]\label{thm-congr-weq}
	...
	$\pNet[\pNetQ']_{j_0}$ are weak FH-bisimilar.
\end{theorem}
 \begin{proof}  Consider $Q$ weak FH-bisimilar to $Q'$.  It means, there exist a FH-bisimulation $\mathcal{R}_{Q,Q'}$ relating the two pNets $Q$ and $Q'$. We define a relation $\mathcal{R}$ relating states of $P[Q]_{j_0}$ with states of $P[Q']_{j_0}$: 
\[\mathcal{R} = \{(\ostate{S_P\uplus S_Q},\ostate{S_P\uplus S'_Q}, \Pred_{Q,Q'})|\,(s_Q,s_{Q'}, \Pred_{Q,Q'})\in\mathcal{R}_{Q,Q'}\}\]


To prove weak FH-bisimulation of $\pNet[\pNetQ]_{j_0}$ and 
	$\pNet[\pNetQ']_{j_0}$, we consider  an open transition of $\pNet[\pNetQ]_{j_0}$, and an equivalent state of $\pNet[\pNetQ']_{j_0}$, and we try to find a family of WOT of 	$\pNet[\pNetQ']_{j_0}$ that simulates it.
...
\end{theorem}
 ...
 \begin{proof}   Consider $P$ weak FH-bisimilar to $P'$.  It means, there exist a FH-bisimulation $\mathcal{R}_{P,P'}$ relating the two pNets $Q$ and $Q'$. We define a relation $\mathcal{R}$ relating states of $P[Q]_{j_0}$ with states of $P[Q']_{j_0}$: 
\[\mathcal{R} = \{(\ostate{S_P\uplus S_Q},\ostate{S_P\uplus S'_Q}, \Pred_{Q,Q'})|\,(s_Q,s_{Q'}, \Pred_{Q,Q'})\in\mathcal{R}_{Q,Q'}\}\]
