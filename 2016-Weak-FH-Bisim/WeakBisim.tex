...
As mentioned in page \ref{def:open-automaton}, we adopt here a semantic view on open automata. More precisely, in \cite{hou:hal-02406098}, we  define
...
Let us point out that the weak OT loops ($WI_1$,$WI_2$ and $W_\tau$) on state ${000}$ are also present in all other states, we did not repeat them. Additionally, many WOTs are similar, and numbered accordingly as 3, 3a, 3b, 3c and 8, 8a, 8b, 8c respectively: they only differ by their respective source or target states; the "variant" WOTs appear in blue in   Figure~\ref{SimpleProtCounter:ImplWOA2}, note that composed WOTs also appear in blue.
\end{example}
...
\subsection{Weak FH-bisimulation for open pNets}
