...
conventions, that brings some user-friendly features that are mappeds by the editor into pure pNet constructs.
...
\begin{itemize}
  ...
  \item the variables in the guards of synchronisation vectors do not need to be explicitely quantified: all variables in a guard that do not appear inside the vector actions will be recognised as bound by a \emph{forall} quantifier inside the guard.
    \item the tools will check that everything is correctly declared, and that variables are used properly and do not conflict between different objetcs, that vectors have coherent length, etc.
\end{itemize}
...
\medskip That is trivialy true, choosing \texttt{p-a1=p-a2} for each given \texttt{p-a1}.
