...
\begin{theorem}[Congruence]\label{thm-congr-eq}
	...
	$\Sort(Q)=\Sort(Q')=S_{j_0}$\footnote{Note that $\Sort(Q)=\Sort(Q')$ is ensured by 
	stronfg bisimilarity.}. Then 
	$\pNet[Q]_{j_0}$ and 
...
of \emph{invisible actions} that are considered internal to some
...
\emph{invisible actions} depending on a specific action algebra.
...
\TODO{Eric: ask Elie, Hubert or others about interesting weak
  equationnal theories for Lotos -- DONE, waiting for answer...}
...
\[\mathcal{R}'' = \Big\{(s^1,s^3|\Pred\,''\phi)\Big|\begin{array}{l}
\displaystyle\Pred\,''=\bigvee_{ \begin{array}{l}
(s^1,s^2|\Pred)\in\mathcal{R}\\ (s^2,s^3|\Pred\,')\in\mathcal{R}' 	
\end{array}
}\,\Pred\land\Pred\,'\\
\land~ \phi \text{ renames variables of $\mathcal{T}_2$ into fresh variables}
\end{array}\Big\}\]
...
Free variables are renamed to avoid further clashes with variables of $\mathcal{T}_2$; in 
practice, because having name clashes between different terms of the disjunction is not 
an issue, there is no need to have a different $\phi$ for each predicate, and we will use 
a global renaming $\phi$ in the following.
...
$(s^1,s^2_p|\Pred_p)\in\mathcal{R}$,  and $(s^2_p,s^3|\Pred\,'_p)\in\mathcal{R}$. We have 
the 
following by definition of bisimulation.
For any open transition $OT$ in $\mathcal{T}_1$ originating from $s^1$:
Additionally we have $\Pred\,'' = (\bigvee_{p\in P} \Pred_p\land\Pred_p\,')\phi$.
\begin{mathpar}
...
\Pred_{target_{pxy}})\phi\implies 
\Pred\,'_{target_{pxy}}$ (this is one element of the  disjunction defining the 
predicate 
relating ${s^{1}}'$ and $s^{3}_{pxy}$ in the definition of $\mathcal{R}''$, note the use 
of the global substitution $\phi$ ensuring that $\Pred\,'_{target_{pxy}}$ does not use 
the variables of $\mathcal{T}_2$).
...
Additionally, the Post substitutions do not modify the predicates that only use 
non-substituted 
variables, and because of the domain of the substitutions:\\
...
\phi\\
%
\implies\Pred\,'_{target_{pxy}}\subst{\Post_{OT}}\subst{\Post_{OT_{px}\phi}}
\subst{\Post_{OT_{pxy}}}\\
%
= 
\Pred\,'_{target_{pxy}}\subst{\Post_{OT}}\subst{\Post_{OT_{pxy}}}\subst{\Post_{OT_{px}}\phi}\\
 $
Finally, gathering the previous results:
...
\left( \forall j. b_j=b_{jpxy}  \Rightarrow \Pred_{OT_{pxy}}
\land v\!=\!v_{pxy} \land
\Pred\,'_{target_{pxy}}\subst{\Post_{OT}}\subst{\Post_{OT_{pxy}}}\subst{\Post_{OT_{px}}\phi}\right)
$

This is the expected property except for the $\subst{\Post_{OT_{px}}\phi}$ however, now, 
this substitution only modifies internal variables of $\Pred\,'_{target_{pxy}}$ that are 
part of no other term of the assertion (in particular neither $\Pred\,''$ nor 
$\Pred_{OT}$), it can thus safely be removed to obtain the needed property.
