...
$\subst{y\gets x}$ is the substitution operation that replaces in a term all occurrences 
...
\RAB{diff entre Leaves(p) et overline\{Leaves\}?} 
...
\RAB{ce pred est-il diff de celui de la def 4?}
...
\RAB{notation S, T or underline\{S\}, underline\{T\}? pas clair...}
...
\RAB{que signifie notation uplus Post? Pourquoi fresh(v), et v existe satisfait Pred? aussi b\_j?} 
...
\begin{minipage}{0.69\linewidth} 	Suppose that
   ...
   and of the same sort.  %\LUDO{changed:} 
      Let 
      \ERIC{$\mathcal{R}=\{(s^1,s^2|\Pred_{s^1,s^2})\}$} be a
...
%       
       	...
       	\[\mathcal{R}'' = \{(s^1,s^3|\Pred\,'')|\,\Pred\,''=\bigvee_{ \begin{array}{l}
       		(s^1,s^2|\Pred)\in\mathcal{R}\\ (s^2,s^3|\Pred\,')\in\mathcal{R}' 	
       		\end{array}
       	}\,\Pred\land\Pred\,'\}\]

This relation is the adaptation of the transitivity to the conditional relationship that 
defines a bisimulation. Indeed the global disjunction together with the conjuction of 
