...
where
\[\set{\gamma'}=\mybigdotcup_{h=0}^k\vis{\set{\beta_{1h}}} \dotcup \vis{{\beta_{j}}^{(j\in J\setminus\{j_0\})} \uplus \set{\beta_2}} \dotcup \mybigdotcup_{h=0}^{k'}\vis{\set{\beta_{3h}}}\]
\[
\Pred_c=\begin{array}[t]{l}\displaystyle\bigwedge_{h=0}^k{\Pred_{1h}}\subst{\bigotimes_{i=h-1}^{0}\Post_i^1}\land
\left(Pred\land\Pred_2\land \alpha_Q=\beta_{j_0 }\right)\subst{\bigotimes_{i=k}^{0}{\Post_{1i}}}\land\\\displaystyle
...
Note also that $\Post$ only acts on variables of $P$ while $\Post_{1i}$ only acts on state variables of $Q$. A
\end{proof}
...
$\displaystyle{\gamma_j^{j\in J}=\mybigdotcup_{p=0}^{m_1} \vis{\set{\beta_{1p}}} \dotcup  \vis{\set{\beta_2}} \dotcup \mybigdotcup_{p=0}^{m_2} \vis{\set{\beta_{3p}}}}\\
