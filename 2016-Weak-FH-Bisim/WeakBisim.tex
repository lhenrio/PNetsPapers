...
We use ${\displaystyle \bigodot_{i=n}^{1} \Post_i}$ to denote  $\Post_n\otimes .. \otimes \Post_0$. By convention ${\displaystyle \bigodot_{i=0}^{1} \Post_i}$ is the identity. 
...
%\inferrule{
 \openrule
         {
           \set{\beta_{11}},\Pred_{11},\Post_{11}}
         {s_{11} \OTarrow {\tau} s_{12}} \in \mathcal{T}%}
\end{mathpar}
...
{ \openrule
       {
           \vis{\set{\beta_{11}}}\!,\Pred_{11},\Post_{11}
				 } {s_{11} \OTWeakarrow {\tau}s_{12}} \in \WT
}
\end{mathpar} 
...
\inferrule {\openrule
       {
           \vis{\set{\beta_{11}}}\!,\Pred_{11},\Post_{1}
				 } {s_{11} \OTWeakarrow {\tau}s_{12}}\! \in\! \WT
\\
\openrule
         {
...
\\\\
\set{\gamma''}= \vis{\set{\beta_{11}}} \oplus \set{\gamma}\subst{\Post_{1}}\\
\Pred\,''\!=\!\Pred_{11}\land\Pred\subst{\Post_{11}}\\\\%\land (\Pred_3\subst{\Post_2})\subst{\Post_{11}}
\Post\,''=\Id(s'')\shortodot\Post\shortodot\Post_{11} \\ \alpha''=\alpha'\subst{\Post_{11}}
}
{
\openrule
         {
         ...
         {s_{11} \OTWeakarrow {\alpha''} s'} \in\!\WT
} \WTTrois
\end{mathpar}
where we obtain the conclusion of the lemma, as required with the following assertions (derived from previous assertions):
\begin{align*}
s_{11}=s_{11} &\wedge s_{1(n+2)}=s_{2} \wedge s'_2 = s_{31} \wedge s_{3(m+1)}=s'\\
\alpha''=&\alpha\subst{\bigodot_{j=n+1}^{2}\Post_{1j}}\subst{\Post_{11}} = \alpha\subst{\bigodot_{j=n+1}^{1}\Post_{1j}}\\
...
where \\
$ 
s=s_{11} \wedge s_{1{(n_1+1)}}\!=s',\quad
\displaystyle{\set{\gamma_1}= \bigoplus_{i=1}^{n_1} \vis{\set{\beta_{1 i}}\subst{\bigodot_{j=i-1}^{1} \Post_{1 j} } }} \\
\Pred_1\!=\!\bigwedge_{i=1}^{n_1}(\Pred_{1i} \subst{\bigodot_{j=i-1}^{1}\Post_{i j}} ), \quad \Post_1= \bigodot_{i=n_1}^{1} \subst{\Post_{1i}} 
$
...
\displaystyle{\set{\gamma_3}= \bigoplus_{i=1}^{m_1} \vis{\set{\beta_{3 i}}\subst{\bigodot_{j=i-1}^{1} \Post_{3 j} } }} 

\land 

\Pred_3\!=\!\bigwedge_{i=1}^{m_1}(\Pred_{3i} \subst{\bigodot_{j=i-1}^{1}\Post_{3 j}} )

\land 

 \Post_3= \bigodot_{i=m_1}^{1} \subst{\Post_{3i}} 
\end{mathpar}
 ...
 \openrule
    {
       \beta_{3j}^{j \in J_3'},\Pred_3,\Post_3   }
         {s_{31} \OTarrow {\tau} s_{32}} \in \mathcal{T}          
\end{mathpar}
...
\text{\qquad and \qquad}\gamma_{j_0}=[\beta_0..\beta_n]
	\vspace{-2.2ex}\\\text{and for all $p\in[1..n]$~~}
		%\[
		Q\models{\openrule
			{
...
\\
		\text{such that~\qquad}  \bigcup_{p=1}^nJ_{p} = J\cap\Holes(Q) 
\text{, }
 \gamma_j^{j\in J\cap\Holes(Q)}= \bigoplus_{p=1}^n (\gamma_{p j}^{j\in J_p})\subst{\bigodot_{i=p-1}^{1}\Post_i}, \\
{~}\Pred \iff \Pred\,'
		\land \!\!\bigwedge_{p=1}^n(\alpha_p\subst{\bigodot_{i=p-1}^{1}\Post_i} = \beta_p\land \Pred_p\subst{\bigodot_{i=p-1}^{1}\Post_i} ),
\\
{~\hspace{2cm}}\Post=\Post\,' \uplus \bigodot_{p=n}^1
		\Post_p,   \text{and\quad} \forall i \in L_Q.\, s_{(n+1) i} = s_i'\land s_{1 i} = s_i\\
		\end{mathpar} 
...
Let 
\begin{mathpar}
J_Q=\displaystyle{\bigcup_{p=1}^nJ_{p}}


\forall{i\in L_Q}.\,s_i=s_{0i}~~\land~~s'_i=s_{(n+1) i}

\forall j\in J_p,\,\gamma_j= \displaystyle{\bigoplus_{p=1}^n \gamma_{p j}\subst{\bigodot_{k=p}^{1}{\Post_k}}}

\Pred\,'=\Pred\land \bigwedge_{p=1}^{n}(\alpha_p=\beta_p\land \Pred_p)\subst{\bigodot_{i=p-1}^{1}\Post_i} 
...
\land \bigwedge_{t=1}^{n} \Pred_t\bigodot_{i=t-1}^{1}\!\!\Post_i%\\&\qquad
