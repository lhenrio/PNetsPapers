...
\end{mathpar}
For each  weak open transition obtained above, we have  $(s_{11},t_x|\Pred_{s_{11},t_x})\in \mathcal{R}$  and $s_{11}=s_{20}$, so we have  $(s_{20},t_x|\Pred_{s_{20},t_x})\in \mathcal{R}$. Again 
...
\tau \land \alpha\!=\! \alpha_{x y} \subst{\Post_{OT_x}} \land (\Pred_{s_{21},t_{xy}} \land  \Pred_3) \subst{\Post_2\shortotimes \Post_1\uplus \Post_{OT_{x y}}\shortotimes\Post_{OT_x}} \Big)$\\ 
...
\tau \!\land \alpha\!=\!\alpha_{x y} \subst{\Post_{OT_x}}\land \alpha_{x y z}\!=\!\tau \land \Pred_{s_{31},t_{x y z}}  \subst{\underbrace{\Post_3\shortotimes\Post_2\shortotimes\Post_1}_{\Post_{OT}}\uplus \underbrace{\Post_{OT_{x y z}}\shortotimes \Post_{OT_{x y}}\shortotimes\Post_{OT_x}}_{\Post}} \Big)$\\
Having $s_{31}=s'$ we can conclude:\\
$\Pred_{s,t} \land \Pred_{OT}\\
\hspace{1cm} \implies\!\!\! \displaystyle{\bigvee_{x\in X}} \displaystyle{\bigvee_{y\in Y}} \displaystyle{\bigvee_{z\in Z}} 
\Big(\forall j\in J_{x y z}. \gamma_j =\gamma_{j x y z} \Rightarrow  \Pred \land \alpha =\alpha_{x y}\subst{\Post_{OT_x}} \land \Pred_{s',t_{x y z}}\subst{\Post_{OT}\uplus \Post} \Big)$\\
 This is exactly what we needed to prove \TODO{je dois generaliser}. \qed
\end{proof}
