...
\\
 ...
 \text{and\quad} \forall i \in L_Q.\, s_{(n+1) i} = s_i'\land s_{0 i} = s_i\\{\hspace{-3ex}}\text{and~~} \Pred = \Pred\,'
		\land \!\!\bigwedge_{p=0}^n((\alpha_p\bigotimes_{i=p-1}^{0}\Post_i) = \beta_p\land (\Pred_p\bigotimes_{i=p-1}^{0}\Post_i) )
\\
\text{and~}
...
Concerning $Q$, we reduce the sequence of OTs to a path for which it moves in all steps. In other words, if there is $Q$ does not move at step $q$, then we have $\ostate{s_{q_l}^{l\in L_Q}} = \ostate{{s}_{(q+1) l}^{l\in L_Q}}$ , then we skip the state $\ostate{{s}_{(q+1) l}^{l\in L_Q}}$, i.e.  we rename all the following states  $\ostate{s_{p l}^{l\in L_Q}}$ where $p\geq q+1$ by $\ostate{{s}_{(p-1) l}^{l\in L_Q}}$. Note that if $Q$ does an OT staying at the same state, we keep this necessary OT. We proceed in the same way  for states named $u$. To simplify the proof, we suppose that in case 3, $Q$  moves, else transition 3 of $Q$ should be skipped and the last $s_{p l}$ are equal to the first $u_{0 l}$. So we have: \\ 
$\forall p\! \in\! [0..n_1] Q\models{\openrule
			{
				(\beta_{1p})^{j\in J\cap\Holes(Q)}, {{\Pred''_{1p}}},  
				{{\Post''_{1p}}}}
				{\ostate{s_{pl}^{l\in L_Q}} \OTarrow {\alpha_{1p}}
				\ostate{s_{(p+1)l}^{l\in L_Q}}}
				}%\]
...
$\\ 
In this equation, and using case 1 above for all $k\in [0\ldots n_1]$ there is a $p$ (following the re-indexing done in the removal of steps where $Q$ does not move) $\Pred_{1p}\implies \alpha_{1p}=\beta_{1 p j_0}=\alpha_k$ and thus $\Pred_{1p}\subst{\bigotimes_{j=i-1}^{0}\!\!\Post_{1j}}\implies \beta_{1 p j_0}\subst{\bigotimes_{j=i-1}^{0}\!\!\Post'_{1j}}=\alpha_k\subst{\bigotimes_{j=i-1}^{0}\!\!\Post''_{1j}}$ (because $\beta_{1 p j_0}$ only contains variables of $P$ and $\alpha_k$ only variables of $Q$). Finally because $\Post''_{1j}$ is only definesd when $Q$ moves:
\[\Pred_{1p}\subst{\bigotimes_{j=i-1}^{0}\!\!\Post_{1j}}\implies
 \beta_{1 p j_0}\subst{\bigotimes_{j=i-1}^{0}\!\!\Post'_{1j}}=\alpha_k\subst{\bigotimes_{j=k-1}^{0}\!\!\Post''_{j}}\]
We have similar equations for $\alpha_{n_1+1}$ related with $\beta_{2 j_0}$ and the following $\alpha_k$ related with $\beta_{3 j_0}$ for some $p$. Note that the substitutions are however more complex. \TODO{detailler les equations?}

Overall we obtain (we skip here the details about the three cases 1, 2, and 3 above that all fall into the same equation because of the re-indexing we perform) \[\Pred \implies \gamma_{j_0}=
...
\end{mathpar}
$\alpha_k'=\alpha_{n_k}\displaystyle{\subst{\bigotimes_{j=n_k-1}^{n'_{k-1}}\!\!\Post''_{p}}}$\\
$\forall l \in L_Q. v_l=v_{(n'_{k-1})l} \wedge v_l'=v_{(n'_k-1)l}$, \\
$\displaystyle{\gamma_{kj}^{j\in J\cap\Holes(Q)} }=\displaystyle{\mybigdotcup_{i=n'_{k-1}}^{(n_k\!-\!1)} \vis{{\beta_{ij}^{j\in J\cap\Holes(Q)}}\subst{\bigotimes_{l=i-1}^{n'_{k-1}}\!\!\!\Post''_{l}}}\dotcup \vis{{\beta_{n_k j}^{j\in J\cap\Holes(Q)}}\subst{\bigotimes_{l=n_k-1}^{n'_{k-1}}\!\!\!\Post''_{l}}}} \dotcup\\
~~\qquad \mybigdotcup_{i=n_k+1}^{n_k'-1}
\vis{{\beta_{ij}^{j\in J\cap\Holes(Q)}} \subst{\bigotimes_{l=i-1}^{n_k\!+\!1}\!\Post''_{l}\shortotimes\Post''_{n_k}\shortotimes\!\!\bigotimes_{l=n_k-1}^{n'_{k-1}}\!\!\!\Post''_{l}} }\\
%
\Pred_k=\bigwedge_{i=n'_{k-1}}^{n_k\!-\!1}\!\!\Pred''_{i}\subst{\bigotimes_{j=i-1}^{n'_{k-1}}\!\!\Post_{j}}\land\Pred''_{n_k} \subst{\bigotimes_{j=n_k-1}^{n'_{k-1}}\!\!\Post''_{j}}\land\\ 
~\qquad\quad\bigwedge_{i=n_k+1}^{n_k'-1}\!\!\!\Pred''_{i}\subst{\bigotimes_{j=i-1}^{n'_{k-1}}\!\!\!\Post''_{j}\shortotimes\Post''_{n_k}\shortotimes\!\!\bigotimes_{j=n_k-1}^{n'_{k-1}}\!\!\!\Post''_{j}}
\\
\Post_k= \bigotimes_{j=n_k'-1}^{n'_{k-1}}\!\!\Post''_{j}$\\
...
Note that \[\alpha_{n_k}\displaystyle{\subst{\bigotimes_{j=n_k-1}^{0}\!\!\Post''_{p}}} = \alpha_k'\displaystyle{\subst{\bigotimes_{j=n'_{k-1}}^{0}\!\!\Post''_{p}}} = 
\alpha_k'\displaystyle{\subst{\bigotimes_{j=k-1}^{0}\!\!\Post_{p}}}\]
We now have: \TODO{add ref to eq 1 page above?}
\[\Pred \implies \gamma_{j_0}=
\vis{[\alpha'_{p}\subst{\bigotimes_{j=p-1}^{0}\!\!\Post_{j}}|p\in[1..n]]}\]
 
%$\displaystyle{\gamma_{j_0}}=\mybigdotcup_{i=0}^{m_1}  \vis{\beta_{1i{j_0}}} \dotcup  \vis{{\beta_{2j_0}}} \dotcup \mybigdotcup_{i=0}^{m_2} \vis{\beta_{3i{j_0}}}$. Additionally, we have
%$[\alpha'_0.. \alpha'_n]$ is the list of non-$\tau$ $\alpha_p$.
%% such that $p\in [0..(n_1\!+\!n_2\!+\!2)] \land \alpha_p\neq \tau$ where for all $k\in [0\ldots n_1+n_2+2]$, $\beta_{kj_0}=\alpha_k$. 
%In other words, $\displaystyle{\mybigdotcup_{i=0}^{m_1}  \vis{\beta_{1i{j_0}}} \dotcup  \vis{{\beta_{2j_0}}} \dotcup \mybigdotcup_{i=0}^{m_2} \vis{\beta_{3i{j_0}}} = [\alpha'_0.. \alpha'_n]}$. %This ensures that $\gamma_{j_0}=[\alpha'_0.. \alpha'_n]$.
%
%
%Consequently 
...
\TODO{stopped here not sure about the following}
