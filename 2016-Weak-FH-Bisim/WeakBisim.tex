...
\TODO{TODO ... or rather a dedicated section}
...
The semantics of open \pNet s will be defined  as an open automaton. An open
...
In this translation, the states of the open automata will be obtained from the states of the \pLTS s at the leaves of the composition. The predicates on the transition will be obtained both from the predicates on the \pLTS s transitions and from the
\TODO{Ce paragraph a eu un probleme, je ne sais pas quand.... Vous avez une vieille version sous la main ?}
This allows us to derive a bismulation theory for open \pNet s. As \pNet s are composition structures, it then makes sense to prove composition lemmas: we prove that the composition of bisimilar pNets are themselves bisimilar.
...
As  our symbolic operational semantics provides an open automaton, we can apply the notion of
	strong (symbolic) bisimulation on automata to open \pNet s:
\begin{definition}[FH-bisimulation for open \pNet s]\label{def:bisim-pnets}
...
\openrule
         {
           \overline{\gamma_1}\dotcup \overline{\gamma_2}\dotcup\overline{\gamma_3},
		\Pred,
...
\begin{lemma} Let $A = <J,\mathcal{S}, s_0,
    \mathcal{T}>$ be open automata and $<J,\mathcal{S}, s_0,
    \WT>$ be the
weak open automata derived from $A$.  For any weak open transition $\symb{WOT} \in \WT$, there exists an open transitions $\symb{OT} \in \mathcal{T}$ such that:
...
\Bigg(\forall i \in [0\ldots n].\openrule
    {
       \overline{\beta^1_i},\Pred^1_i,\Post^1_i   }
         {s^1_i \OTarrow {\tau} s^1_{i+1}} \in \mathcal{T} \quad \wedge
\quad
\openrule
         {
           \overline{\beta^2},\Pred\,^2,\Post^2 }
         {s^2 \OTarrow {\alpha} s'^2} \in \mathcal{T}
\quad \wedge\\
\forall i \in [0\ldots m].\openrule
         {
           \overline{\beta^3_i},\Pred\,^3_i,({x^3_k\gets e^3_k})^{k\in K}    }
         {s^3_i \OTarrow {\tau} s^3_{i+1}} \in \mathcal{T}
\Bigg)        
       \Longleftrightarrow\\  \Bigg( 
 \Big(\alpha = \tau \wedge  \vis{\overline{\beta^2}}=\emptyset \wedge \Pred\,^2 =\symb{True} \wedge \Post^2 =\symb{Id(s)}\Big) \Bigg) \bigvee  {
\openrule
         {
           \overline{\gamma},
		\Pred, \Post
...
where\\
\begin{mathpar} 
s=s^1_0 \wedge s^1_n=s^2 \wedge s'^2 = s^3_0 \wedge s^3_m=s', \quad
\overline{\gamma}=\biguplus_{i=0}^n \vis{\overline{\beta^1_i}} \dotcup  \vis{\overline{\beta^2}} \dotcup \biguplus_{i=0}^m \vis{\overline{\beta^3_i}}
\\
...
\begin{proof} ($\Rightarrow$) We proceed by induction on the number of transitions $l$ with which we  build  a weak transition (introduced in Definition \ref{def:buildweakOT}), in particular we have  $l=n+m+1$.
\begin{itemize}
...
%}
\end{mathpar} 
by rule (2) we directly conclude 
 \begin{mathpar}
 % \mprset {vskip=.5ex}
%\inferrule{
% \openrule
...
\end{mathpar} 
\item For the inductive step, first we have by induction hypothesis that the formula holds for some length $l$.   Induction step is to infer that formula holds for transitions of length $l + 1$. In particular, for $n+1$ or $m+1$, by adding the transition:
\begin{mathpar}
 % \mprset {vskip=.5ex}
%\inferrule{
 \openrule
...
First by using rule (2) we can directly conclude :
\begin{mathpar}
 % \mprset {vskip=.5ex}
%\inferrule{
% \openrule
...
Second by using rule (3) \TODO{To complete}

\end{itemize}
\noindent ($\Leftarrow$) We proceed also  by induction on the number of transitions $l$ with which we  build  a weak transition.

\begin{itemize}
\item Basis: suppose that $\OTWeakarrow {\alpha}$ cannot be subdivided. So for a given weak transition:
...
We have two cases:
\begin{itemize}
\item either $\alpha = \tau$, $s=s'$, $\overline{\beta}=\emptyset$, $\Pred = \symb{True}$ and $\Post=\symb{Id}(s)$. So by rule (2) we have: 
\begin{mathpar}
{ \openrule
         {
           \emptyset,\symb{True},\symb{Id}(s)
				 } {s \OTarrow {\tau} s} \in \mathcal{T}
}
\end{mathpar}
\item Otherwise, by rule (2) we conclude that we have: 
\begin{mathpar}
\openrule
         {
           \overline{\beta},\Pred,\Post}
         {s \OTarrow {\alpha} s'} \in \mathcal{T}
\end{mathpar}
where $\overline{\gamma} = \vis{\overline{\beta}}$
\end{itemize}
\item Step: by using rule (3) we can rewrite a weak transition:
...
such that \\
$\overline{\gamma}=\overline{\gamma_1} \dotcup \overline{\gamma_2} \dotcup \overline{\gamma_3}$, 
$\Pred=\Pred_1\land\Pred_2\subst{\Post_1}\land (\Pred_3\subst{\Post_2})\subst{\Post_1}$, and $\Post=$ \TODO{Voir comment l'exprimer}\\


\begin{enumerate}

\item By induction hypothesis and simplification we have: 
\begin{mathpar}
%\inferrule{
\bigg( \openrule
         {
...
where 
\begin{mathpar} 
s=s^1_0 \wedge\, s^1_{n_1}=s_1 \quad
\overline{\gamma_1}=\biguplus_{i=0}^{n_1} \vis{\overline{\beta^1_i}} 
\quad
\Pred_1=\bigwedge_{i=0}^{n_1}\Pred^1_i\quad \Post_1= \biguplus_{i=0}^{n_1} \Post_i^1
\end{mathpar}
...
\bigg( \forall i \in [0\ldots m_1].\openrule
    {
       \overline{\beta^3_i},\Pred^3_i,\Post^3_i   }
         {s^3_i \OTarrow {\tau} s^3_{i+1}} \in \mathcal{T}         \bigg)
\end{mathpar}
where
\begin{mathpar} 
s_2=s^3_0 \wedge s^1_{m_1}=s'\quad
\overline{\gamma_3}=\biguplus_{i=0}^{m_1} \vis{\overline{\beta^3_i}}\quad
\Pred_3\!=\!\bigwedge_{i=0}^{m_1}\Pred^3_i \quad \Post_3= \biguplus_{i=0}^{m_1} \Post_i^3
\end{mathpar}
...
where\\
\begin{mathpar} 
s_1=s^1_0 \wedge s^1_n=s^2_0 \wedge s'^2 = s^3_0 \wedge s^3_{m_2}=s_2,
\overline{\gamma_2}=\biguplus_{i=0}^{n_2} \vis{\overline{\beta^1_i}} \dotcup  \vis{\overline{\beta^2}} \dotcup \biguplus_{i=0}^{m_2} \vis{\overline{\beta^3_i}}
\\
\Pred_2=\bigwedge_{i=0}^{n_2}\Pred^1_i\land\Pred\,^2\subst{ \biguplus_{i=0}^{n_2}\Post^1_i}\land \Big(\bigwedge_{i=0}^{m_2}\Pred\,^3_i\subst{\Post^2}\Big)\subst{\biguplus_{i=0}^{n_1}\Post^1_i}
\\
\Post_2= ({x^3_k\gets (e^3_k\subst {\Post^2})\subst{\biguplus_{i=0}^{n_2} \Post^1_i}})^{k\in K}
\end{mathpar}
\end{enumerate}
...
where\\
\begin{mathpar} 
s_1=s^1_0 \wedge s^1_{n_1+n_2}=s^2 \wedge s'^2 = s^3_0 \wedge s^3_{m_1+m_2}=s'\\
\overline{\gamma}=\biguplus_{i=0}^{n_1+n_2} \vis{\overline{\beta^1_i}} 
\dotcup  \vis{\overline{\beta^2}} \dotcup \biguplus_{i=0}^{m_1+m_2} \vis{\overline{\beta^3_i}}
\\
\Pred=\bigwedge_{i=0}^{n_2}\Pred^1_i\land\Pred\,^2\subst{ \biguplus_{i=0}^{n_2}\Post^1_i}\land \Big(\bigwedge_{i=0}^{m_2}\Pred\,^3_i\subst{\Post^2}\Big)\subst{\biguplus_{i=0}^{n_1}\Post^1_i}
\\
\Post= ({x^3_k\gets (e^3_k\subst {\Post^2})\subst{\biguplus_{i=0}^{n_2} \Post^1_i}})^{k\in K}
\end{mathpar}
