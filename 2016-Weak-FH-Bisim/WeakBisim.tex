...
Among the approaches for modelling open systems, where both a component and  its environment (other components) can be designed, by automata we find the theory of interface automata \cite {deAlfaro:2001} and conditional transition systems \cite{Beohar:2020}. Our formalism open automata is different from both, in that they are parametrised models.

\TODO{LUDO idees: \cite{Beohar:2020} conditions de transitions et bisim basees la dessus, leur bisimulation est plus logique et plus riche de ce cote, la notre plus structurelle et plus orientee composition, de facon liee ils voient l environnement comme repondant a des relaations l=ogiques avec certaines rpoprietes, on ne prend pas en compte un environnement mais on a des regles de compositions entre systemes et etablit une bisim prenant ca en compte}
