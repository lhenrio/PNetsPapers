...
\begin{example}
  ...
  $\openrule
         ...
         {\text{b1} \OTWeakarrow \tau \text{b1}} \forall n \geq 0$.  Finally,  applying again \WTTrois, and using the central open transition having \texttt{\underline{out(b1\_m,b1\_ec)}}  as $\alpha$, we get the resulting weak open transition between b1 and b0 (as shown in Figure \ref{WOT2}).
 %     $\openrule
...
$ WI_{8c}(n) = \openrule
         ...
         {210 \OTWeakarrow {\underline{\texttt{out(r1\_msg,r1\_ec)}\subst{(\texttt{r1\_msg}\gets \texttt{s1\_msg}, \texttt{r1\_ec}\gets \texttt{s2\_msg}+n)}}} 202} \forall n\ge 0
        $
...
\hline
  ...
  {\text{b0} \OTarrow {\underline{\texttt{in(m1)}}} \text{b1}}
  $
...
\land True
\land \underline{\texttt{in(m1)}} = \underline{\texttt{in(m2)}} \land 
\\
...
$ WI_3(n) = \openrule
  {\{\texttt{P}\mapsto \texttt{p-send(m)}\}, True,
    (\texttt{s1\_msg}\gets \texttt{m}, \texttt{s1\_ec}\gets n)}
  { \{000,202\} \OTWeakarrow {\underline{\texttt{in(m)}}} 100}
$

$ WI_{3a}(n) = \openrule
  {\{\texttt{P}\mapsto \texttt{p-send(m)}\}, True,
    (\texttt{m1\_msg}\gets \texttt{m}, \texttt{m1\_ec}\gets n, \texttt{s2\_ec}\gets n)}
  { \{000,202\} \OTWeakarrow {\underline{\texttt{in(m)}}} 210}
$

$ WI_{3b}(n) = \openrule
  {\{\texttt{P}\mapsto \texttt{p-send(m)}\}, True,
    (\texttt{s2\_ec}\gets n)}
  { \{000,202\} \OTWeakarrow {\underline{\texttt{in(m)}}} 220}
$

$ WI_{3c}(n) = \openrule
  {\{\texttt{P}\mapsto \texttt{p-send(m)}\}, True,
    (\texttt{r1\_msg}\gets \texttt{m}, \texttt{r1\_ec}\gets n)}
  { \{000,202\} \OTWeakarrow {\underline{\texttt{in(m)}}} 201}
$
...
$ WI_8 = \openrule
         ...
         {201 \OTWeakarrow {\underline{\texttt{out(r1-msg,r1-ec)}}} \{202, 000\}}
         \ \forall n\ge 0$

$ WI_{8a}(n) = \openrule
         {\{\texttt{Q}\mapsto \texttt{q-recv(m1\_msg,m1\_ec}+n)\}, True, ()}
         {210 \OTWeakarrow {\underline{\texttt{out(m1\_msg,m1\_ec}+n)}} \{202, 000\}}
         \ \forall n\ge 1$

 $ WI_{8b}(n) = \openrule
         {\{\texttt{Q}\mapsto \texttt{q-recv(??\_msg,s2\_ec}+n)\}, True, ()}
         {220 \OTWeakarrow {\underline{\texttt{out(??\_msg,m2\_ec}+n)}} \{202, 000\}}
         \ \forall n\ge 1$

 $ WI_{8c}(n) = \openrule
         {\{\texttt{Q}\mapsto \texttt{q-recv(s1\_msg,s1\_ec}+n)\}, True, ()}
         {100 \OTWeakarrow {\underline{\texttt{out(s1\_msg,s1\_ec}+n)}} \{202, 000\}}
         \ \forall n\ge 1$
