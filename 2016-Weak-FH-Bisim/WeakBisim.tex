...
\begin{proof} ($\Rightarrow$) We present an induction on $n$ and $m$, focusing on the incrementation on $n$: we prove that the property is valid for $-1$, $-1$ prove a general induction for going from $n$ to $n+1$, a similar induction can be applied to go from $m$ to $m+1$
\begin{itemize}
