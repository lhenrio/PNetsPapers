	...
	{\openrule
		{
			...
			\Pred\land\Pred^2\land \alpha_t=\beta^i_{p j_0 },  
			\Post\uplus \Post^2}
{\ostate{s^{i\in L}_{i}\uplus s_0^2} \OTarrow {\alpha_p} \ostate{{s'}^{i\in L}_{i}\uplus s_1^2}}
	}
	\]
...
\set{\gamma'}, \Pred_c,
\Post_c}
         {\ostate{s^1_0\uplus s^{i\in L}_{i}} \OTWeakarrow {\alpha^l} 
	\ostate{{{s'}^{i\in L}_{i}\uplus s_{k'}^3}}}\] 
where
...
\left(Pred\land\Pred^2\land \alpha_t=\beta^i_{p j_0 }\right)\subst{\bigotimes_{i=0}^{k}{\Post_i^1}}\land\\\displaystyle
