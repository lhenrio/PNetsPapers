...
\\
\Pred=\bigwedge_{i=0}^n\Pred^1_i\land\Pred\,^2\subst{\Post_1}\land (\bigwedge_{i=0}^m\Pred_i^3\subst{\Post^2})\subst{\Post^1_i}\\
...
\\
\Pred_2=\bigwedge_{i=0}^{n_2}\Pred^3_i\land\Pred\,^4\subst{ \biguplus_{i=0}^{n_2}\Post^3_i}\land \Big(\bigwedge_{i=0}^{m_2}\Pred\,^5_i\subst{\Post^4}\Big)\subst{\biguplus_{i=0}^{n_1}\Post^1_i}
\\
\Post_2= ({x^5_k\gets (e^5_k\subst {\Post^4})\subst{\biguplus_{i=0}^{n_2} \Post^3_i}})^{k\in K}
$
\end{enumerate}
~~\\
...
\displaystyle{\Pred=\bigwedge_{i=0}^{n_2}\Pred^6_i\land\Pred^7\subst{ \biguplus_{i=0}^{n_2}\Post^6_i}\land \Big(\bigwedge_{i=0}^{m_2}\Pred^6_i\subst{\Post^7}\Big)\subst{\biguplus_{i=0}^{n_1}\Post^6_i}}
\\
\displaystyle{\Post= ({x^8_k\gets (e^8_k\subst {\Post^7})\subst{\biguplus_{i=0}^{n_2} \Post^6_i}})^{k\in K}}
$\\
...
\begin{corollary} The definition of weak bisimulation given in  Definition \ref{def-Weak-bisim} is equivalent to the following one:
...
For the sake of simplicity, a single tau open transition will be considered; however the proof may be easily generalized to the multiple tau open transitions by using the same reasoning and (\WTTrois)~rule. 

\paragraph{1st transition}
...
by  Definition \ref{def-Weak-bisim} we have for an open transition:
...
\hspace{1cm} \implies\!\!\! \displaystyle{\bigvee_{x\in X}}
   \left( \forall j\in J_x. \vis{\beta^1_j}=\gamma_{jx}  \Rightarrow \Pred_{OT_x}
     ...
     \Pred_{s'^1,t_x}\subst{\Post_i^1}\subst{\Post_{OT_x}}\right)$
   
Note that, because $\AlgE\cap\AlgA=\emptyset$ and $\alpha_x\!=\!\tau$ we have 
directly\NOTE{the argumet is not trivial!}:
\begin{mathpar}
    \openrule
         {
           \gamma_{j x}^{j\in J_{x}}, \Pred_{OT_x},\Post_{OT_x}}
...
\end{mathpar}
\paragraph{2nd transition}
         
Concerning the middle open transition, we have
 \begin{mathpar}
     \openrule
         {
...
We have $(s'^1,t_x|\Pred_{s'^1,t_x})\in \mathcal{R}$. Then
 by Definition \ref{def-Weak-bisim}:

 there exist weak open transitions $\symb{WOT}_{x y}^{x\in X,y\in Y} \subseteq \WT_2$:
 \begin{mathpar}
    \openrule
         {
           \gamma_{j y}^{j\in J_{y}}, \Pred_{OT_y},\Post_{OT_y}}
         {t_x \OTWeakarrow {\alpha_y} t_{x y}}
\end{mathpar}
 such that  $\forall x,\{j\in J^2|\beta^2_j\neq\tau\}=J_{x}, 
(s'^2,t_y|\Pred_{s'^2,t_{x y}})\in \mathcal{R}$; \\
 $\Pred_{s^2,t'} \land \Pred\,^2\\
\hspace{1cm} \implies\!\!\! \displaystyle{\bigvee_{y\in Y}}
   \left( \forall j\in J_y. \vis{\beta^2_j}=\gamma_{jy}  \Rightarrow \Pred_{OT_y}
     \land \alpha\!=\!\alpha_y \land  
     \Pred_{s'^2,t_y}\subst{\Post^2}\subst{\Post_{OT_y}}\right)$


 \paragraph{third  transition}
...
$(s'^2,t_y|\Pred_{s'^2,t_{x y}})\in \mathcal{R}$
and by Definition \ref{def-Weak-bisim}:

there exist weak open transition $\symb{WOT}_{x y z}^{x,y,z\in Z} \subseteq \WT_2$:
 \begin{mathpar}
    \openrule
         {
           \gamma_{j z}^{j\in J_{z}}, \Pred_{OT_z},\Post_{OT_z}}
         {t_{x y} \OTWeakarrow {\alpha_x} t_{x y z}}
\end{mathpar}
 such that  $\forall z,\{j\in J^3|\beta^3_j\neq\tau\}=J_{3}, 
(s',t_{x y z}|\Pred_{s',t_{x y z}})\in \mathcal{R}$. \\
 $\Pred_{s'^2,t_{x y}} \land \Pred_i^3\\
\hspace{1cm} \implies\!\!\! \displaystyle{\bigvee_{z\in Z}}
   \left( \forall j\in J_z. \vis{\beta^3_j}=\gamma_{jz}  \Rightarrow \Pred_{OT_z}
     \land \alpha_z\!=\!\tau \land  
     \Pred_{s'^3,t_z}\subst{\Post_i^3}\subst{\Post_{OT_z}}\right)$
         

\medskip     

On other hand, by applying (\WTTrois)~rule  \TODO{on each  $\symb{WOT}_x$} we have:
...
\openrule
         {
           \gamma_{j y}^{j\in J_{y}}, \Pred_{OT_y},\Post_{OT_y}}
         {t' \OTWeakarrow {\alpha_y} t_y}
\qquad
\openrule
         {
           \gamma_{j z}^{j\in J_{z}}, \Pred_{OT_z},({x_k\gets e_k})^{k\in K}}
         {t'' \OTWeakarrow {\tau} t_z}
\\
\Pred=\Pred_{OT_x}\land\Pred_{OT_y}\subst{\Post_{OT_x}}\land (\Pred_{OT_z}\subst{\Post_{OT_y}})\subst{\Post_{OT_x}}
}
{
\openrule
         {
           \gamma_{j x}^{j\in J_{x}} \dotcup \gamma_{j y}^{j\in J_{y}}\dotcup\gamma_{j z}^{j\in J_{z}},
		\Pred,
				({x_k\gets (e_k\subst {\Post_{OT_y}})\subst{\Post_{OT_y}}})^{k\in K} } 
         {t \OTWeakarrow {\alpha_x} t_z} 
} 
 \end{mathpar}
 such that $t_x=t' \wedge t_y=t''$


\newpage
-----------------------------------------------------

The (\WTDeux)~ and (\WTTrois)~  rules can be applied repeatedly to produce:
\begin{mathpar}
%\inferrule{
 \forall i \in [0\ldots n].\openrule
    {
       \set{\beta^1_i},\Pred^1_i,\Post^1_i   }
         {s^1_i \OTarrow {\tau} s^1_{i+1}} \in \mathcal{T}          \implies
 \openrule
         {
           \set{\gamma_1},\Pred_1,\Post_1   }
         {s \OTWeakarrow {\tau} s_1} \in\! \WT 
...
where 
\begin{mathpar}
s=s^1_0 \wedge s^1_{n}\!=s_1,
\displaystyle{\set{\gamma_1}=\mybigdotcup_{i=0}^{n} \vis{\set{\beta^1_i}}}, 
\Pred_1\!=\!\bigwedge_{i=0}^{n}\Pred^1_i, \Post_1= \biguplus_{i=0}^{n} \Post_i^1\\
\end{mathpar}
And similarly, a series of $m$ open transitions can be simplified:
\begin{mathpar}
%\inferrule{
\forall i \in [0\ldots m].\openrule
         {
...
where 
\begin{mathpar}
s_2=s^3_0 \wedge s^3_{m}\!=s',
\displaystyle{\set{\gamma_3}=\mybigdotcup_{i=0}^{m} \vis{\set{\beta^3_i}}}, 
\Pred_3\!=\!\bigwedge_{i=0}^{m}\Pred^3_i 
%\Post_3= \biguplus_{i=0}^{n_1} \Post_i^1\\
\end{mathpar}
Concerning the middle open transition, by Definition \ref{def-Weak-bisim} we know that for each open transition $OT$ in $\mathcal{T}_1$:
 \begin{mathpar}
     \openrule
         {
           \beta_j^{J^2},\Pred\,^2,\Post^2 }
         {s^2 \OTarrow {\alpha} s'^2} 
\end{mathpar}
 there exist weak open transitions $\symb{WOT}_x^{x\in X} \subseteq \WT_2$:
 \begin{mathpar}
    \openrule
         ...
         {t \OTWeakarrow {\alpha_x} t_x}
\end{mathpar}
 such that  $\forall x,J^2=J_{x}, (s'^2,t_x|\Pred_{s'^2,t_x})\in \mathcal{R}$; 
 and  \TODO{$\{j\in J^2|\beta_j\neq\tau\}$ useless} \\
 $\Pred \land \Pred\,^2\\
\hspace{1cm} \implies\!\!\! \displaystyle{\bigvee_{x\in X}}
   \left( \forall j\in J_x. \vis{\beta_j}=\gamma_{jx}  \Rightarrow \Pred_{OT_x}
     \land \alpha\!=\!\alpha_x \land  
     \Pred_{s'^2,t_x}\subst{\Post^2}\subst{\Post_{OT_x}}\right)$
...
\openrule
         {
           \set{\gamma_3},\Pred_3,({x_k\gets e_k})^{k\in K}    }
         {s_2 \OTWeakarrow {\tau} s'} 
\\
\Pred=\Pred_1\land\Pred_2\subst{\Post_1}\land (\Pred_3\subst{\Post_{OT_x}})\subst{\Post_1}
}
{
\openrule
         {
           \set{\gamma_1}\dotcup\gamma_{j x}^{j\in J_{x}} \dotcup\set{\gamma_3},
		\Pred,
				({x_k\gets (e_k\subst {\Post_{OT_x}})\subst{\Post_1}})^{k\in K} } 
         {s \OTWeakarrow {\alpha_x} s'} 
} 
 \end{mathpar}
such that
$s_1=t \wedge t_x=s_2$  \TODO{Attention les states; I'm not convinced}\\
Therefore, we can deduce that:

For any open transition $\symb{WOT}$ in $\WT_1$:
 \begin{mathpar}
     \openrule
         {
           \gamma_j^{j\in J_1},\Pred_{OT},\Post_{OT}}
...
\end{mathpar}
\TODO{Side Conditions to discuss}\\
 such that  $\forall x, J_1=J_{x}, (s',t_x|\Pred_{s',t_x})\in \mathcal{R}$; 
 and  \\
$\Pred \land \Pred_{OT}\\
\displaystyle{\implies\!\!\! \bigvee_{x\in X}
...
\noindent ($\Leftarrow$) Now, we prove that the definition above implies Definition \ref{def-Weak-bisim}.

Suppose that for any weak open transition $\symb{WOT}$ in $\WT_1$:
 \begin{mathpar}
     \openrule
         {
           \gamma_j^{j\in J_1},\Pred_{OT},\Post_{OT}}
...
By applying (\WTDeux)~rule we can directly deduce that we have:

\begin{mathpar}
     \openrule
         {
           \vis{\beta_j^{j\in J_1}},\Pred_{OT},\Post_{OT}}
         {s \OTWeakarrow {\alpha} s'}

\end{mathpar}

Then by transitivity,   it is straightforward to  deduce that for any open transition $\symb{OT}$ in $\mathcal{T}_1$:
 \begin{mathpar}
     \openrule
         {
           \gamma_j^{j\in J_1},\Pred_{OT},\Post_{OT}}
...
\\
		\text{and~~}  \bigcup_{\begin{array}{l}
p\in[0..n]
\end{array}} J_{p} = J\cap\Holes(Q) \qquad
 \text{and\qquad} \forall j\in J_p,\,\gamma_j= \mathop{\dot{\bigcup}}_{p\in [0..n]} \gamma_{p j} \\
 \text{and\quad} \forall i \in L_Q.\, s_{(n+1) i} = s_i'\land s_{0 i} = s_i\\{\hspace{-3ex}}\text{and~~} \Pred \iff \Pred\,'
		\land \!\!\bigwedge_{p\in[0..n]}(\alpha_p=\beta_p\land \Pred_p) \qquad\text{and~}
