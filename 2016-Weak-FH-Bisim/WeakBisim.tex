...
values of $s^2$ are collected by a disjunction.
 we will show
that $\mathcal{R}''$ is an FH-bisimulation. Let
$(s^1,s^2|\Pred)\in\mathcal{R}$,  we have the followings since 

\TODO{LUDO: I stopped rewriting here}
...
such that  $\forall x, J_1=J_{2x}, (s^{1'},s^{2}_x|\Pred_{target_x})\in 
\mathcal{R}$;
and  \\
...
for any open transition $OT_x$, since
$(s^2,s^3|\Pred^\prime)\in\mathcal{R}$ there exist open transitions
$OT_{xy}^{x\in X, y\in Y} \subseteq \mathcal{T}_3$: 

\begin{mathpar}

%    \left( fresh \ \overline{a_i}, \overline{b_j}, v_x.\ \
\inferrule*[myfraction=\reddottedrule]
{\{s^{3}_i\xrightarrow{a_{i x y}}_i s^{3}_{i xy}\}^{i\in I_{3 xy}},
	\{\xrightarrow{b_{j x y}}_j\}^{j\in J_{3xy}}, \Pred_{OT_{xy}},\Post_{OT_{xy}}}
{s^3 \xrightarrow {v_{xy}} s^{3}_{xy}}
%         \right)

\end{mathpar}
\TODO{LUDO: I have trouble doing computing the next line without intermediates: it is an 
iterative application of many FH bisim definitions with reordering of terms and 
manipulation of quantifiers}
such that  $\forall x, \forall y, J_1=J_{2x}=J_{3xy}, 
(s^{1'},s^{3}_{xy}|\Pred_{target_{xy}})\in \mathcal{R}$; and  \\
$\Pred\,' \land \Pred_{OT}\\
\hspace{1cm} \implies\!\!\! \bigvee_{x\in X}\bigvee_{y\in Y}
\left( \forall j. b_j=b_{jx}=b_{jxy}  \Rightarrow \Pred_{OT_{xy}}
\land v\!=\!v_{xy} \land
\Pred_{target_{xy}}\subst{\Post_{OT}}\subst{\Post_{OT_{xy}}}\right)$.

\TODO{the proof is incomplete because it does not deal with predicates and does not 
explicit what is the right $pred''$ $\implies$ TODO}
