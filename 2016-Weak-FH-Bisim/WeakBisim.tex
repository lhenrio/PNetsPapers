...
Considering the set of $\symb{OT}_p$ of $Q$ such that $p \in S$ and $S \subseteq [0\ldots m+n+1]$, at each time a single open transition moves.   So we have for each open transition:
...
By definition \ref{def:buildweakOT} we can conclude that we have for all $p\in S$:
...
Where\\ $\displaystyle{\bigcup_{p=0}^nJ_{p}} = J\cap\Holes(Q)$, $\forall j\in J_p,\,\gamma_j= \displaystyle{\mybigdotcup_{p=0}^n \gamma_{p j}}$, $\forall l\in L_Q,\, s_{0l}=s_l$, $s_{(n+1)l}=s'_l$  \\
  $\Pred \iff \Pred\,'
		\land \displaystyle{\bigvee_{p=0}^n}(\alpha_p= \beta_{pj_0}\land \Pred_p), \text{and~}
\Post=\Post\,'\uplus\biguplus_{p=0}^n
		\Post_p \quad$  

\TODO{We will discuss quantif.}
