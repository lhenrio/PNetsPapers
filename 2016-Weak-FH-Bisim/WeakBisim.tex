...
\implies
    \openrule
         {
           \set{\beta}\subst{\Post}, \Pred\,'\subst{\Post}\land\Pred,\Post\otimes\Post\,'}
...
\\
\Pred=\Pred_1\land\Pred_2\subst{\Post_1}\land \Pred_3\subst{\Post_2\otimes\Post_1}
\\
\set{\gamma}=\set{\gamma_1}\dotcup \set{\gamma_2}\subst{\Post_1}\dotcup\set{\gamma_3}\subst{\Post_2\otimes \Post_1}\\
...
\openrule
         {\set{\gamma}
           ,
		\Pred,
				\Post_3\otimes\Post_2\otimes\Post_1} 
         {s \OTWeakarrow {\alpha'} s'} \in\WT
} \WTTrois
\end{mathpar}
...
\hspace{1cm} \implies\!\!\! \bigvee_{x\in X}
   \left( \forall j\in J_x. \vis{\beta_j}=\gamma_{jx}  \Rightarrow \Pred_{OT_x}
     \land \alpha\!=\!\alpha_x \land  
     \Pred_{s',t_x}\subst{\Post_{OT}\otimes\Post_{OT_x}}\right)$
...
\begin{lemma}[Open transition composition]\label{lem-compose-2} ~\\	Suppose $j_0\not\in J$ and $L_Q=\Leaves({Q})$:\\[-2ex]
\begin{mathpar}
%\[
P\models{\openrule
	{
...
\forall i \in [0..m].\openrule
         {
           \set{\beta^3_i},\Pred^3_i,({x^3_{ik}\gets e^3_{ik}})^{k\in K_i}    }
...
\\
\Pred=\bigwedge_{i=0}^n\Pred^1_i\bigotimes_{j=0}^{i-1}\subst{\Post_j^1}\land\Pred^2 \bigotimes_{j=0}^{n}\subst{\Post_j^1}\land\\ 
~\qquad\quad\Big(\bigwedge_{i=0}^m\Pred_i^3\bigotimes_{j=0}^{i-1}\subst{\Post_j^3}\subst{\Post^2}\bigotimes_{j=0}^{n}\subst{\Post^1_j}\Big)\\
\Post=\big({x^3_{mk}\gets e^3_{mk}\bigotimes_{j=0}^{m-1}\subst{\Post_j^3}\subst{\Post^2}\bigotimes_{j=0}^{n}\subst{\Post^1_j}\big)^{k\in K_m}}
$
...
\bigwedge_{h=0}^{k'}{\Pred^3_h}\subst{\bigotimes_{i=0}^{h-1}{\Post_i^3}\otimes({\Post\uplus \Post^2})\otimes\bigotimes_{i=0}^{k}{\Post_i^1}}
\end{array}
\]

\[\Post_c=(\bigotimes_{i=0}^{k'}{\Post_i^3})\otimes({\Post\uplus \Post^2})\otimes\bigotimes_{i=0}^{k}{\Post_i^1}
\]
...
\bigwedge_{h=0}^{k'}{\Pred^3_h}\subst{\bigotimes_{i=0}^{h-1}{\Post_i^3}\otimes({\Post_p\uplus \Post^2})\otimes\bigotimes_{i=0}^{k}{\Post_i^1}}
\end{array}
\]

\[\Post_c=(\bigotimes_{i=0}^{k'}{\Post_i^3})\otimes({\Post_p\uplus \Post^2})\otimes\bigotimes_{i=0}^{k}{\Post_i^1}
\]
