...
\begin{minipage}{0.69\linewidth} 	Suppose that
   $A_1 = <L_1,J,\mathcal{S}_1, s_0,
   \mathcal{T}_1>$ and $A_2 = <L_2,J,\mathcal{S}_2,t_0, \mathcal{T}_2>$
...
$s\in\mathcal{S}_1$ and $t\in\mathcal{S}_2$, $(s,t|\Pred)\in\mathcal{R}$, we 
have
the following:
\end{minipage}
\hspace{2mm}
...
%    \left( fresh \ \overline{a_i}, \overline{b_j}, v_x.\ \
    \inferrule*[myfraction=\reddottedrule]
         {\{t_i\xrightarrow{a_{i x}}_i t_{i x}\}^{i\in I_{2 x}},
           \{\xrightarrow{b_{j x}}_j\}^{j\in J_{2x}}, \Pred_{OT_x},\Post_{OT_x}}
         {t \OTarrow {v_x} t_x}
%         \right)
\end{mathpar}
 such that  $\forall x, J_1=J_{2x}, (s',t_x|\Pred_{s',t_x})\in 
 \mathcal{R}$; 
 and  \\
 $\Pred \land \Pred_{OT}\\
...
\end{mathpar}
 such that  $\forall x, J_1=J_{2x}, (s',t_x|\Pred_{s^{1'},t_x})\in \mathcal{R}$; 
 and  \\
 $\Pred \land \Pred_{OT}\\
\hspace{1cm} \implies\!\!\! \bigvee_{x\in X}
   \left( \forall j. b_j=b_{jx}  \Rightarrow \Pred_{OT_x}
