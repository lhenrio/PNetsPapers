...
\ead{rabea.ameur-boulifa@telecom-paris.fr}     

\author{Ludovic Henrio\corref{cor1}}
...
Establishing equivalences between programs or systems is crucial both for verifying correctness of programs and for justifying optimisations and program transformations. There exist several equivalence relations for programs, and bisimulations are among the most versatile of these equivalences. Among bisimulations one distinguishes strong bisimulation that requires that each action of a program is simulated by a single action of the equivalent program, and weak bisimulation that  allows some of the actions to be
  invisible, and thus not simulated by the equivalent program.

pNet is a generalisation of automata that model open systems. They feature variables and hierarchical composition. Open pNets are pNets with holes, i.e. placeholders inside the hierarchical structure  that can be filled later by sub-systems. 
...
Among these properties we are interested in compositionality: if two systems are proven equivalent they will be undistinguishable by their context, and they will also be indistinguishable when their holes are filled with equivalent systems.
...
method for these processes. Thirty years later,  no verification platform use this kind of approaches to provide proof methods for value-passing processes or open process expressions, perhaps because of the difficulty to apply these methods on industrial systems. 
...
The semantics of pNets can be expressed as a translation to a labelled transition system (LTS), but only if the pNet has no parameter and no hole. Adding parameters to a LTS is quite standard \ERIC{\cite{Lin:96}} but enabling holes inside LTSs is not a standard notion.
...
\ERIC{Beware that even if open transitions may look similar to the notion of Transition System Specification \cite{GROOTE1992202,GROOTE20063} and other forms of SOS rules, they are \emph{not} structural rules, but rules defining the behaviour of the global states of the system.
}
...
\item An illustrative example based on a simple transport protocol, showing the construction of the weak open transitions, and the proof of weak FH-bisimulation.
\end{itemize}
...
cases where the original transition can be triggered. %\TODO{Eric: is this like in symbolic bisimulation ?}
\ERIC{This is similar to the early and late symbolic bisimulation equivalences for value-passing CCS \cite{HENNESSY:1995}, though we use more general definitions in our setting.}
...
\ERIC{This is quite different from the works on contextual equivalences, e.g. \cite{Larsen87, Larsen:91}; we will give more details on this in section \ref{sec:RW}.}
...
This approach also allows us to specify a sufficient condition on allowed transitions to make weak bisimilarity compositional; namely it is not possible to synchronise on invisible actions from the holes or prevent them to occur.
\ERIC{This is loosely related to works on the syntactic conditions on SOS rules to check whether weak bisimulation is a congruence for some process algebra operators \cite{GrootVaandrager1992}. Our approach is semantical and more global: our sufficient condition applies to a whole (open) system, and is expressed on its open automaton (see Definition \ref{def:non-obspNet}).}
%\TODO{Eric: relations with SOS rule formats?}
...
compositionality properties of strong bisimularity for open
pNets. 
...
The  actions that distinguish input variables will be used in the definition of \pLTS\ below and are defined as follows:
\[
...
The disjoint union on sets is $\uplus$ and we only use $A\uplus B$ when $A$ and $B$ are disjoint. We extend it to   union  of indexed sets provided they are indexed over disjoint families; is $\uplus$  is then defined by the merge of the 
two sets.
The elements
...
The  subtraction operation on indexed sets is $\setminus$, it reduces the set of indexes such that $\dom(A\setminus B)=\dom(A)\setminus B$.
...
The pNets model is hierarchical in the structure of the processes, in contrast to the Statecharts formalism \cite{Harel:87}, which is widely used to model high-level behaviour,  that organises the states (but not processes) in a hierarchy. 

We introduce the notion of pNets through a simple example below, and define formally pLTSs and pNets afterwards:

\begin{example}[CCS choice]\label{ex:CCSplus}
...
The left hole is
  indexed $L$ the right hole $R$. The sub-net is an labelled transition system (LTS) with three states and emitting actions $l$ and $r$. 
...
\RAB{In the same vein as for context systems \cite{Larsen:91}, pNets is a formalism for modular and possibly incomplete  description of concurrent systems. The two formalisms are however different as the theory of contexts relies on a form of rewrite rules, while  pNets rely on parametric automata to express the system behaviour.  }

\RAB{
PNets have features of "known" compositional specification theory  allowing for modular and possibly incomplete  description   and  structural compositions \cite{HuttelL89,Larsen:91} of systems.}

%\TODO{Rabea: look for papers by Larsen end Liu on context systems, and Larsen and Huttel on static constructs}
...
on finite branching of the transition relation. Compared to our previous works~\cite{henrio:Forte2016,AmeurBoulifa2017} make variables global, which makes the model easier to use.
...
% Note that, in this definition of sorts we do not  
%distinguish
%input variables from other expressions.
%and only  and we remove the
%\emph{input marker} (?) of variables.\TODO{??? ceci ne se retrouve pas dans la def ci-dessous, c'est une vieille version ?}
\[
\begin{array}{l}
...
Note that  $\Holes(\pNet)$is a set of indexes because holes are characterized only by their indices, while entities at the leaves ar pLTSs and thus $\Leaves(\pNet)$ is a set of pLTSs.
...
%\TODO{Eric: j'ai 2 pbs ici, d'une part, on ne l'utilise que dans la composition de pNet ci-dessous, ou il nous faudrait plutot l'inclusion des sorts, que l'egalite. D'autre part on la definie (Definition 3) comme un ensemble de signatures, mais on cite a plusieurs endroits comme un ensemble d'actions parametrees... }
\end{definition}
...
\begin{definition}[pNet composition]
 ...
 can be (partially) filled by providing  a pNet $\pNetQ$ to fill one of  its holes.	
	Suppose $j_0\in J$ and $\Sortop(\pNetQ) \subseteq \Sort_{j_0}$, then: % are compatible:
...
While the definition and usage of the open transition can be considered purely syntactically, we take in this article a semantics and logical understanding of open automata. 
...
\ERIC{Indeed, our definition uses both hypotheses on the behaviour of holes, as in \cite{deSimone85}, and symbolic manipulation of action expressions, as in symbolic bisimulations of \cite{HENNESSY:1995}. }
%\TODO{Eric: combination of RdS, symb bisim, and more}
...
Bisimulation should allow it to be matched by the two following ones (depending on the value of $x$), to prove the bisimulation $\mathcal{R}=\{(s_1,s_2,\True),(s_1',s_2',\True)\}$
\[ \openrule
         {
...
\begin{thm}[FH-bisimularity is an equivalence]\label{thm-equiv} FH-bisimilarity is 
...
This section defines the semantics of an open pNet via translation into an open automaton. 
...
%Consequently, the state of a pNet is not only characterized the tuple of pLTS states but also contains the value of its variables $\vars(P)$.
...
% but it is easy to build the predicate constructively by brute-force unification of the sub-pNets actions with the corresponding vector actions, possibly followed by a simplification step.
...
$\alpha$, $\fresh(\alpha)$ means all variables in $\alpha$ are fresh.
	...
	\begin{figure}[tb]\label{fig:Tr1Tr2}
\[\inferrule
		{ s \xrightarrow{\langle \alpha,~e_b,~(x_j\!:= {e}_j)^{j\in
					J}\rangle} s'\in \to  }
...
\item	The  rule \TrDeux\ deals with pNet nodes: for each possible
	...
	I_2$}\footnote{We could replace $I_1$ and $I_2$ by their formal definition in \TrDeux\ but the rule would be more difficult to read.}.   
\end{itemize}                                                                  
%\begin{mathpar}
...
\begin{lem}[Open transition decomposition\label{lem-decompose}] Consider two pNets $P$ and $Q$ that are not pLTSs\footnote{A similar lemma can be proven for a pLTS $Q$}.
	...
	\vspace{-2.2ex}\\\text{and~\qquad\qquad~}
		%\[
		Q\models{\openrule
			{
...
%\TODO{Eric: discuss rel with Vaandrager - ou pas !!}
...
The weak open automaton \emph{derived} from $A$ is an open automaton  $\mylangle J,\mathcal{S},s_0,V,\WT\myrangle$ where $\WT$ is derived from $\mathcal{T}$ by saturation, applying the rules of Figure~\ref{fig:WT}:
...
\begin{mathpar}
 {\openrule
         ...
         {s \OTWeakarrow {\tau} s} \in \WT  \quad \WTUn}
\qquad\quad
\text{and}
...
\\
\set{\gamma}=\set{\gamma_1}\oplus \set{\gamma_2}\subst{\Post_1}\oplus\set{\gamma_3}\subst{\Post_2\shortodot \Post_1}\\
\alpha'=\alpha\subst{\Post_1}\\
\Pred=\Pred_1\land\Pred_2\subst{\Post_1}\land \Pred_3\subst{\Post_2\shortodot\Post_1}
}
{
\openrule
         {\set{\gamma}
           ,
		\Pred,
				\Post_3\shortodot\Post_2\shortodot\Post_1} 
         {s \OTWeakarrow {\alpha'} s'} \in\WT
} \WTTrois
\end{mathpar}
\caption{Weak transition definition}\label{fig:WT}
\end{figure}
\end{definition}
...
\end{figure}
 ...
 For readability, we only give names to the  weak open transitions of \symb{SimpleProtocolImpl} in Figure \ref{SimpleProtCounter:ImplWOA2}; we detail some of these transitions below and the full list is included in  \ref{Appendix:FullExample}.
...
\begin{example}[CCS Choice] Consider the $+$ operator of CCS, shown in  Example~\ref{ex:CCSplus}. 
...
It is well-known that weak bisimularity is not a congruence in CCS, this corresponds to the fact that the $+$ operator can observe the $\tau$ transitions. Thus, even if we can define a weak FH-bisimilarity for CCS with $+$ it does not verify the necessary requirements for being a congruence.
...
mCRL2~\cite{} is another effective model for specifying and proving properties of concurrent systems. mCRL2 has an established tool-suite and share similarities with pNets. However, pNets feature hierarchical composition with more structure than mCRL2 that composes processes with a parallel operator. Synchronisation of processes is expressed very differently; it is difficult to precisely compare multi-actions of mCRL2 with synchronisation vectors of pNets but synchronisation vector ogf pNets enforce a  synchronisation based on the structure while in mCRL2 synchronisation is specified in a versatile, flexible, but less structured way.
...
\LUDO{
pNets support the refinement checking at the automaton level through a simulation approach, with symbolic evaluation of
...
\beta_j=\alpha'_j \land 
 ...
 and by definition of $\Pred$ (as obtained by applying \TrDeux~rule),
	...
	the leaves: $\displaystyle{\biguplus_{m\in I_1\cup\{j_0\}\uplus I_2} 
        ...
        built by \TrDeux\ rule as follows:
	\noindent
	\begin{small}
\begin{mathpar}
    \mprset {vskip=.8ex}
...
\end{mathpar}  
\end{small}
   This provides the desired conclusion.   \qed
...
\displaystyle{
\set{\gamma}=\bigoplus_{i=0}^n \vis{\set{\beta_{1i}}\subst{\bigodot_{j=i-1}^{0} \Post_{1j} } }  \oplus  \vis{\set{\beta_2}\subst{\bigodot_{j=n}^{0}\Post_{1j}}} \oplus}\\
{}~\qquad \displaystyle{\bigoplus_{i=0}^m \vis{\set{\beta_{3i}} \subst{\bigodot_{j=i-1}^{0}\Post_{3j}\shortodot\Post_2\shortodot\bigodot_{j=n}^{0}\Post_{1j}} }}
\\
...
\set{\gamma}=&\bigoplus_{i=1}^{n+1} \vis{\set{\beta_{1i}}\subst{\bigodot_{j=i-1}^{1} \Post_{1j} } }  \oplus  \vis{\set{\beta_2}\subst{\bigodot_{j=n+1}^{1}\Post_{1j}}} \oplus\\
& \bigoplus_{i=0}^m \vis{\set{\beta_{3i}} \subst{\bigodot_{j=i-1}^{0}\Post_{3j}\shortodot\Post_2\shortodot\bigodot_{j=n}^{0}\Post_{1j}} }\\
...
\\\\
\set{\gamma''}= \vis{\set{\beta_{10}}} \oplus \set{\gamma}\subst{\Post_{10}}\\
\Pred\,''\!=\!\Pred_{10}\land\Pred\subst{\Post_{10}}\\\\%\land (\Pred_3\subst{\Post_2})\subst{\Post_{10}}
\Post\,''=\Id(s'')\shortodot\Post\shortodot\Post_{10} \\ \alpha''=\alpha'\subst{\Post_{10}}
}
{
\openrule
         {
...
\alpha''=&\alpha\subst{\bigodot_{j=n+1}^{1}\Post_{1j}}\subst{\Post_{10}} = \alpha\subst{\bigodot_{j=n+1}^{0}\Post_{1j}}\\
\set{\gamma''}=&\bigoplus_{i=0}^{n+1} \vis{\set{\beta_{1i}}\subst{\bigodot_{j=i-1}^{1} \Post_{1j} } }  \oplus  \vis{\set{\beta_2}\subst{\bigodot_{j=n+1}^{1}\Post_{1j}}} \oplus\\
&\bigoplus_{i=0}^m \vis{\set{\beta_{3i}} \subst{\bigodot_{j=i-1}^{0}\Post_{3j}\shortodot\Post_2\shortodot\bigodot_{j=n}^{0}\Post_{1j}} }
\\
...
%where 
%$\set{\gamma}=\set{\gamma_1} \oplus \set{\gamma_2} \dotcup \set{\gamma_3}$,
...
\displaystyle{\set{\gamma_1}= \bigoplus_{i=0}^{n_1} \vis{\set{\beta_{1 i}}\subst{\bigodot_{j=i-1}^{0} \Post_{1 j} } }} \\
\Pred_1\!=\!\bigwedge_{i=0}^{n_1}(\Pred_{1i} \subst{\bigodot_{j=i-1}^{0}\Post_{i j}} ), \quad \Post_1= \bigodot_{i=n_1}^{0} \subst{\Post_{1i}} 
$
...
\displaystyle{\set{\gamma_3}= \bigoplus_{i=0}^{m_1} \vis{\set{\beta_{3 i}}\subst{\bigodot_{j=i-1}^{0} \Post_{3 j} } }} 

\land 

\Pred_3\!=\!\bigwedge_{i=0}^{m_1}(\Pred_{3i} \subst{\bigodot_{j=i-1}^{0}\Post_{3 j}} )

\land 

 \Post_3= \bigodot_{i=m_1}^{0} \subst{\Post_{3i}} 
\end{mathpar}
...
\set{\gamma_2}=&\bigoplus_{i=0}^{n_2} \vis{\set{\beta_{2i}}\subst{\bigodot_{j=i-1}^{0} \Post_{2 j} } }  \oplus  \vis{\set{\beta'}\subst{\bigodot_{j=n_2}^{0}\Post_{2j}}} \oplus\\
&
 \bigoplus_{i=0}^{m_2} \vis{\set{\beta'_{2i}} \subst{\bigodot_{j=i-1}^{0}\Post\,'_{2j}\shortodot\Post\,'\shortodot\bigodot_{j=n_2}^{0}\Post_{2j}}}
\\
...
\set{\gamma}=&\bigoplus_{i=0}^{n_1+n_2} \vis{\set{\beta_{4i}}\subst{\bigodot_{j=i-1}^{0} \Post_{4 j} } }  \oplus  \vis{\set{\beta'}\subst{\!\!\bigodot_{j=n_1+n_2}^{0}\!\!\!\Post_{4j}}}\oplus \\
&\bigoplus_{i=0}^{m_1+m_2}\!\!\vis{\set{\beta_{5i}}\subst{\bigodot_{j=i-1}^{0}\Post\,'_{5i}\shortodot\Post\,'\shortodot\!\!\bigodot_{j=n_1+n_2}^{0}\!\!\!\Post_{4i}} }\\
\Pred=&
...
\gamma_j^{j\in J'}=&\bigoplus_{i=0}^n \vis{\set{\beta_{1i}}\subst{\bigodot_{j=i-1}^{0} \Post_{1j} } }  \oplus  \vis{\set{\beta_2}\subst{\bigodot_{j=n}^{0}\Post_{1j}}} \oplus\\
&\bigoplus_{i=0}^m \vis{\set{\beta_{3i}} \subst{\bigodot_{j=i-1}^{0}\Post_{3j}\shortodot\Post_2\shortodot\bigodot_{j=n}^{0}\Post_{1j}} }
\\
...
\openrule
         ...
         {v_{b} \OTWeakarrow {\tau} w_{c}}
\\ 
\set{\gamma'}=\gamma_{j a}^{j\in J_{a}} \oplus \gamma_{jb}^{j\in J_{b}} \subst{\Post_{OT_a}} \oplus\gamma_{jc}^{j\in J_{c}} \subst{\Post_{OT_{ b}}\shortodot\Post_{OT_a}}\\
\Pred=\Pred_{OT_a}\land\Pred_{OT_{b}}\subst{\Post_{OT_a}}\land \Pred_{OT_{c}}\subst{\Post_{OT_{b}}\shortodot\Post_{OT_a}}\\
\Post= \Post_{OT_{c}} \shortodot \Post_{OT_{b}}\shortodot\Post_{OT_{a}}\quad \alpha''=\alpha_{2}\subst{\Post_{OT_a}}\\
}
{
\openrule
         {
...
$\Pred_{s,t} \land \Pred\implies
 \displaystyle{\bigvee_{x\in X}} 
  \left( \forall j\in J. \gamma_j'=\gamma_{j}  \land \Pred
     \land \alpha \!=\! \alpha'' \land  
     \Pred_{s',x}\subst{\Post_{OT}\uplus\Post\,}\right)$\\
We have:\\
$\Pred_{OT}= \Pred_1 \wedge \Pred_2\subst{\Post_1} \wedge \Pred_3\subst{\Post_2\shortodot\Post_1}$\\
$\Post_{OT}=\Post_3\shortodot \Post_2\shortodot\Post_1$\\
...
\Pred_{s,t} \land \Pred_1)\wedge\Pred_2\subst{\Post_1}\implies \\
 \bigvee_{a\in A}
  \!\! \big( \forall j\in J_a. \vis{\beta_{1j}}\!=\!\gamma_{ja}\!  \land\! \Pred_{OT_a}\! \land\\
...
\fussy
%\begin
\noindent{\bf Theorem~\ref{thm-weak-equiv}}. \emph{Weak FH-bisimilarity is an equivalence}.
 Suppose $\mathcal{R}$ 
...
\text{, }
 \gamma_j^{j\in J\cap\Holes(Q)}= \bigoplus_{p=0}^n (\gamma_{p j}^{j\in J_p})\subst{\bigodot_{i=p-1}^{0}\Post_i}, \\
{~}\Pred \iff \Pred\,'
		\land \!\!\bigwedge_{p=0}^n(\alpha_p\subst{\bigodot_{i=p-1}^{0}\Post_i} = \beta_p\land \Pred_p\subst{\bigodot_{i=p-1}^{0}\Post_i} ),
\\
...
\gamma_j^{j\in J}=&\bigoplus_{i=0}^{m_1} \vis{\set{\beta_{1i}}\subst{\bigodot_{j=i-1}^{0}\!\! \Post_{1j}}}\oplus\\&  \vis{\set{\beta_2}\subst{\bigodot_{j=m_1}^{0}\Post_{1j}}} \oplus  
{\bigoplus_{i=0}^{m_2} \vis{\set{\beta_{3i}} \subst{\bigodot_{j=i-1}^{0}\!\Post_{3j}\shortodot\Post_2\shortodot\bigodot_{j=m_1}^{0}\!\!\Post_{1j}} }}
\\
...
{(\gamma_j')}^{j\in (J\setminus \Holes(Q)) \cup \{j_0\}}=&\displaystyle{\bigoplus_{i=0}^{m_1} \vis{\set{\beta_{1i}}\subst{\bigodot_{j=i-1}^{0}\!\!\Post\,'_{1j}}}\oplus  \vis{\set{\beta_{2}}\subst{\bigodot_{j=m_1}^{0}\Post\,'_{1j}}}} \oplus\\
& \bigoplus_{i=0}^{m_2}
...
%\[\gamma'_{j_0}=\displaystyle{\bigoplus_{i=0}^{m_1} \vis{{\alpha_{1i}}\subst{\bigodot_{j=i-1}^{0}\!\!\Post'_{1j}}}\dotcup  \vis{\set{\beta_{2}}\subst{\bigodot_{j=m_1}^{0}\Post'_{1j}}}} \dotcup\\
%~~\qquad \bigoplus_{i=0}^{m_2}
%\vis{\set{\beta_{3i}} \subst{\bigodot_{j=i-1}^{0}\!\Post'_{3j}\shortotimes\Post'_2\shortotimes\bigodot_{j=m_1}^{0}\!\!\Post'_{1j}} }\]
...
\begin{eqnarray*}
\Pred_\beta &\iff& \gamma_{j_0}\!=\!
\bigoplus_{p=0}^{p=n_1+n_2+2}
\vis{\alpha'_{p}\subst{\bigodot_{j=p-1}^{0}\!\!\Post_{j}}}\\
%&\iff&\gamma_{j_0}=
%\bigoplus_{p=0}^{p=n_1+n_2+2}
%\vis{\alpha_{p}\subst{\bigodot_{j=p-1}^{n'_p}\!\!\Post_{j}}\subst{\bigodot_{j=n'_p-1}^{0}\!\!\Post_{j}}}\\
%\vis{[\alpha_{p}\subst{\bigodot_{j=p-1}^{0}\!\!\Post_{j}}|p\in[0..n_1+n_2+2]]}\\
%&\iff& \gamma_{j_0}=
%\vis{[\alpha'_{p}\subst{\bigodot_{j=p-1}^{0}\!\!\Post_{j}}|p\in[1..n]]}\\
&\iff& \gamma_{j_0}=
...
\begin{equation*}
\begin{split}
\gamma_j^{j\in J}=&\bigoplus_{i=0}^{m_1} \vis{\set{\beta_{1i}}\subst{\bigodot_{j=i-1}^{0}\!\! \Post_{1j}}}\oplus  \vis{\set{\beta_2}\subst{\bigodot_{j=m_1}^{0}\Post_{1j}}} \oplus\\& \bigoplus_{i=0}^{m_2} \vis{\set{\beta_{3i}} \subst{\bigodot_{j=i-1}^{0}\!\Post_{3j}\shortodot\Post_2\shortodot\bigodot_{j=m_1}^{0}\!\!\Post_{1j}} }\\
...
\begin{split}
\gamma_j^{j\in J\cap \Holes(Q)}&=\bigoplus_{i=0}^{m_1} \vis{\set{(\beta_{1i}')}\subst{\bigodot_{j=i-1}^{0}\!\! \Post''_{1j}}}\oplus  \vis{\set{\beta'_2}\subst{\bigodot_{j=m_1}^{0}\Post''_{1j}}} \oplus\\&\qquad\qquad \bigoplus_{i=0}^{m_2} \vis{\set{\beta'_{3i}} \subst{\bigodot_{j=i-1}^{0}\!\Post''_{3j}\shortodot\Post''_2\shortodot\bigodot_{j=m_1}^{0}\!\!\Post''_{1j}} }\\
&=\bigoplus_{k=0}^n \left( \bigoplus_{i=n'_k}^{n'_{k+1}-1}\vis{\beta_{ij}^{j\in J\cap \Holes(Q)}\subst{\bigodot_{j=i-1}^{n'_k}\!\!\Post''_{j}\bigodot_{j=n'_k-1}^{0}\!\!\Post''_{j}}}\right)\\
&=\bigoplus_{k=0}^n \left( \gamma_{kj}^{j\in J\cap \Holes(Q)}\subst{\bigodot_{j=k-1}^{0}\!\!\Post_{k}}\right)
\end{split}\end{equation*}
}
Next, we have:
...
%%$\set{\gamma}= \displaystyle{\bigoplus_{p=0}^{n}
%%\vis{\set{\beta_{p}}}\subst{\bigodot_{p=n}^{0} \Post_{p}}}$\\
%Furthermore, still by hypothesis we have:\\
%$\Pred=\displaystyle{\bigwedge_{i=0}^{m_1}\Pred_{1i}\subst{\bigodot_{j=i-1}^{0}\!\!\Post_{1j}}\land\Pred_2 \subst{\bigodot_{j=m_1}^{0}\!\!\Post_{1j}} }\land \\
%~\qquad \quad \bigwedge_{i=0}^{m_2}\Pred_{3i}\subst{\bigodot_{j=i-1}^{0}\!\!\Post_{3j}\shortotimes\Post_2\shortotimes\bigodot_{j=m_1}^{0}\!\!\Post_{1j}}$\\
...
%		\land {\Pred''_{1i}}\land \alpha_{1i}=\beta_{1ij_0}) \bigodot_{j=i-1}^{0}\subst{\Post_{1j}}\land}\\
%~~\qquad\qquad({\Pred'_2}\land {\Pred''_2}\land \alpha_2=\beta_{2j_0})	\bigodot_{i=m_1}^{0}\subst{\Post_{1i}}\land\\
%~~\qquad\qquad\bigwedge_{i=0}^{m_2} ({\Pred'_{3i}}\land {\Pred''_{3i}}\land \alpha_{3i}=\beta_{3ij_0})
%\subst{\bigodot_{j=i-1}^{0}\!\!\Post_{3j}\shortotimes\Post_2\shortotimes\bigodot_{j=m_1}^{0}\!\!\Post_{1j}}$\\
...
%{\Pred'_{1i}}\bigodot_{j=i-1}^{0}\subst{{\Post'_{1j}}}\big)
%\land{\Pred'_2}\bigodot_{i=m_1}^{0}\subst{{\Post'_{1i}}}\land} \\
%~~\qquad\qquad\bigwedge_{i=0}^{m_2}{\Pred'_{3i}}\bigodot_{j=i-1}^{0}\subst{{\Post'_{3j}}}\subst{{\Post'_2}}\bigodot_{j=m_1}^{0}\subst{{\Post'_{1j}}} \land\\	
%~\qquad\qquad\bigwedge_{i=0}^{m_1}({\Pred''_{1i}}\land \alpha_{1i}=\beta_{1ij_0}) \bigodot_{j=i-1}^{0}\subst{\Post_{1j}}\land({\Pred''_2}\land \alpha_2=\beta_{2j_0})	\bigodot_{i=m_1}^{0}\subst{\Post_{1i}}\land\\
%~\qquad\qquad\bigwedge_{i=0}^{m_2} ({\Pred''_{3i}}\land \alpha_{3i}=\beta_{3ij_0})\bigodot_{j=i-1}^{0}\subst{\Post_{3j}}\subst{\Post_2}  \bigodot_{i=m_1}^{0}\subst{\Post_{1i}}
%$\\		
%Thus, we can write the formula as follows:\\
%$\Pred= \Pred\,' \wedge \displaystyle{\bigwedge_{p=0}^{n}}((\alpha_p= \beta_{pj_0}\land \Pred_p)\bigodot_{i=p-1}^{0}\Post_i)$\\
%We proceed the same way with the postcondition,  we have:\\
%$\Post=\displaystyle{\bigodot_{j=m_2}^{0}\!\!\Post_{3j}\shortotimes\Post_2\shortotimes\bigodot_{j=m_1}^{0}\!\!\Post_{1j}}$\\
...
%$\Post= \displaystyle{\bigodot_{i=m_2}^{0}\subst{{\Post'_{3i}}\uplus 
%		{\Post''_{3i}}} \subst{{\Post'_2}\uplus 
%		{\Post''_2}} \bigodot_{j=m_1}^{0}\subst{ {\Post'_{1i}}\uplus 
%		{\Post''_{1i}}}}\\
%~~\qquad= \bigodot_{i=m_2}^{0}{\Post'_{3i}}\subst{{\Post'_2}}\bigodot_{j=m_1}^{0}\subst{{\Post'_{1j}}} \uplus \bigodot_{i=m_2}^{0}{\Post''_{3i}}\subst{{\Post''_2}}\bigodot_{j=m_1}^{0}\subst{{\Post''_{1j}}} \\
%~~\qquad= \Post' \uplus \bigodot_{j=n}^{0} \Post_j $\\
\qed
\end{proof}
...
%\forall j\in J_t,\,\gamma_j= \displaystyle{\bigoplus_{t=0}^n \gamma_{t j}}\text{\quad and\quad}
...
\end{align*}
\begin{align*}
\set{\gamma}=&\bigoplus_{h=0}^k \vis{\set{\beta_{1h}}\subst{\!\!\bigodot_{j=h-1}^{0}\!\! \Post_{1j} } }  \oplus  \vis{\set{\beta_2}\subst{\bigodot_{j=k}^{0}\!\!\Post_{1j}}} \oplus\\&
		%&
 \bigoplus_{h=0}^{k'} \vis{\set{\beta_{3h}} \subst{\!\!\bigodot_{j=h-1}^{0}\!\!\Post_{3j}\shortodot\Post_2\shortodot\bigodot_{j=k}^{0}\Post_{1j}} }\\
\Pred_Q=&\bigwedge_{h=0}^{k}\!\Pred_{1h}\subst{\!\!\bigodot_{j=h-1}^{0}\!\!\!\!\Post_{1 j}}\land \Pred_2\subst{\bigodot_{h=k}^{0}\Post_{1h}}\land\\
&
	\!\bigwedge_{h=0}^{k'}\!\Pred_{3h}\subst{\!\! 	\bigodot_{j=h-1}^{0}\!\!\!{\Post_{3j}}\odot{\Post_2}  \odot\bigodot_{h=k}^{0}\!\!\Post_{1h}}\\
\Post_Q=&\bigodot_{h=k'}^{0}\!\!\Post_{3h}\odot\Post_2\odot\bigodot_{j=k}^{0}\!\!\Post_{1j}
\end{align*}
\end{small}
...
\begin{small}
\begin{align*}
\set{\gamma_c}=&\bigoplus_{h=0}^k \vis{\set{\beta_{1h}}\subst{\bigodot_{i=h-1}^{0}\!\! \Post_{1i} } }  \oplus  \vis{({\beta_{j}}^{(j\in J\setminus\{j_0\})} \uplus \set{\beta_2})\subst{\bigodot_{i=k}^{0}\Post_{1i}}} \oplus\\
&\bigoplus_{h=0}^{k'} \vis{\set{\beta_{3h}} \subst{\bigodot_{i=h-1}^{0}\!\!\Post_{3i}\shortodot\Post_2\shortodot\bigodot_{i=k}^{0}\Post_{1i}} }\\
...
\forall j\in J_p,\,\gamma_j= \displaystyle{\bigoplus_{p=0}^n \gamma_{p j}\subst{\bigodot_{k=p}^{0}{\Post_k}}}

\Pred\,'=\Pred\land \bigwedge_{p=0}^{n}(\alpha_p=\beta_p\land \Pred_p)\subst{\bigodot_{i=p-1}^{0}\Post_i} 
...
\alpha=&\alpha'{\subst{\bigodot_{j=m_1}^{0}\Post_{1j}}} \\
\gamma_j^{j\in J}=&{\bigoplus_{i=0}^{m_1} \vis{{\beta_{1ij}^{j\in J_{1p}}}\subst{\bigodot_{k=i-1}^{0}\!\! \Post_{1k} } }  \oplus \, \vis{{\beta_{2j}^{j\in J_{2}}}\subst{\bigodot_{k=m_1}^{0}\Post_{1k}}}}\, \oplus\\
&\bigoplus_{i=0}^{m_2} \vis{\beta_{3ij}^{j\in J_{3p}} \subst{\bigodot_{k=i-1}^{0}\Post_{3k}\shortodot\Post_2\shortodot\bigodot_{k=m_1}^{0}\Post_{1k}}}\\
\Pred=&{\bigwedge_{p=0}^{m_1}\!\big(\Pred_{1p}\subst{\!\bigodot_{j=p-1}^{0}\!\!\Post_{1j}}\big)\land\Pred_2\subst{\!\bigodot_{p=m_1}^{0}\!\!\Post_{1p}}\land}\\ 
&
{\bigwedge_{p=0}^{m_2}\!\Pred_{3p}\subst{\!\bigodot_{j=p-1}^{0}\!\!\Post_{3j}\odot\Post_2 \odot\!\bigodot_{p=m_1}^{0}\!\!\Post_{1p}}}\\
\Post=&{\bigodot_{p=m_2}^{0}\Post_{3p}\odot\Post_2\odot\bigodot_{j=m_1}^{0}\Post_{1j}}
\end{align*}
}
...
\gamma_{j_0}\!=&~[\beta_0..\beta_n]\\
 =&\!\!
{\bigoplus_{\substack{i=0\\j_0\in J_{1i}}}^{m_1}\!\!\! \vis{{\beta_{1ij_0}}\subst{\!\bigodot_{k=i-1}^{0} \!\!\!\!\Post_{1k} } }  \,\oplus\, \vis{{\beta_{2j_0}}\subst{\!\bigodot_{k=m_1}^{0}\!\!\!\Post_{1k}}}}\oplus\! \\
&\qquad\qquad
{ \!\bigoplus_{\substack{i=0\\j_0\in J_{3i}}}^{m_2}\!\! \vis{{\beta_{3ij_0}} \subst{\!\bigodot_{k=i-1}^{0}\!\!\!\Post_{3k}\shortodot\Post_2\shortodot
...
\set{\gamma'}=&
 \bigoplus_{i=0}^{m_1}\set{\gamma'_{1i}}\subst{\bigodot_{k=i-1}^{0}\Post\,'_{1k}}\,\oplus\,
\set{\gamma'_{2}}\subst{\bigodot_{k=m_1}^{0}\Post\,'_{1k}}\,\oplus\\&\qquad
 \bigoplus_{i=0}^{m_2}\set{\gamma'_{3i}}\subst{\bigodot_{k=i-1}^{0}\Post\,'_{3k}\shortodot\Post\,'_2\shortodot\bigodot_{k=n}^{0}\Post\,'_{1k}}
\displaybreak[0]\\
\Pred\,''=&\bigwedge_{i=0}^{m_1}\Pred\,'_{1i}\subst{\bigodot_{j=i-1}^{0}\Post\,'_{1j}}\land\Pred\,'_2 \subst{\bigodot_{j=m_1}^{0}\Post\,'_{1j}}\land\\ 
...
\begin{split}\gamma'_j=&\bigoplus_{i=0}^{m_1} \vis{{\beta_{1ij}}\subst{\bigodot_{k=i-1}^{0}\! \Post_{1k} } }  \oplus  \vis{{\beta_{2j}}\subst{\bigodot_{k=m_1}^{0}\!\Post_{1k}}}\, \oplus\\&
\bigoplus_{i=0}^{m_2} \vis{\beta_{3ij} \subst{\!\bigodot_{k=i-1}^{0}\!\!\!\Post_{3k}\shortodot\Post_2\shortodot\bigodot_{k=m_1}^{0}\!\!\Post_{1k}}}
\end{split}
\end{equation*}
}
...
\begin{split}
\gamma'_j =&
\bigoplus_{i=0}^{m_1}{\gamma'_{1ij}}\subst{\bigodot_{k=i-1}^{0}\Post\,'_{1k}}\oplus
 {\gamma'_{2 j}}\subst{\bigodot_{k=m_1}^{0}\Post\,'_{1k}}\,\oplus\\&\quad \bigoplus_{i=0}^{m_2}{\gamma'_{3ij}}\subst{\bigodot_{k=i-1}^{0}\Post\,'_{3k}\shortodot\Post\,'_2\shortodot\bigodot_{k=n}^{0}\Post\,'_{1k}}\\
=&
{\bigoplus_{t=0}^{n} {\gamma_{tj}}\subst{\bigodot_{k=t-1}^{0} \Post_{k}}}   
\end{split}
\end{equation*}
}
...
%\begin{split}
%\Pred\,''&=\bigwedge_{i=0}^{m_1}\Pred\,'_{1i}\subst{\bigodot_{j=i-1}^{0}\Post\,'_{1j}}\land\Pred\,'_2 \subst{\bigodot_{j=m_1}^{0}\Post\,'_{1j}}\land\\
%&~~{\bigwedge_{i=0}^{m_3}\Pred\,'_{3i}\subst{\bigodot_{j=i-1}^{0}\Post\,'_{3j}\shortotimes\Post\,'_2\shortotimes\bigodot_{j=m_1}^{0}\Post\,'_{1j}}}\\
%&=\big(\bigwedge_{p=0}^{m_1}\!\Pred_{1p}\subst{\bigodot_{j=p-1}^{0}\Post_{1j}}\land\Pred_2\subst{\bigodot_{p=m_1}^{0}\Post_{1p}}\land\\
%&~~~\bigwedge_{p=0}^{m_2}\!\Pred_{3p}\subst{\bigodot_{j=p-1}^{0}\Post_{3j}\otimes\Post_2 \otimes\bigodot_{p=m_1}^{0}\Post_{1p}}\big) \land \bigwedge_{t=0}^{n} \Pred_t\bigodot_{i=t-1}^{0}\Post_i \land
%%$~\qquad\qquad\,\displaystyle{\bigwedge_{i=0}^{m-1}(\beta_{1{n_i}{j_0}}=\alpha_i)\subst{\bigodot_{j={n_i}-1}^{0}\Post\,'_{1j}}\land(\beta_{2{j_0}}=\alpha_m) \subst{\bigodot_{j=m_1}^{0}\Post\,'_{1j}}\land}
%\end{split}
%\end{equation*}
%%$~\qquad\qquad\,\displaystyle{\bigwedge_{i=m+1}^{n}(\beta_{3{n'_i}{j_0}}=\alpha_i)\subst{\bigodot_{j=n'_i-1}^{0}\Post\,'_{3j}\shortotimes\Post\,'_2\shortotimes\bigodot_{j=m_1}^{0}\Post\,'_{1j}}\Big)}$\\
%\end{small}
\begin{footnotesize}
\begin{align*}
\Pred\,''&=\Big(\bigwedge_{i=0}^{m_1}\Pred\,'_{1i}\subst{\bigodot_{j=i-1}^{0}\!\!\Post\,'_{1j}}\land\Pred\,'_2 \subst{\bigodot_{j=m_1}^{0}\!\!\Post\,'_{1j}}\\ &\qquad\qquad \land
%~\quad
...
\land \bigwedge_{t=0}^{n} \Pred_t\bigodot_{i=t-1}^{0}\!\!\Post_i%\\&\qquad
\land \Big( \bigwedge_{i=0}^{m-1}(\beta_{1{n_i}{j_0}}=\alpha_i)\subst{\bigodot_{j={n_i}-1}^{0}\Post\,'_{1j}}\land(\beta_{2{j_0}}=\alpha_m) \subst{\bigodot_{j=m_1}^{0}\!\!\Post\,'_{1j}}\\ &\qquad
~\quad\land\bigwedge_{i=m+1}^{n}(\beta_{3{n'_i}{j_0}}=\alpha_i)\subst{\bigodot_{j=n'_i-1}^{0}\Post\,'_{3j}\shortodot\Post\,'_2\shortodot\bigodot_{j=m_1}^{0}\!\!\Post\,'_{1j}}\Big)\displaybreak[0]\\
...
\land \Big(\bigwedge_{i=0}^{m-1}(\beta_{i}=\alpha_i)\subst{\bigodot_{j={i-1}}^{0}\!\!\Post_{j}}\land(\beta_m=\alpha_m )\subst{\bigodot_{j=m}^{0}\Post_{j}}\\ &\qquad
~\quad\land\bigwedge_{i=m+1}^{n}(\beta_i=\alpha_i)\subst{\bigodot_{j=i-1}^{m}\!\!\Post_j\shortodot\Post_m\shortodot\bigodot_{j=m-1}^{0}\!\!\Post_{j}}\Big)\\
&=\Pred
\end{align*}
\end{footnotesize}
...
\bigskip
         ...
         Note that if we were using the alternative weak FH-bisimulation relation from Appendix \ref{app-WFH-equiv}, Lemma \ref{lem-rel-OT-WOT}, that is checking strong FH-bisimulation between the corresponding 
