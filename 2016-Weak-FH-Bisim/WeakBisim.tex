...
\end{example}
  ...
  \begin{example}
    Figures \ref{SimpleProtCounter:WeakSpecOA} and \ref{SimpleProtCounter:ImplOA} respectively show the weak automata of the simple protocol specification and implementation.
    \TODO{comment...}

    \TODO{introduce the notation for parametrized OTs}
    
    For readability, we did not include the full details of the Implementation weak open transitions in Figure \ref{SimpleProtCounter:ImplOA}, they will appear below.
    First, we the weak OT loops on state $ostate{000}$ are also present in all other states, we did not repeat them. Then many WOTs are similar, and numbered accordingly as 3, 3', 3'', and 8, 8', 8'',8''' respectively: they only differ by the name of the state variables in their respective source or target states.
\end{example}
...
$post_4 = (\texttt{m1\_msg}\gets \texttt{s1\_msg}, \texttt{m1\_ec}\gets \texttt{s1\_ec})$

$post_5 = (\texttt{m2\_ec}\gets \texttt{m1\_ec})$

$post_6 = (\texttt{s1\_ec}\gets \texttt{m2\_ec+1})$
...
So if we denote ${Post_{456}}^*$ any iteration of this loop, we get ${Post_{456}}^* = (\texttt{s1\_ec}\gets \texttt{s1\_ec}+n)$ for any $n\ge 0$, and we will get e.g. the Post of the weak OT number 3' as $ post_4\shortotimes {Post_{456}}^*\shortotimes\ post_3 = (\texttt{m1\_msg}\gets m, \texttt{m1\_ec}\gets n), \forall n\ge 0$
\medskip
...
$ WI_3(n) = \openrule
  ...
  $ WI_4(n) = \openrule
         {\{\}, True, (\texttt{m1\_msg}\gets \texttt{s1\_msg}, \texttt{m1\_ec}\gets \texttt{s1\_ec}+n)}
...
Preliminary remarks:
  ...
  Now consider relation $\mathcal{R}$ defined by the following triples:
...
\hline
    ...
    b0 & $\ostate{000}$ & True\\
    b0 & $\ostate{202}$ & True\\
    b1 & $\ostate{100}$ & $\texttt{b1\_msg = s1\_msg} \land \texttt{b1\_ec = s1\_ec}$\\
    b1 & $\ostate{210}$ & $\texttt{b1\_msg = m1\_msg} \land \texttt{b1\_ec = m1\_ec}$\\
    b1 & $\ostate{220}$ & $\texttt{b1\_msg = m2\_msg} \land \texttt{b1\_ec = m2\_ec}$\\
    b1 & $\ostate{201}$ & $\texttt{b1\_msg = r1\_msg} \land \texttt{b1\_ec = r1\_ec}$\\
    \hline
    \end{tabular}
  ...
  Consider the first triple from the table, and transition $WS_3(n)$ from state $\ostate{b0}$ for some fixed integer $n$. Its easy to guess that it will correspond to $WI_3(n)$ of $\ostate{000}$.

  $ WS_3(n) = \openrule
  {\{\texttt{P}\mapsto \texttt{p-send(m)}\}, True,
    (\texttt{b1\_msg}\gets \texttt{m}, \texttt{b1\_ec}\gets n)}
  {000 \OTWeakarrow {\underline{\texttt{in(m)}}} 100}
  $
...
\begin{itemize}
  ...
  \item Triple ($\ostate{b1},  \ostate{100},  \texttt{b1\_msg = s1\_msg} \land \texttt{b1\_ec = s1\_ec}$) is in $\mathcal{R}$.
...
\TODO{Insert here $\forall \phi(m1,m2)$ }

$True \land True\hspace{1cm} \implies\\ 
...
there exist   $\set{\beta_{1i}}$, $\set{\beta_{2i}}$, and $\set{\beta_{3i}}$, $\Pred_{1i}$, $\Pred_{3i}$, $\Post_{1i}$, and  $\Pred_2$, $\Post_2$ s.t.\footnote{$n=0$ (resp. $m=0$) corresponds to the case where there is no $\tau$ transition before (resp. after) the transition $\alpha$.}:
\begin{mathpar}
%\mprset {vskip=.5ex}
%\inferrule 
...
where\\
$
\alpha'=\alpha\displaystyle{\subst{\bigotimes_{j=n}^{1}\Post_{1j}}}\\
s=s_{11} \wedge s_{1(n+1)}=s_2 \wedge s'_2 = s_{31} \wedge s_{3(m+1)}=s'\\
\displaystyle{
\set{\gamma}=\mybigdotcup_{i=1}^n \vis{\set{\beta_{1i}}\subst{\bigotimes_{j=i-1}^{1} \Post_{1j} } }  \dotcup  \vis{\set{\beta_2}\subst{\bigotimes_{j=n}^{1}\Post_{1j}}} \dotcup}\\
~\qquad \displaystyle{\mybigdotcup_{i=1}^m \vis{\set{\beta_{3i}} \subst{\bigotimes_{j=i-1}^{1}\Post_{3j}\shortotimes\Post_2\shortotimes\bigotimes_{j=n}^{1}\Post_{1j}} }}
\\
\Pred=\bigwedge_{i=1}^n\Pred_{1i}\subst{\bigotimes_{j=i-1}^{1}\Post_{1j}}\land\Pred_2 \subst{\bigotimes_{j=n}^{1}\Post_{1j}}\land\\ 
~\qquad\quad\Big(\bigwedge_{i=1}^m\Pred_{3i}\subst{\bigotimes_{j=i-1}^{1}\Post_{3j}\shortotimes\Post_2\shortotimes\bigotimes_{j=n}^{1}\Post_{1j}}\Big)\\
\Post=\bigotimes_{j=m}^{1}\Post_{3j}\shortotimes\Post_2\shortotimes\bigotimes_{j=n}^{1}\Post_{1j}
$
...
\begin{itemize}
\item In the base case there is one transition, which corresponds to $n=0$ and $m=0$, we have:
\begin{mathpar}
 % \mprset {vskip=.5ex}
%\inferrule{
 \openrule
...
We consider the case $n'=n+1$ ($m'=m+1$ is similar).  We want to prove (1) $\Rightarrow$ (2) in the lemma, and in (1) we  focus on the case where there is a set of open transitions (this is the case: $s\neq s'$). In other words, we consider the sequence of ($n+m+2$) open transitions: 
\begin{mathpar}
...
where\\
$
\alpha'=\alpha\displaystyle{\subst{\bigotimes_{j=n+1}^{2}\!\!\Post_{1j}}}\\
s''=s_{12} \wedge s_{1(n+2)}=s_2 \wedge s'_2 = s_{31} \wedge s_{3(m+1)}=s'\\
\displaystyle{
\set{\gamma}=\mybigdotcup_{i=2}^{n+1} \vis{\set{\beta_{1i}}\subst{\bigotimes_{j=i-1}^{1} \Post_{1j} } }  \dotcup  \vis{\set{\beta_2}\subst{\bigotimes_{j=n+1}^{2}\Post_{1j}}} \dotcup}
\\
{}~\qquad 
\displaystyle{\mybigdotcup_{i=1}^m \vis{\set{\beta_{3i}} \subst{\bigotimes_{j=i-1}^{1}\Post_{3j}\shortotimes\Post_2\shortotimes\bigotimes_{j=n}^{2}\Post_{1j}} }}
\\
\Pred=\bigwedge_{i=2}^{n+1}\Pred_{1i}\subst{\bigotimes_{j=i-1}^{1}\Post_{1j}}\land\Pred_2 \subst{\bigotimes_{j={n+1}}^{2}\Post_{1j}}\land
\\ 
{}~\qquad\quad
\Big(\bigwedge_{i=1}^m\Pred_{3i}\subst{\bigotimes_{j=i-1}^{1}\Post_{3j}\shortotimes\Post_2\shortotimes\bigotimes_{j={n+1}}^{2}\Post_{1j}}\Big)\\
\Post=\bigotimes_{j=m}^{1}\Post_{3j}\shortotimes\Post_2\shortotimes\bigotimes_{j={n+1}}^{2}\Post_{1j}
$
...
\\
\set{\gamma''}= \vis{\set{\beta_{11}}} \dotcup \set{\gamma}\subst{\Post_{11}}\quad
\Pred\,''=\Pred_{11}\land\Pred\subst{\Post_{11}}\\%\land (\Pred_3\subst{\Post_2})\subst{\Post_{11}}
\Post\,''=\Id(s'')\shortotimes\Post\shortotimes\Post_{11} \quad \alpha''=\alpha'\subst{\Post_{11}}
}
{
\openrule
         {
...
{}~\qquad~\displaystyle{ \mybigdotcup_{i=1}^m \vis{\set{\beta_{3i}} \subst{\bigotimes_{j=i-1}^{1}\Post_{3j}\shortotimes\Post_2\shortotimes\bigotimes_{j=n}^{0}\Post_{1j}} }}
\\
\Pred=\bigwedge_{i=1}^{n+1}\Pred_{1i}\subst{\bigotimes_{j=i-1}^{1}\Post_{1j}}\land\Pred_2 \subst{\bigotimes_{j={n+1}}^{1}\Post_{1j}}\land\\ 
{}~\qquad\quad\Big(\bigwedge_{i=1}^m\Pred_{3i}\subst{\bigotimes_{j=i-1}^{1}\Post_{3j}\shortotimes\Post_2\shortotimes\bigotimes_{j={n+1}}^{1}\Post_{1j}}\Big)\\
\Post=\bigotimes_{j=m}^{1}\Post_{3j}\shortotimes\Post_2\shortotimes\bigotimes_{j={n+1}}^{1}\Post_{1j}
$
...
In Figure \ref{Appendix:SpecOA-partial} we show the weak oepn automaton of our implementation pNet. Details of the weak transitions is listed here:
...
$ 3 = \openrule
  {\{\texttt{P}\mapsto \texttt{p-send(m)}\}, True,
    (\texttt{s1\_msg}\gets m, \texttt{s1\_ec}\gets n)}
  {000 \OTWeakarrow {\underline{\texttt{in(m)}}} 100}
  \ \forall n\ge 0$
