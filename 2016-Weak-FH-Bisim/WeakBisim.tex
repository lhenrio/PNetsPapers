...
\ERIC{This is similar to the early and late symbolic bisimulation equivalences for value-passing CCS \cite{HENNESSY:1995}, though we need more general definitions in our setting.}
...
\begin{example}
The simulation of one transition by many others is one non-standard aspect of this definition. This is made necessary by the parameterised nature of our model. Consider the open transition:
\[ \openrule
         {
           \set{\beta}, \True,\subst{y\leftarrow x}}
          {s \OTarrow {\alpha(x)} s'}\]
Bisimulation should allow it to be matched by the two following ones (depending on the value of $x$):
\[ \openrule
         {
...
Quotienting and decomposition techniques \cite{Larsen:91,LaveauxWillemse:ICE21} seem closer to our approach as they propose to 
