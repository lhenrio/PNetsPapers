...
\hl{Additionally we require that:} $\{x_j|j\in J\} = \vars(s')$.
...
\item[$\bullet$] $\symb{SV}_k^{k\in K}$ is a set of
  ...
  \symb{SV}_k\!=\!\SV{\alpha_{l}^{l\in I_k \uplus J_k}}{\alpha'_k}{e_k}$ \hl{where}
...
\item The set of variables of a pNet $P$, denoted $vars(P)$ is the set of variables of all states of all pLTSs that compose $P$.
...
\TODO{J'ai change les $\underline{in}$ et $\underline{out}$ en $in$ et $out$ dans le texte. Rabea, peux-tu le faire dans toutes les figures (1, 2, 3,4,5,6) ?}
...
\begin{definition}[Open transitions]\label{def:OT}
	...
	where $x_k$ are\ \ERIC{the state variables of $s'$}\footnote{all of them, completed by identity if needed}, and $e_k$ are expressions
        over the \ERIC{source or target state variables}\footnote{On pourrait du coup y mettre ici n'importe quelle variable du OA...}. Open transitions are identified
...
inference rule. They correspond to two different logical levels.
 The open transition is itself a logical implication, but using a simple logic (this logic includes the boolean expressions $\AlgB$, boolean operators, and term equality). \hl{We differentiate classical (black) inference rules that use  an expressive logic and are paper rules from open transition rules that use a simpler  logic,  will be embedded into automata transitions, and could typically be handled in a 
 mechanical way.}
...
\hl{the action algebra} $\mathcal{A}_P$. Then it is decidable whether the relation 
...
A pLTS being its own single leave:
  $States(\mylangle S,s_0, \to\myrangle) = \{\triangleleft s \triangleright| s \in S, \}$. To be precise, the state of a pLTS is not reduced to a state identity, it also contains the value of its variables $vars(P)$. 
...
with the $\symb{Sender}$ within the \emph{SimpleProtocol}) is:
 ...
 \[  OT_1  = \openrule{
    ...
    value of the sender state variable named $\texttt{s1\_msg}$.
 ...
 %%                      }
 %%    {\ostate{s_1,m_0,r_0} \OTarrow{\nounderline{send(SV2\_m)}} \ostate{s_2,m_1,r_0}}
 %%    $
 ...
 %%                      }
 %%    {\ostate{s_1,m_0,r_0} \OTarrow{\nounderline{send(s_1\_m)}} \ostate{s_2,m_1,r_0}}
 %%    $
...
\hl{The semantics of a pNet} $\pNet$ is an open automaton $A\!= 
...
%%   The rule uses \TrUn\ for the $\delta$ transition of $C_3$, for the $l$ transition of $C_4$, then combines the result using the $a_4$ vector of the bottom pNet node, and the $\nounderline{\delta(x)}$ vector of the top node.
...
\paragraph{Example} \hl{Figure} \ref{SimpleProtCounter:SpecOA} shows the open automaton computed from the Specification pNet given in Figure \ref{SimpleProt:Spec}. 
...
\TODO{Ca serai tpas mal de trouver une notation plus intuitive au lieu de ce $\vis{\set\beta}$ ?}
...
\begin{definition}[Building a weak open automaton]\label{def:buildweakOT}
  Let $A = <J,\mathcal{S},s_0,V,\mathcal{T}>$ be an open automaton. 
The weak open automaton \emph{derived} from $A$ is an open automaton  $<J,\mathcal{S},s_0,V,\WT>$ where $\WT$ is derived from $\mathcal{T}$ as follows: 
...
\begin{figure}[ht]
   \centerline{\includegraphics[width=12cm]{XFIG/WOT2-result}}
...
\begin{example}
  ...
  $\openrule
        ...
        {\text{b1} \OTWeakarrow \tau \text{b1}} \forall n \geq 0$.  Finally,  applying again \WTTrois, and using the central open transition having \texttt{\nounderline{out(out(b1\_m,b1\_ec)}}  as $\alpha$, we get the resulting weak open transition between b1 and b0 (as shown in Figure \ref{WOT2}).  
...
\end{figure}
 ...
 For readability, we did not include the full details of the Implementation weak open transitions in Figure \ref{SimpleProtCounter:ImplWOA2}, they will appear below.
First, we point out that the weak OT loops ($WI_1$,$WI_2$ and $W_\tau$) on state ${000}$ are also present in all other states, we did not repeat them. Then many WOTs are similar, and numbered accordingly as 3, 3a, 3b, 3c and 8, 8a, 8b, 8c respectively: they only differ by the name of the state variables in their respective source or target states.
\end{example}
...
Now let us give some details about the construction of the weak automaton of the implementation pNet, obtained by application of the weak rules as explained above. We concentrate on weak open transitions $WI_3$ and $WI_4$. Let us denote as $post_n$ the effect (= substitution) of the strong open transitions $OT_n$ from Figure \ref{SimpleProtCounter:ImplOA}:
\smallskip

$post_3 = (\texttt{s1\_msg}\gets \texttt{m}, \texttt{s1\_ec}\gets 0)$
...
So if we denote ${post_{456*}}$ any iteration of this loop, we get ${post_{456*}} = (\texttt{s1\_ec}\gets \texttt{s1\_ec}+n)$ for any $n\ge 0$, and the \Post~ of the weak OT  $WI_{3}$ is:\\
 ${post_{456*}}\shortotimes\ post_3 = (\texttt{s1\_msg}\gets m, \texttt{s1\_ec}\gets n), \forall n\ge 0$ and \Post~ of  $WI_{3a}$  is:\\ $post_4\shortotimes {post_{456*}}\shortotimes\ post_3 = (\texttt{m1\_msg}\gets m, \texttt{m1\_ec}\gets n), \forall n\ge 0$.
\medskip
...
$ WI_4(n) = \openrule
         {\{\}, True, 
   (\texttt{m1\_msg}\!\gets\! \texttt{s1\_msg}, \texttt{m1\_ec}\!\gets\! \texttt{s1\_ec}+n, \texttt{s2\_ec}\!\gets\! \texttt{s1\_ec}+n)}
...
$ WI_{456}(n) = \openrule
         {\{\}, True, 
    (\texttt{s1\_ec}\gets \texttt{s1\_ec}+n)}
...
$
\begin{array}{ll}
Post_{6a}= post_{4}\shortotimes\ post_{456*}\shortotimes\ post_{6} =&  (\texttt{m1\_msg}\gets \texttt{s1\_msg}, \texttt{m1\_ec}\gets \texttt{s1\_ec}, \texttt{s2\_ec}\gets \texttt{s1\_ec})\,
\shortotimes \\ &(\texttt{s1\_ec}\gets \texttt{s1\_ec}+n) 
\shortotimes (\texttt{s1\_ec}\gets \texttt{s2\_ec}+1) =\\
&(\texttt{m1\_msg}\!\gets \texttt{s1\_msg}, \texttt{m1\_ec}\!\gets \texttt{s2\_ec}+1\!+\!n, \texttt{s2\_ec}\!\gets \texttt{s2\_ec}+1\!+\!n)
\end{array}$\\

$ WI_{6a}(n) = \openrule
         {\{\}, True, 
         (\texttt{m1\_ec}\gets \texttt{s1\_ec}+1+n, \texttt{s2\_ec}\gets \texttt{s2\_ec}+1+n)}
         {220 \OTWeakarrow {\tau} 210} \forall n\ge 0
         $

$Post_{8c}= post_{8}\shortotimes\ post_{7}\shortotimes\ post_{4}\shortotimes\ post_{456*} 
= (\texttt{r1\_msg}\gets \texttt{s1\_msg}, \texttt{r1\_ec}\gets \texttt{s2\_msg}+1+n), \forall n\ge 0$
         
$ WI_{8c}(n) = \openrule
         {\{\texttt{Q}\mapsto \texttt{q-recv(s1\_msg,s2\_ec}+n)\}, True, ()}
<<<<<<< HEAD
        {210 \OTWeakarrow {\nounderline{\texttt{out(r1\_msg,r1\_ec)}\subst{(\texttt{r1\_msg}\gets \texttt{s1\_msg}, \texttt{r1\_ec}\gets \texttt{s2\_msg}+n)}}} 202}
=======
         {210 \OTWeakarrow {\nounderline{\texttt{out(r1\_msg,r1\_ec)}\subst{(\texttt{r1\_msg}\gets \texttt{s1\_msg}, \texttt{r1\_ec}\gets \texttt{s2\_msg}+n)}}} 202}
>>>>>>> 43db4faa763ea798b44dcee241ba0b0ba2526abe
        $($\forall n\ge 0$)
...
\hline
    ...
    b1 & $220$ & $\texttt{b1\_msg = s2\_msg} \land \texttt{b1\_ec = s2\_ec}$\\
...
\begin{itemize}
  ...
  \item Triple ($\text{b1},  100,  \texttt{b1\_msg = s1\_msg} \land \texttt{b1\_ec = s1\_ec}$) is in $\mathcal{R}$.
...
\\
(\texttt{b1\_msg\!\! = \!\!s1\_msg} \land \texttt{b1\_ec\!\! =\!\! s1\_ec}) \subst{(\texttt{b1\_msg}\!\gets\! \texttt{m1}, \texttt{b1\_ec}\!\gets\! 0)\uplus (\texttt{s1\_msg}\!\gets\! \texttt{m2},\texttt{s1\_ec}\!\gets\! 0)})\}$
...
\end{itemize}
  ...
  $ WS_4(n) = \openrule
         {\{\}, True, (\texttt{b1\_ec}\gets \texttt{b1\_ec}+n)}
         {\text{b1} \OTWeakarrow {\tau} \text{b1}}
         \ \forall n\ge 0
         $
  
  \medskip       A similar reasonning gives us\TODO{Refait, please check. But I think it does not show much more than first case, can we find a better case ?}

\begin{itemize}
  \item No active holes and the pair of ressulting states $<210,b1>$ is in $\mathcal{R}$.
\item $fv(SI_4) = \{\}, fv(WS_4(n)) = \{n\}$, \TODO{Is that correct ? Is it true that considering $n$ as a free variable allows Z3 to find  a suitable value...?}
\item and the verification condition\\

         $(\texttt{b1\_msg} = \texttt{s1\_msg} \land \texttt{b1\_ec} = \texttt{s1\_ec}) \land True\hspace{3mm} \implies
\exists n.(True \land \tau=\tau \land \\ (\texttt{b1\_msg} = \texttt{m1\_msg} \land \texttt{b1\_ec} = \texttt{m1\_ec})  \subst{\texttt{m1\_msg}\!\gets\! \texttt{s1\_msg}, \texttt{m1\_ec}\!\gets\! \texttt{s1\_ec}, \texttt{s2\_ec}\gets \texttt{s1\_ec}}
\uplus \subst{\texttt{b1\_ec}\gets \texttt{b1\_ec}+n})
$
...
\begin{figure}[h]
   \centerline{\includegraphics[width=10cm]{XFIG/SimpleProtImpl-WOA}}
  \caption{Weak Open Automaton of the implementation}
   \label{Appendix:ImplOA}
\end{figure}

In Figure \ref{Appendix:ImplOA} we show the weak open automaton of our implementation pNet. 
\TODO{This version is very uncomplete, at least 12 WOTs are missing, including: all variants of WI3 starting from 202; the inverse of WI4, WI5, WI6, WI8, etc}
\TODO{See also the last bunch of ransitions, at the end of the list, for combinations of any $\tau$ transition with an external action of P or Q}

\begin{figure}[h]
   \centerline{\includegraphics[width=11cm]{XFIG/SimpleProtImpl-WOA2}}
  \caption{Weak Open Automaton of the implementation: alternative}
   \label{Appendix:ImplOA2}
\end{figure}

Figure \ref{Appendix:ImplOA2} shows a more compact version of this weak automation, that will be easier to read for humans...
...
$ WI_{3a}(n) = \openrule
  {\{\texttt{P}\mapsto \texttt{p-send(m)}\}, True,
    (\texttt{m1\_msg}\gets \texttt{m}, \texttt{m1\_ec}\gets n, \texttt{s2\_ec}\gets n)}
...
$ WI_{3c}(n) = \openrule
  {\{\texttt{P}\mapsto \texttt{p-send(m)}\}, True,
    (\texttt{r1\_msg}\gets \texttt{m}, \texttt{r1\_ec}\gets n)}
...
$ WI_{4a}(n) = \openrule
         {\{\}, True, 
    (\texttt{s2\_ec}\gets \texttt{s1\_ec}+n)}
         {100 \OTWeakarrow {\tau} 220}
$

$ WI_5(n) = \openrule
         {\{\}, True, (\texttt{s2\_ec}\gets \texttt{s2\_ec}+n)}
         {210 \OTWeakarrow {\tau} 220}
         \ \forall n\ge 0$

$ WI_{5a}(n) = \openrule
         {\{\}, True, (\texttt{s1\_ec}\gets \texttt{s2\_ec}+1+n)}
...
= \left( (\texttt{m1\_msg}\gets \texttt{s1\_msg}, \texttt{m1\_ec}\gets \texttt{s1\_ec}, \texttt{s2\_ec}\gets \texttt{s1\_ec})
\shortotimes (\texttt{s1\_ec}\gets \texttt{s1\_ec}+n) \right)
\shortotimes (\texttt{s1\_ec}\gets \texttt{s2\_ec}+1) \\
= (\texttt{m1\_msg}\gets \texttt{s1\_msg}, \texttt{m1\_ec}\gets (\texttt{s2\_ec}+1)+n, \texttt{s2\_ec}\gets (\texttt{s2\_ec}+1)+n)$

\medskip
This result ($WI_{6a}$) is INCORRECT: the Post cannot use $s1\_msg$, this variable is not available in state 220

$ WI_{456*}(n) = \openrule
         {\{\}, True, 
    (\texttt{s1\_msg}\gets \texttt{s1\_msg}, \texttt{s1\_ec}\gets \texttt{s1\_ec}+n)}
  {100 \OTWeakarrow {\tau} 100}
        $

Note that the loop itself does not assign $s1\_msg$, but as we are in a self-loop of state 100, we can add the identity assignment on it.
\medskip

$ WI_{564*}(n) = \openrule
         {\{\}, True, 
    (\texttt{m1\_msg}\gets \texttt{s1\_msg}, \texttt{s2\_ec}\gets \texttt{s2\_ec}+1+n, \texttt{m1\_ec}\gets \texttt{s2\_ec}+1+n)}
  {210 \OTWeakarrow {\tau} 210}
        $

Because 

$Post_{564}=\ post_{4}\shortotimes\ post_{6}\shortotimes\ post_{5}\\
= \left( (\texttt{m1\_msg}\gets \texttt{s1\_msg}, \texttt{m1\_ec}\gets \texttt{s1\_ec}, \texttt{s2\_ec}\gets \texttt{s1\_ec})
\shortotimes (\texttt{s1\_ec}\gets \texttt{s2\_ec}+1) \right)
\shortotimes () \\
= (\texttt{m1\_msg}\gets \texttt{s1\_msg}, \texttt{m1\_ec}\gets \texttt{s2\_ec}+1, \texttt{s2\_ec}\gets \texttt{s2\_ec}+1$

This result is INCORRECT: the Post cannot use $s1\_msg$, this variable is not available in state 210


$ WI_{645*}(n) = \openrule
         {\{\}, True, 
    (\texttt{s2\_ec}\gets \texttt{s2\_ec}+1+n)}
...
\medskip
$ WI_7(n) = \openrule
         {\{\}, True, (\texttt{r1\_msg}\gets \texttt{s1\_msg}, \texttt{r1\_ec}\gets \texttt{s2\_ec}+n)}
         {210 \OTWeakarrow {\tau} 201}
         \ \forall n\ge 0$
         
$WI_{7a}$ shows the same problem, as it  includes a dependency on $s1\_msg$:

$ WI_{7a}(n) = \openrule
         {\{\}, True, (\texttt{r1\_msg}\gets \texttt{s1\_msg}, \texttt{r1\_ec}\gets \texttt{m1\_ec}+n)}
         {220 \OTWeakarrow {\tau} 201}
         \ \forall n\ge 0$
         
$ WI_{7b}(n) = \openrule
         {\{\}, True, (\texttt{r1\_msg}\gets \texttt{m1\_msg}, \texttt{r1\_ec}\gets \texttt{s2\_ec}+n)}
         {100 \OTWeakarrow {\tau} 201}
         \ \forall n\ge 1$
         
         
\RAB{Erreur sur les deux transition $WI_{7}$, $WI_{7a}$ et $WI_{7b}$}
...
$ WI_{8a}(n) = \openrule
         {\{\texttt{Q}\mapsto \texttt{q-recv(m1\_msg,m1\_ec}+n)\}, True, ()}
         {210 \OTWeakarrow {\nounderline{\texttt{out(m1\_msg,m1\_ec}+n)}} \{202, 000\}}
         \ \forall n\ge 1$

 $ WI_{8b}(n) = \openrule
         {\{\texttt{Q}\mapsto \texttt{q-recv(??\_msg,s2\_ec}+n)\}, True, ()}
         {220 \OTWeakarrow {\nounderline{\texttt{out(??\_msg,m2\_ec}+n)}} \{202, 000\}}
         \ \forall n\ge 1$

 $ WI_{8c}(n) = \openrule
         {\{\texttt{Q}\mapsto \texttt{q-recv(s1\_msg,s1\_ec}+n)\}, True, ()}
         {100 \OTWeakarrow {\nounderline{\texttt{out(s1\_msg,s1\_ec}+n)}} \{202, 000\}}
         \ \forall n\ge 1$
...
\TODO{ajouter les boucles de $WI_1$ et $WI_2$, dans le cas ou $p_a$ (resp $q_b$) sont des $\tau$:}

$WI_{12*}(n) = \openrule
{\{(\texttt{P}\mapsto \tau|\texttt{Q}\mapsto \tau)^n\},true, ()}
  {s \OTWeakarrow {\tau} s} 
s \in S$
...
We reacall here the realtion $\mathcal{R}$ that is the candidate for our weak bisimulation relation:
...
$SI_3 << WS_3$
         ...
         And these numbers raise faster for other states, e.g. 13 WOTs for state 100.
