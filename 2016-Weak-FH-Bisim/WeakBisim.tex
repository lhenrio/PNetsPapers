...
\TODO{REMOVE NExT RULE : IT IS WRONG: it should have different variables, etc.}
	\noindent
\begin{mathpar}
    \mprset {vskip=.8ex}
\inferrule
...
\forall m\!\!\in\!\! I_2.		{ \pNet_m 
    ...
    {\mylangle {\pNet}_m^{m\in I}, \set{\Sort}, \symb{SV}_k^{k\in K}\myrangle
    	...
    	{\openrule
    		{
    		{\beta_j}^{j\in J\setminus \Holes(Q)\uplus \{j_0\}}, \Pred',  \biguplus_{m\in I_1\uplus I_2} 
    		\Post_m'}
    		{\ostate{s_i^{i\in L\setminus L_Q}} \OTarrow {\alpha}
...
\begin{lemma}[Weak OT decomposition]\label{lem-decompose} 
	...
	\]
		with  $J\cap\Holes(Q)\neq\emptyset$ or $\exists i\in L_Q.\,s_i\neq s'_i$, i.e. $Q$ takes part in the reduction. Suppose $\gamma_{j_0}=[\beta_0..\beta_n]$
		 then there exist $\Pred\,'$,  
...
\openrule
    {
       \set{\beta^1_i},\Pred^1_i,\Post^1_i   }
         {\ostate{s^1_i} \OTarrow {\tau} \ostate{s^1_{i+1}}}$
~and~ $\forall i \in [0\ldots m] \, P[Q]_{j_0}	\models \openrule
         {
           \set{\beta^3_i},\Pred\,^3_i, \Post_i^3   }
         {\ostate{s^3_i} \OTarrow {\tau} \ostate{s^3_{i+1}}}
$\\
and $ P[Q]_{j_0}	\models \openrule
         {
           \set{\beta^2},\Pred\,^2,\Post^2 }
         {\ostate{s^2} \OTarrow {\alpha} \ostate{s'^2}}
$ where\\
$
s=s^1_0 \wedge s^1_n=s^2 \wedge s'^2 = s^3_0 \wedge s^3_m=s',
\displaystyle{\gamma_j^{j\in J}=\mybigdotcup_{i=0}^n \vis{\set{\beta^1_i}} \dotcup  \vis{\set{\beta^2}} \dotcup \mybigdotcup_{i=0}^m \vis{\set{\beta^3_i}}}
\\
\Pred=\bigwedge_{i=0}^n\Pred^1_i\land\Pred\,^2\bigotimes_{i=0}^{n}\subst{\Post_i^1}\land (\bigwedge_{i=0}^m\Pred\,^3_i\subst{\Post^2})\bigotimes_{i=0}^{n}\subst{\Post^1_i}\\
\Post=\biguplus_{i=0}^{m}(\Post^3_i\subst{\Post^2})\bigotimes_{i=0}^{n}\subst{\Post^1_i}
$

We can apply Lemma \ref{lem-decompose} on each $\symb{OT}$,  we have obtain of each case:
\begin{enumerate}
\item $P\models{\openrule
			{
				\beta_{ij}^{1j\in (J\setminus \Holes(Q)) \cup \{j_0\}}, 
				{\Pred_i^1}',  
				{\Post_i^1}'}
			{\ostate{s_{il}^{1l\in L\setminus L_Q}} \OTarrow {\tau}
				\ostate{s_{il}'^{\,1l\in L\setminus L_Q}}}
		}%\]
	\,\text{and~~}
		%\[
		Q\models{\openrule
			{
				\beta_{ij}^{1j\in J\cap\Holes(Q)}, {\Pred_i^1}'',  
				{\Post_i^1}''}
			{\ostate{s_{il}^{1l\in L_Q}} \OTarrow {\alpha^1_Q}
				\ostate{s_{il}'^{\,1l\in L_Q}}}
		}%\]
$and  $\Pred_i^1 \iff {\Pred_i^1}'
		\land {\Pred_i^1}''\land \alpha^1_Q=\beta^1_{ij_0}$, $\Post_i^1={\Post_i^1}'\uplus 
		{\Post_i^1}''$ where ${\Post_i^1}''$ is the restriction of $\Post_i^1$ over variables of $\Leaves(Q)$.
\item
 $P\models{\openrule
			{
				\beta_{ij}^{3j\in (J\setminus \Holes(Q)) \cup \{j_0\}}, 
				{\Pred_i^3}',  
				{\Post_i^3}'}
			{\ostate{s_{il}^{3l\in L\setminus L_Q}} \OTarrow {\tau}
				\ostate{s_{il}'^{\,3l\in L\setminus L_Q}}}
		}%\]
	\,\text{and~~}
		%\[
		Q\models{\openrule
			{
				\beta_{ij}^{3j\in J\cap\Holes(Q)}, {\Pred_i^3}'',  
				{\Post_i^3}''}
			{\ostate{s_{il}^{3l\in L_Q}} \OTarrow {\alpha^3_Q}
				\ostate{s_{il}'^{\,3l\in L_Q}}}
		}%\]
$and  $\Pred_i^3 \iff {\Pred_i^3}'
		\land {\Pred_i^3}''\land \alpha^3_Q=\beta^3_{ij_0}$, $\Post_i^3={\Post_i^3}'\uplus 
		{\Post_i^3}''$ where ${\Post_i^3}''$ is the restriction of $\Post_i^3$ over variables of $\Leaves(Q)$.
		
\item 
 $
		P\models{\openrule
			{
				\beta_j^{2j\in (J\setminus \Holes(Q)) \cup \{j_0\}}, 
				{\Pred^2}',  
				{\Post^2}'}
			{\ostate{s_l^{2l\in L\setminus L_Q}} \OTarrow {\alpha}
				\ostate{s_l'^{\,2l\in L\setminus L_Q}}}
		}%\]
	\,\text{and~~}
		%\[
		Q\models{\openrule
			{
				\beta_j^{2j\in J\cap\Holes(Q)}, {\Pred^2}'',  
				\Post\,''}
			{\ostate{s_l^{2l\in L_Q}} \OTarrow {\alpha^2_Q}
				\ostate{s_l'^{\,2l\in L_Q}}}
		}%\]
		$
		and  $\Pred^2 \iff {\Pred^2}'
		\land {\Pred^2}''\land \alpha^2_Q=\beta^2_{j_0}$, $\Post^2={\Post^2}'\uplus 
		{\Post^2}''$ where ${\Post^2}''$ is the restriction of $\Post^2$ over variables of $\Leaves(Q)$.
\end{enumerate}
By Lemma \ref{lem-rel-OT-WOT} we can conclude that we have:
...
\TODO{j'explicite Pred', Post', ect... Mais je ne sais pas trop pour Q???}

\end{proof}
\begin{lemma}[WOT decomposition]\label{lem-wot-decompose} 
	...
	\]
		with  $J\cap\Holes(Q)\neq\emptyset$ or $\exists i\in L_Q.\,s_i\neq s'_i$, i.e. $Q$ takes part in the reduction\footnote{Note that necessarily $\alpha_Q\neq \tau$ as soon as silent actions are not observable Definition~\ref{def:Non-ObsTau}.}.
		 Then there exist $\alpha_Q$, $\Pred\,'$, $\Pred\,''$, 
		$\Post\,'$, $\Post\,''$ s.t.:\\[-2ex]
		%\[
		\begin{mathpar}
	...
	\vspace{-2.2ex}\\\text{and~~}
		...
		Q\models{\openrule
			{
				\gamma_j^{j\in J\cap\Holes(Q)}, \Pred\,'',  
				\Post\,''}
			{\ostate{s_i^{i\in L_Q}} \OTWeakarrow {\alpha_Q}
				\ostate{s_i'^{\,i\in L_Q}}}
		}%\]
		\end{mathpar}
		and  $\Pred \iff \Pred\,'
		\land \Pred\,''\land \gamma_{j_0}=[\alpha_Q]$, $\Post=\Post\,'\uplus 
		\Post\,''$ where $\Post\,''$ is the restriction of $\Post$ over variables of 
		$\Leaves(Q)$.
