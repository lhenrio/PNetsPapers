...
\]
We can apply Lemma \ref{lem-compose} on each   $\symb{OT}_p$ of $P$ in the form: 
$P\!\models		
\openrule
    {
       {(\beta^i_p)^{p\in J}},\Pred_p,\Post_p   }
         {\ostate{s^{i\in L}_{pi}} \OTarrow {\alpha} \ostate{s^{i\in L}_{(p+1)i}}}$ 
At the beginning $Q$ is at state $\ostate{s_{q0}}$. We proceed by iteration over the list  $\gamma_{j_0}=[\beta_0..\beta_n]$.\\
For each step when $P$ moves, we have two cases:
\begin{itemize}
\item If $j_0\in J$ then Q moves by one open transition, and by hypothesis  $\beta^i_{j_0}=\beta_t$ with $\beta^i_{j_0}\neq \tau$ for some $t$ such that $t$ has value $0$ at beginning and that is incremented whenever this condition is satisfied. In this case, we have:
\[Q\models{\openrule
	{
		\gamma_{t j}^{j\in J_t},
		 \Pred_t,  
...
\[Q\models{\openrule
	{
		\gamma_{t j}^{j\in J_t},
		 \Pred_t,  
		\Post_t}
	{\ostate{s_{t i}^{i\in L_Q}} \OTarrow {\alpha_t}
		\ostate{s_{(t+1) i}^{\, i\in L_Q}}}
}
\]
\begin{enumerate}
...
\item For the observable open transition. We have $\alpha_t=\beta_{j_0}$  by Lemma \ref{lem-compose} we obtain:\\ 
\[P[Q]_{j_0}\!\models		
\openrule
    {
       \beta_p^{(p\in J\setminus\{j_0\}) \uplus J_Q},{\Pred_p}\!\!'',{\Post_p}\!\!''   }
