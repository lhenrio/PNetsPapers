...
trivial). 
We use \ERIC{sets of assignments of the form $x_i:=e_i$}, ranged over by $\Post$; $\subst{\Post}$ is the substitution that applies all the substitutions defined by $\Post$ in a parallel manner. $\otimes$ is the composition operator on substitutions: $(x_k\gets e_k)^{k\in K}\shortotimes \Post =  (x_k\gets e_k\subst{\Post})^{k\in K}$
...
The pNet has a tree-like structure.  The root node of the tree {\it SimpleSystemSpec} is the top level of the pNet structure. It acts as the parallel operator. It consists of tree nodes: two Holes $P$ and $Q$ and one subnet, noted {\it PrefectMedium}. Nodes of the tree are synchronized using four synchronization vectors, that  express the possible synchronization between the parameterized actions of a subset of the nodes. For instance, in the vector "$< p\_send(m),in(m),\_> \rightarrow \underline{in(m)}$ only $P$ and {\it PrefectMedium} nodes are involved in the synchronization. In particular, this means the synchronization between these processes occurs when  process $P$  performs $p\_send(m)$  action sending a message, and  the medium accepts the message through $in(m)$ action  at the same time. And the result that will be returned at upper level is the internal action  $\underline{in(m)}$.


Fig. \ref{SimpleProt:Impl} shows the pNet model of the protocol implementation. When the $Medium$ detects an error (modeled by a local $\tau$ action), it sends back a $m\_error$ message to the $Sender$. The $Sender$ increments its local error counter $ec$, and resends the message (including $ec$) to the $Medium$, that will, eventually, transmit $m,ec$ to the $Receiver$.
...
\begin{definition}[Open transitions]\label{def:OT}
	...
	\ERIC{assignments that are effective \emph{after the open transition}}, they are
...
with the $Sender$ within the \emph{SimpleProtocol}) is:
 ...
 \smallskip\noindent
 \[  OT_1  = \openrule{
      \{P\mapsto p\_send(m)\} \qquad [m=m'] \qquad \{s1\_m\gets m\}
                      }
    {\ostate{s_0,a_0,r_0} \OTarrow{\underline{in(m')}} \ostate{s_1,a_0,r_0}}
    \]
...
\paragraph{Example} Figure \ref{SimpleProtCounter:SpecOA} shows the open automaton computed from the Specification pNet in Figure \ref{SimpleProt:Spec}. 
\RAB{Here, in the open automaton drawing, we use a user-friendly notation for the OTs, in the form ``$\{actions of holes\} [guard] action \{assigns\}$'', rather than the full OT form with the red-dotted rule. Note that the resulting action of the OTs appear here between the guard and the set of assignments. For example, in the OT where $\beta=\{P\mapsto p\_a\}$, $\Pred=P\_a\neq p\_send$, and $\alpha=P\_a$ is denoted in the open automata \texttt{\{P->p-a\} [P-a!=p-send] P-a}.
}
...
Figure \ref{SimpleProtCounter:ImplOA} shows the full open automaton of the implementation $SimpleSystem$. 
Note that, again for readability,  we have also simplified the form of the variable dependencies in the OTs replacing e.g. in OT1 $\{P\mapsto p\_send(m)\} \quad [m=m']$ and $\alpha= \underline{in(m')}$ by the shorter \texttt{\{P->p\_send(m)\} in(m)}.
...
%% We show here the first open transition, written here in the form of the tool full output.

%% $  OT_1  = \openrule{
%%   (P\mapsto p\_send(m:sva\_SV6:1:1)) \\
%%   [( msg =m: sva\_SV6 :1:1) \wedge (( in( msg ))=( in(m: sva_SV6 :1:1)))]\\
%%   \{ s1\_msg := msg , s1\_c := 0\}
%%                       }
%% {\ostate{s_0,m_0,r_0} \OTarrow{\underline{in(m:sva\_SV6:1:1)}} \ostate{s_1,m_0,r_0}}
%% $
    \ERIC{Also we have short labels for states, representing $\ostate{s_0,m_0,r_0}$ by \texttt{000}. The resulting behavior is quite simple: In every state, holes $P$ and $Q$ can do any action independent of the protocol (we only have labelled these on state \texttt{000}). Then we have a main loop including receiving a message from $P$ and transmitting the same message to $Q$, with some intermediate $\tau$ actions from the internal communications between the protocol processes. In most of the transitions, you can observe that data is propagated between the successive state variables (holding the message, and the error counter value). On th eright of the figure, you have a loop of $\tau$ transitions showing the handling of errors and corresponding increments of the error counter.}
    \TODO{j'ai comment\'e Fig 3, vous pouvez relire ?}
...
automata that is an FH-bisimulation and their initial states are in the relation (with some initial predicate).
\TODO{check}
\end{definition}
...
\begin{example}
  Figures \ref{WOT2} shows the construction of one of the weak transitions of the Specification OA. We show on the top one possible sequence corresponding to a weak transition, on the bottom left the concerned subset of the original OA, and on the right the generated weak transition.
\end{example}

  
%  In the case of WOT1, we start with a tau loop, then change state with an action of hole $P$ to state 1, initializing the $ec$ counter. On state 1, we have two tau loops, on of them increasing the $ec$ counter. The resulting weak transition is represented here as a regular expression including assignemnts, rather than the substitutions of the formal definition. It can the be abbreviated as: $P\mapsto \texttt{p-send}\ \{(ec:=ec+1)^*$.

  WOT2 here is the transition delivering the result of the algorithm to hole $Q$. Remark that it includes a loop increasing the counter, occuring before the action of $Q$, and this action is using the value of the counter. Let us go into the details, using rules WT1/2/3. For simplification we ignore the pure $\tau$-loops before and after the central \texttt{q-recv(ec)} action, they have no effect on the construction. First consider application of Rule WT3 on the first 3 transitions. Hole actions and predicates are empty, we just concentrate on the Post operations. Rule WT2 transforms each 3 OTs into WOTs. In the drawing we use \texttt{ec++} to abbreviate the Post: $\subst{ec\gets ec+1}$. Then WT3 produces the weak OT:
  $\openrule
         {\set{\tau}, True,
			\subst{\texttt{ec++}\shortotimes\texttt{ec++}\shortotimes\emptyset} }
         {s \OTWeakarrow {\tau\subst{\texttt{ec++}}} s'}$
  ...
  $\openrule
         ...
         Now you can iterate this construction an arbitrary number of times, getting for any natural number $n$ the weak transition:
  $\openrule
         {\emptyset, True,
			\subst{ec\gets ec+n} }
         {s \OTWeakarrow \tau s'}$

         For any of these, apply again WT3, using the central \texttt{q-recv(ec)} transition as $\alpha$, and you get:
      $\openrule
         {\gamma=\{\texttt{q-revc(ec)}\subst{ec\gets ec+n}\}, True,
			\subst{ec\gets ec+n} }
         {s \OTWeakarrow {\texttt{q-recv(ec)}\subst{ec\gets ec+n}} s'}$
     
  \begin{example}
    Figure \ref{SimpleProtCounter:WeakSpecOA} shows the weak automaton of the simple protocol specification.
    \TODO{comment...}
\end{example}


\begin{figure}[ht]
   \centerline{\includegraphics[width=10cm]{ATG/SPSpecWeakOpen}}
  \caption{Weak Open Automaton of the Specification}
   \label{SimpleProtCounter:WeakSpecOA}
\end{figure}

  \begin{example}
  Figure \ref{SimpleProtCounter:SpecOA-partial} shows a partial view of the weak automaton of the simple protocol implementation.
  
\TODO{Explain the partial OA}
\end{example}

\begin{figure}[t]
   \centerline{\includegraphics[width=12cm]{ATG/SPImplWeakOpen}}
  \caption{Weak Open Automaton of the implementation, partial}
...
Let $A$ be the open automaton expressing the semantics of an open pNet $\pNet$; let $<J,\mathcal{S},s_0,  \WT>$ be the weak open automata derived from $A$. For any $\WOT\in\WT$, we denote $\pNet \models \WOT$, this defines the semantics of the pNet $\pNet$ as a weak open automaton. \TODO{strange sentence}
\end{definition}
...
\subsection{Running example}
- In section \ref{} we have shown the full satured weak automaton for both the specification and the implementation of the simple protocol.
...
\TODO{Je ne pense pas que nous ayons un algo de bisim weak operationnel pour la publi, mais ca n'est pas completement impossible nonp[l;us...  Au pire, argumenter intuitivement sur papier, en donnat les classes d'equivalence et les predicats.}
  ...
  \TODO{Eric: this is still ABP ! rewrite completely, baesd on saturation example 2 pages before.}
...
open transition from each sequence. Here is one example of a short sequence,
starting with the open transition $OT_1$ of page \pageref{OT:SimpleProt},
then transmitting the message to the receiver, and to the environment. The two
