...
$t\in\AlgT$.
\TODO{Ludo: footnote or remark on Sorts/types/vars/signatures}
...
Two open automata are weak FH-bisimilar if there exists a weak FH-bisimulation relation that relates their initial states.
...
Borgstrom et al., Liu et al, Delaune et al. and Buscemi et al. providing symbolic semantic and equivalence for different variants of pi calculus respectively \cite{BorgstromBN04,DelauneKR:2007,LiuL:2010,BuscemiM:2008}; and more recently
 Feng et al. provide a symbolic bisimulation for quantum processes \cite{feng2014symbolic}.
All the above works, did not give a complete approach for verification \TODO{Ludo}, and the models on which these works based are definitely different from ours.
...
The semantics of open pNets relies on the definition of open automata that are automata with holes and parameters, but no hierarchy \TODO{Ludo: flatten}. Open automata are somehow labelled transition systems with parameters and holes, a notion that is useful to define semantics, but makes less sense when modelling a system, compared to pNets. To be precise, it is on open automata that we define our FH-bisimilarity relations.

This article defines a strong and a weak bisimulation relation that are adapted to parameterised systems and hierarchical composition. Our FH-bisimulation principle handles pNet parameters in the sense that two states might be or not in relation depending on the value of parameters. Our strong  FH-bisimilarity is compositional by nature in the sense that FH-bisimilarity is maintained when composing processes. We also identified a simple and realistic condition on the semantics of non-observable actions that allows weak FH-bisimilarity to be also compositional \TODO{Eric: recall on rule formats}. Overall we believe that this article paved the way for a solid theoretical foundation for compositional verification of parallel and distributed systems.
