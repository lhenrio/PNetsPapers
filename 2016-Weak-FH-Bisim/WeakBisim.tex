...
\begin{example}
  ...
  $\openrule
         ...
         {\text{b}1 \OTWeakarrow \tau b1} \forall n \geq 1$.  Finally,  applying again \WTTrois, and using the central open transition having \texttt{\underline{out(m,ec)}}  as $\alpha$, we get the resulting weak open transition between b0 and b1 (as shown in Figure \ref{WOT2}).
 %     $\openrule
...
\end{example}
  ...
  \begin{example}
    ...
    First, we the weak OT loops on state ${000}$ are also present in all other states, we did not repeat them. Then many WOTs are similar, and numbered accordingly as 3, 3', 3'', and 8, 8', 8'',8''' respectively: they only differ by the name of the state variables in their respective source or target states.
\end{example}
