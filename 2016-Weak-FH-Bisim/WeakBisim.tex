...
%        $\beta_j$ and $\alpha$, and the state variables of $s$.
	...
	where $\forall k.\, x_k\in V$, and $e_k$ are expressions
        ...
        in the different terms
       $\beta_j$ and $\alpha$. Open transitions are identified
...
A pLTS being its own single leave:
  $States(\mylangle S,s_0,V, \to\myrangle) = \{\triangleleft s \triangleright| s \in S, \}$. To be precise, the state of a pLTS is not reduced to a state identity, it also contains the value of its variables $vars(P)$. 
...
\paragraph{Example} \hl{paragraph} Figure~\ref{SimpleProtCounter:SpecOA} shows the open automaton computed from the Specification pNet given in Figure \ref{SimpleProt:Spec}. 
