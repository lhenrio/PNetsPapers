...
An open automaton $A = <J,\mathcal{S},s_0,\mathcal{T}>$ \emph{cannot observe silent actions} if and only if for all $j$ in $J$ we have
\[ \openrule
         {
...
\begin{definition}[Weak open transitions]
  Let $A = <J,\mathcal{S},s_0,\mathcal{T}>$ be an open automaton. We derive the  weak invisible transition $\OTWeakarrow {\tau}$ from $\mathcal{T}$ as follows:
\begin{mathpar}
 \openrule
         {
...
And the general weak open transition $\OTWeakarrow {\alpha}$ as follows:
\begin{mathpar}
\inferrule{\openrule
         {
