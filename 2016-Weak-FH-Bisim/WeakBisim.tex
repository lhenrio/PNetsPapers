...
\\
\forall j\in J_t,\,\gamma_j= \displaystyle{\mybigdotcup_{t=0}^n \gamma_{t j}\subst{\bigotimes_{k=t}^{0}{\Post_k}}}\text{\quad and\quad}
...
\[\alpha=\alpha'\displaystyle{\subst{\bigotimes_{j=m_1}^{0}\Post_{1j}}}\]
\begin{multline*}
\gamma_j^{j\in J}=\mybigdotcup_{i=0}^{m_1} \vis{{\beta_{1ij}^{j\in J_{1p}}}\subst{\bigotimes_{j=i-1}^{0} \Post_{1j} } }  \dotcup  \vis{{\beta_{2j}^{j\in J_{2}}}\subst{\bigotimes_{j=m_1}^{0}\Post_{1j}}} \\
\dotcup \mybigdotcup_{i=0}^{m_2} \vis{\beta_{3ij}^{j\in J_{1p}} \subst{\bigotimes_{j=i-1}^{0}\Post_{3j}\shortotimes\Post_2\shortotimes\bigotimes_{j=m_1}^{0}\Post_{1j}}}
\end{multline*}
\begin{multline*}
\Pred=\bigwedge_{p=0}^{m_1}\!\big(\Pred_{1p}\subst{\bigotimes_{j=p-1}^{0}\Post_{1j}}\big)\land\Pred^2\subst{\bigotimes_{p=m_1}^{0}\Post_{1p}}\land\\ \bigwedge_{p=0}^{m_2}\!\Pred_{3p}\subst{\bigotimes_{j=p-1}^{0}\Post_{3j}\otimes\Post_2 \otimes\bigotimes_{p=m_1}^{0}\Post_{1p}}
\end{multline*}
\begin{equation*}
\begin{split}
...
\begin{enumerate}
\item $p\in [0..m_1]$. Either there is $i$ such that $p=n_i$, and thus $\beta_i$ and $\beta_{1 p j_0}$ are defined. In this case by Lemma~\ref{lem-Weakcompose1}, we have: 
...
with 
\begin{mathpar}
\set{\gamma'_{1p}}=\set{\vis{\beta_{1p}}}\\
\Pred\,'_{1p}=\Pred_{1p}\\
\Post\,'_{1p}=\Post_{1p}
\end{mathpar}

\item idem for the middle OT with label $\alpha$:
\[P[Q]_{j_0}\models\openrule
         {	\set{\gamma'_2},\Pred_2',\Post_2' }
...
\item and for the last $\tau$ OTs:
for $p\in [0..m_2]$. Either there is $i$ such that $p=n'_i$, and thus $\beta_i$ and $\beta_{1 p j_0}$ are defined. In this case by Lemma~\ref{lem-Weakcompose1}, we have: 
...
What we need now is an adaptation of Lemma~\ref{lem-rel-OT-WOT} where transitions are WOTs and not OTs ... \TODO{comment depending on the status of lemma 4 bis (Rabea). Thus by Lemma 4 bis:}
 We obtain:
