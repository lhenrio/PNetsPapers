...
such that\\
\begin{mathpar} 
s=s^1_0 \wedge s^1_n=s^2_0 \wedge s'^2 = s^3_0 \wedge s^3_m=s'
\\\\
...
\\
\Pred=\bigwedge_{i=0}^n\Pred^1_i\land\Pred^2\subst{\Post_1}\land (\bigwedge_{i=0}^m\Pred^3_i\subst{\Post^2})\subst{\Post^1_i}

\end{mathpar}

\begin{mathpar}
\Longleftrightarrow  \left \{ \begin{array}{l} 
\alpha = \tau \wedge  \vis{\overline{\beta^2}}=\emptyset \wedge \Pred^2 =\symb{True} \wedge \Post^2 =\symb{Id(s)}\\
\\
\bigvee \\
\\
{
\openrule
         {
           \overline{\gamma},
		\Pred,
				({x^3_k\gets (e^3_k\subst {\Post^2})\subst{\Post^1_i}})^{k\in K} } 
         {s \OTWeakarrow {\alpha} s'} \in\WT
}
\end{array}
\right.
...
\begin{proof} ($\Rightarrow$) We proceed by induction by induction the number of transitions with which we  build  a weak transition (introduced in Definition \ref{def:buildweakOT}).
The base case  is if $n=0$ and $m=0$ then 
\begin{mathpar}
 % \mprset {vskip=.5ex}
%\inferrule{
 \openrule
...
\end{mathpar} 
For the inductive step, first we have by induction hypothesis that the formula is valid for some length $l$ such that $l=n+m+1$, we have 
\TODO{I will complete}

on other hand by rule (2) we have
\begin{mathpar}
 % \mprset {vskip=.5ex}
\inferrule{
 \openrule
...
\noindent ($\Leftarrow$) We proceed also by induction on the depth of the inference of 
\begin{mathpar}
{ \openrule
         {
           \overline{\gamma}\!,\Pred,\Post
