...
\begin{itemize}
 \item  For any weak open transition $wOT_1$ in $WT_1$:
 \begin{mathpar}
     \openrule
         {
           \gamma_j^{j\in J_1},\Pred_{OT_1},\Post_{OT_1}}
         {s \OTWeakarrow {\alpha} s'}

\end{mathpar}
 there exists weak open transition $wOT_2$ in $WT_2$ :
 \begin{mathpar}
    \openrule
         {
           {\gamma\,'}_{j}^{j\in J_2}, \Pred_{OT_2},\Post_{OT_2}}
         {t \OTWeakarrow {\alpha} t'}
\end{mathpar}
 such that  $J_1=J_{2}, (s'\!,t'|\Pred_{s',t'})\in \mathcal{R}$; 
 and  \\
 $\Pred_{s,t} \land \Pred_{OT_1}\\
\hspace{1cm} \implies\!\!\! \bigvee_{x\in X}
   \left( \forall j\in J_1. \gamma_j=\gamma'_{j}  \Rightarrow \Pred_{OT_2}
     \land \alpha\!=\!\alpha_x \land  
     \Pred_{s'\!,t'}\subst{\Post_{OT_1}}\subst{\Post_{OT_2}}\right)$
