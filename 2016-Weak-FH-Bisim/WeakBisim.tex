...
Idea: we define $OT\implies OT$ to check whether an OT is a particular case of another one (refining free action variables and predicates)
 \begin{mathpar}
    \openrule
         {
           \beta_{j}^{j\in J_{x}}, \Pred_{OT},\subst{x_k\leftarrow e_k}^{k\in K}}
          {t \OTarrow {\alpha} t'}\in\mathcal{T}
\implies
    \openrule
         {
           (\beta_{j}\Post)^{j\in J}, \Pred_{OT}\Post\land\Pred,\subst{x_k\leftarrow e_k\Post}^{k\in K}}
         {t \OTarrow {\alpha\Post} {t'}}\in\mathcal{T}
\end{mathpar}
...
\forall m\!\!\in\!\! I_2.		{ \pNet_m 
    	...
    	{\ostate{s_m} \OTarrow {\alpha_m}
    		\ostate{s_m'}} }\\\\
%     J' = \biguplus_{m\in I_1}\!\! J_m \uplus J \frac{num}{den}	\\
...
\[ \openrule
         {
           A,\Pred,\Post}
         {s \OTarrow {\alpha} s'}
         \in \mathcal{T} \] If there is a $j$ such that $A(j)=\tau$ then we have \[ \alpha=\tau\land s=s'\land \Pred=\True\land\Post=\Id(s) \land A=(j\mapsto \tau)
\]
\end{enumerate}
\end{definition}
The first statement of the definition state that the open automaton must allow a hole to do a silent action at any time, and must not observe it, i.e. cannot change its internal state because a hole did a $\tau$ transition. The second statement ensures that there cannot be in the open automaton other transitions that would be able to observe a silent action from a hole. The condition $A=(j\mapsto \tau)$ is a bit restrictive, it could safely be replaced by $\forall h\in\dom(A).\, A(h)=\tau$, allowing the other holes to perform silent transitions too (because these silent actions cannot be observed).
...
\qquad
\Pred\,'=\Pred\land \bigwedge_{p=0}^{n}(\alpha_p=\beta_p\land \Pred_p)\bigotimes_{i=0}^{p-1}\Post_i
\\
\forall j\in J_p,\,\gamma_j= \displaystyle{\mybigdotcup_{p=0}^n \gamma_{p j}}\text{\quad and\quad}
\Post\,'=\Post\uplus\bigotimes_{p=0}^{n}
		\Post_p 
\end{mathpar}
Then, we have\\[-2ex]
...
\end{mathpar}
By Lemma \ref{lem-rel-OT-WOT} this implies the following: \\			
$\forall p\!\in\![0..m_1].\openrule
    {\set{\beta^1_p},\Pred^1_p,\Post^1_p   }
         {\ostate{{(s^1_{pi})}^{i\in L}} \OTarrow {\tau} \ostate{{(s^1_{(p+1)i})}^{i\in L}}}\!\in\!\mathcal{T} \wedge
\openrule
         {\set{\beta^2},\Pred^2,\Post^2 }
         {\ostate{({s_{0i}^2})^{i\in L}} \OTarrow {\alpha} \ostate{({s_{1i}^2})^{i\in L}}}\! \in\! \mathcal{T}
\wedge\\
\forall p \in [0..m_2].\openrule
         {
           \set{\beta^3_p},\Pred^3_p, \Post^3_p}
         { \ostate{(s^3_{pi})^{i\in L}} \OTarrow {\tau} \ostate{(s^3_{(p+1)i})^{i\in L}}} \in \mathcal{T}$ 
\\
...
Note that if $(\beta^i_p)_{j_0}=\tau$ because of Definition \ref{def:Non-ObsTau}, $P$ necessarily makes a $\tau$ open transition and remains in the same state, ($s_p = s_{p+1}$). Thus without loss of generality, we can bypass such an open transition and obtain another decomposition of the WOT without the open transition that requires ${\beta^i_{p{j_0}}}=\tau$. We can thus  suppose that for all $p$ and $i$ we have ${\beta^i_{p{j_0}}}\neq\tau$.

Moreover, we have for all $p\in[0..n]$:
\begin{mathpar}
Q\models{\openrule
	{
		\gamma_{p j}^{j\in J_p},
...
We proceed by iteration on the list of   $\symb{OT}_p$ of $P$ identified above with an additional variable $t$, OTs are of the form: 
$P\!\models		
\openrule
    {
       {(\beta^i_{p j})^{j\in J}},\Pred_p,\Post_p   }
         {\ostate{s^{i\in L}_{pi}} \OTarrow {\alpha} \ostate{s^{i\in L}_{(p+1)i}}}$ 
At the beginning $Q$ is at state $\ostate{s_{0}}$ and $t=0$. 
For each step, we have two cases:
\begin{itemize}
\item If $j_0\in J$ then Q moves by one open transition. By the hypothesis above $\beta^i_{j_0}\neq \tau$. Additionally, we have
$\displaystyle{\gamma_j^{j\in J}=\mybigdotcup_{p=0}^{m_1} \vis{\set{\beta^1_p}} \dotcup  \vis{\set{\beta^2}} \dotcup \mybigdotcup_{p=0}^{m_2} \vis{\set{\beta^3_p}}}$ and\\
 $\gamma_{j_0}=[\beta_0..\beta_n]$, thus
  $\beta^i_{j_0}=\beta_t$ with  for some $t$ such that $t$ has value $0$ at beginning and that is incremented whenever this condition is satisfied. 
...
\]
By  Lemma \ref{lem-rel-OT-WOT} we can decompose $\WOT_t$ into  a series of $k+k'$ tau open transitions and the observable open transition:
...
\[Q\models{\openrule
	{
		\gamma_{t j}^{j\in J_t},
		 \Pred_t,  
		\Post_t}
	{\ostate{s^2_{t}}\OTarrow {\alpha_t}
		\ostate{s^3_{t}}}
}
\]
\begin{enumerate}

\item For the first $k$ open tau transitions, by Definition \ref{def:Non-ObsTau} $P$ can make a tau open transition and the hole makes also tau action. So by Lemma \ref{lem-compose} we obtain $k$ open transitions in the form: 
\[P[Q]_{j_0}\!\models		
\openrule
    {
       \beta_p^{(p\in J\setminus\{j_0\}) \uplus J_Q},{\Pred_p},{\Post_p}   }
         {\ostate{s^{i\in L \uplus L_Q}_{pi}} \OTarrow {\tau} \ostate{s^{i\in L\uplus L_Q }_{(p+1)i}}}\]

\item We proceed in the same way for $k'$ last weak open transitions, and we obtain $k'$ open tau transitions.\\
\item For the observable open transition. Because of the definition of $\Pred'$ we can suppose that $\alpha_t=\beta_{j_0}$ \TODO{because else the original WOT does not trigger}. By Lemma \ref{lem-compose} we obtain:\\ 
\[P[Q]_{j_0}\!\models		
\openrule
    {
       \beta_p^{(p\in J\setminus\{j_0\}) \uplus J_Q},{\Pred_p}\!\!'',{\Post_p}\!\!''   }
...
\openrule
 ...
 \item Else, $j_0\not\in J$. $t$ does not move  and $Q$ does not move, i.e. we have: $\ostate{(s_{ti})^{i\in L_Q}} = \ostate{({s}_{(t+1)i})^{i\in L_Q}}$....
\end{itemize}
