...
\TODO{Lemma revu mais convaincant...}
...
There exist  sets of $\beta^1_i$, $\beta^2_i$, and $\beta^3_i$, $\Pred^1_i$, $\Pred^3_i$, $\Post^1_i$, and  $\Pred^2$, $\Post^2$ s.t.:
\begin{mathpar}
%\mprset {vskip=.5ex}
%\inferrule 
...
\Bigg)        
       \bigvee\\   
 \Big(\alpha = \tau \wedge  \overline{\gamma}=\emptyset \wedge \Pred =\True \wedge \Post =\Id(s)\wedge s=s'\Big)
 \end{mathpar}
If only if there exist a set of $\beta$, $\Pred$, $\Post$ s.t.
 \begin{mathpar}
%   \Longleftrightarrow  {
\openrule
         {
...
\displaystyle{\overline{\gamma}=\mybigdotcup_{i=0}^n \vis{\overline{\beta^1_i}} \dotcup  \vis{\overline{\beta^2}} \dotcup \mybigdotcup_{i=0}^m \vis{\overline{\beta^3_i}}}
\\
...
\Bigg)        
       \bigvee\\   
 \Big(\alpha = \tau \wedge  \overline{\gamma}=\emptyset \wedge \Pred =\True \wedge \Post =\Id(s)\wedge s=s'\Big)  \implies  {
\openrule
         {
           \overline{\gamma},
		\Pred, \Post
...
}\! \implies\! \Big(\alpha = \tau \wedge \overline{\gamma}=\emptyset \wedge \Pred =\True \wedge \Post =\Id(s)\wedge s=s'\Big)
\end{mathpar}
...
\item By induction hypothesis this means each tau weak open transition can be written as a series of $n_1$ tau transitions such $n_1=n+m+1$, hence by simplification we have: 
\begin{mathpar}
%\inferrule{
\bigg( \openrule
         {
...
\displaystyle{\overline{\gamma_1}=\mybigdotcup_{i=0}^{n_1} \vis{\overline{\beta^1_i}}}, 
...
\bigg( \forall i \in [0\ldots m_1].\openrule
    {
       \overline{\beta^2_i},\Pred^2_i,\Post^2_i   }
         {s^2_i \OTarrow {\tau} s^2_{i+1}} \in \mathcal{T}         \bigg)
\end{mathpar}
where\\
$
s_2=s^2_0 \wedge s^2_{m_1}=s',
\displaystyle{\overline{\gamma_2}=\mybigdotcup_{i=0}^{m_1} \vis{\overline{\beta^2_i}}},
...
\openrule
         {
           \overline{\beta^4},\Pred^4,\Post^4 }
         {s^4 \OTarrow {\alpha} s'^4} \in \mathcal{T}
 \wedge
\forall i \in [0\ldots m_2].\openrule
         {
           \overline{\beta^5_i},\Pred^5_i,({x^5_k\gets e^5_k})^{k\in K}    }
...
\displaystyle{\overline{\gamma_2}=\mybigdotcup_{i=0}^{n_2} \vis{\overline{\beta^3_i}} \dotcup  \vis{\overline{\beta^4}}  \mybigdotcup_{i=0}^{m_2} \vis{\overline{\beta^5_i}}}
\\
...
\Bigg(\forall i\in [0\ldots(n_1\!+\!n_2)].\openrule
    {
       \overline{\beta^6_i},\Pred^6_i,\Post^6_i   }
         {s^6_i \OTarrow {\tau} s^6_{i+1}} \in\mathcal{T} \wedge
         
\openrule
         {
           \overline{\beta^7},\Pred^7,\Post^7 }
         {s^7 \OTarrow {\alpha} s'^7} \in \mathcal{T}
\wedge
\forall i \in [0\ldots(m_1\!+\!m_2)].\openrule
         {\overline{\beta^8_i},\Pred^8_i,({x^8_k\gets e^8_k})^{k\in K}    }
...
~~\\
\displaystyle{\overline{\gamma}=\mybigdotcup_{i=0}^{n_1+n_2}\!\! \vis{\overline{\beta^6_i}}
\dotcup\vis{\overline{\beta^7}}\dotcup\!\mybigdotcup_{i=0}^{m_1+m_2}\!\!\vis{\overline{\beta^8_i}}
}\\
...
\begin{corollary} The definition of weak bisimulation given in  Definition \ref{def-Weak-bisim} is equivalent to the following weak FH-bisimulation: Let $A_1 = <J,\mathcal{S}_1, s_0,
    \mathcal{T}_1>$ and $A_2 = <J,\mathcal{S}_2,t_0,  \mathcal{T}_2>$ be open automata.
Let $<J,\mathcal{S}_1, s_0,
    \WT_1>$ and $<J,\mathcal{S}_2,t_0,  \WT_2>$ be the
weak open automata derived from $A_1$ and $A_2$ respectively.
...
\end{mathpar}
 ...
 \begin{mathpar}
    \openrule
         ...
         {t \OTWeakarrow {\alpha_x} t_x}
\end{mathpar}
 such that  $\forall x, \{j\in J_1|\gamma_j\neq\emptyset\}=J_{x}, (s',t_x|\Pred_{s',t_x})\in \mathcal{R}$; 
 and  \\
$\Pred \land \Pred_{OT}\\
...
1) By lemma \ref{lem-rel-OT-WOT} we have:
...
\openrule
 ...
 \Bigg(\forall i \in [0\ldots n].\openrule
    {
       \overline{\beta^1_i},\Pred^1_i,\Post^1_i   }
         {s^1_i \OTarrow {\tau} s^1_{i}} \in \mathcal{T} \quad \wedge
\quad
\openrule
         {
           \overline{\beta^2},\Pred\,^2,\Post^2 }
...
\Bigg) 
 \end{mathpar}
2) From Definition \ref{def-Weak-bisim} we have:

For each open transition $OT$ in $\mathcal{T}_1$:
 \begin{mathpar}
     \openrule
         {
           \beta_j^{j\in J'},\Pred_{OT},\Post_{OT}}
...
\hspace{1cm} \implies\!\!\! \displaystyle{\bigvee_{x\in X}}
 ...
 3) From Definition \ref{def:buildweakOT} we have:
...
\end{mathpar}
 ...
 \TODO{Trouver comment decrire la recurrence/fusionner les Pred}
