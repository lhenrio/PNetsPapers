...
and $\P$ a set of variables. Within $\Talg$, we distinguish a set of
 expressions $\Exprs$, including a set of boolean
expressions $\BoolExprs$ ($\BoolExprs\subseteq\Exprs$). We let $e_i$ range over expressions ($e_i\in\Exprs$).
...
integers, bounded or not. We use indexed sets in pNets because we want to consider a set of processes, and provide an additional structure that explains how to synchronise them. Basically this could be done by tuples (but not by sets), however indexed sets are more general, can be infinite, and give a compact representation (the position is easier to express).
...
\AlgB$ is a guard, and the variables $x_j\in P$ \TODO{P?}
