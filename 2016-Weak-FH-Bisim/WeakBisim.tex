...
%                       1 \xrightarrow{send(m,sb)}_{Sender} 2 ~~
  ...
  %  {\ostate{00} \xrightarrow{\underline{\delta(x1)}} \ostate{10}}
    ...
    \smallskip
    Here the global states $Prot_1$ and $Prot_2$ encode the states of
    the 4 pLTS composing the system, namely
    $Prot_1=\ostate{s_1,m_0,m_0,r_0}$ and
    $Prot_2=\ostate{s_2,m_1,m_0,r_0}$, showing that the sender and the
    first medium have evolved during this transition.
...
%                            \xrightarrow{in(?m,?b)}_{M1}
  ...
  %  {\ostate{00} \xrightarrow{\underline{\delta(x1)}} \ostate{10}}
    ...
    \smallskip
    The global states $ABP_1$ and $ABP_2$ have the same structure than
    $Prot_1$ and $Prot_2$, as the holes have no states. The assignment
...
\begin{lemma}[OT decomposition]\label{lem-decompose} 
	...
	\]
		...
		%\[
		\begin{mathpar}
		P\models{\mprset{myfraction=\reddottedrule}\inferrule
			{
...
Consider rule TR2 in 
    ...
    $\inferrule
    {
	\forall m\!\!\in\!\! I_1. {\pNet_m \models\mprset{myfraction=\reddottedrule}\inferrule
    	{
    	\{{b_{j}}\}^{j\in J_m}, \Pred_m, \Post_m}
    	{\ostate{s_{i}^{i \in L_m}} \OTarrow {v_m}
    		\ostate{(s_i^\prime)^{i\in L_m}}} }	
