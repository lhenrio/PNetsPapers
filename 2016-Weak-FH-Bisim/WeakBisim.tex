...
\begin{figure}[ht]
   \centerline{\includegraphics[width=7cm]{XFIG/WOT2-result}}
  \caption{Construction of WOT2}
   \label{WOT2}
\end{figure}

\begin{example}
  Figure \ref{WOT2} shows the construction of one of the weak transitions of the Specification OA. On the top we show the subset of the original open automaton (from Figure \ref{SimpleProtCounter:SpecOA}) considered here, and at the bottom the generated weak transition.


  WOT2 here is the transition delivering the result of the algorithm to hole $Q$. Remark that it includes a loop increasing the error counter, occuring before the action of $Q$, and this action is using the resulting value of the counter. Let us go into the details, using rules WT1/2/3.  First WT1 adds a $\tau$ loop on each state. Then consider application of Rule WT3 on a sequence 3 internal actions ``$\tau\subst{ec\gets ec+1};\tau\subst{ec\gets ec+1};\tau$''. Hole actions and predicates are empty, we just concentrate on the Post operations. Rule WT2 transforms each 3 OTs into WOTs. In the drawing we use \texttt{ec++} to abbreviate the Post: $\subst{ec\gets ec+1}$. Then WT3 produces the weak OT:
  $\openrule
         {\{\}, True,
  ...
  $\openrule
      ...
      $\openrule
         {\{Q\mapsto\texttt{q-revc(ec)}\subst{ec\gets ec+n}\}, True,
			\subst{ec\gets ec+n} }
         {s \OTWeakarrow {\texttt{out(m,ec)}\subst{ec\gets ec+n}} s'}$
\end{example}
...
\TODO{Eric: insert here a table with the details of the WOTs...}
...
$$ 1 = \openrule
         {\{P\mapsto\texttt{p-a}\}, [\forall m.p-a!=P\_send(m)], () }
         {000 \OTWeakarrow {\texttt{p-a}} 000}$$
\\
$$6 = \openrule
         {\{\}, [True], () }
         {100 \OTWeakarrow {\tau\big(\subst{m1\_c:=s1\_c,m1\_m:=s1\_m,s2\_c:=s1\_c}\subst{s1\_c:=s2\_c+1}\big)^* \subst{m1\_c:=s1\_c,m1\_m:=s1\_m,s2\_c:=s1\_c} } 210}
         $$
