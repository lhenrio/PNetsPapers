...
\begin{theorem}[Congruence]\label{thm-congr-weq}
	...
	$\pNet[\pNetQ']_{j_0}$ are weak FH-bisimilar.
\end{theorem}
 \begin{proof}  (Sketch of proof) Consider $Q$ weak FH-bisimilar to $Q'$.  It means, according to Lemma  \ref{lem-alternative-weak-bisim} for any weak open transition $WOT$ in $Q$ there exist a family of weak open transitions $WOT_{x}^{x \in X}$ in $Q'$. 
...
On the other hand, $Q$ weak FH-bisimilar to $Q'$, so according to  Lemma  \ref{lem-alternative-weak-bisim} there exist a family of weak open transitions $WOT_{x}$:
 \begin{mathpar}
    \openrule
         {
           \gamma_{j x}^{j\in J_{x}}, \Pred_{Q_x},\Post_{Q_x}}
         {\ostate{t_{i}^{i\in L_{Q_x}}}\OTWeakarrow {\alpha_x} \ostate{t_{ix}^{i\in L_{Q_x}}}}
\end{mathpar}
