...
\begin{proof} ($\Rightarrow$) We proceed by induction on  $l=n+m+1$, which is in fact the number of transitions with which we  build  a weak transition (introduced in Definition \ref{def:buildweakOT}).
\begin{itemize}
\item The base case $l=1$, so $n=0$ and $m=0$, we have:
\begin{mathpar}
 % \mprset {vskip=.5ex}
%\inferrule{
 \openrule
...
We consider the case $n'=n+1$, and $m'=m+1$ are similar. Side conditions are omitted for readability. We first focus on the left part of the disjunction. Let us consider the sequence of ($n+m+1$) open transitions:
\begin{mathpar}
\Big(\forall i \in [0..n].\openrule
    {
       \set{\beta^1_i},\Pred^1_i,\Post^1_i   }
         {s^1_i \OTarrow {\tau} s^1_{i+1}} \in \mathcal{T} \quad \wedge
\quad
...
~~\qquad\qquad\forall i \in [0..m].\openrule
         {
           \set{\beta^3_i},\Pred^3_i,({x^3_k\gets e^3_k})^{k\in K}    }
         {s^3_i \OTarrow {\tau} s^3_{i+1}} \in \mathcal{T}
\Big) 
\end{mathpar}

By recurrence hypothesis we suppose that the statement holds for some  set of open transitions of length $l$. In particular, we have:\\
...
\\
\set{\gamma''}= \vis{\set{\beta}} \dotcup \set{\gamma}\subst{\Post\,'}\quad
\Pred\,''=\Pred\,'\land\Pred\subst{\Post\,'}\\%\land (\Pred_3\subst{\Post_2})\subst{\Post_1}
\Post\,''=\Id(s'')\shortotimes\Post\shortotimes\Post\,' \quad \alpha''=\alpha'\subst{\Post\,'}
}
{
\openrule
         {
...
\openrule
         {
           \set{\gamma_3},\Pred_3,({x_k\gets e_k})^{k\in K}    }
         {s'' \OTWeakarrow {\tau} s'''} \in\WT
\\
\Pred=\Pred_1\land\Pred_2\subst{\Post_1}\land (\Pred_3\subst{\Post_2})\subst{\Post_1}
}
...
\openrule
         {
           \set{\gamma_1}\dotcup \set{\gamma_2}\dotcup\set{\gamma_3},
		\Pred,
...
\displaystyle{\set{\gamma_1}=\mybigdotcup_{i=0}^{n_1} \vis{\set{\beta_{1i}}}}, 
\Pred_1\!=\!\bigwedge_{i=0}^{n_1}\Pred_{1i}, \Post_1= \bigotimes_{i=0}^{n_1} \subst{\Post_{1i}}\\
$
\TODO{$\Post_1= \biguplus_{i=0}^{n_1} \Post_{1i}$ or bigotimes?}
...
\displaystyle{\set{\gamma_3}\!=\!\mybigdotcup_{i=0}^{m_1}\vis{\set{\beta_{3i}}}},
\Pred_3\!=\!\bigwedge_{i=0}^{m_1}\Pred_{3i},\Post_3\!=\!\bigotimes_{i=0}^{m_1} \subst{\Post_{3i}}\\
$
...
\forall i \in [0..m_2].\openrule
         {
           \set{\beta_{2i}'},\Pred_{2i}^{\,\prime},({x_k\gets e_k})^{k\in K}    }
...
\displaystyle{\set{\gamma_2}=\mybigdotcup_{i=0}^{n_2} \vis{\set{\beta_{2i}}} \dotcup  \vis{\set{\beta'}}  \mybigdotcup_{i=0}^{m_2} \vis{\set{\beta_{2i}'}}}
\\
\Pred_2=\bigwedge_{i=0}^{n_2}\Pred_{2i}\land\Pred\,'\bigotimes_{i=0}^{n_2}\subst{\Post_{2i}}\land \big(\bigwedge_{i=0}^{m_2}\Pred_{2i}^{\,\prime}\subst{\Post\,'}\big)\bigotimes_{i=0}^{n_1}\subst{\Post_{2i}}
\\
\Post_2= \big({x_k\gets e_k\subst {\Post\,'}\bigotimes_{i=0}^{n_2}\subst{\Post_{2i}}}\big)^{k\in K}
$
\end{enumerate}
~~\\
...
\forall i \in [0..(m_1\!+\!m_2)].\openrule
         {\set{\beta^3_i},\Pred^3_i,({x^3_k\gets e^3_k})^{k\in K}    }
         {s^3_i\OTarrow {\tau} s^3_{i+1}}\in\mathcal{T}\bigg)  
\end{mathpar}
...
\right. \\
~~\\
s\!=\!s^1_0 \wedge s^1_{(n_1+n_2+1)}\!=s_0^2 \wedge s_1^2 = s^3_0 \wedge s^3_{(m_1+m_2+1)}\!=\!s'''\\
~~\\
\displaystyle{\set{\gamma}=\mybigdotcup_{i=0}^{n_1+n_2}\!\! \vis{\set{\beta^1_i}}
\dotcup\vis{\set{\beta^2}}\dotcup\!\mybigdotcup_{i=0}^{m_1+m_2}\!\!\vis{\set{\beta^3_i}}
}\\
\displaystyle{\Pred=\bigwedge_{i=0}^{n_2}\Pred^1_i\land\Pred^2\bigotimes_{i=0}^{n_2}\subst{\Post^1_i}\land \Big(\bigwedge_{i=0}^{m_2}\Pred^1_i\subst{\Post^2}\Big)\bigotimes_{i=0}^{n_1}\subst{\Post^1_i}}
\\
\displaystyle{\Post= \big({x^3_k\gets e^3_k\subst {\Post^2}\bigotimes_{i=0}^{n_2}\subst{\Post^1_i}}\big)^{k\in K}}
$\\
This concludes the inductive step, showing  that the decomposition expressed by the $\Leftarrow$ direction of the lemma is always possible. \qed
\end{itemize}
\end{proof}
