...
We will check here 2 of these conditions.
Preliminary remarks:
  \begin{itemize}
    \item Both pNets trivialy verify the ``non-observability''
      condition: the only vectors having $\tau$ as an action of a
...
variables using suffix 1 for the Specification, and 2 for the Implementation.
  ...
  \medskip
Let us start with the first pair: $SS_1 << WI_1$, where:
...
\medskip That is trivialy true, choosing pa-2 = pa-1 for each p-a1.

\end{itemize}

  \bigskip
  All others pairs from this set are just as trivialy prove true.
