...
\Post^a=\bigotimes_{i=0}^{m_2}{\Post_i^{3a}}\subst{{\Post^{2a}}}\bigotimes_{j=0}^{m_1}\subst{{\Post_j^{1a}}}
$
...
Q\models{\openrule
			{
			(\gamma'_{kj})^{j\in J\cap\Holes(Q)},  {{\Pred^b_k}},  
				{{\Post^b_k}}}
...
\displaystyle{\gamma'_{kj} }=\mybigdotcup_{i=0}^{(n_k'-1)}\!\!\vis{\beta^1_{ij}}\,\dotcup\,\vis{\set{\beta^2_j}}\,\dotcup\!\!\!\mybigdotcup_{i=(n_k'+1)}^{(n_k-1)}\!\!\!\vis{\beta^3_{ij}}
\\
\Pred^b_k=\Big(\!\!\!\bigwedge_{i=0}^{(n_k'-1)}\!\!
...
\Post^b_k=\bigotimes_{i=(n_k'+1)}^{(n_k-1)}\!\!\!{\Post_i^{3b}}\Big(\subst{{\Post^{2b}}}\bigotimes_{i=0}^{(n_k'-1)}\subst{{\Post_i^{1b}}}\Big)
$\\
...
and $\displaystyle{\bigcup_{p=0}^nJ_{p}} = J\cap\Holes(Q)$, $\forall j\in J_p,\,\gamma_j= \displaystyle{\mybigdotcup_{p=0}^n \gamma_{p j}}$, $\forall l\in L_Q,\, s_{0l}=s_l$, $s_{(n+1)l}=s'_l$  \\
  $\Pred \iff \Pred\,'
		\land \displaystyle{\bigwedge_{p=0}^n}(\alpha_p= \beta_{pj_0}\land \Pred_p), \text{and~}
\Post=\Post\,'\uplus\bigotimes_{p=0}^n
		\Post_p \quad$  \\
------------------------------------------------------\\
We have:\\
...
By replacing ${\Pred^{1}_i}$,  $\Pred^2$, and ${\Pred^{3}_i}$ we obtain:\\
...
$\\		
Therefore, we have:\\
$\Pred\iff \Pred^a \wedge\\
~\qquad\qquad\quad\displaystyle{\bigwedge_{i=0}^{m_1}({\Pred^{1b}_i}\land \alpha^1_i=\beta^1_{ij_0}) \bigotimes_{j=0}^{i-1}\subst{\Post_j^1}\land({\Pred^{2b}}\land \alpha^2=\beta^2_{j_0})	\bigotimes_{i=0}^{m_1}\subst{\Post_i^1}}\land\\
~\qquad\qquad\quad\bigwedge_{i=0}^{m_2} ({\Pred^{3b}_i}\land \alpha^3_i=\beta^3_{ij_0})\bigotimes_{j=0}^{i-1}\subst{\Post^3_j}\subst{\Post^2}  \bigotimes_{i=0}^{m_1}\subst{\Post_i^1}
$		
		
\TODO{Je n'ai pas d'idee comment simplifier le reste!}\\
...
By replacing ${\Post^{1}_i}$,  $\Post^2$, and ${\Post^{3}_i}$ we obtain:\\
$\Post= \displaystyle{\bigotimes_{i=0}^{m_2}\subst{{\Post^{3a}_i}\uplus 
		...
		{\Post^{1b}_i}}}\\
~~\qquad= \bigotimes_{i=0}^{m_2}{\Post_i^{3a}}\subst{{\Post^{2a}}}\bigotimes_{j=0}^{m_1}\subst{{\Post_j^{1a}}} \uplus \ldots  $
\end{proof}
\TODO{l'autre terme a voir!}
...
\end{mathpar}
As well, by Lemma \ref{lem-rel-OT-WOT} this implies the following:\\
...
\[ P[Q]_{j_0}  
	...
	{\openrule
		{
			\beta_j^{(j\in J\setminus\{j_0\}) \uplus J_Q}, 
			\Pred\land\Pred\,'\land \alpha'_p=\beta_{j_0},  
			\Post\uplus \Post\,'}
		{\ostate{s_i^{i\in L\uplus L_Q}} \OTarrow {\alpha}
			\ostate{s_i'^{\, i\in L\uplus L_Q}}}
	}
