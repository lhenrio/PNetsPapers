...
Concerning $Q$, we consider only the paths for which it moves. So for each, state such that we have $\ostate{(s_{p})_l^{l\in L_Q}} = \ostate{({s}_{(p+1) l})^{l\in L_Q}}$, we skip the state $\ostate{({s}_{(p+1) l})^{l\in L_Q}}$ and  we rename all its following states  $\ostate{(s_{p})_l^{l\in L_Q}}$ by $\ostate{({s}_{(p-1) l})^{l\in L_Q}}$. We proceed in the same way as for states named $u$.
...
\TODO{We will discuss}
 
For each  $\symb{OT}_p$ of $Q$ such that $p \in S$ and $S \subseteq [0\ldots (m_1\!+\!m_2)\!+\!1]$ in the form:
\begin{mathpar}
Q\models{\openrule
			{
