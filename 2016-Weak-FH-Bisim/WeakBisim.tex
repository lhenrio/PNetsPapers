...
This article defines bisimulation relations for the pNet modelfor the comparison of systems specified as pNets.
...
More precisely, for any pair $(s,t)\in \mathcal{S}_1\times \mathcal{S}_2$, there is a 
   single
      ...
      predicate $\Pred_{s,t}$. By nature, this is well-defined if the variales of the two open automata are disjoint, i.e.  $V_1\cap V_2=\emptyset$.
...
Preliminary remarks:
  \begin{itemize}
    ...
    \item We must take care of variable name conflicts: in our example, the variables of the 2 systems already have different names, but the action parameters occurring in the transitions (m, msg, ec) are the same, that is not correct. In the tools, this is managed by the static semantic layer; in the following example, we have renamed all conflicting variables with 1 for the Spec, and 2 for the Impl.
    \end{itemize}
...
\hline
  ...
  We give here one  example: consider the second triple from the table, and transition $SS_3$ from state $b0$. Its easy to guess that it will correspond to $WI_3(0)$ of state $202$.
...
\hl{With}~\ref{eqproofbeta}, we now have. Recall that the actions $\alpha_k$ are the actions $\alpha'_{p}$ that are observable: 
\begin{eqnarray*}
\Pred_\beta &\iff& \gamma_{j_0}=
...
All the following transitions are parametrized by an integer $n\in \mathcal{N}$, meaning they stand for the (infinite) set of weak OTs. 
In some cases, this set is further restricted (see e.g. $WI_{7b}(n)$), in which case we have added an explicit quantifier.
\smallskip
...
\bigskip
Considering the first triple <b0, 000, True>, we have to prove the following properties, in which $OT<<WOT$ means that the (strong) open transition $OT$ is covered, in the sense of definition \ref{def-Weak-bisim}, by the weak transition $WOT$ (it could be a set, but this will not be used here):
...
weak automata, we would have a lot more transitions to examine, as we  have 4 weak transitions for $b0$ in the specification weak automaton, and 7 WOTs from 000 in the implementation automaton.
...
Preliminary remarks:
  ...
  \medskip
We will check here 2 of these conditions, starting with the first one: $SS_1 << WI_1$
