...
\TODO{J'ai un probleme ici, comme on a dit que les AO etaient satures par substitution, ils sont par nature infinis... Donc le theoreme, tel qu'il est exprime, est vrai, mais inapplicable. La solution est dans le papier de PEPM, mais complique a expliquer...}
\TODO{pour faire simple, on vire le theoreme, et on met une simple note disant qu'on a une definition dans PEPM d'une description structurelle finie, et un algo avec terminaison ?}
\TODO{[LUDO->ERIC] D'accord: tu ecris comme tu connais le mieux le papier PEPM?}
...
In this translation, the states of the open automata are obtained from the states of the pLTSs at the leaves of the composition. The predicates on the transitions are obtained both from the predicates on the pLTSs transitions and from the synchronisation vectors involved in the transition.
\TODO{Meme remarque ici, on ne genere qu'une representation finie des OAs}
...
\RAB{Here, in the open automaton drawing, we use a user-friendly notation for the OTs, in the form ``$\{actions of holes\} [guard] action \{assigns\}$'', rather than the full OT form with the red-dotted rule. Note that the resulting action of the OTs appear here between the guard and the set of assignments. For examplein the upper left of the figure, the OT where $\beta=\{P\mapsto p\_a\}$, $\Pred=[\forall m. P\_a\neq p\_send(m)]$, and $\alpha=P\_a$ is denoted as ``\texttt{\{P->p-a\} [Forall m.P-a!=p-send(m)] P-a}''.
}
...
\end{figure}
    ...
    Figure \ref{SimpleProtCounter:ImplOA} shows the open automaton of the implementation $SimpleSystem$.
    \ERIC{In this drawing, we have short labels for states, representing $\ostate{s_0,m_0,r_0}$ by \texttt{000}. For readability we only have written the \emph{external} actions of holes $P$ and $Q$ on the loop transitions of the first staes; they are also present on all other states. 
The resulting behavior is quite simple: In every state, holes $P$ and $Q$ can do any action independent of the protocol (we only have labelled these on state \texttt{000}). Then we have a main loop including receiving a message from $P$ and transmitting the same message to $Q$, with some intermediate $\tau$ actions from the internal communications between the protocol processes. In most of the transitions, you can observe that data is propagated between the successive state variables (holding the message, and the error counter value). On the right of the figure, you have a loop of $\tau$ transitions showing the handling of errors and corresponding increments of the error counter.}
...
\begin{figure}[ht]
   \centerline{\includegraphics[width=8cm]{XFIG/WOT2-result}}
  \caption{Construction of WOT2}
   \label{WOT2}
\end{figure}

\begin{example}
  Figure \ref{WOT2} shows the construction of one of the weak transitions of the Specification OA. We show on the top the subset of the original open automaton (from Figure \ref{SimpleProtCounter:SpecOA}) consider here, and at the bottom the generated weak transition.


  WOT2 here is the transition delivering the result of the algorithm to hole $Q$. Remark that it includes a loop increasing the counter, occuring before the action of $Q$, and this action is using the value of the counter. Let us go into the details, using rules WT1/2/3.  First consider application of Rule WT3 on the first 3 transitions. Hole actions and predicates are empty, we just concentrate on the Post operations. Rule WT2 transforms each 3 OTs into WOTs. In the drawing we use \texttt{ec++} to abbreviate the Post: $\subst{ec\gets ec+1}$. Then WT3 produces the weak OT:
  $\openrule
         {\set{\tau}, True,
...
\TODO{Eric: Explain the partial OA}
\end{example}
...
\ERIC{
  => Context VCE, ref to FASE paper, citer Avocs et  PEPM\\
...
pNet, that was drwan in Figure \ref{SimpleProt:Impl}, page \pageref{SimpleProt:Impl}. This code should be intuitive enough to read, with the following
