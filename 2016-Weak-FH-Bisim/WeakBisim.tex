...
\hspace{1cm} \implies\!\!\! \bigvee_{x\in X}
   \left( \forall j\in J_x. \vis{\beta_j}=\gamma_{jx}  \Rightarrow \Pred_{OT_x}
     \land \alpha\!=\!\alpha_x \land  
     \Pred_{s',t_x}\subst{\Post_{OT}\shortotimes\Post_{OT_x}}\right)$
...
\medskip
       	...
       	\Pred_{{px}}\subst{\Post_{OT}\shortotimes\Post_{OT_{px}}}\right)$\\
...
For any open transition $OT_{px}$, since
       	...
       	\Pred_{{pxy}}\subst{\Post_{OT_{px}}\shortotimes\Post_{OT_{pxy}}}\right)$.\\
...
variable in $\mathcal{T}_1$.
       	...
       	\begin{small}
       		...
       		\Pred\,'_{{pxy}}\subst{\Post_{OT}\shortotimes\Post_{OT_{pxy}}}\right)$.
       	\end{small}
From (*) and (**) we have, for all $p$
       	\begin{small}
       		...
       		\left( \forall j. \beta_j=\beta_{jpx}  \Rightarrow \Pred_{OT_{px}}
       		\land \alpha\!=\!\alpha_{px} \land
       		\Pred_{{px}}\subst{\Post_{OT}\shortotimes\Post_{OT_{px}}}\right)\land\Pred_{p,u}\\
       		%
       		\hspace{1cm} \implies\!\!\! \bigvee_{x\in X}
       		...
       		\Pred_{{pxy}}\subst{\Post_{OT_{px}}\shortotimes\Post_{OT_{pxy}}}\big))
       		\land \alpha\!=\!\alpha_{px} \land
       		\Pred_{{px}}\subst{\Post_{OT}\shortotimes\Post_{OT_{px}}}\big)\\
       		%
       		\hspace{1cm} \implies\!\!\! \bigvee_{x\in X} \bigvee_{y\in Y}
       		...
       		\Pred_{{pxy}}\subst{\Post_{OT_{px}}\shortotimes\Post_{OT_{pxy}}}
       		\land
       		\Pred_{{px}}\subst{\Post_{OT}\shortotimes\Post_{OT_{px}}}\big)\big)
       		$
...
substitutions  and of the predicates, we have:\\
       	$\Pred_{{px}}\subst{\Post_{OT}\shortotimes\Post_{OT_{px}}}\land 
       	\Pred_{{pxy}}\subst{\Post_{OT_{px}}\shortotimes\Post_{OT_{pxy}}}\\
       	\Leftrightarrow % we changed =
       	\Pred_{{px}}\subst{\Post_{OT}\shortotimes\Post_{OT_{px}}\shortotimes\Post_{OT_{pxy}}}\land
       	\Pred_{{pxy}}\subst{\Post_{OT}\shortotimes\Post_{OT_{px}}\shortotimes\Post_{OT_{pxy}}}\\
       	\implies\Pred\,'_{{pxy}}\subst{\Post_{OT}\shortotimes\Post_{OT_{px}}\shortotimes\Post_{OT_{pxy}}}\\
       	\Leftrightarrow % we changed = 
       	 \Pred\,'_{{pxy}}\subst{\Post_{OT}\shortotimes\Post_{OT_{pxy}}} $\\
...
\Pred_{OT_{pxy}}
\land \alpha\!=\alpha_{px}\!=\!\alpha_{pxy}\\~\hspace{3em}~ \land
\Pred\,'_{{pxy}}\subst{\Post_{OT}\shortotimes\Post_{OT_{pxy}}}\big)\big)
\\
\hspace{1cm} \implies\!\!\! \bigvee_{x\in X}\bigvee_{y\in Y}\bigvee_{p\in P}
\left( \forall j. \beta_j=\beta_{jpxy}  \Rightarrow \Pred_{OT_{pxy}}
\land \alpha\!=\!\alpha_{pxy} \land
...
The proof is trivial.
 ...
 \Pred_{s x}\subst{\Post\,''\shortotimes\Post_{OT_x}}\right)$
...
% terms using the fact that $v_Q=v_x$.
 ...
 \begin{itemize}
 	...
 	$\Pred_{s x}\subst{\Post_{OT}\shortotimes(\Post\,'\uplus \Post_{OT_x})}$ 
 	, i.e.:
 	$\Pred_{s x}\subst{(\Post\,'\uplus 
 		\Post\,'')\shortotimes(\Post\,'\uplus \Post_{OT_x})}$. We 
...
\land \Pred_{s x}\subst{\Post\,'\shortotimes\Post_{OT_x}}) \land
\Pred\,''\\ % 
			%FH for P
 \implies  \hspace{-2ex}
	{\displaystyle{\bigvee_{x\in X}\!(\forall j\!\in\! J'. \beta_j\!=\!\beta_{jx}  
...
\land \alpha\!=\!\alpha_x}} 
\land \Pred_{s x}\subst{\Post\,'\shortotimes\Post_{OT_x}}) 	
$\end{small}
...
\begin{itemize}
	...
	does not 
	change the validity of the formula (those variables are not used).
	\item We need $\Pred_{s x}\subst{\Post_{OT}\shortotimes(\Post_{OT_x}\uplus \Post\,'')}$, i.e.:
	$\Pred_{s x}\subst{(\Post\,'\uplus \Post\,'')\shortotimes(\Post_{OT_x}\uplus \Post\,'')}$. We 
...
\forall i \in [0..n].\openrule
    {
       \set{\beta^1_i},\Pred^1_i,\Post^1_i   }
         {s^1_i \OTarrow {\tau} s^1_{i+1}} \in \mathcal{T} \quad \wedge
\quad
\openrule
         {
           \set{\beta^2},\Pred^2,\Post^2 }
         {s_0^2 \OTarrow {\alpha} s_1^2} \in \mathcal{T}
\quad \wedge\\
\forall i \in [0..m].\openrule
         {
           \set{\beta^3_i},\Pred^3_i,\Post_i^3    }
         {s^3_i \OTarrow {\tau} s^3_{i+1}} \in \mathcal{T}
 \end{mathpar}
...
s=s^1_0 \wedge s^1_{n+1}=s_0^2 \wedge s_1^2 = s^3_0 \wedge s^3_{m+1}=s'\\
\displaystyle{
\set{\gamma}=\mybigdotcup_{i=0}^n \vis{\set{\beta^1_i}\subst{\bigotimes_{j=i-1}^{0} \Post_j^1 } }  \dotcup  \vis{\set{\beta^2}\subst{\bigotimes_{j=n}^{0}\Post_j^1}} \dotcup \mybigdotcup_{i=0}^m \vis{\set{\beta^3_i} \subst{\bigotimes_{j=i-1}^{0}\Post_j^3\shortotimes\Post^2\shortotimes\bigotimes_{j=n}^{0}\Post^1_j} }}
\\
\Pred=\bigwedge_{i=0}^n\Pred^1_i\subst{\bigotimes_{j=i-1}^{0}\Post_j^1}\land\Pred^2 \subst{\bigotimes_{j=n}^{0}\Post_j^1}\land\\ 
~\qquad\quad\Big(\bigwedge_{i=0}^m\Pred_i^3\subst{\bigotimes_{j=i-1}^{0}\Post_j^3\shortotimes\Post^2\shortotimes\bigotimes_{j=n}^{0}\Post^1_j}\Big)\\
\Post=\bigotimes_{j=m}^{0}\Post_j^3\shortotimes\Post^2\shortotimes\bigotimes_{j=n}^{0}\Post^1_j
$
...
We consider the case $n'=n+1$ (because $m'=m+1$ is similar).  We first focus on the left part of the disjunction. Let us consider the sequence of ($n+m+1$) open transitions:
\begin{mathpar}
...
~~\qquad\qquad\forall i \in [0..m].\openrule
         {
           \set{\beta^3_i},\Pred^3_i,\Post_{i 3}   }
         {s^3_i \OTarrow {\tau} s^3_{i+1}} \in \mathcal{T}
\Big) 
\end{mathpar}

By recurrence hypothesis we suppose that the statement holds for $n$ and $m$ (we remove the first tau transition). We have:\\
...
\Big(\forall i \in [1..n+1].\openrule
    {
       \set{\beta^1_i},\Pred^1_i,\Post^1_i   }
         {s^1_i \OTarrow {\tau} s^1_{i+1}} \in \mathcal{T} \,\wedge\,
\openrule
         {
           \set{\beta^2},\Pred^2,\Post^2 }
         {s_0^2 \OTarrow {\alpha} s_1^2} \in \mathcal{T}
\, \wedge\\
~~\hspace{5.25cm}\forall i \in [0..m].\openrule
         {
           \set{\beta^3_i},\Pred^3_i,\Post_{i 3}   }
         {s^3_i \OTarrow {\tau} s^3_{i+1}}\! \in\! \mathcal{T}
\Big) \\       
          \bigvee \Big(\alpha = \tau \wedge  \set{\gamma}=\emptyset \wedge \Pred =\True \wedge \Post =\Id(s)\wedge s=s'\Big)\!  \implies\!  {
\openrule
         {
           \set{\gamma},
		\Pred, \Post
...
s''=s^1_0 \wedge s^1_{n+2}=s_0^2 \wedge s_1^2 = s^3_0 \wedge s^3_{m+1}=s'\\
\displaystyle{
\set{\gamma}=\mybigdotcup_{i=1}^{n+1} \vis{\set{\beta^1_i}\subst{\bigotimes_{j=i-1}^{1} \Post_j^1 } }  \dotcup  \vis{\set{\beta^2}\subst{\bigotimes_{j=n+1}^{1}\Post_j^1}} \dotcup \mybigdotcup_{i=0}^m \vis{\set{\beta^3_i} \subst{\bigotimes_{j=i-1}^{0}\Post_j^3\shortotimes\Post^2\shortotimes\bigotimes_{j=n}^{0}\Post^1_j} }}
\\
\Pred=\bigwedge_{i=1}^{n+1}\Pred^1_i\subst{\bigotimes_{j=i-1}^{1}\Post_j^1}\land\Pred^2 \subst{\bigotimes_{j={n+1}}^{1}\Post_j^1}\land\\ 
~\qquad\quad\Big(\bigwedge_{i=0}^m\Pred_i^3\subst{\bigotimes_{j=i-1}^{0}\Post_j^3\shortotimes\Post^2\shortotimes\bigotimes_{j={n+1}}^{1}\Post^1_j}\Big)\\
\Post=\bigotimes_{j=m}^{0}\Post_j^3\shortotimes\Post^2\shortotimes\bigotimes_{j={n+1}}^{1}\Post^1_j
$
...
%\inferrule{
 \openrule
         {
           \set{\beta_{0}^{1}},\Pred_0^1,\Post_0^1}
         {s_0^1 \OTarrow {\tau} s_1^1} \in \mathcal{T}%}
\end{mathpar}
...
{ \openrule
       {
           \vis{\set{\beta_{0}^{1}}}\!,\Pred_0^1,\Post_0^1
				 } {s_0^1 \OTWeakarrow {\tau}s_1^1} \in \WT
}
\end{mathpar} 

On the other hand, by rule (\WTUn) we can write the following weak open transition:
\begin{mathpar}
 \openrule
         {
          \emptyset,\True,\Id(s')
...
\\
\set{\gamma''}= \vis{\set{\beta_{0}^{1}}} \dotcup \set{\gamma}\subst{\Post_0^1}\quad
\Pred\,''=\Pred_0^1\land\Pred\subst{\Post_0^1}\\%\land (\Pred_3\subst{\Post_2})\subst{\Post_0^1}
\Post\,''=\Id(s'')\shortotimes\Post\shortotimes\Post_0^1 \quad \alpha''=\alpha'\subst{\Post_0^1}
}
{
\openrule
         {
         ...
         {s_0^1 \OTWeakarrow {\alpha''} s'} \in\WT
} \WTTrois
\end{mathpar}
where we obtain the conclusion of the lemma, as required with the following assertions (derived from previous assertions):\\
$
\alpha''=\alpha\displaystyle{\subst{\bigotimes_{j=n+1}^{1}\Post_j^1}}\subst{\Post_0^1} = \alpha\displaystyle{\subst{\bigotimes_{j=n+1}^{0}\Post_j^1}}\\
s^1_0=s^1_0 \wedge s^1_{n+2}=s_0^2 \wedge s_1^2 = s^3_0 \wedge s^3_{m+1}=s'\\
\displaystyle{
\set{\gamma''}=\mybigdotcup_{i=0}^{n+1} \vis{\set{\beta^1_i}\subst{\bigotimes_{j=i-1}^{1} \Post_j^1 } }  \dotcup  \vis{\set{\beta^2}\subst{\bigotimes_{j=n+1}^{1}\Post_j^1}} \dotcup \mybigdotcup_{i=0}^m \vis{\set{\beta^3_i} \subst{\bigotimes_{j=i-1}^{0}\Post_j^3\shortotimes\Post^2\shortotimes\bigotimes_{j=n}^{0}\Post^1_j} }}
\\
\Pred=\bigwedge_{i=0}^{n+1}\Pred^1_i\subst{\bigotimes_{j=i-1}^{0}\Post_j^1}\land\Pred^2 \subst{\bigotimes_{j={n+1}}^{0}\Post_j^1}\land\\ 
~\qquad\quad\Big(\bigwedge_{i=0}^m\Pred_i^3\subst{\bigotimes_{j=i-1}^{0}\Post_j^3\shortotimes\Post^2\shortotimes\bigotimes_{j={n+1}}^{0}\Post^1_j}\Big)\\
\Post=\bigotimes_{j=m}^{0}\Post_j^3\shortotimes\Post^2\shortotimes\bigotimes_{j={n+1}}^{0}\Post^1_j
$
...
\Pred_1\!=\!\bigwedge_{i=0}^{n_1}(\Pred_{1i} \subst{\bigotimes_{j=i-1}^{0}\Post_{i j}} ), \Post_1= \bigotimes_{i=0}^{n_1} \subst{\Post_{1i}} 
$
...
\Pred_3\!=\!\bigwedge_{i=0}^{m_1}(\Pred_{3i} \subst{\bigotimes_{j=i-1}^{0}\Post_{3 j}} ), \Post_1= \bigotimes_{i=0}^{m_1} \subst{\Post_{3i}} 
$
...
\displaystyle{
\set{\gamma_2}=\mybigdotcup_{i=0}^{n_2} \vis{\set{\beta_{2i}}\subst{\bigotimes_{j=i-1}^{0} \Post_{2 j} } }  \dotcup  \vis{\set{\beta'}\subst{\bigotimes_{j=n}^{0}\Post_j^1}} }\\~\qquad
\dotcup \mybigdotcup_{i=0}^{m_2} \vis{\set{\beta'_{2i}} \subst{\bigotimes_{j=i-1}^{0}\Post'_{2j}\shortotimes\Post'\shortotimes\bigotimes_{j=n}^{0}\Post_{2j}} }
\\
\Pred_2=\bigwedge_{i=0}^{n_2}\Pred_{2i}\subst{\bigotimes_{j=i-1}^{0}\Post_{2j}}\land\Pred' \subst{\bigotimes_{j={n_2}}^{0}\Post_{2j}}\land\\ 
~\qquad\quad\Big(\bigwedge_{i=0}^{m_2}\Pred_i^3\subst{\bigotimes_{j=i-1}^{0}\Post'_{2j}\shortotimes\Post'\shortotimes\bigotimes_{j={n_2}}^{0}\Post_{2j}}\Big)\\
...
\\
 ...
 and similarly for $Pred_{4i}$, $Pred_{5i}$, $Pred_{4i}$, and $Pred_{4i}$  \TODO{define or OK this way?}
...
~\qquad\quad\Big(\bigwedge_{i=0}^{m_1+m_2}\Pred_{5i}\subst{\bigotimes_{j=i-1}^{0}\Post_{5j}\shortotimes\Post'\shortotimes\bigotimes_{j={n_1+n_2}}^{0}\Post_{4j}}\Big)\\
...
Suppose that $\Pred_{s,t}\in \mathcal{R}$ and for any open transition $OT$ in $\mathcal{T}_1$:
 \begin{mathpar}
     \openrule
         {
           \beta_j^{j\in J},\Pred_{OT},\Post_{OT}}
...
and  suppose the following statement holds:
\begin{mathpar}
     \openrule
         {
...
\openrule
         {
           {(\beta_j^2)}^{j\in J_2},\Pred\,^2,\Post^2 }
         {s_0^2 \OTarrow {\alpha} s_1^2} \in \mathcal{T}
 \wedge
\forall i \in [0\ldots m].\openrule
         {
           \set{\beta^3_i},\Pred\,^3_i,  \Post^3_i  }
         {s^3_i \OTarrow {\tau} s^3_{i+1}} \in \mathcal{T}
\bigg)
\end{mathpar}
where\\
...
\displaystyle{\gamma_j^{j\in J}=\vis{\beta_j^{j\in J}}=\mybigdotcup_{i=0}^n \vis{\set{\beta^1_i}} \dotcup  \vis{\set{\beta^2}} \dotcup \mybigdotcup_{i=0}^m \vis{\set{\beta^3_i}}}
\\
...
\begin{enumerate}
\item For the first open transition:
\begin{mathpar}
%\inferrule{
 \openrule
    {
       \set{\beta^1},\Pred^1,\Post^1   }
         {s_0^1 \OTarrow {\tau} s_1^1} \in \mathcal{T}          
\end{mathpar}
we have $(s,t|\Pred_{s,t})\in\mathcal{R}$, hence $(s_0^1,t|\Pred_{s_0^1,t})\in\mathcal{R}$, since $s=s_0^1$. By  Definition \ref{def-Weak-bisim} we  can deduce there exist weak open transitions $\symb{WOT}_x^{x\in X} \subseteq \WT_2$:
 \begin{mathpar}
    \openrule
         {
           \gamma_{j x}^{j\in J_{x}}, \Pred_{OT_x},\Post_{OT_x}}
         {t \OTWeakarrow {\alpha_x} t_x}
\end{mathpar}
 such that  $\forall x,J_{x}=\{j\in J_1|\beta^1_j\neq\tau\}, (s_1^1,t_x|\Pred_{s_1^1,t_x})\in \mathcal{R}$. \\
...
\hspace{1cm} \implies\!\!\! \displaystyle{\bigvee_{x\in X}}
  \!\! \left( \forall j\in J_x.\vis{\beta^1_j}=\gamma_{jx}  \Rightarrow \Pred_{OT_x}
     ...
     \Pred_{s_1^1,t_x}\subst{\Post^1\shortotimes\Post_{OT_x}}\right)$
...
\item Concerning the middle open transition, we have:
 \begin{mathpar}
     \openrule
         {
           \beta_j^{j \in J_2},\Pred\,^2,\Post^2 }
         {s_0^2 \OTarrow {\alpha} s_1^2} 
\end{mathpar}
For each of the weak open transition obtained above, we have the following.
We have $(s_1^1,t_x|\Pred_{s_1^1,t_x})\in \mathcal{R}$, then $(s_0^2,t_x|\Pred_{s_0^2,t_x})\in \mathcal{R}$. Again 
 by Definition \ref{def-Weak-bisim} we can deduce there exist weak open transitions $\symb{WOT}_{x y}^{x\in X,y\in Y} \subseteq \WT_2$:
 \begin{mathpar}
    \openrule
         {
           \gamma_{j x y}^{j\in J_{x y}}, \Pred_{OT_{x y}},\Post_{OT_{x y}}}
         {t_x \OTWeakarrow {\alpha_{x y}} t_{x y}}
\end{mathpar}
 such that  $\forall x,\forall y, J_{xy}=\{j\in J_2|\beta^2_j\neq\tau\}, 
(s_1^2,t_{x y}|\Pred_{s_1^2,t_{x y}})\in \mathcal{R}$; \\
 $\Pred_{s_1^1,t_x} \land \Pred\,^2\\
\hspace{1cm} \implies\!\!\! \displaystyle{\bigvee_{y\in Y}}
 \!\!  \left( \forall j\in J_{x y}. \vis{\beta^2_j}\!=\!\gamma_{jx y}\Rightarrow \Pred_{OT_{x y}}\!\land \alpha\!=\!\alpha_{x y} \land  
     \Pred_{s_1^2,t_{x y}}\subst{\Post^2\shortotimes\Post_{OT_{x y}}}\right)$


\item 
And similarly, for the third  open transition:
     \begin{mathpar}
%\inferrule{
 \openrule
    {
       \set{\beta^3},\Pred^3,\Post^3   }
         {s_0^3 \OTarrow {\tau} s_1^3} \in \mathcal{T}          
\end{mathpar}
Additionally, we have 
$(s_1^2,t_{x y}|\Pred_{s_0^3,t_{x y}})\in \mathcal{R}$, then $(s_0^3,t_{x y}|\Pred_{s_0^3,t_{x y}})\in \mathcal{R}$
and by Definition \ref{def-Weak-bisim} there exist weak open transition $\symb{WOT}_{x y z}^{x\in X,y\in Y,z\in Z} \subseteq \WT_2$:
 \begin{mathpar}
    \openrule
         ...
         {t_{x y} \OTWeakarrow {\alpha_{x y z}} t_{x y z}}
\end{mathpar}
 such that  $\forall x, \forall y, \forall z,J_{xyz}= \{j\in J_3|\beta^3_j\neq\tau\}, 
(s^3_1,t_{x y z}|\Pred_{s_1^3,t_{x y z}})\in \mathcal{R}$. \\
 $\Pred_{s_1^2,t_{x y}} \land \Pred^3\\
\hspace{1cm} \implies\!\!\! \displaystyle{\bigvee_{z\in Z}}
   \!\left( \forall j\in J_{xyz}. \vis{\beta^3_j}\!=\!\gamma_{j x y z} \! \Rightarrow\! \Pred_{OT_{x y z}}\!
     \land\! \alpha_{x y z}\!=\!\tau \land  
     \Pred_{s^3_1,t_{x y z}}\subst{\Post^3\shortotimes\Post_{OT_{x y z}}}\right)$
         
\end{enumerate}
\medskip     

Moreover, by applying (\WTTrois)~rule on the resulting $\symb{WOT}$s we have:
...
\openrule
         {
           \gamma_{j x y z}^{j\in J_{x y z}}, \Pred_{OT_{x y z}}, \Post_{OT_{x y z}}}
         {t_{x y} \OTWeakarrow {\tau} t_{x y z}}
\\
\Pred=\Pred_{OT_x}\land\Pred_{OT_{x y}}\subst{\Post_{OT_x}}\land \Pred_{OT_{x y z}}\subst{\Post_{OT_{x y}}\shortotimes\Post_{OT_x}}
}
{
\openrule
         {
           \gamma_{j x}^{j\in J_{x}} \dotcup \gamma_{j x y}^{j\in J_{x y}}\dotcup\gamma_{j x y z}^{j\in J_{x y z}},
		\Pred, \Post_{OT_{x y z}}\subst {\Post_{OT_{x y}}\shortotimes\Post_{OT_{x}}}}  
         {t \OTWeakarrow {\alpha_{x y}} t_{x y z}} 
} 
 \end{mathpar}
So we need to prove that the following statement holds:\\
$\Pred_{s,t} \land \Pred\\
\hspace{1cm} \implies\!\!\! \displaystyle{\bigvee_{z\in Z}}
   \left( \forall j\in J_{x y z}. \gamma_j=\gamma_{j x y z}  \Rightarrow \Pred_{OT_{x y z}}
     \land \alpha \!=\! \alpha_{x y} \land  
     \Pred_{s',t_{x y z}}\subst{\Post_{OT}\shortotimes\Post\,}\right)$
We recall that we have:\\
$\Pred_{OT}= \Pred^1 \wedge \Pred^2\subst{\Post^1} \wedge \Pred^3\subst{\Post^2\shortotimes\Post^1}$\\
$\Post_{OT}=\Post^3\subst{\Post^2\shortotimes\Post^1}$\\
...
\hspace{1cm} \implies\!\!\! \displaystyle{\bigvee_{x\in X}}
 \! \! \left( \forall j\in J_x. \vis{\beta^1_j}=\gamma_{jx}  \Rightarrow \Pred_{OT_x}
     \land \alpha_x\!=\!\tau \land  
     \Pred_{s_1^1,t_x}\subst{\Post^1\shortotimes\Post_{OT_x}}\right)$\\
The conjunction of the term $\Pred^2\subst{\Post^1}$ with the both sides of the implication, we get:\\  
$(\Pred_{s,t} \land \Pred^1)\wedge\Pred^2\subst{\Post^1}\\
 \hspace{1cm} \implies\!\!\! \displaystyle{\bigvee_{x\in X}}
  \!\! \left( \forall j\in J_x. \vis{\beta^1_j}=\gamma_{jx}  \Rightarrow \Pred_{OT_x}
     \land \alpha_x\!=\!\tau \land  
     \Pred_{s_1^1,t_x}\subst{\Post^1\shortotimes\Post_{OT_x}}\right) \wedge\Pred^2\subst{\Post^1\shortotimes\Post_{OT_x}}  \\
$
Note because of the domain independence of the substitution, we have also added the substitution $\subst{\Post_{OT_x}}$ on the right side of the implication without affecting the validity of the statement. Hence a little rewriting gives:\\
$(\Pred_{s,t} \land \Pred^1)\wedge\Pred^2\subst{\Post^1}\\
 \hspace{1cm} \implies\!\!\! \displaystyle{\bigvee_{x\in X}}
\!\!   \left(\forall j\in J_x. \vis{\beta^1_j}\!=\!\gamma_{jx}\Rightarrow \Pred_{OT_x}
     \!\land\! \alpha_x\!=\!\tau\land(\Pred_{s_1^1,t_x}\wedge\Pred^2)\subst{\Post^1\shortotimes\Post_{OT_x}} \right)$\\
By replacing the inner sub-term $(\Pred_{s_1^1,t_x}\wedge\Pred^2)$ by the consequent of the corresponding implication given in case 2,  the formula becomes: \\
$(\Pred_{s,t} \land \Pred^1)\wedge\Pred^2\subst{\Post^1}\\
 \hspace{1cm} \implies\!\!\! \displaystyle{\bigvee_{x\in X}}
   \!\!\Big(\forall j\in J_x. \vis{\beta^1_j}\!\!=\!\!\gamma_{jx}  \Rightarrow \Pred_{OT_x}
     \!\land\! \alpha_x\!=\!\tau \land\Big(\displaystyle{\bigvee_{y\in Y}}
   ( \forall j\in J_{x y}. \vis{\beta^2_j}\!\!=\!\!\gamma_{jx y}\Rightarrow\\~~\qquad\quad \Pred_{OT_{x y}}\land \alpha\!=\!\alpha_{x y}\! \land \Pred_{s_1^2,t_{x y}}\subst{\Post^2\shortotimes\Post_{OT_{x y}}})\Big)\subst{\Post^1\shortotimes\Post_{OT_x}} \Big)$\\
Furthermore, the rewriting of the implication  gives:\\
$(\Pred_{s,t} \land \Pred^1)\wedge\Pred^2\subst{\Post^1}\\
\hspace{1cm} \implies\!\!\! \displaystyle{\bigvee_{x\in X}} \displaystyle{\bigvee_{y\in Y}} 
\Big(\forall j\in J_x. \vis{\beta^1_j}\!\!=\!\!\gamma_{jx} \land  \forall j \in J_{x y}. \vis{\beta^2_j}\!\!=\!\gamma_{jx y}\Rightarrow\Pred_{OT_x} \land  \Pred_{OT_{x y}} \\\subst{\Post^1\shortotimes\Post_{OT_x}} \land \alpha_x\!=\!\tau \land \alpha\!=\!\alpha_{x y} \land \Pred_{s_1^2,t_{x y}}\subst{\Post^2\shortotimes\Post^1\shortotimes \Post_{OT_{x y}}\shortotimes \Post_{OT_x}}\Big)$\\ 
Finally, the conjunction with the term $\Pred^3\subst{\Post^2\shortotimes\Post^1}$ of the both sides of the implication and rewriting, we get:\\
$(\Pred_{s,t} \land \Pred^1)\wedge\Pred^2\subst{\Post^1} \land \Pred^3\subst{\Post^2\shortotimes\Post^1}\\
\hspace{1cm} \implies\!\!\! \displaystyle{\bigvee_{x\in X}} \displaystyle{\bigvee_{y\in Y}} 
...
\tau \land \alpha\!=\! \alpha_{x y} \land (\Pred_{s_1^2,t_{xy}} \land  \Pred^3) \subst{\Post^2\shortotimes \Post^1\shortotimes \Post_{OT_{x y}}\shortotimes\Post_{OT_x}} \Big)$\\ 
Again because of the domain independence of the substitution, we deleted $\subst{\Post^1}$, and  we have added  $\subst{\Post_{OT_{x y}}\shortotimes\Post_{OT_x}}$ on the right side of the implication.
Finally, by replacing $(\Pred_{s_1^2,t_{xy}} \land  \Pred^3)$
by the consequent of the corresponding implication given in case 3, we get:\\
$(\Pred_{s,t} \land \Pred^1)\land \Pred^2\subst{\Post^1} \land \Pred^3\subst{\Post^2\shortotimes\Post^1}\\
\hspace{1cm} \implies\!\!\! \displaystyle{\bigvee_{x\in X}} \displaystyle{\bigvee_{y\in Y}} \displaystyle{\bigvee_{z\in Z}} 
\Big(\forall j\in J_x.\vis{\beta^1_j}\!\!=\!\!\gamma_{jx} \land\!  \forall j \!\in\! J_{x y}.\vis{\beta^2_j}\!\!=\!\!\gamma_{jx y} \forall j\!\in\! J_{x y z}. \vis{\beta^3_j}\!\!=\!\!\gamma_{j x y z} \Rightarrow  \Pred_{OT_x}\land\Pred_{OT_{x y}}\subst{\Post_{OT_x}} \land \Pred_{OT_{x y z}}\!\subst{\Post_{OT_{x y}}\shortotimes\Post_{OT_x}} \land \alpha_x\!=\!
\tau \!\land \alpha\!=\!\alpha_{x y} \land \alpha_{x y z}\!=\!\tau \land \Pred_{s^3_1,t_{x y z}}  \subst{\underbrace{\Post^3\shortotimes\Post^2\shortotimes\Post^1}_{\Post_{OT}}\,\shortotimes \Post_{OT_{x y z}}\shortotimes \Post_{OT_{x y}}\shortotimes\Post_{OT_x}} \Big)$
Therefore, having $s^3_1=s'$ we can conclude:\\
$(\Pred_{s,t} \land \Pred^1)\land \Pred^2\subst{\Post^1} \land \Pred^3\subst{\Post^2\shortotimes\Post^1}\\
\hspace{1cm} \implies\!\!\! \displaystyle{\bigvee_{x\in X}} \displaystyle{\bigvee_{y\in Y}} \displaystyle{\bigvee_{z\in Z}} 
\Big(\forall j\in J_{x y z}. \vis{\beta^1_j}.\vis{\beta^2_j}.\vis{\beta^3_j}\!=\!\gamma_{j x y z} \Rightarrow  \Pred_{OT_x}\land\Pred_{OT_{x y}}\subst{\Post_{OT_x}} \land \Pred_{OT_{x y z}}\subst{\Post_{OT_{x y}}\shortotimes\Post_{OT_x}} \land \alpha\!=\!\alpha_{x y} \land \Pred_{s',t_{x y z}}\subst{\Post_{OT}\shortotimes\Post_{OT_{x y z}}\shortotimes\Post_{OT_{x y}}\shortotimes\Post_{OT_x}} \Big)$
...
\\
 ...
 \text{and\quad} \forall i \in L_Q.\, s_{(n+1) i} = s_i'\land s_{0 i} = s_i\\{\hspace{-3ex}}\text{and~~} \Pred \iff \Pred\,'
...
\begin{mathpar}
 P[Q]_{j_0} \models \openrule
			...
			{\ostate{s_l^{l\in L}} \OTWeakarrow {\alpha}
				\ostate{s_l'^{\, l\in L}}} 
\end{mathpar}
By Lemma \ref{lem-rel-OT-WOT} this implies the following \TODO{replace $\alpha$ by $\alpha'$}: \\			
$ \forall p\! \in [0..m_1]\,		
	P[Q]_{j_0}\!\models		
\openrule
    {
...
$\displaystyle{
\set{\gamma}=\mybigdotcup_{i=0}^{m_1} \vis{\set{\beta^1_i}\subst{\bigotimes_{j=i-1}^{0} \Post_j^1 } }  \dotcup  \vis{\set{\beta^2}\subst{\bigotimes_{j=n}^{0}\Post_j^1}} \dotcup \mybigdotcup_{i=0}^{m_2} \vis{\set{\beta^3_i} \subst{\bigotimes_{j=i-1}^{0}\Post_j^3\shortotimes\Post^2\shortotimes\bigotimes_{j=m_1}^{0}\Post^1_j} }}
\\
\Pred=\bigwedge_{i=0}^{m_1}\Pred^1_i\subst{\bigotimes_{j=i-1}^{0}\Post_j^1}\land\Pred^2 \subst{\bigotimes_{j=m_1}^{0}\Post_j^1}\land\\ 
~\qquad\quad\Big(\bigwedge_{i=0}^{m_2}\Pred_i^3\subst{\bigotimes_{j=i-1}^{0}\Post_j^3\shortotimes\Post^2\shortotimes\bigotimes_{j=m_1}^{0}\Post^1_j}\Big)\\
\Post=\bigotimes_{j=m_2}^{0}\Post_j^3\shortotimes\Post^2\shortotimes\bigotimes_{j=m_1}^{0}\Post^1_j$
...
$P\models{\openrule
	...
	\,\text{~~and~~} 
		Q\models{\openrule
			{
				(\beta^1_{pj})^{j\in J\cap\Holes(Q)}, {{\Pred^{1b}_p}},  
				{{\Post^{1b}_p}}}
				{\ostate{s_{pl}^{l\in L_Q}} \OTarrow {\alpha^1_{p}}
				\ostate{s_{(p+1)l}^{l\in L_Q}}}
				}\\%\]\\
$
such that  $\Pred^1_p \iff {\Pred^{1a}_p}
		\land {\Pred^{1b}_p}\land \alpha^1_p=\beta^1_{pj_0}$, $\Post^1_p={\Post^{1a}_p}\uplus 
		{\Post^{1b}_p}$ where ${{\Post^{1b}_p}}$ is the restriction of $\Post^1_p$ over variables of $\Leaves(Q)$.\\
...
$
	...
	Q\models{\openrule
			{
				(\beta^2_j)^{j\in J\cap\Holes(Q)}, {\Pred^{2b}},  
				{\Post^{2b}}}
			{\ostate{t_l^{l\in L_Q}} \OTarrow {\alpha^2}
				\ostate{t_l'^{\,l\in L_Q}}}
		}%\]
$\\
such that  $\Pred^2 \iff {\Pred^{2a}}
		\land {\Pred^{2b}}\land \alpha^2=\beta^2_{j_0}$, $\Post^2={\Post^{2a}}\uplus 
		{\Post^{2b}}$ where ${\Post^{2b}}$ is the restriction of $\Post^2$ over variables of $\Leaves(Q)$.
...
\\
\Pred\,'=\big(\bigwedge_{i=0}^{m_1}
{\Pred_i^{1a}}\bigotimes_{j=0}^{i-1}\subst{{\Post_j^{1a}}}\big)
\land{\Pred^{2a}}\bigotimes_{i=0}^{m_1}\subst{{\Post_i^{1a}}}\land \\
~~\qquad\quad\big(\bigwedge_{i=0}^{m_2}{\Pred_i^{3a}}\bigotimes_{j=0}^{i-1}\subst{{\Post_j^{3a}}}\subst{{\Post^{2a}}}\bigotimes_{j=0}^{m_1}\subst{{\Post_j^{1a}}}\big)\\
\Post'=\bigotimes_{i=0}^{m_2}{\Post_i^{3a}}\subst{{\Post^{2a}}}\bigotimes_{j=0}^{m_1}\subst{{\Post_j^{1a}}}
$
...
$\forall p\! \in\! [0..n_1] Q\models{\openrule
			{
				(\beta^1_{pj})^{j\in J\cap\Holes(Q)}, {{\Pred_p^{1b}}},  
				{{\Post_p^{1b}}}}
				{\ostate{s_{pl}^{l\in L_Q}} \OTarrow {\alpha^1_{p}}
				\ostate{s_{(p+1)l}^{l\in L_Q}}}
				}%\]
...
$\forall p \in [0..n_2] Q\models{\openrule
			{
				(\beta^3_{pj})^{j\in J\cap\Holes(Q)}, {{\Pred_p^{3b}}},  
				{{\Post_p^{3b}}}}
				{\ostate{u_{pl}^{l\in L_Q}} \OTarrow {\alpha^3_{p}}
...
$\forall p \in [0..(n_1\!+\!n_2\!+\!2)]\,\, Q\models{\openrule
			{
				(\beta_{pj})^{j\in J\cap\Holes(Q)}, {{\Pred_p^b}},  
				{{\Post_p^b}}}
...
$Q\models{\openrule
			{
				(\beta^2_j)^{j\in J\cap\Holes(Q)}, {{\Pred^{2b}}},  
				{{\Post^{2b}}}}
				{\ostate{v_{n_k'l}^{l\in L_Q}} \OTarrow {\alpha'_k}
...
Q\models{\openrule
			{
			(\gamma_{kj})^{j\in J\cap\Holes(Q)},  {{\Pred^b_k}},  
				{{\Post^b_k}}}
...
\displaystyle{\gamma_{kj} }=\mybigdotcup_{i=0}^{(n_k'-1)}\!\!\vis{\beta^1_{ij}}\,\dotcup\,\vis{\set{\beta^2_j}}\,\dotcup\!\!\!\mybigdotcup_{i=(n_k'+1)}^{(n_k-1)}\!\!\!\vis{\beta^3_{ij}}
\\
\Pred_k=\Big(\!\!\!\bigwedge_{i=0}^{(n_k'-1)}\!\!
{\Pred_i^{1b}}\bigotimes_{j=0}^{(i-1)}\subst{{\Post_j^{1b}}}\Big)
\land\Big({\Pred^{2b}}\bigotimes_{i=0}^{(n_k'-1)}\subst{{\Post_i^{1b}}}\Big)\land\\
~\qquad\qquad\Big(\!\!\!\bigwedge_{i=(n_k'+1)}^{(n_k-1)}\!\!\!{\Pred_i^{3b}}\bigotimes_{j=n_k'+1}^{(i-1)}\subst{{\Post_j^{3b}}}\subst{{\Post^{2b}}}\bigotimes_{i=0}^{(n_k'-1)}\subst{{\Post_i^{1b}}}\Big)\\
\Post_k=\bigotimes_{i=(n_k'+1)}^{(n_k-1)}\!\!\!{\Post_i^{3b}}\Big(\subst{{\Post^{2b}}}\bigotimes_{i=0}^{(n_k'-1)}\subst{{\Post_i^{1b}}}\Big)
$\\
...
We have: 
$\displaystyle{\gamma_{j_0}}=\mybigdotcup_{i=0}^{m_1}  \vis{\beta^1_{i{j_0}}} \dotcup  \vis{\set{\beta^2_i}} \dotcup \mybigdotcup_{i=0}^{m_2} \vis{\beta^3_{i{j_0}}}$ 

Additionally, we have
$[\alpha'_0.. \alpha'_n]$ is a a list of $\alpha_p$ such that $p\in [0..(n_1\!+\!n_2\!+\!2)] \land \alpha_p\neq \tau$ where for all $k\in [0\ldots n_1+n_2+2]$, $\beta_{kj_0}=\alpha_k$. In other words, $\displaystyle{\mybigdotcup_{i=0}^{m_1}  \vis{\beta^1_{i{j_0}}} \dotcup  \vis{\set{\beta^2_i}} \dotcup \mybigdotcup_{i=0}^{m_2} \vis{\beta^3_{i{j_0}}} = [\alpha'_0.. \alpha'_n]}$. This ensures that $\gamma_{j_0}=[\alpha'_0.. \alpha'_n]$.
...
Q\models{\openrule
			...
			{\ostate{s_{pl}^{l\in L_Q}} \OTWeakarrow {\alpha_{k}}
				\ostate{s_{(p+1)l}^{l\in L_Q}}}
		}%\]
\end{mathpar}
and $\displaystyle{\bigcup_{p=0}^nJ_{p}} = J\cap\Holes(Q)$, $\forall j\in J_p,\,\gamma_j= \displaystyle{\mybigdotcup_{p=0}^n \gamma_{p j}}$, $\forall l\in L_Q,\, s_{0l}=s_l$, $s_{(n+1)l}=s'_l$  \\
  $\Pred \iff \Pred\,'
		\land \displaystyle{\bigwedge_{p=0}^n}((\alpha_p= \beta_{pj_0}\land \Pred_p)\bigotimes_{i=0}^{p-1}\Post_i),\text{and~}$\\
$\displaystyle{\Post=\Post\,'\uplus\bigotimes_{p=0}^n
		\Post_p}$  \\
We have:\\
$\Pred=\displaystyle{\bigwedge_{i=0}^{m_1}\!\big(\Pred^1_i\bigotimes_{j=0}^{i-1}\subst{\Post_j^1}\big)\land\Pred^2\bigotimes_{i=0}^{m_1}\subst{\Post_i^1}\land \bigwedge_{i=0}^{m_2}\!\Pred_i^3\big(\bigotimes_{j=0}^{i-1}\subst{\Post^3_j}\subst{\Post^2}  \bigotimes_{i=0}^{m_1}\subst{\Post_i^1}}\big)$\\
By replacing ${\Pred^{1}_i}$,  $\Pred^2$, and ${\Pred^{3}_i}$ by their equivalent formula  we obtain:\\
$\Pred \iff\displaystyle{\bigwedge_{i=0}^{m_1}({\Pred^{1a}_i}
		\land {\Pred^{1b}_i}\land \alpha^1_i=\beta^1_{ij_0}) \bigotimes_{j=0}^{i-1}\subst{\Post_j^1}\big)\land}\\
~~\qquad\qquad\qquad({\Pred^{2a}}\land {\Pred^{2b}}\land \alpha^2=\beta^2_{j_0})	\bigotimes_{i=0}^{m_1}\subst{\Post_i^1}\land\\
~~\qquad\qquad\bigwedge_{i=0}^{m_2} ({\Pred^{3a}_i}\land {\Pred^{3b}_i}\land \alpha^3_i=\beta^3_{ij_0})\big(\bigotimes_{j=0}^{i-1}\subst{\Post^3_j}\subst{\Post^2}  \bigotimes_{i=0}^{m_1}\subst{\Post_i^1}\big)$\\
By distribution of the substitution operation over the logical operator we can deduce:\\
$\Pred\iff \displaystyle{\big(\bigwedge_{i=0}^{m_1}
{\Pred_i^{1a}}\bigotimes_{j=0}^{i-1}\subst{{\Post_j^{1a}}}\big)
\land{\Pred^{2a}}\bigotimes_{i=0}^{m_1}\subst{{\Post_i^{1a}}}\land} \\
~~\qquad\qquad\quad\big(\bigwedge_{i=0}^{m_2}{\Pred_i^{3a}}\bigotimes_{j=0}^{i-1}\subst{{\Post_j^{3a}}}\subst{{\Post^{2a}}}\bigotimes_{j=0}^{m_1}\subst{{\Post_j^{1a}}}\big) \land\\	
~\qquad\qquad\quad\bigwedge_{i=0}^{m_1}({\Pred^{1b}_i}\land \alpha^1_i=\beta^1_{ij_0}) \bigotimes_{j=0}^{i-1}\subst{\Post_j^1}\land({\Pred^{2b}}\land \alpha^2=\beta^2_{j_0})	\bigotimes_{i=0}^{m_1}\subst{\Post_i^1}\land\\
~\qquad\qquad\quad\bigwedge_{i=0}^{m_2} ({\Pred^{3b}_i}\land \alpha^3_i=\beta^3_{ij_0})\bigotimes_{j=0}^{i-1}\subst{\Post^3_j}\subst{\Post^2}  \bigotimes_{i=0}^{m_1}\subst{\Post_i^1}
$\\		
Therefore, we have:\\
$\Pred\iff \Pred\,' \wedge \displaystyle{\bigwedge_{p=0}^{n}}((\alpha_p= \beta_{pj_0}\land \Pred_p)\bigotimes_{i=0}^{p-1}\Post_i)$\\ where $n=m_1+m_2+1$	\\
Concerning the postcondition we have:\\
$\Post=\displaystyle{\bigotimes_{i=0}^{m_2}\Post^3_i\subst{\Post^2}\bigotimes_{j=0}^{m_1}\subst{\Post^1_j}}$\\
By replacing ${\Post^{1}_i}$,  $\Post^2$, and ${\Post^{3}_j}$ and because the ${\Post_i^{ja}}$ and ${\Post_i^{jb}}$ are independent (the ${\Post_i^{ja}}$ concern the leaves of $P$ while the ${\Post_i^{jb}}$ concern the leaves of $Q$), we obtain:\\
$\Post= \displaystyle{\bigotimes_{i=0}^{m_2}\subst{{\Post^{3a}_i}\uplus 
		{\Post^{3b}_i}} \subst{{\Post^{2a}}\uplus 
		{\Post^{2b}}} \bigotimes_{j=0}^{m_1}\subst{ {\Post^{1a}_i}\uplus 
		{\Post^{1b}_i}}}\\
~~\qquad= \bigotimes_{i=0}^{m_2}{\Post_i^{3a}}\subst{{\Post^{2a}}}\bigotimes_{j=0}^{m_1}\subst{{\Post_j^{1a}}} \uplus \bigotimes_{i=0}^{m_2}{\Post_i^{3b}}\subst{{\Post^{2b}}}\bigotimes_{j=0}^{m_1}\subst{{\Post_j^{1b}}} \\
~~\qquad= \Post' \uplus \bigotimes_{j=0}^{n} \Post_j $\\
 where $n=m_1+m_2+1$	
\end{proof}
