...
\item  intro et orga globale du papier
\end{itemize}
...
\item proofs of new theorems -> fin aout
...
\TODO{From Definition \ref{def:buildweakOT} we can prove following lemma that enables us to conclude FH-weak bisimulation coincides with  FH-bisimulation ... what does coincide mean here?}

\begin{lemma} \TODO{!!! formalisation of the lemma is cumbersome}
$s (\OTarrow{\tau})\!\!* (\OTarrow{\alpha})(\OTarrow{\tau})\!\!* s'$ iff $s \OTWeakarrow {\alpha} s'$
\end{lemma}
...
We will show that $\mathcal{R}''$ is a weak FH-bisimulation. Consider 
$(s,u|\Pred_{s,u})\in\mathcal{R}''$. Then there is a set of states of $\mathcal{S}_3$ relating $s$ and $u$, let $(p)^{p\in \mathcal{S}_2}$ be this family.  We have $\Pred_{s,u} = \bigvee_{p\in \mathcal{S}_2} \Pred_{s,p}\land\Pred_{p,u}$.
...
For any open transition $OT$ in $\mathcal{T}_1$ originating from $s$.
       	...
       	There exist weak open transitions $OT_{x}^{x\in X} \subseteq \mathcal{T}_2$:
       	
       	\begin{mathpar} 
       	%    \left( fresh \ \overline{a_i}, \overline{b_j}, v_x.\ \
       	\openrule
       	{
       		\gamma_{j x}^{j\in J_{x}}, \Pred_{OT_{x}},\Post_{OT_{x}}}
       	{p \OTWeakarrow {\alpha_{x}} p'_{x}}\qquad (2)
       	%         \right)
       	\end{mathpar}
       	
       	such that  $\forall x, J_1=J_{x}, (s',p'_{x}|\Pred\,'_{{x}})\in 
       	\mathcal{R}$;
       	and  \\
       	
       	$\Pred_{s,p} \land \Pred_{OT}\\
       	\hspace{1cm} \implies\!\!\! \bigvee_{x\in X}
       	\left( \forall j. \vis{\beta_j}=\gamma_{jx}  \Rightarrow \Pred_{OT_{x}}
       	\land \alpha\!=\!\alpha_{x} \land
       	\Pred\,'_{{x}}\subst{\Post_{OT}}\subst{\Post_{OT_{x}}}\right)$\\
       	


For any open transition $OT_{x}$, since
       	$(p,u_x|\Pred_{x})\in\mathcal{R}'$ there exist open transitions
       	$OT_{xy}^{y\in Y} \subseteq \mathcal{T}_3$: 
       	
       	\begin{mathpar}  	
       	%    \left( fresh \ \overline{a_i}, \overline{b_j}, v_x.\ \
       	\openrule
       	{
       		\gamma_{j  x y}^{j\in J_{xy}}, 
       		\Pred_{OT_{xy}},\Post_{OT_{xy}}}
       	{u \OTWeakarrow {\alpha_{xy}} u'_{xy}}\qquad (3)
       	%         \right)    	
       	\end{mathpar}
       	such that  $\forall y, J_{x}=J_{xy}, 
       	(p'_{x},u'_{xy}|\Pred\,'_{{x,xy}})\in \mathcal{R}'$; and  \\
       	$\Pred_{p,u} \land \Pred_{OT_{x}}\\
       	\hspace{1cm} \implies\!\!\! \bigvee_{y\in Y}
       	\left( \forall j. \gamma_{jx}=\gamma_{jxy}  \Rightarrow \Pred_{OT_{xy}}
       	\land \alpha_{x}\!=\!\alpha_{xy} \land
       	\Pred\,'_{{x,xy}}\subst{\Post_{OT_{x}}}\subst{\Post_{OT_{xy}}}\right)$.\\
       	
This is verified for all $p \in \mathcal{S}_2$. Overall,  we have a family of open 
       	transitions 
       	$OT_{xy}^{x\in X, 
       		y\in Y} \subseteq \mathcal{T}_3$ that should simulate \emph{OT}.
 
We have  $\forall y, \forall x,  J_1=J_{x}=J_{xy}, 
       	({s}',u'_{xy}|\Pred\,'_{{xy}})\in \mathcal{R}''$ for some $\Pred\,'_{{xy}}$.  Indeed,  for any $x$ and $y$, we have $p'_{x}$ relates ${s}'$ and $u'_{xy}$, indeed we have
$(s',p'_{x}|\Pred\,'_{{x}})\in \mathcal{R}$
       	and $(p'_{x},u'_{xy}|\Pred\,'_{{x,xy}})\in \mathcal{R}'$. For states ensuring the transitivity relation we have   
       		 for all  $x$, $y$, $\Pred\,'_{x}\land 
       	\Pred\,'_{{x,xy}}\implies 
       	\Pred\,'_{{xy}}$. 
...
\TODO{We need to prove that we have the following  relation:}  \\     	
 ...
 $\Pred_{s,u} \land \Pred_{OT}\\
       		...
       		\left( \forall j. \vis{\beta_j}=\gamma_{jxy}  \Rightarrow \Pred_{OT_{xy}}
       		\land \alpha\!=\!\alpha_{xy} \land
       		\Pred\,'_{{xy}}\subst{\Post_{OT}}\subst{\Post_{OT_{xy}}}\right)$.
       	\end{small}
       	
\noindent By rule given in (2) we have for all state $p$:\\
\begin{small}
$\big(\Pred_{s,p} \land \Pred_{OT}\big)\land\Pred_{p,u}\\
       		\hspace{1cm} \implies\!\!\! \Big(\bigvee_{x\in X}
       		\left( \forall j. \vis{\beta_j}=\gamma_{jx}  \Rightarrow \Pred_{OT_{x}}
       		\land \alpha\!=\!\alpha_{x} \land
       		\Pred\,'_{{x}}\subst{\Post_{OT}}\subst{\Post_{OT_{x}}}\right)\Big)\land\Pred_{p,u}\\
       		%
       		\hspace{1cm} \implies\!\!\! \bigvee_{x\in X}
       	\Big( \forall j. \vis{\beta_j}=\gamma_{jx}  \Rightarrow 
       		(\Pred_{OT_{x}}\land\Pred_{p,u})
       		\land \alpha\!=\!\alpha_{x} \land
       		\Pred\,'_{{x}}\subst{\Post_{OT}}\subst{\Post_{OT_{x}}}\Big).\\
       		%
       		$
  \end{small}     		
  
\noindent By rule given in (3) we get:     \\	
	\begin{small}
$    \big(\Pred_{s,p} \land \Pred_{OT}\big)\land\Pred_{p,u}\\
\hspace{1cm} \implies\!\!\! \bigvee_{x\in X}
       		\bigg( \forall j. \vis{\beta_j}=\gamma_{jx}  \Rightarrow \Big(\bigvee_{y\in Y} 
       		\big( \forall j'.\gamma_{j'x}=\gamma_{j'xy}  \Rightarrow \Pred_{OT_{xy}}
       		\land \alpha_{x}\!=\!\alpha_{xy}\land\\
       		%
 \Pred\,'_{{x,xy}}\subst{\Post_{OT_{x}}}\subst{\Post_{OT_{xy}}}\Big)
       		\land \alpha\!=\!\alpha_{x} \land
       \Pred\,'_{{x}}\subst{\Post_{OT}}\subst{\Post_{OT_{x}}}\bigg)\\
       		%
 \hspace{1cm} \implies\!\!\! \bigvee_{x\in X} \bigvee_{y\in Y}
       		\bigg(\forall j, j'.\vis{\beta_j}=\gamma_{jx} \land \gamma_{j'x}=\gamma_{j'xy}
       		\Rightarrow \Big( 
       		\Pred_{OT_{xy}}
       		\land \alpha\!=\alpha_{x}\!=\!\alpha_{xy} \land
       \Pred\,'_{{x,xy}}\subst{\Post_{OT_{x}}}\subst{\Post_{OT_{xy}}}
       		\land
\Pred\,'_{{x}}\subst{\Post_{OT}}\subst{\Post_{OT_{x}}}\Big)\bigg).
       		$
       \end{small}
       
\medskip
\noindent Because of variables domains independence we  have:\\ 
    $\Pred\,'_{{x}}\subst{\Post_{OT}}\subst{\Post_{OT_{x}}}\land 
       	\Pred\,'_{{x,xy}}\subst{\Post_{OT_{x}}}\subst{\Post_{OT_{xy}}}
       	\Leftrightarrow\\ % we changed =
\Pred\,'_{{x}}\subst{\Post_{OT}}\subst{\Post_{OT_{x}}}\subst{\Post_{OT_{xy}}}\land
       \Pred\,'_{{x,xy}}\subst{\Post_{OT}}\subst{\Post_{OT_{x}}}\subst{\Post_{OT_{xy}}} \Leftrightarrow
\big(\Pred\,'_{{x}} \land   \Pred\,'_{{x,xy}}\big)   \subst{\Post_{OT}}\subst{\Post_{OT_{x}}}\subst{\Post_{OT_{xy}}} \\	\implies\Pred\,'_{{xy}}\subst{\Post_{OT}}\subst{\Post_{OT_{x}}}\subst{\Post_{OT_{xy}}}
   \Leftrightarrow  \Pred\,'_{{xy}}\subst{\Post_{OT}}\subst{\Post_{OT_{xy}}} $\\
       	
\noindent This allows us to conclude, with $\Pred_{s,u} = \bigvee_{p\in \mathcal{S}_2} 
       	\Pred_{s,p}\land\Pred_{p,u}$:

      	\begin{small}     	
$\Pred_{s,u} \land \Pred_{OT}\\
\hspace{1cm} \implies\!\!\! \bigvee_{p\in \mathcal{S}_2} \big(
	\Pred_{s,p}\land\Pred_{p,u}\big) \land \Pred_{OT}\\
\hspace{1cm} \implies\!\!\!\bigvee_{p\in \mathcal{S}_2}\bigg(\!\!
 \bigvee_{x\in X}\!\!\bigvee_{y\in Y}\!\!
\Big( \forall j, j'.\vis{\beta_j}=\gamma_{jx} \land \gamma_{j'x}=\gamma_{j'xy}
\Rightarrow \Big( 
\Pred_{OT_{xy}}
\land \alpha\!=\alpha_{x}\!=\!\alpha_{xy}\\~\hspace{3em}~ \land
\Pred\,'_{{xy}}\subst{\Post_{OT}}\subst{\Post_{OT_{xy}}}\Big)\Big)\bigg)
\\
\hspace{1cm} \implies\!\!\! \bigvee_{x\in X}\!\!\bigvee_{y\in Y}\!\!\bigvee_{p\in \mathcal{S}_2}\!\!
\Big(\forall j.\vis{\beta_j}\!=\!\gamma_{jxy}  \Rightarrow \Pred_{OT_{xy}}
\land\, \alpha\!=\!\alpha_{xy} \land
\Pred\,'_{{xy}}\subst{\Post_{OT}}\subst{\Post_{OT_{xy}}}\Big)$.
  \end{small}
...
\begin{lemma}[Weak OT decomposition]\label{lem-decompose} 
	...
	\]
		...
		$\Post\,'$,   $\gamma_{j_0}=[\beta_0..\beta_n]$, and for all $p\in[0..n]$ there exist $\Pred_p$, $\Post_p$ and a family $\gamma_{p j}^{j\in J_p}$ and $s_{p i}^{\,i\in L_Q}$. s.t.:\\[-2ex]
		%\[
		\begin{mathpar}
		P\models{\openrule
			{
		...
		Q\models{\openrule
			{
				...
				\Post_p}
			{\ostate{s_i^{i\in L_Q}} \OTWeakarrow {\alpha_i}
				\ostate{s_{p i}^{\,i\in L_Q}}}
		}%\]
\\
		\text{and~~}  \bigcup_{\begin{array}{l}
p\in[0..n]\\{j\in J_p}
\end{array}} J_{p j} = J\cap\Holes(Q) \qquad
 \text{and\quad} \forall i \in L_Q.\, s_{n i} = s_i'\\{\hspace{-3ex}}\text{and~~} \Pred \iff \Pred\,'
