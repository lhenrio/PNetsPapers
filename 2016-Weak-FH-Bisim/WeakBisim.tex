...
This allows us to derive a bismulation theory for open pNets. As pNets are composition structures, it then makes sense to prove composition lemmas: we prove that the composition of bisimilar pNets are themselves bisimilar.
...
\[ \openrule
         ...
         {s \OTarrow {\alpha} s'}
         \in \mathcal{T} \] we have \[ \alpha=\tau\land s=s'\land \Post=\Id(s)
\]
\end{enumerate}
\end{definition}
...
A pNet $\mylangle \pNet_i^{i\in I} , \Sort_j^{j\in J}, \overline{\symb{SV}}\myrangle$
 \emph{cannot observe silent actions} if it verifies:\\ $\forall i\in I\uplus J.\, \SV{(i\mapsto \tau)}{\tau}{\True}\in \overline{\symb{SV}}$ and $\forall SV_k\in \overline{\symb{SV}}, \forall i,j\in I_k\uplus J_k.\,i\neq j \Rightarrow \alpha'_k\neq\tau$
\end{definition}
...
\begin{theorem}[Context equivalence for  weak bisimulation]\label{weak-thm-ctxt-eq}
	...
	\overline{\symb{SV}}\myrangle$ and 	$\pNet' = \mylangle {\pNet'}_i^{i\in I}, 
	\Sort_j^{j\in 
	J}, 	\overline{\symb{SV'}}\myrangle$ that are weak FH-bisimilar
	(recall they must have the same holes to be bisimilar).
...
\Bigg)        
       \bigvee\\   
 \Big(\alpha = \tau \wedge  \overline{\gamma}=\emptyset \wedge \Pred =\True \wedge \Post =\Id(s)\Big)  \Longleftrightarrow  {
\openrule
         {
           \overline{\gamma},
		\Pred, \Post
...
\end{mathpar} 
\item For the inductive step, first we have by induction hypothesis that the formula holds for some length $l$.   Induction step is to infer that formula holds for transitions of length $l + 1$. As the condition holds for $l$, it means that there is $n$  $m$, $n'$,$m'$ such that $l=n+m+1$, and $l+1=n'+m'+1$ and either $n'=n+1$ or $m'=m+1$ (exclusively).
We consider the case $n'=n+1$, $m'=m+1$ is similar. Side conditions are omitted for readability. We first focus on the left part of the disjunction
Suppose
$
...
\Bigg)        
       \bigvee\\   
 \Big(\alpha = \tau \wedge  \overline{\gamma}=\emptyset \wedge \Pred =\True \wedge \Post =\Id(s)\Big)  \implies  {
\openrule
         {
           \overline{\gamma},
		\Pred, \Post
...
We need to prove the same implication with 
...
Finally by using rule (3) we compose the three rules:
...
\begin{mathpar}
\Big(\alpha = \tau \wedge  \overline{\gamma}=\emptyset \wedge \Pred =\True \wedge \Post =\Id(s)\Big)
\end{mathpar}
is handled trivially by rule (WT1) 

\end{itemize}
\TODO{add rule numbers WT1 WT2 WT3, and refer to them}
...
\item case rule (WT2). \TODO{write inference}
\begin{mathpar}
\openrule
         {
           \overline{\beta},\Pred,\Post}
...
This proves the implication with $n=0$ and $m=0$.
...
%
%
%Basis: suppose that $\OTWeakarrow {\alpha}$ cannot be subdivided. So for a given weak transition:
%
%\begin{mathpar}
%{ \openrule
%         {
...
%We have two cases:
%\begin{itemize}
\item either $\alpha = \tau$, $s=s'$, $\overline{\beta}=\emptyset$, $\Pred = \True$ and $\Post=\Id(s)$. So by rule (2) we have: 
