...
\author{Ludovic Henrio  \ \ \  Eric Madelaine \ \ \ Rab\'ea}
...
To illustrate this work, we use a simple transport protocol, that provides safe transport of data between two processes,
over unsafe media.
...
$receiver$. The underlying network is modelled by a medium entity transporting messages from the sender to the receiver, and that is able to detect some transport error, and signal it to the sender. The return $ack$ message from $receiver$ to $sender$ is supposed to be safe. The final transmission of the message to the recipient (Hole Q) include the value of the ``error counter'' $ec$.

This is only a graphical syntax,
with an obvious semantics for building the corresponding pNets.

Fig. \ref{SimpleProt:Spec} (right) shows the Schema of this system, and on the right side, its global specification, expressed as a pNet. 
The full system should be equivalent (e.g. through weak bisimulation)
to the two processes connected simply through a perfect medium. Fig. \ref{SimpleProt:Spec}, right, shows the pNet representing this global specification.

Fig. \ref{SimpleProt:Impl} show the pNet model of the protocol. When the medium detects an error (modeled by a local $\tau$ action), it sends back a $m\_error$ message to the $Sender$. The $Sender$ increments its error counter $c$, and resend the message (including $c$) to the medium, that will, eventually, transmit $m,c$ to the receiver.
...
\subsection{Open Automata}
 Open automata (OA) are not composition structures but they are made of transitions that are dependent of the actions of the holes, and they can reason on variables (potentially with only symbolic values). Formaly, each state of an OA has set of \emph{state variables}, and all state variable sets are disjoint.
...
\begin{definition}[Open transitions]\label{def:OT}
	...
	equations that hold \emph{after the open transition}, they are
...
\TODO{:  and maybe one example on the Impl. OTs presented in latex readable form??}

\begin{figure}[t]
   \centerline{\includegraphics[width=9cm]{ATG/SPSpecOpen}}
   \caption{Specification OA (version with the error counter)}
...
\paragraph{Lemma~\ref{lem-compose}} {\bf Open transition composition}\\
...
\end{definition}
\TODO{Predicate=True me pose un probleme... typiquement, on a besoin d'une ``condition initiale'', pour avoir des pNets data-oriented equivalents a leur version state-oriebted}
...
Now the semantics of an open pNet is defined both as an open automaton, and as a weak open automaton. Like the open automaton, the weak open automaton features valuable composition properties. We can exhibit  a composition property and a decomposition property that relate open pNet composition with their semantics, defined as weak open automata. These are technically more complex than the ones for open automata because each hole performs a set of actions, and thus a composed transition is the composition of one transition of the top-level pNet and a sequence of transitions of the sub-pnet that fills its hole.
...
open trnsition from each sequence. Here is one example of a short sequence,
...
firsts open transitionTs are internal, the last one is visible, so it constitute a
...
The proof is trivial.
 ...
 \\~\qquad\quad \Pred\,'\land\alpha_Q=\beta_{j_0} \\
 	\implies
 	 \hspace{-2ex}
 	{\displaystyle\bigvee_{x\in X}
 	...
 	~\qquad\quad 	\Pred_{s, x}\subst{\Post\,''\uplus\Post_{OT_x}})
 	 $\end{small}
...
\end{mathpar}
 such that  $\forall a, J_{a}=\{j\in J_1'|\beta_{1j}\neq\tau\},   (s_{11},u_a|\Pred_{s_{11},u_a})\in \mathcal{R}$ and \\
...
$(\Pred_{s,t} \land \Pred_1)\wedge\Pred_2\subst{\Post_1}\\
 \hspace{1cm} \implies\!\!\! \displaystyle{\bigvee_{a\in A}}
   \!\!(\forall j\in J_a. \vis{\beta_{1j}}\!\!=\!\gamma_{ja}  \land \Pred_{OT_a}
    ...
    \forall j\in J_{b}. \vis{\beta_{2j}}\!=\!\gamma_{jb}  \land \Pred_{OT_{b}}\land \\~\qquad\qquad\alpha'\!=\!\alpha_{2 b}\! \land \Pred_{s_{21},v_{b}}\subst{\Post_2\uplus\Post_{OT_{b}}})\subst{\Post_1\uplus\Post_{OT_a}})$\\
This can be rewritten into:\\
$(\Pred_{s,t} \land \Pred_1)\wedge\Pred_2\subst{\Post_1}\\
\hspace{1cm} \implies\!\!\! \displaystyle{\bigvee_{a\in A}}\, \displaystyle{\bigvee_{b\in B}} 
(\forall j\in J_a. \vis{\beta_{1j}}\!\!=\!\gamma_{ja} \land  \forall j \in J_{b}. (\vis{\beta_{2j}}\!=\!\gamma_{jb})\subst{\Post_1\uplus\Post_{OT_a}}   \land\Pred_{OT_a} \land  \Pred_{OT_{b}} \subst{\Post_1\uplus\Post_{OT_a}} \land  (\alpha'\!=\!\alpha_{2 b}) \subst{\Post_1\uplus\Post_{OT_a}} \land \\ \Pred_{s_{21},v_{b}}\subst{\Post_2\shortotimes\Post_1\uplus \Post_{OT_{b}}\shortotimes \Post_{OT_a}})$\\ 
...
$(\Pred_{s,t} \land \Pred_1)\wedge\Pred_2\subst{\Post_1}\\
\hspace{1cm} \implies\!\!\! \displaystyle{\bigvee_{a\in A}}\, \displaystyle{\bigvee_{b\in B}} 
(\forall j\in J_a. \vis{\beta_{1j}}\!=\!\gamma_{ja} \land  \forall j \in J_{b}. \vis{\beta_{2j}}\subst{\Post_1}=\gamma_{jb} \subst{\Post_{OT_a}}  \land\Pred_{OT_a} \land  \Pred_{OT_{b}} \subst{\Post_{OT_a}} \land \alpha'\subst{\Post_1}=\alpha_{2 b} \subst{\Post_{OT_a}} \land \Pred_{s_{21},v_b}\\\subst{\Post_2\shortotimes\Post_1\uplus \Post_{OT_{b}}\shortotimes \Post_{OT_a}})$\\ 
...
$\Pred_{s,t} \land \Pred_{OT}\\
\hspace{1cm} \implies\!\!\! \displaystyle{\bigvee_{a\in A}} \displaystyle{\bigvee_{b\in B}} \displaystyle{\bigvee_{c\in C}} 
(\forall j\in J.\gamma_j'=\gamma_j \land  \Pred \land \alpha=\alpha''  \land \Pred_{s',w_{c}}\subst{\Post_{OT}\uplus\Post\,}$\\
...
$\Pred_{s,t} \land \Pred_{OT}\\
\hspace{1cm} \implies\!\!\! \displaystyle{\bigvee_{x\in X}} 
(\forall j\in J.\gamma_j'=\gamma_j \land  \Pred \land \alpha=\alpha''  \land \Pred_{s',t_{x}}\subst{\Post_{OT}\uplus\Post\,}$\\
\qed
\end{proof}
...
\item Finally, for the open transition in the form ($\set{\beta_{2}}=\beta_{2j}^{j\in J_2}$): $P[Q]_{j_0}	\models \openrule
         {
           \beta_{2j}^{j\in J_2},\Pred_2,\Post_2 }
...
Note that for all $k\in[0..n-1]$, $v_{kl}'=v_{(k+1)l}$,  $v_{0l}=s_{0l}=s_l$, and $v'_{nl}=v_{(n_1+n_2+3) l}=u_{(n_2+1) l} = s'_l$.

By definition of $\Post_k$, we have $\displaystyle{\bigotimes_{j=n'_{k}-1}^{0}\!\!\Post''_{j} = \bigotimes_{j=k-1}^{0}\!\!\Post_{j}}$.
...
\end{mathpar}
such that\\ 
{\footnotesize $\alpha_Q=\alpha'_Q\displaystyle{\subst{\bigotimes_{j=k}^{0}\Post_{1j}}}$\\
...
$\alpha=\alpha'\displaystyle{\subst{\bigotimes_{j=m_1}^{0}\Post_{1j}}}$ \\
%\begin{multline*}
$\gamma_j^{j\in J}=\displaystyle{\mybigdotcup_{i=0}^{m_1} \vis{{\beta_{1ij}^{j\in J_{1p}}}\subst{\bigotimes_{k=i-1}^{0} \Post_{1k} } }  \dotcup \, \vis{{\beta_{2j}^{j\in J_{2}}}\subst{\bigotimes_{k=m_1}^{0}\Post_{1k}}}} \\
~~\qquad~\dotcup \mybigdotcup_{i=0}^{m_2} \vis{\beta_{3ij}^{j\in J_{3p}} \subst{\bigotimes_{k=i-1}^{0}\Post_{3k}\shortotimes\Post_2\shortotimes\bigotimes_{k=m_1}^{0}\Post_{1k}}}$\\
%\end{multline*}
%\begin{multline*}
...
~~\quad =\displaystyle{\mybigdotcup_{\substack{i=0\\j_0\in J_{1i}}}^{m_1} \vis{{\beta_{1ij_0}}\subst{\bigotimes_{k=i-1}^{0} \Post_{1k} } }  \dotcup \vis{{\beta_{2j_0}}\subst{\bigotimes_{k=m_1}^{0}\Post_{1k}}}} \\
~~\quad~\displaystyle{ \dotcup \mybigdotcup_{\substack{i=0\\j_0\in J_{3i}}}^{m_2} \vis{{\beta_{3ij_0}} \subst{\bigotimes_{k=i-1}^{0}\Post_{3k}\shortotimes\Post_2\shortotimes\bigotimes_{k=m_1}^{0}\Post_{1k}}}}$\\
...
%\\
\Pred\,''=\bigwedge_{i=0}^{m_1}\Pred\,'_{1i}\subst{\bigotimes_{j=i-1}^{0}\Post\,'_{1j}}\land\Pred\,'_2 \subst{\bigotimes_{j=m_1}^{0}\Post\,'_{1j}}\land\\ 
~\qquad\quad\Big(\bigwedge_{i=0}^{m_2}\Pred\,'_{3i}\subst{\bigotimes_{j=i-1}^{0}\Post\,'_{3j}\shortotimes\Post\,'_2\shortotimes\bigotimes_{j=m_2}^{0}\Post\,'_{1j}}\Big)\\
\Post\,''=\bigotimes_{j=m_2}^{0}\Post\,'_{3j}\shortotimes\Post\,'_2\shortotimes\bigotimes_{j=m_1}^{0}\Post\,'_{1j}
$
...
\begin{multline*}
\gamma'_j=\mybigdotcup_{i=0}^{m_1} \vis{{\beta_{1ij}}\subst{\bigotimes_{k=i-1}^{0} \Post_{1k} } }  \dotcup  \vis{{\beta_{2j}}\subst{\bigotimes_{k=m_1}^{0}\Post_{1k}}} \\
\dotcup \mybigdotcup_{i=0}^{m_2} \vis{\beta_{3ij} \subst{\bigotimes_{k=i-1}^{0}\Post_{3k}\shortotimes\Post_2\shortotimes\bigotimes_{k=m_1}^{0}\Post_{1k}}}
\end{multline*}
...
\mybigdotcup_{i=0}^{m_1}{\gamma'_{1ij}}\subst{\bigotimes_{k=i-1}^{0}\Post\,'_{1k}}\dotcup
 ...
 \mybigdotcup_{i=0}^{m_2}{\gamma'_{3ij}}\subst{\bigotimes_{k=i-1}^{0}\Post\,'_{3k}\shortotimes\Post\,'_2\shortotimes\bigotimes_{k=n}^{0}\Post\,'_{1k}}\\
&=
...
&=\Big(\bigwedge_{p=0}^{m_1}\!\Pred_{1p}\subst{\bigotimes_{j=p-1}^{0}\Post_{1j}}\land\Pred_2\subst{\bigotimes_{p=m_1}^{0}\Post_{1p}}\land\\&\qquad \bigwedge_{p=0}^{m_2}\!\Pred_{3p}\subst{\bigotimes_{j=p-1}^{0}\Post_{3j}\otimes\Post_2 \otimes\bigotimes_{p=m_1}^{0}\Post_{1p}}\Big)\\&\qquad
...
This allows us to conclude concerning the lemma. \qed
\end{proof}
...
\begin{figure}[t]
   \includegraphics[width=5.5cm]{XFIG/SimpleProt-Schema}
   \includegraphics[width=6.5cm]{XFIG/SimpleProt-Spec}
   \caption{Specification pNet Schema and Specification}
   \label{SimpleProt:Spec}
...
\begin{figure}[t]
  \centerline{\includegraphics[width=12cm]{XFIG/SimpleProt-pNet}}
  \caption{Composed pNet with the Simple Protocol Implementation}  \label{SimpleProt:Impl}
\end{figure}

The first piece of code is the textual definition of the SimpleProtSec
pNet. This code should be intuitive enough to read, with the following
conventions:
...
state a0
transition in(msg) -> a1 {a1_msg:=msg}

state a1
vars a1_msg:Data {m1_c:=0}:Int

transition out(a1_msg, m1_c) -> a0
transition synchro($tau)  -> a1 {m1_c:=m1_c+1}
...
vector SV0 <p_send(m), in(m),_>->p_send(m)
vector SV1 <p_a,_,_>->p_a [p_a != p_send(m)]
vector SV2 <_, out(m,c), q_recv(m)>->q_recv(m,c)
vector SV3 <_,_,q_a>->q_a [q_a != q_recv(m)]

  \end{lstlisting}

The corresponding generated Open Automaton was given in Figure \ref{},
page \pageref{}.
...
const s_send,s_ack,s_error,r_recv,r_ack: Action
...
state s0
transition in(msg) -> s1 {s1_msg:=msg, s1_c:=0}

state s1
vars s1_msg:Data s1_c:Int
transition s_send(s1_msg, s1_c) -> s2 
...
vars m:Data c:Int
vector SV0 <in(m), _, _>->in(m)
vector SV1 <s_send(m,c), m_recv(m,c), _>->synchro($tau)
vector SV2 <_, m_send(m), r_recv(m)>->synchro($tau)
vector SV3 <$s_ack, _, $r_ack>->synchro($tau)
vector SV4 <$s_error, $m_error, _>->synchro($tau)
vector SV5 <_, _, out(m,c)>->out(m,c)

pNet SimpleProtImpl2
holes P,Q
subnets P,medium,Q
vars p_a,q_a:Action  m:Data c:Int
vector SV0 <p_send(m), in(m),_>->in(m)
vector SV1 <p_a,_,_>->p_a [p_a != p_send(m)]
vector SV2 <_, out(m,c), q_recv(m,c)>->out(m,c)
...
SV12 SVs: <_, _tau_, _>-->_tau_
SV13 SVs: <_, _tau_, _>-->_tau_
SV14 SVs: <_, _tau_, _>-->_tau_
SV15 SVs: <_, _tau_, _>-->_tau_
Holes: P
Subnets: medium
- medium
...
Finaly, we list here, in raw format, the 24 OTs computed by the tool:

\begin{lstlisting}[basicstyle=\scriptsize\ttfamily, language=java, frame=single]
INFO  SimpleProtImpl2 - Total: 24 OTs
INFO  SimpleProtImpl2 - 
		{P|->p_send(m:sva_SV6:1:1)},   [((in(msg))=:ra:Sender:121:1)/\(:ra:Sender:121:1=(in(m:sva_SV0:12:1)))/\((in(m:sva_SV0:12:1))=(in(m:sva_SV6:1:1)))],   {s1_msg := msg, s1_c := 0}
	OT:1 ------------------------------------------------------------
		<s0_a0_r0>-----in(m:sva_SV6:1:1)----><s1_a0_r0>

INFO  SimpleProtImpl2 - 
		{P|->p_a:sva_SV7:1:2},   [(forall:m:sva_SV7:1:2. (p_a:sva_SV7:1:2!=(p_send(m:sva_SV7:1:2)))],   {}
	OT:2 ------------------------------------------------------------
		<s0_a0_r0>-----p_a:sva_SV7:1:2----><s0_a0_r0>

INFO  SimpleProtImpl2 - 
		{Q|->q_a:sva_SV9:1:4},   [(forall:m:sva_SV9:1:4,c:sva_SV9:1:4. (q_a:sva_SV9:1:4!=(q_recv(m:sva_SV9:1:4, c:sva_SV9:1:4)))],   {}
	OT:3 ------------------------------------------------------------
		<s0_a0_r0>-----q_a:sva_SV9:1:4----><s0_a0_r0>

INFO  SimpleProtImpl2 - 
		{P|->p_a:sva_SV7:1:2},   [(forall:m:sva_SV7:1:2. (p_a:sva_SV7:1:2!=(p_send(m:sva_SV7:1:2)))],   {}
	OT:4 ------------------------------------------------------------
		<s1_a0_r0>-----p_a:sva_SV7:1:2----><s1_a0_r0>

INFO  SimpleProtImpl2 - 
		{Q|->q_a:sva_SV9:1:4},   [(forall:m:sva_SV9:1:4,c:sva_SV9:1:4. (q_a:sva_SV9:1:4!=(q_recv(m:sva_SV9:1:4, c:sva_SV9:1:4)))],   {}
	OT:5 ------------------------------------------------------------
		<s1_a0_r0>-----q_a:sva_SV9:1:4----><s1_a0_r0>

INFO  SimpleProtImpl2 - 
		{},   [((s_send(s1_msg, s1_c))=:ra:Sender:1144:1)/\(:ra:Sender:1144:1=(s_send(m:sva_SV1:114:2, c:sva_SV1:114:2)))/\((m_recv(msg, c))=:ra:M1:1145:1)/\(:ra:M1:1145:1=(m_recv(m:sva_SV1:114:2, c:sva_SV1:114:2)))/\(_tau_=_tau_)],   {a1_msg := msg, a1_c := c}
	OT:6 ------------------------------------------------------------
		<s1_a0_r0>-----_tau_----><s2_a1_r0>

INFO  SimpleProtImpl2 - 
		{},   [((s_send(s1_msg, s1_c))=:ra:Sender:1174:1)/\(:ra:Sender:1174:1=(s_send(m:sva_SV1:117:2, c:sva_SV1:117:2)))/\((m_recv(msg, c))=:ra:M1:1175:1)/\(:ra:M1:1175:1=(m_recv(m:sva_SV1:117:2, c:sva_SV1:117:2)))/\(_tau_=_tau_)],   {a1_msg := msg, a1_c := c}
	OT:7 ------------------------------------------------------------
		<s1_a0_r0>-----_tau_----><s2_a1_r0>

INFO  SimpleProtImpl2 - 
		{},   [((s_send(s1_msg, s1_c))=:ra:Sender:1204:1)/\(:ra:Sender:1204:1=(s_send(m:sva_SV1:120:2, c:sva_SV1:120:2)))/\((m_recv(msg, c))=:ra:M1:1205:1)/\(:ra:M1:1205:1=(m_recv(m:sva_SV1:120:2, c:sva_SV1:120:2)))/\(_tau_=_tau_)],   {a1_msg := msg, a1_c := c}
	OT:8 ------------------------------------------------------------
		<s1_a0_r0>-----_tau_----><s2_a1_r0>

INFO  SimpleProtImpl2 - 
		{},   [((s_send(s1_msg, s1_c))=:ra:Sender:1234:1)/\(:ra:Sender:1234:1=(s_send(m:sva_SV1:123:2, c:sva_SV1:123:2)))/\((m_recv(msg, c))=:ra:M1:1235:1)/\(:ra:M1:1235:1=(m_recv(m:sva_SV1:123:2, c:sva_SV1:123:2)))/\(_tau_=_tau_)],   {a1_msg := msg, a1_c := c}
	OT:9 ------------------------------------------------------------
		<s1_a0_r0>-----_tau_----><s2_a1_r0>

INFO  SimpleProtImpl2 - 
		{},   [((s_send(s1_msg, s1_c))=:ra:Sender:1264:1)/\(:ra:Sender:1264:1=(s_send(m:sva_SV1:126:2, c:sva_SV1:126:2)))/\((m_recv(msg, c))=:ra:M1:1265:1)/\(:ra:M1:1265:1=(m_recv(m:sva_SV1:126:2, c:sva_SV1:126:2)))/\(_tau_=_tau_)],   {a1_msg := msg, a1_c := c}
	OT:10 ------------------------------------------------------------
		<s1_a0_r0>-----_tau_----><s2_a1_r0>
...
INFO  SimpleProtImpl2 - 
		{P|->p_a:sva_SV7:1:2},   [(forall:m:sva_SV7:1:2. (p_a:sva_SV7:1:2!=(p_send(m:sva_SV7:1:2)))],   {}
	OT:11 ------------------------------------------------------------
		<s2_a1_r0>-----p_a:sva_SV7:1:2----><s2_a1_r0>

INFO  SimpleProtImpl2 - 
		{Q|->q_a:sva_SV9:1:4},   [(forall:m:sva_SV9:1:4,c:sva_SV9:1:4. (q_a:sva_SV9:1:4!=(q_recv(m:sva_SV9:1:4, c:sva_SV9:1:4)))],   {}
	OT:12 ------------------------------------------------------------
		<s2_a1_r0>-----q_a:sva_SV9:1:4----><s2_a1_r0>

INFO  SimpleProtImpl2 - 
		{},   [(_tau_=:ra:M1:11420:1)/\(:ra:M1:11420:1=_tau_)/\(_tau_=_tau_)],   {}
	OT:13 ------------------------------------------------------------
		<s2_a1_r0>-----_tau_----><s2_a2_r0>

INFO  SimpleProtImpl2 - 
		{},   [(_tau_=:ra:M1:11720:1)/\(:ra:M1:11720:1=_tau_)/\(_tau_=_tau_)],   {}
	OT:14 ------------------------------------------------------------
		<s2_a1_r0>-----_tau_----><s2_a2_r0>

INFO  SimpleProtImpl2 - 
		{},   [(_tau_=:ra:M1:12020:1)/\(:ra:M1:12020:1=_tau_)/\(_tau_=_tau_)],   {}
	OT:15 ------------------------------------------------------------
		<s2_a1_r0>-----_tau_----><s2_a2_r0>

INFO  SimpleProtImpl2 - 
		{},   [(_tau_=:ra:M1:12320:1)/\(:ra:M1:12320:1=_tau_)/\(_tau_=_tau_)],   {}
	OT:16 ------------------------------------------------------------
		<s2_a1_r0>-----_tau_----><s2_a2_r0>

INFO  SimpleProtImpl2 - 
		{},   [(_tau_=:ra:M1:12620:1)/\(:ra:M1:12620:1=_tau_)/\(_tau_=_tau_)],   {}
	OT:17 ------------------------------------------------------------
		<s2_a1_r0>-----_tau_----><s2_a2_r0>
...
INFO  SimpleProtImpl2 - 
		{P|->p_a:sva_SV7:1:2},   [(forall:m:sva_SV7:1:2. (p_a:sva_SV7:1:2!=(p_send(m:sva_SV7:1:2)))],   {}
	OT:18 ------------------------------------------------------------
		<s2_a2_r0>-----p_a:sva_SV7:1:2----><s2_a2_r0>

INFO  SimpleProtImpl2 - 
		{Q|->q_a:sva_SV9:1:4},   [(forall:m:sva_SV9:1:4,c:sva_SV9:1:4. (q_a:sva_SV9:1:4!=(q_recv(m:sva_SV9:1:4, c:sva_SV9:1:4)))],   {}
	OT:19 ------------------------------------------------------------
		<s2_a2_r0>-----q_a:sva_SV9:1:4----><s2_a2_r0>

INFO  SimpleProtImpl2 - 
		{},   [(s_error=:ra:Sender:11413:1)/\(:ra:Sender:11413:1=s_error)/\(m_error=:ra:M1:11414:1)/\(:ra:M1:11414:1=m_error)/\(_tau_=_tau_)],   {s1_c := s2_c+1}
	OT:20 ------------------------------------------------------------
		<s2_a2_r0>-----_tau_----><s1_a0_r0>

INFO  SimpleProtImpl2 - 
		{},   [(s_error=:ra:Sender:11713:1)/\(:ra:Sender:11713:1=s_error)/\(m_error=:ra:M1:11714:1)/\(:ra:M1:11714:1=m_error)/\(_tau_=_tau_)],   {s1_c := s2_c+1}
	OT:21 ------------------------------------------------------------
		<s2_a2_r0>-----_tau_----><s1_a0_r0>

INFO  SimpleProtImpl2 - 
		{},   [(s_error=:ra:Sender:12013:1)/\(:ra:Sender:12013:1=s_error)/\(m_error=:ra:M1:12014:1)/\(:ra:M1:12014:1=m_error)/\(_tau_=_tau_)],   {s1_c := s2_c+1}
	OT:22 ------------------------------------------------------------
		<s2_a2_r0>-----_tau_----><s1_a0_r0>

INFO  SimpleProtImpl2 - 
		{},   [(s_error=:ra:Sender:12313:1)/\(:ra:Sender:12313:1=s_error)/\(m_error=:ra:M1:12314:1)/\(:ra:M1:12314:1=m_error)/\(_tau_=_tau_)],   {s1_c := s2_c+1}
	OT:23 ------------------------------------------------------------
		<s2_a2_r0>-----_tau_----><s1_a0_r0>

INFO  SimpleProtImpl2 - 
		{},   [(s_error=:ra:Sender:12613:1)/\(:ra:Sender:12613:1=s_error)/\(m_error=:ra:M1:12614:1)/\(:ra:M1:12614:1=m_error)/\(_tau_=_tau_)],   {s1_c := s2_c+1}
	OT:24 ------------------------------------------------------------
		<s2_a2_r0>-----_tau_----><s1_a0_r0>

INFO  SimpleProtImpl2 - Total: 24 SATISFIABLE OTs
INFO  SimpleProtImpl2 -        278 UNSATISFIABLE OTs
INFO  SimpleProtImpl2 - Total Time: 1056 ms
