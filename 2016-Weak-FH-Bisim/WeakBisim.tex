...
\RAB{The weak open transition here is the transition delivering the result of the algorithm to hole $Q$ by applying or rules: \WTUn,\WTDeux, and \WTTrois. First rule \WTUn~ adds an $\texttt{WT}_\tau$ loop on each state. Rule \WTDeux~ transforms each 3 OTs into WOTs.   Then consider application of Rule \WTTrois~ on a sequence 3  WOTs.   $\openrule
  ...
  $\openrule
         ...
         {b1 \OTWeakarrow \tau b1} \forall n \geq 0$.  Finally,  applying again \WTTrois, and using the central open transition having \texttt{q\_recv(ec)}  as $\alpha$, we get:
      $\openrule
         {\{Q\mapsto\texttt{q\_revc(ec)}\subst{ec\gets ec+n}\}, True,
			(\texttt{ec}\gets \texttt{ec}+n) }
         {b1 \OTWeakarrow {\texttt{out(m,ec)}\subst{ec\gets ec+n}} b0} \forall n \geq 0$. } 
