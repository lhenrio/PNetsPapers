...
% REMOVED BECAUSE USELESS: $\iv(\alpha)\!\subseteq\! \vars(s')$, 
		...
		$\vars(e_b)\!\subseteq\! \vars(s)\cup\varsigma(\alpha)$, and
		$\forall j\!\in\! J .\,\vars(e_j)\!\subseteq\! \vars(s)\cup\varsigma(\alpha)\land 
...
$\Label(\symb{SV}_k) = \alpha'_k$. $g_k \in \AlgB$ is a guard associated to the vector 
...
in a pNet node and its subnets are all disjoint:
  \[\begin{array}{l}
\Holes(\mylangle S,s_0, \to\myrangle) \!=\! \mylangle S,s_0, \to\myrangle\\
...
inductively as:
\TODO{Not very nice but at least of the right kind for the definition of composition}
\[\begin{array}{l}
...
\TODO{this definition is not valid because L is undefined and leaves only exist for pNets 
-> REMOVE? this would mean that a pNet is not a pLTS ...}
...
%	equations of a predicate.
	...
	\LUDO{now with the magical operator ... $s_m$ and $s_m'$ are a bit ill-typed terms. 
	Not very happy of the new rule}
	\begin{description}
		\item[{\bf Tr2:}]
	\end{description}
    ...
    \alpha'_k \\
    	...
    	\\\forall m\in I_k. \CreateISet{s_m}{i}=s_{i}^{i \in L_m} \\
		\forall m\in I_k. \CreateISet{s_m'}{i}=(s_i^\prime)^{i\in L_m} \\
