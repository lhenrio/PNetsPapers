...
\footnote{when showing  the result of $Posts$ composition, we will Z omit the identity substitutions introduced bythe $\shortotimes$ definition in page \pageref{def:substitutions}}
\medskip
...
With~\ref{eqproofbeta}, we now have. Recall that the actions $\alpha_k$ are the actions $\alpha'_{p}$ that are observable: 
\begin{eqnarray*}
\Pred_\beta &\iff& \gamma_{j_0}=
