...
\RAB{d'ou vient l'action v? notation S\_j ou bien Sort\_j}
...
\begin{definition}[Operational semantics of open pNets]
	...
	\mathcal{T}>$ where:
	\begin{itemize}
		\item $J$ is the indices of the holes: $Holes(p)= H_j^{j\in J}$. \RAB{Holes(p)= J en ref a page 4}
...
\[\mathcal{R}'' = \Big\{(s^1,s^3|\Pred\,''\phi)\Big|
\displaystyle\Pred\,''=\bigvee_{ \begin{array}{l}
(s^1,s^2|\Pred)\in\mathcal{R}\\ (s^2,s^3|\Pred\,')\in\mathcal{R}' 	
\end{array}
}\!\!\!\!\!\exists \phi.\begin{array}[t]{l} (\Pred\land\Pred\,')\phi\\
\land \fv((\Pred\land\Pred)\phi)\subseteq \fv(\mathcal{T}_1)\cup\fv(\mathcal{T}_3)
\\
\land \dom(\phi)\subseteq \fv(\mathcal{T}_2)
\end{array}\Big\}\]

\TODO{is it satisfactory as a notion of transitive composition? It would be easy to 
extend it a bit to allow having several substitutions for the same s2, eg one that works 
for different values of variables in T1 and T3. It would make the notations more complex 
but it would be OK. I do not think I can do more than that}
...
Finally, a substitution should ensure that the 
free variables of the predicates are only the ones of $\mathcal{T}_1$ and 
$\mathcal{T}_1$. This substitution somehow matches the two predicates by eliminating the 
free variables.
