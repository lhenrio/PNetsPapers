...
\end{example}
  ...
  \begin{example}
    ...
    For readability, we did not include the full details of the Implementation weak open transitions in Figure \ref{SimpleProtCounter:ImplOA}, they will appear below. In this automaton, many WOT are similar, and numbered accordingly as 3, 3', 3'', and 8, 8', 8'' respectively: they only differ by the name of the state variables in their respective source or target states.
\end{example}
...
$post_7 = (\texttt{r1\_msg}\gets \texttt{m1\_msg}, \texttt{r1\_ec}\gets \texttt{m1\_ec})$
...
We can now show some of the weak OTs of Figure \ref{SimpleProtCounter:SpecOA-partial} (the full table is included in Appendix \ref{Appendix:FullExample}) :
\smallskip
...
$ 3' = \openrule
  {\{\texttt{P}\mapsto \texttt{p-send(m)}\}, True,
    (\texttt{m1\_msg}\gets m, \texttt{m1\_ec}\gets n)}
...
$ 8 = \openrule
         {\{\}, True, ()}
         {201 \OTWeakarrow {\underline{\texttt{out(m,c)}}\subst{(m\gets \texttt{r1\_msg}, c\gets \texttt{r1\_ec}}} 202}
         $
