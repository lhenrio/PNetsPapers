...
only a few entries will also be denoted $(3\mapsto a)$. \TODO{We need to define update as $A \uplus \{j \mapsto a\}$ or $A[j \mapsto a]$}
...
\[\Pred_{\symb{sv}}(SV_k, a_i^{i\in I}, b_j^{j\in J}, v)\Leftrightarrow \bigg(
\begin{array}{l}
\exists {(a'_i)}^{i\in I},
...
\begin{minipage}{0.30\linewidth}
	...
	\TODO{change fig: s1,s2 $\to$ s,t. DONE we will check if it is OK. Why did we use tilde notation for R}
\end{minipage}
...
\begin{definition}[pNet composition]
 ...
 can be (partially) filled by providing  a pNets $\pNet'$ of the
	...
	\]
\end{definition}
\TODO{$\uplus\{j_0\mapsto \pNet'\}$ non defini}
...
\begin{lemma}[OT decomposition]\label{lem-decompose} 
	...
	\]
		with $Q$ ``moving'' \TODO{or is reduced?}, i.e. $J\cap\Holes(Q)\neq\emptyset$ or $I\cap L_Q\neq 
		\emptyset$.
...
\TODO{Revoir la def des leaves set ou multisets}
...
\TODO{We need to add: Note that $J_{j_0}=	J\cap\Holes(Q)$ this is a direct consequence of Tr2??}
...
$SV=clone(\alpha_j^{j\in I_k\uplus\{j_0\}\uplus 
	J_k})$. $\Leaves(P[Q]_{j_0})=\Leaves(P)\uplus \Leaves(Q)$. All sub-pNets can be 
...
\end{small}
 ...
 t=t'\uplus s'' \land s''\in \mathcal{S}_P \land (s',t'|\Pred_{s,t})\in\mathcal{R}\}$ \TODO{$\uplus$ entre states?} 
