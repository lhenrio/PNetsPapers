...
Fig. \ref{SimpleProt:Impl} show the pNet model of the protocol implementation. When the $Medium$ detects an error (modeled by a local $\tau$ action), it sends back a $m\_error$ message to the $Sender$. The $Sender$ increments its local error counter $c$, and resends the message (including $c$) to the $Medium$, that will, eventually, transmit $m,c$ to the $Receiver$.
