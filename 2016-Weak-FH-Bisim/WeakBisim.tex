...
\RAB{La notation overline n'a pas ete introduite auparavant? Aussi, je n'ai pas trop bien 
saisi quand on utilise overline et la la notation k in K}\LUDO{DONE}
...
\RAB{Exemple  Fig3 a revoir: Les regles ne passent... }
...
More precisely, for any pair $(s^1,s^2)$, there is a 
   single
      $(s^1,s^2|\Pred_{s^1,s^2})\in\mathcal{R}$  stating that $s^1$ and $s^2$ are related 
      if $\Pred_{s^1,s^2}$ 
      is 
      true, i.e. the states are related when the variables in $s^1$ and $s^2$ verify the 
      predicate $\Pred_{s^1,s^2}$.
      
\RAB{ I propose:
Let $\mathcal{R}=\{(s^1,s^2|\Pred_{s^1,s^2})\}$ be a relation over the set 
$\mathcal{S}_1$ and $\mathcal{S}_2$ constrained by a predicate: if $(s^1,s^2) \in 
\mathcal{R}$ then $\Pred_{s^1,s^2}$ is  true.}  \LUDO{No : this is not the meaning}   
