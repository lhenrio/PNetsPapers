...
\hline
  ...
  {b0 \OTarrow {\underline{\texttt{in(m1)}}} b1}
  $
...
\subst{(\texttt{b1\_msg}\gets \texttt{m1}, \texttt{b1\_ec}\gets 0)
  \uplus (\texttt{s1\_msg}\gets \texttt{m2}, \texttt{s1\_ec}\gets 0)})\}
$
...
\exists n.
( True \land \tau=\tau
\land (b1_msg = m1_msg \land b1_ec = m1_ec)  \subst{\texttt{m1\_msg}\gets \texttt{s1\_msg}, \texttt{m1\_ec}\gets \texttt{s1\_ec}, \texttt{s2\_ec}\gets \texttt{s1\_ec}}
