...
\begin{example}
 ...
 WOT2 here is the transition delivering the result of the algorithm to hole $Q$. Remark that it includes a loop increasing the error counter, occurring before the action of $Q$, and this action is using the resulting value of the counter. Let us go into the details, using rules WT1/2/3.  First WT1 adds a $\tau$ loop on each state. Then consider application of Rule WT3 on a sequence 3 internal actions ``$\tau\subst{ec\gets ec+1};\tau\subst{ec\gets ec+1};\tau$''. Hole actions and predicates are empty, we just concentrate on the Post operations. Rule WT2 transforms each 3 OTs into WOTs.  Then WT3 produces the weak OT:
  ...
  $\openrule
         ...
         For any of these, apply again WT3, using the central \texttt{q\_recv(ec)} transition as $\alpha$, and you get:
      $\openrule
         {\{Q\mapsto\texttt{q\_revc(ec)}\subst{ec\gets ec+n}\}, True,
			\subst{ec\gets ec+n} }
...
Now let us give some details about the construction of the weak automation of the implementation pNet, obtained by application of the weak rules as explained above. Here we denote as $post_n$ the effect (= susbtitution) of the Strong transition $OT_n$ from Figure \ref{SimpleProtCounter:ImplOA}:
\smallskip
