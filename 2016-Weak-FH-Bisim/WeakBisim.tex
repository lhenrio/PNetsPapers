...
\begin{enumerate}
\item For  $\symb{OT}$ in the form: 
$P[Q]_{j_0}	\models		\openrule
    {
       \set{\beta}_p,\Pred_p,\Post_p   }
         {\ostate{s_p} \OTarrow {\tau} \ostate{s^1_{p+1}}}$ \\
We obtain:\\
$P\models{\openrule
			{
				...
				{\Pred_p}\!\!',  
				{\Post_p}\!\!'}
			{\ostate{(s_{p})_l^{l\in L\setminus L_Q}} \OTarrow {\tau}
				\ostate{({s}_{p}')_l^{l\in L\setminus L_Q}}}
		}%\]
	\,\text{~~and~~}
		%\[
		Q\models{\openrule
			{
				...
				{\Post_p''}}
			{\ostate{(s_{p})_l^{l\in L_Q}} \OTarrow {\alpha_p}
				\ostate{({s_{(p+1)}})_l^{l\in L_Q}}}
		}%\]
		$\\
and  $\Pred_p \iff {\Pred_p}\!\!'
		\land {\Pred_p}\!\!''\land \alpha_p=\beta_{pj_0}$, $\Post_p={\Post_p}\!\!'\uplus 
		{\Post_p}\!\!''$ where ${\Post_p''}$ is the restriction of $\Post_p$ over variables of $\Leaves(Q)$.
\item For $\symb{OT}$ in the form: $P[Q]_{j_0}	\models \openrule
         {
           \set{\beta},\Pred,\Post }
         {\ostate{s} \OTarrow {\alpha} \ostate{s'}}$\\
We obtain:\\
		...
		P\models{\openrule
			{
				...
				{\Pred}\,',  
				{\Post}\,'}
			{\ostate{s_l^{l\in L\setminus L_Q}} \OTarrow {\alpha}
		...
		Q\models{\openrule
			{
				\beta_j^{j\in J\cap\Holes(Q)}, {\Pred}\,'',  
				{\Post}\,''}
			{\ostate{s_l^{l\in L_Q}} \OTarrow {\alpha'}
				\ostate{s_l'^{\,l\in L_Q}}}
		}%\]
		$
		and  $\Pred \iff {\Pred}\,'
		\land {\Pred}\,''\land \alpha'=\beta_{j_0}$, $\Post={\Post}\,'\uplus 
		{\Post}\,''$ where ${\Post}\,''$ is the restriction of $\Post$ over variables of $\Leaves(Q)$.

\end{enumerate}
By using  Lemma \ref{lem-rel-OT-WOT} we can conclude that we have:
...
Where \TODO{I will add the rest}

Considering the set of $\symb{OT}_p$ of $Q$ such that $p \in [0\ldots m+n+1]$, at each time a single open transition moves.   So we have for each open transition:
...
By definition \ref{def:buildweakOT} we can conclude that we have for all $p\in[0\ldots n+m]$:

\begin{mathpar}
Q\models{\openrule
			{
				{\gamma_{pj}^{j\in J_p}}, {\Pred_p},  
				{\Post_p}'}
...
Where $J_p = J\cap\Holes(Q)$, ???? $\gamma_{pj}=\vis{\beta_{pj}}$,\\  $\Pred \iff \Pred\,'
