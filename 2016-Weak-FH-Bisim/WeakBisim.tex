...
\openrule
         ...
         {s^2 \OTarrow {\alpha} s'^2} \in \mathcal{T}
\quad \wedge\\
...
\Bigg)        
       \bigvee\\   
 \Big(\alpha = \tau \wedge  \set{\gamma}=\emptyset \wedge \Pred =\True \wedge \Post =\Id(s)\wedge s=s'\Big)  \implies  {
\openrule
         {
           \set{\gamma},
		\Pred, \Post
...
\openrule
         ...
         {
           \set{\beta},\Pred,\Post}
         {s \OTWeakarrow {\alpha} s'} \in \mathcal{T}         
\end{mathpar}
...
\bigg( \forall i \in [0\ldots n_1].\openrule
    {
       \set{\beta^1_i},\Pred^1_i,\Post^1_i   }
         {s^1_i \OTarrow {\tau} s^1_{i+1}} \in \mathcal{T}         \bigg)
\end{mathpar}
where \\
$ 
...
\bigg( \forall i \in [0\ldots m_1].\openrule
    {
       \set{\beta^2_i},\Pred^2_i,\Post^2_i   }
         {s^2_i \OTarrow {\tau} s^2_{i+1}} \in \mathcal{T}         \bigg)
\end{mathpar}
where\\
$
...
\Bigg(\forall i \in [0\ldots n_2].\openrule
    {
       \set{\beta^3_i},\Pred^3_i,\Post^3_i   }
         {s^3_i \OTarrow {\tau} s^3_{i+1}} \in \mathcal{T}  \wedge

\openrule
         {
           \set{\beta^4},\Pred^4,\Post^4 }
         {s^4 \OTarrow {\alpha} s'^4} \in \mathcal{T}
 \wedge
\forall i \in [0\ldots m_2].\openrule
         {
           \set{\beta^5_i},\Pred^5_i,({x^5_k\gets e^5_k})^{k\in K}    }
         {s^5_i \OTarrow {\tau} s^5_{i+1}} \in \mathcal{T}
\Bigg)         
\end{mathpar}
where\\
$
...
\Bigg(\forall i\in [0\ldots(n_1\!+\!n_2)].\openrule
    {
       \set{\beta^6_i},\Pred^6_i,\Post^6_i   }
         {s^6_i \OTarrow {\tau} s^6_{i+1}} \in\mathcal{T} \wedge
         
\openrule
         {
           \set{\beta^7},\Pred^7,\Post^7 }
         {s^7 \OTarrow {\alpha} s'^7} \in \mathcal{T}
\wedge
\forall i \in [0\ldots(m_1\!+\!m_2)].\openrule
         {\set{\beta^8_i},\Pred^8_i,({x^8_k\gets e^8_k})^{k\in K}    }
         {s^8_i\OTarrow {\tau} s^8_{i+1}}\in\mathcal{T}\Bigg)  
\end{mathpar}
...
%\inferrule{
 \openrule
    {
       \set{\beta^3},\Pred^3,\Post^3_i   }
         {s^2 \OTarrow {\tau} s'} \in \mathcal{T}          
\end{mathpar}
...
On other hand, by applying (\WTTrois)~rule  on each  $\symb{WOT}_x$ we have:
...
\hspace{1cm} \implies\!\!\! \displaystyle{\bigvee_{x\in X}}
   \left( \forall j\in J_x. \vis{\beta^1_j}=\gamma_{jx}  \Rightarrow \Pred_{OT_x}
     \land \alpha_x\!=\!\tau \land  
     \Pred_{s^1,t_x}\subst{\Post^1}\subst{\Post_{OT_x}}\right)$\\
Applying the conjunction  $\Pred^2\subst{\Post^1}$ to both sides of the implication, we get:\\  
$(\Pred_{s,t} \land \Pred^1)\wedge\Pred^2\subst{\Post^1}\\
 \hspace{1cm} \implies\!\!\! \Big(\displaystyle{\bigvee_{x\in X}}
   \left( \forall j\in J_x. \vis{\beta^1_j}=\gamma_{jx}  \Rightarrow \Pred_{OT_x}
     \land \alpha_x\!=\!\tau \land  
     \Pred_{s^1,t_x}\subst{\Post^1}\subst{\Post_{OT_x}}\right) \Big) \wedge\Pred^2\subst{\Post^1} \subst{\Post_{OT_x}}  \\
$
Note because of the domain independence of the substitution, we have just added  $\subst{\Post_{OT_x}}$ on the right side. A little rewriting gives:\\
$(\Pred_{s,t} \land \Pred^1)\wedge\Pred^2\subst{\Post^1}\\
 \hspace{1cm} \implies\!\!\! \Big(\displaystyle{\bigvee_{x\in X}}
   \left(\forall j\in J_x. \vis{\beta^1_j}\!=\!\gamma_{jx}\Rightarrow \Pred_{OT_x}
     \!\land\! \alpha_x\!=\!\tau\land(\Pred_{s^1,t_x}\wedge\Pred^2)\subst{\Post^1}\subst{\Post_{OT_x}} \right)\Big)$\\
By replacing $(\Pred_{s^1,t_x}\wedge\Pred^2)$ sub-term,  the formula becomes: \\
$(\Pred_{s,t} \land \Pred^1)\wedge\Pred^2\subst{\Post^1}\\
 \hspace{1cm} \implies\!\!\! \Big(\displaystyle{\bigvee_{x\in X}}
   (\forall j\in J_x. \vis{\beta^1_j}\!\!=\!\!\gamma_{jx}  \Rightarrow \Pred_{OT_x}
     \!\land\! \alpha_x\!=\!\tau \land \Big(\displaystyle{\bigvee_{y\in Y}}
   ( \forall j\in J_{x y}. \vis{\beta^2_j}\!\!=\!\!\gamma_{jx y}\Rightarrow \Pred_{OT_{x y}}\land \alpha\!=\!\alpha_{x y} \land \Pred_{s^2,t_{x y}}\subst{\Post^2}\subst{\Post_{OT_{x y}}})\Big)\subst{\Post^1} \subst{\Post_{OT_x}} )\Big)$\\
...
\Big(\forall j\in J_x. \vis{\beta^1_j}\!\!=\!\!\gamma_{jx} \land  \forall j \in J_{x y}. \vis{\beta^2_j}\!\!=\!\!\gamma_{jx y}\Rightarrow\Pred_{OT_x} \land  \Pred_{OT_{x y}} \subst{\Post^1} \subst{\Post_{OT_x}} \land \alpha_x\!=\!\tau \land \alpha\!=\!\alpha_{x y} \land \Pred_{s^2,t_{x y}}\subst{\Post^2}\subst{\Post^1}\subst{\Post_{OT_{x y}}} \subst{\Post_{OT_x}}\Big)$\\ 
Finally, applying the conjunction $\Pred^3\subst{\Post^2}\subst{\Post^1}$ to both sides of the implication and rewriting, we get:\\
...
\\
\Pred=\bigwedge_{i=0}^{m_1}\Pred^1_i\land\Pred\,^2\bigotimes_{i=0}^{m_1}\subst{\Post_i^1}\land (\bigwedge_{i=0}^{m_2}\Pred\,^3_i\subst{\Post^2})\bigotimes_{i=0}^{m_1}\subst{\Post^1_i}\\
\Post=(\bigotimes_{i=0}^{m_2}\Post^3_i)\subst{\Post^2}\bigotimes_{i=0}^{m_1}\subst{\Post^1_i}
$
...
$P\models{\openrule
			{
				(\beta_{p})_j^{j\in (J\setminus \Holes(Q)) \cup \{j_0\}}, 
				{\Pred_p}\!\!',  
				{\Post_p}\!\!'}
	...
	\,\text{~~and~~} 
		Q\models{\openrule
			{
				(\beta_{p})_j^{j\in J\cap\Holes(Q)}, {{\Pred_p}\!\!''},  
				{{\Post_p}\!\!''}}
...
such that  $\Pred^1_p \iff {\Pred_p}\!\!'
		\land {\Pred_p}\!\!''\land \alpha_p=\beta_{pj_0}$, $\Post^1_p={\Post_p}\!\!'\uplus 
		{\Post_p}\!\!''$ where ${{\Post_p}\!\!''}$ is the restriction of $\Post^1_p$ over variables of $\Leaves(Q)$.\\
Else $Q$ does not move and \\
$P\models{\openrule
			{
				(\beta_{p})_j^{j\in (J\setminus \Holes(Q)) }, 
				{\Pred^1_p},  
				{\Post^1_p}}
			{\ostate{(s_{p l})^{l\in L\setminus L_Q}} \OTarrow {\tau}
...
\item Similarly for tau transitions on states named $u$.

\item And, for the open transition $\symb{OT}$ in the form: $P[Q]_{j_0}	\models \openrule
         {
           \set{\beta^2},\Pred^2,\Post^2 }
...
such that  $\Pred^2 \iff {\Pred}\,'
		\land {\Pred}\,''\land \alpha'=\beta_{j_0}$, $\Post^2={\Post}\,'\uplus 
		{\Post}\,''$ where ${\Post}\,''$ is the restriction of $\Post^2$ over variables of $\Leaves(Q)$.
...
By using  Lemma \ref{lem-rel-OT-WOT} we can conclude from case 1, 2 and 3 that we have:
\begin{mathpar}
		P\models{\openrule
			{
...
$s_l=s_{0l} \wedge s_{{m_1}l}=t_l \wedge t'_l = u_{0l} \wedge s'_l=s_{{m_2}l}$
\TODO{Avant d'ajouter la suite, il faudrait se mettre d'accord sur les notations}\\

Concerning $Q$, we consider only the paths for which it moves. So for each, state such that we have $\ostate{(s_{p})_l^{l\in L_Q}} = \ostate{({s}_{(p+1) l})^{l\in L_Q}}$, we skip the state $\ostate{({s}_{(p+1) l})^{l\in L_Q}}$ and  we rename all its following states  $\ostate{(s_{p})_l^{l\in L_Q}}$ by $\ostate{({s}_{(p-1) l})^{l\in L_Q}}$. We proceed in the same way as for states named $u$. We also consider for the third case $Q$  moves, i.e, $\ostate{t_l^{l\in L_Q}} \neq \ostate{t_l'^{\,l\in L_Q}}$. So we have: \\ 
...
Let us consider the first item, in the series of $(n_1+1)$  actions $\alpha_p$ some of them may be non-observable ($\tau$ transition). By considering $\tau$  actions we split the sequence of $n_1+1$ actions into subsequences, such that each subsequence is a series of  $\tau$ transitions  ending by not more than one observable transition. For each subsequence we can write the  following:\\
$Q\models{\openrule
			{
				(\beta_{i})_j^{j\in J\cap\Holes(Q)}, {{\Pred_i}\!\!''},  
				{{\Post_i}\!\!''}}
				{\ostate{s_{il}^{l\in L_Q}} \OTarrow {\tau}
				\ostate{s_{(i+1)l}^{l\in L_Q}}}
				}%\]
, Q\models{\openrule
			{
				(\beta_{(i+1)})_j^{j\in J\cap\Holes(Q)}, {{\Pred''_{(i+1)}}},  
				{{\Post''_{(i+1)}}}}
				{\ostate{s_{(i+1)l}^{l\in L_Q}} \OTarrow {\tau}
				\ostate{s_{(i+2)l}^{l\in L_Q}}}
				}%\]
\ldots\\
Q\models{\openrule
			{
				(\beta_{k})_j^{j\in J\cap\Holes(Q)}, {{\Pred_k}\!\!''},  
				{{\Post_j}\!\!''}}
				{\ostate{s_{kl}^{l\in L_Q}} \OTarrow {\alpha_k}
				\ostate{s_{(k+1)l}^{l\in L_Q}}}
				}%\]
$\hspace{0.5cm}
where $1\leq i<n_1$ and $i<k$.\\
By Lemma \ref{lem-rel-OT-WOT} we obtain the following weak open transition:
\begin{mathpar}
Q\models{\openrule
			{
			(\gamma_p)_j^{j\in J\cap\Holes(Q)}, {\Pred_p}, {\Post_p}}
			{\ostate{s_{pl}^{l\in L_Q}} \OTWeakarrow {\alpha_{k}}
				\ostate{s_{(p+1)l}^{l\in L_Q}}}
		}%\]
\end{mathpar}

where $s_{il} = s_{pl}$ and $s_{(p+1)l}=s_{(k+1)l}$ 
\TODO{et le reste}
 
Particular attention should be paid to the last subsequence which may not end by an observable action. We proceed in the same way we obtain an non-observable weak open transition; and by applying  rule (\WTTrois) to the resulting non-observable weak open transition and the previous weak open transition we obtain a weak open transition.

By iterating this reasoning over the states $u$ and $t$  one have that: for each  $\symb{OT}_p$ of $Q$ such that $p \in S$ and $S \subseteq [0\ldots (n_1\!+\!n_2)\!+\!1]$ in the form and $\alpha_p \neq \tau$:
\begin{mathpar}
Q\models{\openrule
			{
				(\beta_p)_j^{J_p}, {\Pred_p},  
				{\Post_p}}
			{\ostate{s_{pl}^{l\in L_Q}} \OTarrow {\alpha_{p}}
				\ostate{s_{(p+1)l}^{l\in L_Q}}}
		}%\]
...
\end{mathpar}
We can conclude that we have a $\symb{WOT}_p$:
\begin{mathpar}
Q\models{\openrule
			{
			(\gamma_p)_j^{J_p}, {\Pred_p}, {\Post_p}}
			{\ostate{s_{pl}^{l\in L_Q}} \OTWeakarrow {\alpha_{p}}
				\ostate{s_{(p+1)l}^{l\in L_Q}}}
		}%\]
\end{mathpar}
such that $\gamma_p = \vis{\beta_p}$.
\TODO{le reste une fois mis d'accord sur la notation}\\
...
For  $\gamma_{j_0}=[\beta_0..\beta_n]$ we have $n$ open transitions  $\symb{OT}_p$ s.t $p\in [0\ldots n]$.

Hence, we have forall $p\in [0\ldots n]$ :  
\begin{mathpar}
Q\models{\openrule
			{
			(\gamma_p)_j^{J_p}, {\Pred_p}, {\Post_p}}
