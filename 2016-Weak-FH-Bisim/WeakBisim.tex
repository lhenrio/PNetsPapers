...
\begin{defi}[pNets]\label{def-pnets}
...
\begin{defi}[Sorts, Holes, Leaves, Variables of pNets]\label{def-sortholeleave}~~
...
\begin{defi}[Open transitions]\label{def:OT}
...
We adopt a notation similar to standard relations and denote it
 ...
 \begin{defi}[Strong FH-bisimulation]\label{def-FH-bisim} ~\\
\noindent
	Suppose 
...
\begin{defi}[States of open pNets]\label{def-states}
...
\begin{defi}[Semantics of open pNets]
...
\begin{defi}[FH-bisimulation for open pNets]\label{def:bisim-pnets}
...
\begin{defi}[Non-observability of $\tau$ actions for open automata]\label{def:Non-ObsTau}
...
\begin{defi}[Weak open transition (WOT)]\label{def:weakOT}
...
\begin{defi}[Building a weak open automaton]\label{def:buildweakOT}
...
\begin{defi}[Weak FH-bisimulation]\label{def-Weak-bisim} ~\\
\noindent
...
\begin{defi}[Non-observability of silent actions for pNets]\label{def:non-obspNet}~\\
...
\begin{defi}[Semantics of pNets as a weak open automaton]
...
%\begin{proof}
\proof Note that Definition \ref{def-Weak-bisim} is a particular case of the definition above, thus we only need to prove one direction of the equivalence between the two definitions, namely: \\
...
\Pred_{s,t} \land \Pred_1)\wedge\Pred_2\subst{\Post_1}\implies \\
 \bigvee_{a\in A}
  \!\! \left( \forall j\in J_a. \vis{\beta_{1j}}\!=\!\gamma_{ja}\!  \land\! \Pred_{OT_a}\! \land\!
