...
pNets is a generalisation of automata that includes variables and hierarchical composition. Open pNets are pNets with holes, i.e. placeholders inside the hierarchical structure  that can be filled by sub-systems. Reasoning on open pNets allows us to reason on open systems.
...
These works use the term \emph{parameter} to designate systems where variables defining the system have a strong influence on their behaviour: in parameterised systems, parameters can typically influence the number of processes in the system or the way they interact.
...
Open pNets are pNets for which some elements in the hierarchy are still undefined, such undefined elements are called \emph{holes}; a hole can be \emph{filled} later by providing another pNet.
 The semantics of pNets can be expressed as a translation to a labelled transition system, but only if the pNet has no parameter and no holes. Adding parameters to a LTS is quite standard but enabling holes inside LTSs is not a well-defined notion. 
...
\paragraph{What is new about open automata bisimulation?}
...
Sort comes naturally with a compatibility relation that is similar to a type-compatibility check. We simply say that two sorts are compatible if they consist of the same actions with the same arity. In practice, it is sufficient to check the equality of the two sets of pairs $(\alpha,n)$ of the two sorts.
\end{definition}
...
\begin{figure}[t]
  ...
  \caption{The $\symb{SimpleSystem}$ composed with the $\symb{Impl}$ of $\symb{SimpleProtocol}$}  \label{SimpleProt:Impl}
\end{figure}
...
We adopt a notation similar to standard relations and denote it
 $\mathcal{R}=\{(s,t|\Pred_{s,t})\}$, where: 1) For any pair $(s,t)\in \mathcal{S}_1\times \mathcal{S}_2$, there is a 
  ...
  verify the predicate $\Pred_{s,t}$. 2) The free variables of  $\Pred_{s,t}$ belong to $V_1$ and $V_2$, i.e. $\vars(\Pred_{s,t})\subseteq V_1\cup V_2$, and the variables of the two open automata are disjoint, i.e.  $V_1\cap V_2=\emptyset$.
...
the actions of the involved sub-pNets and the resulting actions. \hl{This predicate verifies}:
...
In \cite{henrio:Forte2016}, we have also shown a detailed example of the construction of a complex open transition, building a deduction tree using rules \TrUn ~and \TrDeux.
...
\begin{figure}[ht]
   ...
   \caption{$\symb{Spec}$ of $\symb{SimpleProtocol}$}
   \label{SimpleProtCounter:SpecOA}
\end{figure}
...
have annotated each global state with the set of corresponding pLTS variables.
 ...
 \begin{figure}[ht]
  ...
  \caption{$\symb{Impl}$ of $\symb{SimpleProtocol}$}  \label{SimpleProtCounter:ImplOA}
\end{figure}
...
pNets have through composition  the ability both to add new holes to a system and to fill holes, obtaining a system with less holes. They support the refinement checking at the automata level through a simulation approach, with symbolic evaluation of
the guards and transitions. The definition of simulation on open automata should be  stronger than a strict simulation since it matches a transition with a family of transitions. Such a relation should be able to check the refinement between two open automata with the same level of abstraction but specified differently, for example, by duplicating states, removing transitions, reinforcing guards, modifying variables. But it also should  support the  comparison of automata that do not have the same number of holes. 
...
There is also a clear relation with the seminal works on rule formats for Structured Operational Semantics, e.g. DeSimone format, GSOS, and conditional rules with or without negative premisses \cite{deSimone85,Bloom88bisimulationcant,GROOTE1992202,VANGLABBEEK2004229}.  Open pNets provides a way to define operators similar to these rules formats, but with quite different aim and approach. A formal comparison would be interesting, though not trivial. What we can say easily is that: the pNet format syntactically encompasses both DeSimone, GSOS, and conditional premisses rules. Then our compositionality result is more powerful than the classical result, but this is not a surprise, as it relies on a (sufficient) syntactic hypothesis on particular systems, rather than the general rules defining an operator.  Last, we intentionaly do not accept negative premisses, that would be more diffcult to put into practice in our implementation.
...
More reecntly we published preliminary work for the case of weak FH-Bisimulation \cite{}. 
...
constructing the weak automaton, but searching \emph{on demand} to construct the required weak transitions.
Alternatively, we explore more pragmatic approaches using weak bisimulation preserving (pattern-based) 
reduction rules \cite{wang:hal-03126313}.
