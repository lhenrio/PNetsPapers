...
$\\ 
In this equation, and using case 1 above for all $k\in [0\ldots n_1]$ there is a $p\in [0..m_1]$ (following the re-indexing done in the removal of steps where $Q$ does not move) $\Pred_{1p}$ contains $ \alpha_{1p}=\beta_{1 p j_0}$ with $\alpha_{1p}=\alpha_k$. Because $\beta_{1 p j_0}$ only contains variables of $P$ and $\alpha_k$ only variables of $Q$:
\[(\alpha_{1p}=\beta_{1pj_0})\subst{\bigotimes_{j=i-1}^{0}\!\!\Post_{1j}}\iff
...
\end{mathpar}
$\alpha_k'=\alpha_{n_k}\displaystyle{\subst{\bigotimes_{j=n_k-1}^{n'_{k-1}}\!\!\Post''_{j}}}$\\
$\forall l \in L_Q. v_{kl}=v_{(n'_{k-1})l} \wedge v_{kl}'=v_{(n'_k-1)l}$\\
...
Also note that for all $k\in[1..n-1]$, $v_{kl}'=v_{(k+1)l}$, and $v_{0l}=s_{0l}=s_l$ and $v'_{nl}=v_{(n_1+n_2+3) l}=u_{n_2 l} = s'_l$.
...
\alpha_{n_k}\displaystyle{\subst{\bigotimes_{j=n_k-1}^{n'_{k-1}}\!\!\Post''_{j}\otimes\bigotimes_{j=n'_{k-1}-1}^{0}\!\!\Post''_{j}}} = \alpha_k'\displaystyle{\subst{\bigotimes_{j=n'_{k-1}-1}^{0}\!\!\Post''_{j}}} = 
...
&\iff& \gamma_{j_0}=
[\alpha'_{p}\subst{\bigotimes_{j=p-1}^{0}\!\!\Post_{j}}|p\in[1..n]]
\end{eqnarray*}
