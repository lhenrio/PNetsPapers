...
\begin{abstract}
This paper provides a solution for the compositional design of systems and their verification. The approach taken consists in designing architectural elements that guaranty safety properties and composing them to build a safe system.

More precisely, this article provides a theory of architectures based on the BIP system.
It extends preliminary work in this domain by handling data: definition of data-domains,  operations on  data-domains, and data transfer. These architectures and the safety properties they ensure can be composed.
We then illustrate how to verify the properties of the architectures in an SMT solver; this is relying on an encoding into an intermediate model called pNets that already supports SMT-based verification. Additionally, the translation into  the open pNet semantic
  model gives us a strong insight on the  coordination mechanisms existing in the two models and their comparison. The approach is illustrated by a case-study where key safety properties are verified.

\keywords{Parametrised verification, composition, safety, interaction models.}
\end{abstract}
...
\addSB{Although we define properties as state predicates, any
  ...
  property \emph{``There is always a possibility to reset the system
    or resume operation after a single failure'' (\ie without
    additional failures occuring in the meantime)} enforced by the
  Failure Monitor architecture can be specified using CTL as $\AG
  \bigl(\PortFail \rightarrow \EU[]{\lnot \PortFail}{(\PortResume \lor
...
additional Boolean value to each port, \add{3)~for each exported variable $x$, create the corresponding fresh variable $x'$ to allow updates during interactions,} 4)~create pNet transitions that reflect the original transitions of
...
\item denoting  $\mathit{init} \bydef{=} \bigl(x := \val{0}{C}(x)\bigr)^{x \in V_C}$;
  \noteSB{Ludo, Eric, pourriez-vous v\'erifier que cela est compr\'ehensible ?}%
...
\todoSB{Update?}\hl{In order to encode these interactions using only one synchronisation vector}, we use
...
\begin{indent}
  description courte et intuitive avec reference au papier d'Avocs
\end{indent}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.8\columnwidth]{TimerOADetailed}
  \caption{The open Automaton of the Failure Monitor Architecture}
...
detail of the open transition, but only the assignement of state
variables, and the resulting action; you can find the full details in
...
behaviour in Fig~\ref{schema:ArchFailure:MaxProgress}. This one
verifies all the properties listed in  \app{properties}.
...
\begin{enumerate}
\item There is always a possibility to reset the system or resume
  operation after a single failure (\ie without additional failures
  occuring in the meantime):
  \begin{equation}
    \label{eq:prop:reset:can}
    \AG \bigl(\PortFail \rightarrow \EU[]{\lnot \PortFail}{(\PortResume \lor \PortReset)}\bigr).
  \end{equation}
...
Holes: E
    ...
    OT1 ------------------------------------------------------------
       ...
       {t := t+1, z := z1}
    OT2 ------------------------------------------------------------
        <t1_s1>-----fail(b0:sva_SV0:1:1)----><t1_s1>
    ...
    OT5 ------------------------------------------------------------
       ...
       {t := t+1, z := z1}
    OT6 ------------------------------------------------------------
        <t1_s2>-----fail(b0:sva_SV0:1:1)----><t1_s2>
    ...
    OT9 ------------------------------------------------------------
       ...
       {t := t+1, z := z1}
    OT10 ------------------------------------------------------------
        <t1_s3>-----fail(b0:sva_SV0:1:1)----><t1_s3>
...
\medskip
  ...
  Now the version with the Max Progress encoding.
 This modifies both the pLTS transitions, and the synchronisation vectors:
...
\end{verbatim}
  ...
  The new (simplied) OA is in figure \ref{schema:ArchFailure:OA-MaxProgress}.
  Here we give the detail only of the modifed OTs:
...
\begin{itemize}
\item OT:2 has changed because $b0=true$ is no longer possible
\item OT7 has disappeared because it is now unsat.
\end{itemize}

\small\begin{verbatim}
	    {t1---tick(z, t, z1)-->t1, s1---fail(zone, true, false)-->s1},
            {B|->fail(b0:sva_SV0:1:1)},
            [(t<(GET_MAX z))/\((tick(z, t, z1))=(tick(z:sva_SV0:1:1, t:sva_SV0:1:1, z1:sva_SV0:1:1)))
              /\((fail(zone, true, false))=(fail(zone:sva_SV0:1:1, bf:sva_SV0:1:1, b1:sva_SV0:1:1)))
              /\(b1:sva_SV0:1:1=>b0:sva_SV0:1:1)
              /\((bf:sva_SV0:1:1/\b0:sva_SV0:1:1)=>b1:sva_SV0:1:1)
              /\(z1:sva_SV0:1:1=(z:sva_SV0:1:1 |Intersection| ((b1:sva_SV0:1:1?(zone:sva_SV0:1:1):(TopInt))+t:sva_SV0:1:1)))],
            {t := t+1, z := z1}
	OT:2 ------------------------------------------------------------
		<t1_s1>-----fail(false)----><t1_s1>
