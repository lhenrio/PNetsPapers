...
The model in \fig{schema:ArchFailure:BIP} has three components.  The
...
\PortResume, \PortFail\}$.  The other two components, $\NameCtrl$ and
$\NameTimer$, are \emph{coordinators}\mdash they are specified
completely, including the states and transitions defining their
\emph{behaviour}.  Transitions are labelled with the ports of the
corresponding components, Boolean guards and update functions on local
variables.  For instance the loop transition $\tstate{1}
...
than $z.u$, where $z = [l, u]$ is a local variable representing an
interval.  Upon firing, this transition decrements the value of $t$ by
$1$.  The guard and update function of the transition $\tstate{1}
\goesto{\PortResume} \tstate{0}$ are omitted.  The default values are
the constant predicate $\true$ and the $\mathit{skip}$ operator
respectively.  The initial valuations $\val{0}{\NameTimer} = \{z
\mapsto [0,0], t \mapsto 0\}$ and $\val{0}{\NameCtrl} =
\{\mathit{zone} \mapsto [\mathrm{Min}, \mathrm{Max}]\}$ are shown by
initialising transitions $\goesto{\TimerInitZone[]} \tstate{0}$ and
$\goesto{\IntervalInit[]} \cstate{0}$ respectively.  The constants
$\mathrm{Min}$ and $\mathrm{Max}$ are the parameters of the
architecture.
...
the context.  Intuitively, for an interaction $(a, g, e)$, $\supp{a}$
...
For instance, in \fig{schema:ArchFailure:BIP}, the two ports
$\NameTimer.\PortStart$ and $\NameCtrl.\PortFail$ are always
synchronised, since they belong to the same binary sub-connector,
...
connector $\NameTimer.\PortStart \bullet\!\!\!-\!\!\!-\!\!\!\bullet
...
Going back to our running example in \fig{schema:ArchFailure:BIP}, the
