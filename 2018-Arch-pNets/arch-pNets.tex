...
\begin{definition}[Composition]
  ...
  the interaction model $\Gamma$ over $(V_i, P_i, \export[i])^{i \in
    I}$ is the component $\Gamma(\cB) = (Q, q^0, V, \val{0}{}, P,
    \export, \goesto{})$, where
%
  $Q = \prod_{i \in I} Q_i$;
%
...
\begin{definition}[Priority model]
  ...
  \export)$ is a strict partial order on interactions: $\pi
  \subseteq \bigl(2^P \times \guards{V} \times \exprs{V} \bigr)^2$.

  We denote $\filter{\alpha} \bydef{=} \setdefb{\beta \in (2^P \times
...
In this paper, we will only consider the so-called \emph{maximal
  progress} priority, where $(a, g, e) \prec (b, h, f)$ iff $a
