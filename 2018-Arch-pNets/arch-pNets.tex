...
\begin{definition}[Open transitions]
	...
	terms, labels, and states $s_i$, $\beta_j$, $s$, $v$. $\Post\in 
	\assigns{\variables}$\ is 
	a set of assignments that are the effects of the transition.
\noteSB{Why $v$? Why not $\alpha$? As now, it is not clear whether this is an action term 
or a variable $v \in \vars(s)$ of some hypothetical state $s$.}\hl{$v$ is a variable 
denoting the resulting action}
\end{definition}
%\begin{definition}[Open transitions]
%	\label{def:OpenTransitions}
...
\begin{multline}
  \Pred_{\symb{sv}}(SV, 
  \alpha_i^{i\in I}, \beta_j^{j\in J}, v)\Leftrightarrow
  \\
%\exists 
%%   \bigg(
%% \begin{array}{l}
\exists {\vars((\alpha'_i)}^{i\in I}),
{\vars((\beta'_j)}^{j\in J}),\vars(v').\, %SV=
%\\
%\land
...
%% \end{array} \bigg)
\\
\text{ Where } SV=\SV{{(\alpha'_i)}^{i\in I}, {(\beta'_j)}^{j\in J}}{ v'}{ g}
\end{multline}
...
k\!\in\! K \qquad SV_k \!=\! \SV{\alpha_m^{m \in I_1\uplus I_2\uplus J}}{\alpha'}{g} \\
\\     	
