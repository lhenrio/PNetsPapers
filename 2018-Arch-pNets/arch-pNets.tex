...
\keywords{Parametrised verification, composition, safety, interaction
  models.}
\end{abstract}
...
The methodology we propose is thus to an encoding of BIP architectures
into open pNet in order to be able to verify, using SMT techniques,
the coordination capabilities of the architecture, and more generally
its properties. The encoding will also give us feedback on the
comparison between the BIP architectures and the pNet formalism.  
...
expressed in the two formalisms. This article is also the opportunity
to
% investigate further on  compositional verification by
relate two different composition mechanisms that provide different
vision on compositional verification. \noteLH{pas tres heureux la fin:
...
preservation of (data dependent) properties by architecture
compositions. 2) An encoding of architectures with data into the open pNet
model, allowing for analysis of their temporal properties using pNet's
software tools.
...
An open transition first
 project the global state into states of the leaves, then apply
pLTS transitions on these states, and compose them with the actions of the holes according to the synchronisation vectors. More precisely, each open transition between two states is defined as a composition of transition from some pLTSs and actions of some holes as follows: 1) the actions of the holes involved in the transition, 2) a guard  built from the synchronisation vectors coordinating the holes and the transitions involved; 3) assignments and global state change are defined by the pLTSs transitions involved. The synchronisation vectors also define the global action. 
...
%
  ...
  %% \end{align}
%
  and $\mu(...)$ denotes the application of the maximal progress priority model.
\end{definition}
...
\defn{up:compat} in \app{arch:appendix} provides a formal definition
of the notion of \emph{upwards compatibility} needed to ensure property preservation when composing architectures relying on the maximal progress priority model.  Here, we present it
...
\addSB{In the general encoding, the global actions of the
  ...
  translation of properties to be verified (\cf \secn{arch:verif}).}
...
Then, in the SV guard, we have to check whether \emph{all ports
  leading to $p$ in the caual tree $\tau(x)$ can be fired} (see
...
various classe infinite-state systems and/or paramaterized topologies
via predicate abstraction and SMT satisfiability:
\cite{DBLP:conf/cade/GhilardiNRZ08,DBLP:journals/corr/CimattiGMT13,DBLP:journals/corr/abs-1806-11459}
...
\todoEM{Il faudrait (mais a-t-on le temps?): faire un dessin de la
  composition au niveau BIP, et au niveau pNet. Un resultat brut, vaguement
  correct, montre 36 OTs, modulo les petites questions de mon mail de
  mercredi midi...
  Au moins dessiner cet automate, et ecrire les proprietes qu'il
  verifie... }

Primary version of the composed pNet:

\begin{verbatim}
SV0: <tick(z, t, z1), fail(zone1, b11), fail(zone2, b12), fail1(b01), fail2(b02), _> --> fail(b01, b02), 
     (z1 = z |Intersection| ((b11?zone1:TopInt)+t) |Intersection| ((b12?zone2:TopInt)+t))
     /\(b11=>b01)/\(b12=>b02)
SV1: <cancel(b1), resume(b21), resume(b22), resume1, resume2, _>-->resume, (b1=b21)/\(b1=b22)
