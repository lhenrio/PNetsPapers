...
\begin{example}[An open-transition]
  ...
  The \texttt{Failure TimerMonitor} pNet of \Fig{FailureTimer:pNet} has \replaceSB{2 controllers 
  and 2 holes. O}{two coordinators and four dangling ports, which can be grouped into two holes (see \secn{encoding} below).  Disregarding the maximal progress priority, o}ne of its possible open-transitions is:\\[-2ex] \noteLH{maximal progress undefined}
...
\begin{small}\[ \openrule
  {\{B\mapsto fail(b0)\},\\
    t<GetMax(z) \land b1=true \land b1=>b0 \land z'= (z \cap (b1?zone;TopInt)+t) {\color{blue}\land (t' = t) \land (zone' = zone)},\\
    \{z:=z', {\color{blue}t := t'+1, zone := zone'}\} }
