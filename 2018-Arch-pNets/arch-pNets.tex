...
$\emptyassign$ to denote the empty assignment, \ie an assignment with
$I = \emptyset$.
...
interval.  Upon firing, this transition increments the value of $t$ by
$1$.  The guard and update assignment of the transition $\tstate{1}
\goesto{\PortResume} \tstate{0}$ are omitted.  The default values are
the constant predicate $\true$ and the empty assignment {\emptyassign}
%$\mathit{skip}$ operator
respectively.  The initial states \tstate{0} and \cstate{0}, and valuations $\val{0}{\NameTimer} = \{z
\mapsto \topinterval, t \mapsto 0\}$ and $\val{0}{\NameCtrl} =
...
%
  ...
  %% \setdefb{\beta \in (2^P \times \guards{V} \times \updates{V})}
