...
\newcommand{\twosynch}{%
  \ensuremath{\bullet\!\!\!-\!\!\!-\!\!\!-\!\!\!\bullet}}
\newcommand{\synchtrig}{%
  \ensuremath{\bullet\!\!\!-\!\!\!-\!\!\!\blacktriangleleft}}
\newcommand{\trigsynch}{%
  \ensuremath{\blacktriangleright\!\!\!-\!\!\!-\!\!\!\bullet}}
\newcommand{\twotrig}{%
  \ensuremath{\blacktriangleright\!\!\!-\!\!\!-\!\!\!\blacktriangleleft}}
...
%
  ...
  \end{multline}
%
  where \noteSB{Can probably write directly $\overline{V_C := \val{0}{C}}$, in which case no need to intrudce $\mathit{init}$.}\hl{$\mathit{init} = \bigl(x := \val{0}{C}(x)\bigr)^{x \in V_C}$}.
\end{itemize}

\noteSBin{Eric uses some kind of initialisation predicate in
  Fig.~\ref{schema:ArchFailure:BIP}.  I do not quite understand how
  this works, so I used a new initial state with explicit
  initialisation.}
...
by the initial valuation $\val{0}{C}$ of $C$ and 2)~define a pNet action algebra that associates, as action parameter, an
...
dangling ports {\PortFail} and {\PortResume} will be provided by a monitoring component, whereas {\PortAsk} and {\PortReset} correspond to
...
as a set of connectors. \noteSB{A legitimate question is whether we
  actually need to do this, instead of listing explicitly the
  individual interactions, then associating a synchronisation vector
  to each one of them?  One of the factors is the encoding
  pNets$\rightarrow$SAT: is it better to have more or fewer SVs if the
  price is additional Boolean variables?  The other factor might be
  the facility of maximal progress encoding\mdash TBC.  Traceability.
  Anything else?  }Indeed, as illustrated by the Failure Monitor
architecture in \fig{schema:ArchFailure:BIP}, each connector can
define several allowed interactions, depending on its hierarchical
structure and port and node syncrhon/trigger typings.  For instance,
the connector {\NameTimer.\PortTick \trigsynch (\PortFail \trigsynch
  \NameCtrl.\PortFail)} defines three interactions (observe that
$\topinterval + \NameTimer.t = \topinterval$ and $\NameTimer.z \cap
\topinterval = \NameTimer.z$):\noteSB{Move this to \secn{components}?}
%
\begin{align*}
  ...
  \\
  &\bigl(\{\NameTimer.\PortTick, \PortFail\},
  &&\true,
  &&\NameTimer.z := \NameTimer.z\bigr)
  \\
  &\bigl(\{\NameTimer.\PortTick, \PortFail, \NameCtrl.\PortFail\},
  &&\true,
  &&\NameTimer.z := \NameTimer.z \cap (\NameCtrl.\mathit{zone} + \NameTimer.t)\bigr)
\end{align*}

In order to encode these using only one synchronisation vector, we use
...
synchronisation vector.  Non-participation of a port corresponds to
firing the loop transition marked by \false.  \Fig{enc:nopri} shows
