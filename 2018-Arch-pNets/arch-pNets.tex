...
First, consider a flat connector with $n$ trigger ports $p_1,\dots,
p_n$ (with $n > 0$) and $m$ synchron ports $q_1,\dots, q_m$
(\fig{connector}).  Similarly to the example above, for any of the
ports $q_1,\dots, q_m$ to participate in an interaction defined by
this connector, at least one of the ports $p_1,\dots, p_n$ must
participate.  This causal dependency can be represented by the Boolean
formula $q_1 \lor \dots \lor q_m \Rightarrow p_1 \lor \dots \lor p_n$.
If there are no synchrons, \ie $m = 0$, this formula simplifies to
$p_1 \lor \dots \lor p_n$.  However, if there are no triggers, \ie $n
= 0$, the connector semantics is that \emph{all} synchron ports must
participate for an interaction to take place, represented by the
Boolean formula $(q_1 = q_2) \land \dots \land (q_1 = q_m)$.

We now generalise this principle to hierarchical connectors, building
the formula bottom-up.  In a flat connector above, we have implicitly
associated a Boolean variable to each port.  In a hierarchical
connector, we associate a fresh Boolean variable to each node.
Similarly to the above example, consider a connector with $n > 0$
trigger sub-connectors and $m$ synchron sub-connectors.  We associate
to this connector a fresh Boolean variable $x$.  Assume that Boolean
variables associated to trigger sub-connectors are $x_1,\dots,x_n$,
whereas those associated to synchron sub-connectors are
$y_1,\dots,y_m$.  The causal dependencies among the interactions
defined by the sub-connectors can be encoded by the formula
%
\begin{equation}
  \label{eq:trig:encoding}
  (y_1 \lor \dots \lor y_m \Rightarrow x) \land (x = x_1 \lor \dots
  \lor x_n).
\end{equation}
%
If there are no trigger sub-connectors, \ie $n = 0$, we use the
formula
%
\begin{equation}
  \label{eq:synch:encoding}
  x = \bigl((y_1 = y_2) \land \dots \land (y_1 = y_m)\bigr)
\end{equation}
%
($x$ is equal to $\true$ if all $x_1, \dots, x_m$ are equal, it is
equal to $\false$ otherwise).

Notice that, in the flat connector example above, the formula was
essentially the same, with $p_i$ instead of $x_i$ and $q_i$ instead of
$y_i$, and without the additional variable $x$.  Indeed, $x$ is used
to encode the causal dependencies between this connector and, if
necessary, those in higher levels of the hierarchy.  The Boolean
encoding of the causal dependencies among the ports of the entire
connector is obtained by taking the conjunction of formulas obtained
for each sub-connector.

Consider again the connector {\NameTimer.\PortTick \trigsynch
  (\PortFail \trigsynch \NameCtrl.\PortFail)}.  Associating the
varaibles $b_0$, $b_1$ and $b_2$ to ports {\PortFail},
{\NameCtrl.\PortFail} and {\NameTimer.\PortTick}, respectively, and
variables $y_1$ and $y_2$ to the top- ({\NameTimer.\PortTick
  \trigsynch $\cdot$}) and second-level (\PortFail \trigsynch \NameCtrl.\PortFail) connectors in the hierarchy,
we obtain the following formula
%
\begin{equation}
  \label{eq:tff:example}
  (b_1 \Rightarrow y_2) \land (y_2 = b_0) \land (y_2 \Rightarrow y_1)
  \land (y_1 = b_2)
  \,.
\end{equation}
%
Observe now that the top-level connector, by its very nature, does not
have any higher level connectors and therefore we can eliminate the
corresponding Boolean variable.  This simplifies \eq{tff:example}, to
%
\begin{equation}
  \label{eq:tff:example:1}
  (b_1 \Rightarrow y_2) \land (y_2 = b_0) \land (y_2 \Rightarrow b_2)
  \,.
\end{equation}
%
In the general case, this corresponds to replacing, \emph{for
  top-level connectors}, equations \eq{trig:encoding} and
\eq{synch:encoding}, respectively, by
%
\begin{gather}
  \label{eq:trig:encoding:1}
  y_1 \lor \dots \lor y_m \Rightarrow x_1 \lor \dots
  \lor x_n
  \\
  \intertext{and}
  \label{eq:synch:encoding:1}
  (y_1 = y_2) \land \dots \land (y_1 = y_m)
  \,.
\end{gather}
%
Furthermore, in the context of one connector (or synchronisation
vector), some interaction must be fired.  Thus, \emph{for top-level
  connectors}, \eq{synch:encoding:1} can be further simplified to 
%
\begin{equation}
  \label{eq:synch:encoding:2}
  y_1 \land \dots \land y_m
  \,.
\end{equation}
