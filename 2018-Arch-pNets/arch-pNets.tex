...
\newcommand{\AF}[1][\ ]{\ensuremath{\mathtt{FG}#1}}
...
\institute{%
  ...
  \and
	Univ Lyon, CNRS, ENS de Lyon, Inria, Universit\'e Claude Bernard Lyon 1, LIP UMR 5668, Lyon, France\\\email{ludovic.henrio@ens-lyon.fr}
  \and
...
\begin{example}
  ...
  \item As long as no failure has occured, the $z$ variable of the
    ...
    \begin{equation}
      \label{eq:prop:z:top}
      \AW[\quad]{(z = T)}{\PortFail} \land\quad
      \AG \bigl(
      \PortReset \rightarrow \AW[]{(z = T)}{\PortFail}
      \bigr).
    \end{equation}
...
\section{Open pNets}
\label{secn:pNets}
This section briefly describes pNets, see~\cite{HMZ-FORTE2016} for a  description of open pNets and their properties.
...
We first define the set of actions a pLTS can use ($a$
...
A  pNet consists of a set of sub-pNets exposing
a set of global actions, each of them resulting from the synchronisation of internal actions in some sub-pNets. The synchronisation between global actions and
internal actions is given by  \emph{synchronisation vectors}.
...
\begin{example}\emph{An open-transition.}
  ...
  and 2 holes. One of its possible open-transitions is:
...
FailureTimerArchi
--------------------
SV0 SVs: <tick(z, t, z1), fail(zone, b1), fail(b0), _>-->fail,
                          [((b1->b0)/\(z1=z/\(((b1?(zone)TopInt)))+t)))]
SV1 SVs: <cancel(b1), resume(b2), resume, _>-->resume, (b1=b2)
SV2 SVs: <timeout, timeout, _, ask>-->ask
SV3 SVs: <_, reset, _, reset>-->reset
SV4 SVs: <init, init, _, _>-->init
pLTS: Timer
