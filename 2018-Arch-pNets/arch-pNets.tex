...
\paragraph{Unifying notations -- remarks}
...
In Archi we have a whereas in pnets we have $\alpha$ where, approximately $\alpha = 
...
states looks ok, but initial state is $s_0$ in pnet and not in archi : $s^0$ . exponent 
...
\begin{definition}[Safety properties]
  ...
  $s^0_{\semopen{B}} \models \Phi$\todoSB{Do we need this?}\hl{;
...
\begin{definition}[Enforcing properties]
  ...
  \prop{arcomp:nice}).  We say that \emph{$A$ enforces $\Phi$ on
    $\cB$} iff, for every state $s = (s_b, s_c)$ reachable in
  $\semopen{A(\cB)}$, with $s_b \in \prod_{B\in \cB}
  S_{\semopen{B}}$ and $s_c \in \prod_{C \in \cC}
  S_{\semopen{C}}$, we have $s_b \models \Phi$.
\end{definition}
...
\begin{proof}
  ...
  We will use $q^i, q^{i\prime}$ to denote the states of
...
%
  \begin{equation}
    ...
    (q^i, \val{i}{})^{i \in I} (q_j, \val{}{j})^{j \in J}
    \goesto{a, \val[\tilde]{}{}}
    (q^{i\prime}, \val{i\prime}{})^{i \in I}
...
%
  \begin{equation}
    \label{eq:lem1:trans}
    (q^i)^{i \in I} (q_j)^{j \in J}
    \goesto{a, G, E}
    (q^{i\prime})^{i \in I} (q_j')^{j \in J}
    \,,
  \end{equation}
%
...
%
  \begin{mathpar}
    ...
    \,,
    \and
    (\val{i\prime}{})^{i \in I} (\val{\prime}{j})^{j \in J}
    = \val[\tilde]{}{}[E]
    \,,
    \and
  ...
  \begin{enumerate}
  \item for $i \in I$,
    $q^i \goesto{a \cap P_{C_i}, g_{C_i}, e_{C_i}} q^{i\prime}$
    is a transition in $C_i$, or
    $a \cap P_{C_i} = \emptyset$ and $q^i = q^{i\prime}$,
  ...
  \item \label{only1:coord}
    ...
    (q^{i\prime})^{i \in I}$ is a transition in $C'$,
    or $a \cap P_{C'} = \emptyset$ and $q^i = q^{i\prime}$,
    for all $i \in [1,m]$,
    $g_{C'} = \true$ and $e_{C'} = \noop$\,;
  \item for $j \in J$,
    $q_j \goesto{a \cap P_{B_j}, g_{B_j}, e_{B_j}} q_j'$
