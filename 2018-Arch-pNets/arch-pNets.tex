...
\emph{compositionally}.  Indeed, it is sufficient to verify that
1)~the applied architectures do enforce their characteristic
properties and 2)~that they are pairwise upwards compatible.  To
a large extent, item~2, \ie verification of the upwards
compatibility, can be carried out syntactically by analysing the
structure of the connectors defining the interaction models:
maximal progress is mostly relevant for connectors involving
triggers; whenever a trigger port is not common to both
architectures, upwards \emph{in}compatibility cannot arise.  The
next section is devoted to the encoding of architectures into
pNets, which addresses item~1 by symbolic verification.
%
}
...
\begin{multline}
  ...
  \rightarrow
  {\color{blue}\alpha \bigl(b_p^{p \in P_x}\bigr)}
  \\
  \left[
...
\addSB{%
%
For the sake of clarity, we simplify the encoding in
...
\paragraph{Practical experiments.}\noteSB{J'ai repris ce paragraphe}
\noteLH{j ai nomme ce paragraphe}
...
SMT engine to check satisfiability of their predicates, minimizing the
...
{\PortFail}). \add{It looks like} the rough behaviour is
...
\label{secn:related}
Previous works on pNets and BIP architectures have been mentioned at the beginning of the paper. We  review here the closest related works outside the scope of pNets and BIP architectures, with a particular focus on compositional and parameterized verification.
...
\add{The following appendices are included for convenience and would be integrated into a research report if the paper is accepted for publication. The first appendices gives complementary informations previously published, while the other appendices provide additional formal details complementing architecture definitions, proofs of theorems, and the case study.}
