...
We extensively use \replaceSB{indexed structures
over some countable indexed sets, which are equivalent to mappings over
the countable set. }{structures indexed over some finite or countable sets of indices, which can be equivalently defined as mappings over such index sets. }% . The indexes will usually be
...
\replaceSB{$a_i^{i\in I}$ }{This notation }defines both $I$ the set over which the family is
...
``indexed set over I''\replaceSB{ when}{, even though} formally we should speak of multisets; and
write $x\in a_i^{i\in I}$ to mean $\exists i\in I.\, x=a_i$.  An empty
family is denoted $\emptyset$. \replaceSB{
We denote classically with an overline -- $\overline{a}$ -- a family when the indexing set 
is
not meaningful.}{We use a classical overline notation, \eg $\overline{a}$, for a family, where the indexing set is irrelevant} \replaceSB{$\uplus$ is }{Finally, we use $\uplus$ to denote }the disjoint union on
indexed sets.
...
\replaceSB{Our models rely }{The pNet model in \secn{pNets} relies }on a notion of parameterised actions, that are
symbolic expressions using data types and variables. As \replaceSB{our model aims }{we aim }at encoding the low-level behaviour of possibly very different
...
variable names}. Action terms are\replaceSB{:}{{} of the form $\alpha=a((?x_i)^{i \in I}, \symb{Expr}_j^{j \in J})$, where $(?x_i)^{i \in I}$ are input variables, $\symb{Expr}_j^{j \in J})$ are expressions.}\noteLH{Eric suggests to remove this: too precise. I 
...
variables. Action terms for pNets are defined as follows:}{have input variables, \ie they have the form $a(e_j^{j \in J})$.}
\removeSBin{
...
\begin{multline}
  \Pred_{\symb{sv}}(SV, a_i^{i\in I}, b_j^{j\in J}, v)\Leftrightarrow
  \\
%%   \bigg(
%% \begin{array}{l}
\exists {(a'_i)}^{i\in I},
{(b'_j)}^{j\in J},v'.\, SV=\bigl({(a'_i)}^{i\in I}, {(b'_j)}^{j\in J}\rightarrow v' | g\bigr)
\\
\land
