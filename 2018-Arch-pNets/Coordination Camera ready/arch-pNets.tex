...
%
\newCoord{We provide a property preservation result, which states that
%
under
monotonicity assumptions on guards and assignments and, in presence of the \emph{maximal progress} special case of
  priority models, additional upwards compatibility assumptions on the architectures,} composition of
...
%
  \newCoord{These extensions require us to define the framework in a
    ...
    proposed by both architectures coincide.  Considering this
    requirement too restrictive, we go beyond by assuming the data
...
\newCoord{Below, we present both the encodings of coordinators and
  ...
  simulating absence of action by an additional loop transition.
  ...
  of variables by treating separately the ``top-level'' triggers, \ie
  those thereof the participation in an interaction is not subject to
  that of any other ports.
%
}
...
\newCoord{In the following theorem, we claim an \emph{isomorphism}
  ...
  architecture and the LTS semantics of the application of this
  architecture to a set of simple components.  We omit the formal
  definition of this isomorphism relation.  However, noting that open
  automata are, essentially, symbolic representations of automata with
  data, we can summarise it as follows: a transition belongs to the
  LTS iff a corresponding open transition belongs to the open
  automaton and the source and target data values of the LTS
  transition satisfy the predicate and implement the assignments of
  the open transition.
%
}
...
\label{secn:related}
%% Aside from previous work on pNets and BIP architectures mentioned in \secn{introduction}. We  review here the closest related works outside the scope of pNets and BIP architectures, with a particular focus on compositional and parameterized verification.
...
\newCoord{The presented work opens a number of avenues for future
  ...
  techniques and 2)~studying the preservation of liveness properties
  by architecture composition under assumptions similar to the
  non-interference property discussed in
...
%  
  the projection of $\Gamma$ onto the coordinators of
...
% 
  \item \label{a1b:start} \label{only1:coord}
    ...
    for all $i \in [1,m]$
    (for convenience, we put $g_{C'} = \true$ and $e_{C'} = \noop$ in
    item~\ref{aux2} below);
  \item for $j \in J$,\\
    $a \cap P_{B_j} \neq \emptyset$ and
    $q_j \goesto{a \cap P_{B_j}, g_{B_j}, e_{B_j}} q_j'$
...
%  
  ...
  update expressions of the underlying transitions in the coordinators
  $C_1$ and $C_2$;
%    
  \begin{mathpar}
...
%
  ...
  Denoting %% \todoSB{One tilde is enough}
...
\begin{proof}[Proof of \thm{combining}]
  ...
  architectures has only one coordinator.  For $i =
...
%% \bigskip
%% \Fig{ArchFailure:OA-TimerV4X2} shows the open automata of
...
%%   \label{fig:ArchFailure:OA-TimerV4X2}
%% \end{figure}
