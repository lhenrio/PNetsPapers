...
\newcommand{\noteInSB}[2][inline,color=green!40]{\todo[#1]{{\bf Note: } {#2}}}
...
units.\noteSB{Here, it would be nice to explain the role of the
  singleton connectors, but that would take too much space. AGREED}

\begin{figure}[t]
  \centering
  \includegraphics[width=0.9\columnwidth]{XFIG/FailureTimerPNET}
  \caption{pNet encoding of the FailureTimer architecture
  \noteEM[inline]{Was a little too small for me, my eyes are getting tired...}}
  \label{schema:ArchFailure:pNet}
\end{figure}


\noteSB{SV2 should not be closed;\\SV0, SV1: $\cup \rightarrow \lor$... DONE}
%\hl{%
%
...
\noteSB{
%
  Eric, could you please check my pNet terminology in this paragraph?
%
}
\noteEM{Done, it's ok.}
%
...
\begin{figure}[h]
\begin{mathpar}
    \openrule{\{s1 \xrightarrow{\OTvar{fail(false)}} s1, t1 \xrightarrow{\OTvar{resume(true)}} t0\}, ~~\{\xrightarrow{hb}\},~~\OTvar{fail(false)}=\OTvar{fail(}\OTvar{b}_1\OTvar{)}\\
    \OTland \OTvar{resume(true)}=\OTvar{start(}\OTvar{b}_2\OTvar{)}\OTland \OTvar{hb}=\OTvar{fail(}\OTvar{b}_0\OTvar{)}\OTland \OTvar{v}=\OTvar{\underline{fail}}\OTland \OTvar{b}_1=\OTvar{b}_2\\
    \OTland \OTvar{(}\OTvar{b}_1\lor\OTvar{b}_2\OTvar{)} \Rightarrow \OTvar{b}_0, ~~\{\}}
    	   {\ostate{s1,t1} \xrightarrow{\OTvar{v}} \ostate{s1,t0}}\\
  \end{mathpar}
...
$\begin{array}{r@{}l}
    ...
    \OTland \OTvar{(}\OTvar{b}_1 \lor \OTvar{b}_2\OTvar{)} \Rightarrow \OTvar{b}_0, ~~\{t := 4\}}
