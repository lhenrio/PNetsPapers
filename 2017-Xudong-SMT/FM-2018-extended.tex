...
\item[$\bullet$] $S_j \subseteq \AlgA$ is a set of action terms,
  denoting the $\Sort$\footnote{The formal definition of $Sorts$ (the
...
$\begin{array}{r@{}l}
    ot  = &\openrule{\{0 \xrightarrow{\OTvar{resume(false)}} 0, 0 \xrightarrow{\OTvar{start(true)}} 1\}, ~~\{\xrightarrow{hb}\},~~\OTvar{resume(false)}=\OTvar{failure(}\OTvar{b}_1\OTvar{)}\\
