...
units.\noteSB{Here, it would be nice to explain the role of the
  singleton connectors, but that would take too much space.}
...
\noteSB{SV2 should not be closed;\\SV0, SV1: $\cup \rightarrow \lor$}
%\hl{%
%
...
Appendix~\ref{appendix:varaibles}). A full output of 
...
\TODO{a more significant example, including state-variable assignements
  ??}
\begin{figure}[h]
%$\begin{array}{@{}r@{}l@{}}
...
$\begin{array}{r@{}l}
    ot  = &\openrule{\{s1 \xrightarrow{\OTvar{fail(false)}} s1, t1 \xrightarrow{\OTvar{resume(true)}} t0\}, ~~\{\xrightarrow{hb}\},~~\OTvar{fail(false)}=\OTvar{fail(}\OTvar{b}_1\OTvar{)}\\
...
\begin{table}\caption{\label{table:BIPalgebra}\leftline{Additional
