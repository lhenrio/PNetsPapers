...
No matter which module emits an "ask" to call for reset, the
controller take the request. Then all of the modules are synchronised
on the "reset". 
So two more new transitions are added to the Timer to ensure the
module can reset at any state. 

There are 327 satisfiable results, out of totally 97920 results, of
the example computed in 105384 milliseconds (~1.45 minutes), 
which was carried out on an Intel Core i7 at 2.5GHz with 16GB of RAM.

Then we found some extra states that never found when running only the subnets.
We figured out that is caused by a input mistake easily comparing it with the results on subnets.
\end{document}
