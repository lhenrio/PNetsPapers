...
$\langle \alpha,~e_b,~(x_j\!:= {e}_j)^{j\in J}\rangle$,
  where $\alpha \in\AlgA_{\P}$ is a parameterized action,
    $e_b \in \AlgB_{\P}$ is a guard, and the
  variables $x_j\in P$ are assigned the expressions \noteEM{Or rather
    $\AlgE_{\P}\cup\AlgA_{P}$ ?}$e_j\in \AlgE_{\P}$. 
If 
$s \xrightarrow{\langle \alpha,~e_b,~(x_j\!:= {e}_j)^{j\in
		J}\rangle} s'\in \to $ then 
%		$\vars(\alpha)\!\subseteq\! \vars(s)$, 
%		\noteSB{I would expect $\vars(e_b)\!\subseteq\!
%                  \vars(s)$, since the guard has to be tested on the
%                 source state}
%                \hl{$\vars(e_b)\!\subseteq\! \vars(s')$}, and
...
of several \noteSB{These should all be pLTSs, right? Or did I miss
  ...
  parameterised actions, but the label has no guard and no assignments}\hl{LTSs} with the
