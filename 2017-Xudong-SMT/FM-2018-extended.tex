...
\begin{example}\emph{An open-transition.}
  This transition is generated by application of vector \texttt{SV0}, synchronizing the initial actions of pLTSs \texttt{Failure} and \texttt{Timer}, with an action of the hole \texttt{B} equal to \texttt{fail}. Th elocal variable \texttt{t} of the \texttt{Timer} is assigned to its initial value \texttt{Max}.
...
Fig.~\ref{schema:smt-lib} shows the translation of this OT predicate
in the SMTlib syntax. Here we display also the proof generated by Z3,
though in practice our tool only needs the brute information ``sat'' or ``unsat''.
...
predicate. 
\TODO{missing some comments on this result OA}.
