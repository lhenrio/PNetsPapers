...
extended with explicit data in synchronizations, and we compute their
...
\begin{table}\caption{\label{table:BIPalgebra}\leftline{Algebra
      Presentation: predefined and BIP Sorts and Operators}}
...
case-study~\cite{CubETH-case-study} realized using BIP.  BIP
(behavior-Interaction-Priority)~\cite{bip} is a framework for the
...
They formalize design patterns, which enforce global properties
characterizing the coordination among the components of the system.
...
synchronization patterns among ports of components.
...
have their \emph{behavior} specified by finite automata extended with
...
enabled interactions at any given execution round.  The default priority is the
...
synchronization vectors.  Each connector that does not involve
triggers is trivially encoded by a synchronization vector comprising
...
synchronization vectors the Boolean predicate encoding the connector
...
%
Although, for the sake of brevity, this encoding does not translate
the maximal progress priority, it can be easily extended to do so, by
introducing an additional Boolean variable for each relevant port.
...
\cite{henrio:Forte2016}. For convenience we include these 
rules in appendix \ref{appendix:semRules}.
...
\def\inactive{\{-\}}
\paragraph{Combining:}
The combining method enumerate all the possible behaviors of the
...
\paragraph{Matching:}
The next step, for each synchronization vector, and each possible
combination of behavior of the subnets as generated by the \emph
...
\begin{itemize}
  \item if some subnet is marked as inactive in the vector, and the
    chosen combination has an active behavior at this position,
  \item if some subnet action expression in the vector does not match (by
    pattern-matching) the corresponding behavior $C_n$,
...
\begin{mathpar}
  ...
  \end{mathpar}
\caption{One of the unsatisfiable open transitions in FailureTimer}  \label{schema:unsat-ot}
\end{figure}
...
vector (SV0) does not match with these actions, as shown the predicate
...
Fig. ~\ref{schema:resultOA}, together with its open transitions.
...
Fig.~\ref{schema:resultOA} is not the one generated by the algorithm,
...
holes using synchronization vectors. %The pNet
...
The variables in each synchronization vector are considered local: for
...
\TODO{INCORRECT, to be rewritten}
State-variable assignments are also treated as a part of the
predicates when checking the satisfiability. % Here we need conduct some
% translations,
For each assignement in a $\Post$ predicate, such as $\{s_0 \leftarrow 1\}$ in section
...
be a pNet. Choose a synchronization vector $SV=clone(SV_k) ={(a'_i)}^{i\in I}, {(b'_j)}^{j\in J}\rightarrow v' \, ,G_k $, for $k\in K$. The
...
$\begin{array}{r@{}l}
    ...
    \OTland \OTvar{start(true)}=\OTvar{start(}\OTvar{b}_2\OTvar{)}\OTland hb_1=\OTvar{fail(}\OTvar{b}_0\OTvar{)}\OTland v_1=\OTvar{\underline{fail}}\OTland \OTvar{b}_1 = \OTvar{b}_2
...
$\begin{array}{r@{}l}
    ...
    \OTland \OTvar{resume(false)}=\OTvar{resume(}\OTvar{b}_2\OTvar{)}\OTland hb_2=\OTvar{fail(}\OTvar{b}_0\OTvar{)}\OTland v_2=\OTvar{\underline{resume}}\OTland \OTvar{b}_1 = \OTvar{b}_2
...
$\begin{array}{r@{}l}
    ...
    \OTland \OTvar{start(false)}=\OTvar{start(}\OTvar{b}_2\OTvar{)}\OTland hb_4=\OTvar{fail(}\OTvar{b}_0\OTvar{)}\OTland v_4=\OTvar{\underline{fail}}\OTland \OTvar{b}_1 = \OTvar{b}_2
    \OTland \OTvar{(}\OTvar{b}_1 \lor \OTvar{b}_2\OTvar{)} \Rightarrow \OTvar{b}_0, ~~\{\}}
...
$\begin{array}{r@{}l}
    ...
    \OTland \OTvar{resume(true)}=\OTvar{resume(}\OTvar{b}_2\OTvar{)}\OTland hb_2=\OTvar{resume(}\OTvar{b}_0\OTvar{)}\OTland v_5=\OTvar{\underline{resume}}\OTland \OTvar{b}_1 = \OTvar{b}_2
    \OTland \OTvar{(}\OTvar{b}_1 \lor \OTvar{b}_2\OTvar{)} \Rightarrow \OTvar{b}_0, ~~\{\}}
...
$\begin{array}{r@{}l}
    ...
    \OTland \OTvar{resume(false)}=\OTvar{resume(}\OTvar{b}_2\OTvar{)}\OTland hb_9=\OTvar{fail(}\OTvar{b}_0\OTvar{)}\OTland v_9=\OTvar{\underline{resume}}\OTland \OTvar{b}_1 = \OTvar{b}_2
    \OTland \OTvar{(}\OTvar{b}_1 \lor \OTvar{b}_2\OTvar{)} \Rightarrow \OTvar{b}_0, ~~\{\}}
