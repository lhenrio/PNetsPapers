...
behavioral properties; \hlEM{we give 2 examples, a safety and a liveness
...
\begin{algorithm}[h]
  ...
  \label{alg1}
\begin{algorithmic}[1]
\Require The pNet pn (cannot be reduce to a hole)
\State Initialize sets $U=\{\mathit{InitState}(pn)\}$, $E=\emptyset$
for unexplored/explored global states, $L=\emptyset$ for the resulting OTs;
\While{!$\mathit{isEmpty}(U)$}
	\State Choose $S$ in $U$; add $S$ in $E$;
	...
	\EndFor
\EndWhile
\State \hlEM{Prune the unsatisfiable open transitions in $L$ using the SMT solver;}
\State \Return $\mathit{OA}=\mathit{Reachable}(\mathit{InitState}(pn),L)$;
...
Appendix~\ref{appendix:semRules}) to build the OT. 
...
as detailed in algorithm~\ref{alg2}. This method also manages directly the
holes of the node, so \emph{MakeTransitions} will never be called on a Hole.
...
\noteInSB{Line 7: There are two type mismatches here: $ot$ is an open
  ...
  $v$ is fresh.)}
\noteInEM{Done, please check the new version}

%\noteInSB{Line 8: When is $\mathit{Filter}(ot)$ called?}

\begin{algorithm}
\caption{MakeTransitions() for pNet nodes}
  \label{alg2}
\begin{algorithmic}[1]
\Require a pNet node $Pn$ with subnets $\overline{sn}$ and holes $\overline{hole}$; a global state $S$.
\State Initialize empty list $l$ and set $L$ for sub-transitions/transitions.
	\For{each $\mathit{Subnet}$ in $\overline{sn}$}
	~~~ $\setminus \setminus$ Recursively applying the semantic rules on the sub-nodes.
                \State Store $\mathit{MakeTransitions}(Subnet, S)$ in $l$;
	\EndFor
	\State \hlEM{$\overline{comb} = \mathit{Combining}(l)$;}
	\For{ \hlEM{each $sv \in SV$, for each $comb \in \overline{comb}$ }}
           \State \hlEM{$ot = \mathit{Matching}(sv, \overline{\mathit{comb}}, \overline{\mathit{hole}})$;} % SB: removed the fourth parameter v here
           \State Store $ot$ in $L$; ~~~ \hl{$\setminus \setminus$ when $\mathit{Matching}(ot)$ succeeds}
	\EndFor
\State \Return $L$;
\end{algorithmic}  
\end{algorithm}
...
\paragraph{Matching.}
\hlEM{The \emph{Matching} method builds the OTs of a pNet node from those of
its subnets (see rule Tr2 in Appendix~\ref{appendix:semRules}).
For each synchronisation vector, and each possible
combination of behaviour of the subnets as generated by the \emph
{Combining} method, it builds the open transition.
By lack of space, we only detail here the construction of the predicate.}
...
\begin{itemize}
  ...
  \item if some subnet action expression in the vector does not match (by
    pattern-matching) the corresponding \hlEM{action expression in $C$},
  \item or if the whole set of active subnet actions in the vector
    cannot be matched (by unification) with \hlEM{the corresponding action expressions
      tuple $C$}.
\end {itemize}
...
with these actions, \hl{since it expects $\NameTimer.\PortStart$. This
  mismatch can be seen} %% as shown 
by the predicate
...
\hl{Note that in the previous sections, the OTs were displayed in a
