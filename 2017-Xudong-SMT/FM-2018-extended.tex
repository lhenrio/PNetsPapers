...
but it also allows 
for more general mechanisms, like gate negotiation in Lotos~\hl{\cite{lotos}}, or broadcast
communications.
...
\begin{definition}
  An \emph{algebra presentation} is a triple $\mathcal{P}=\langle\mathit{Sorts},\mathit{Constrs},\mathit{Ops}\rangle$, where
  ...
  Among constructors, those of arity 0 are called \emph{constants}:
  $\mathit{Consts}(\mathcal{P}) = \{\mathit{Con} \in \mathit{Constrs}\,|\, \mathit{arity}(\mathit{Con})=0\}$.
\end{definition}
...
\item[$\bullet$]
$s_0 \in S$ is the \emph{initial state}.
\item[$\bullet$] $\to \subseteq S \times L \times S$ is the \emph{transition relation}, with
$L$ the set of labels of the form
$\langle \alpha,~e_b,~(x_j\!:= {e}_j)^{j\in J}\rangle$,
where $\alpha \in\AlgA$ is a parameterized action, \noteSB{Shouldn't this be $e_b \in \AlgB_{\mathcal{P}}$, for {\em some} $\mathcal{P}$?}\hl{$e_b \in
...
$s \xrightarrow{\langle \alpha,~e_b,~(x_j\!:= {e}_j)^{j\in
		...
		\noteSB{I would expect $\vars(e_b)\!\subseteq\! \vars(s)$, since the guard has to be tested on the source state}\hl{$\vars(e_b)\!\subseteq\! \vars(s')$}, and
		$\forall j\!\in\! J .\,\vars(e_j)\!\subseteq\! \vars(s)\land 
x_j\!\in\!\vars(s')$.
%\item[$\bullet$]
The $x_j$ are \emph{state variables} of %% the state
$s'$. 
\end{itemize}
\end{definition}
...
where
\begin{itemize}
\item[$\bullet$] \noteSB{Why is one $\I$ and the other $\I_\P$?}\hl{$I \in \I$ and $J\in\I_\P$} are the sets over which are indexed sub-pNets and holes, respectively, such that: $I\cap J=\emptyset$ and  $I\cup J\neq\emptyset$.
...
\item[$\bullet$] $\symb{SV}_k^{k\in K}$, with $K\in\I_\P$, is a set of
  ...
  $\forall i\!\in\! I_k.\,\alpha_{i}\!\in\!\Sort(\pNet_i)$\noteSB{What is the sort of a pNet? Should we define $\Sort(\pNet) \triangleq \{\Label(\symb{SV}_k\,|\,k \in K\}$?}, 
  $\forall j\!\in\!
...
of several \noteSB{These should all be pLTSs, right? Or did I miss something?}\hl{LTSs} with the
...
\begin{definition}[Open transitions]
	...
	\hl{LTS} $\langle S_i,s_{0 i}, \rightarrow_i\rangle$, and $\xrightarrow{b_j}_j$
...
\begin{definition}[Open automaton]
	...
	\begin{itemize}
		...
		\item[$\bullet$]  \noteSB{And one more above}\hl{$LTS_i^{i\in I}$} is a family of \hl{LTSs},
...
\noteSB{This is the first mention of the tool\mdash I would move this text much further.}\hl{In fact this OT is displayed in a simplied, human readable form. The
raw output of the tool is slightly more difficult to read, in
particular it uses structured names for the fresh variables generated
by the algorithm, allowing tracability of the result (see
appendix~\ref{appendix:variables}). A full output of 
...
\noteSB{line 8: $E$ is never updated, so $E = \emptyset$ always;\\
line 11: Would it not be better to prune unsatisfiable transitions on-the-fly, \ie inside the loop?}
...
\State Initialize sets $U=\mathit{InitState}(P)$, $E=\emptyset$ for unexplored/explored global states, $L=\emptyset$ for the resulting OTs;
\While{!$\mathit{isEmpty}(U)$}
	\State Choose $S$ in $U$;
	\State $\mathit{OT} = \mathit{MakeTransitions}(P, S)$;
        \State Store $\mathit{OT}$ in $L$; 
	\ForAll{$\mathit{OT} \in L$}
        \State Let $T$ be the target global state of $\mathit{OT}$
	\IfThen{($T \not\in U$ \&\& \hl{$T \not\in E$})}{Add $T$ into $U$;}
	\EndFor
\EndWhile
\State \hl{Prune the open transitions in $L$ using the SMT solver;}
...
\hl{For pLTSs at the leaves, we simply take the pLTS transitions of the
corresponding local state and build the predicate to generate the
open transition.}
On a pNet node we use two methods, combining and
matching, to generate the predicate of the open transitions in a
hierarchical manner.
%
Then, at the root of the pNet, the predicate of each OT is translated
into SMTlib assertions, and checked for satisfiability. Unsatisfiable
...
\noteInSB{Line 7: There are two type mismatches here: $ot$ is an open
...
\caption{MakeTransitions()}
  \label{alg2}
\begin{algorithmic}[1]
\Require a pNet node $Pn$; a global state $S$.
\State Initialize empty list $l$ and set $L$ for sub-transitions/transitions.
	\For{each $\mathit{Subnet}$ in $Pn$}
	~~~ $\setminus \setminus$ Recursively applying \hl{Tr1 or Tr2} on the sub-nodes.
                \State Store $\mathit{MakeTransitions}(Subnet, S)$ in $l$;
	\EndFor
	\For{each $sv \in SV$}
		\State $\overline{comb} = \mathit{Combining}(l)$;
                 \State \hl{$ot = \mathit{Matching}(sv, \overline{\mathit{comb}}, \mathit{hole})$;} % SB: removed the fourth parameter v here
                 \State Store $ot$ in $L$; ~~~ \hl{$\setminus \setminus$ if $\mathit{Filter}(ot)$ succeeds}
	\EndFor
\State \Return $L$;
\end{algorithmic}  
\end{algorithm}
...
Assume that there is a collection of $n$ subnets.\noteSB{These variables are not used and conflict with the returned set $L$ in Alg.~\ref{alg2}, on one hand, and control states $s_i$ of the subnets, on the other.}\st{
$L = {s_1, s_2, ...,  s_n}$;} We denote $\overline{ot}_i$ the set of open
transitions of the $i$-th
subnet; %% $s_i$ and 
``$-$'' means that the subnet is not involved.
The combination $\overline{comb}$, a set of n-tuples, is the
cartesian product:  
$\overline{comb} = (\inactive\cup \overline{ot}_1) \times (\inactive\cup \overline{ot}_2)\times \dots \times (\inactive\cup \overline{ot}_n)$\hl{, where, for each $i \in [1, n]$, $\overline{ot}_i$ is the set of open transitions obtained from the $i$-th subnet in line~3 of the algorithm}.
...
\hl{From a synchronization vector
$sv = \left((a'_i)^{i\in I} (b'_j)^{j\in J}\rightarrow v'\right) \in SV$
and its guard $G_k$; a tuple of open transitions
$C = ({ot}_i)^{i \in [1,n]} \in \overline{comb}$, such that, for each $i \in [1,n]$, either $ot_i = -$, or the result action of $ot_i$ is $a_i$; the hole behaviors $\mathit{Hole} = (b_j)^{j \in J}$; and a fresh variable $v$, representing the result action of the OT under construction, we build the predicate}
%
\begin{multline*}
  \mathit{Matching}(sv, C, \mathit{Hole}) = 
  \\
...
\begin{itemize}
  ...
  \item if some subnet action expression in the vector does not match (by
    pattern-matching) the corresponding \hl{action in $C$},
  \item or if the whole set of active subnet actions in the vector
    cannot be matched (by unification) with \hl{the tuple $C$}.
\end {itemize}

\st{Each of these possible filters is more precise than the previous
ones. But e}Even when unification succeeds, it is still possible that
...
\hl{we choose to minimize the open automaton (\ie its number of
transitions and states), by checking these predicates} for satisfiability.
...
with these actions, \hl{since it expects $\NameTimer.\PortStart$. This mismatch is materialised} %% as shown
by the predicate
fragment ``$\mathit{resume}(\true)=\mathit{start}(b_2)$''. 

\begin{figure}[t]
\begin{mathpar}
    ot = \openrule{\{s1 \xrightarrow{fail(false)} s1, t1 \xrightarrow{resume(true)} t0\}, ~~\{\xrightarrow{hb}\},~~fail(false)=fail(b_1)\\
    \OTland resume(true)=start(b_2)\OTland hb=fail(b_0)\OTland v=\underline{fail}\OTland b_1=b_2\\
    \OTland (b_1\lor b_2) \Rightarrow b_0, ~~\{\}}
    	   {\ostate{s1,t1} \xrightarrow{v} \ostate{s1,t0}}
  \end{mathpar}
...
section, we describe the translation of the \noteSB{The algebra presentation only appears here for the first time after its definition in Sect.~\ref{section:pnets}}\hl{algebra presentation}, of
...
We have also presented our implementation, which comprises two main parts. Firstly, we
compute all the possible open transitions. \noteSB{I do not understand this sentence. Suggestion: The actions of pLTS and hole behaviours are composed by the algoritm during the generation of predicates needed to match...(?)}\hl{The actions in pLTS and
...
Z3 solver. We implemented our algorithm in the VerCors platform and 
tested the algorithm on available examples, such as encoding expressions from various process algebras. Our case-studies show that our encoding
successfully identifies all unsatisfiable open transitions and that
...
In a previous paper~\hl{\cite{x}}, we
have already found the FH-bisimulation, inspired by~\cite{deSimone85}, to be a
suitable definition. However, the usual approach of computing bisimulation by partition
...
\begin{itemize}
   \item[$\bullet$] $prefix$ identifies the kind of the
     ...
     $SV$), ra (result action), hb\_<B> (behavior of hole $B$).
