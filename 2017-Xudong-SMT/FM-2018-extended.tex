...
\State Initialize empty list $l$ and set $L$ for sub-transitions/transitions.
	...
	\For{ each $sv \in SV$, for each $comb \in \overline{comb}$ }
           \State $ot = \mathit{Matching}(sv, \overline{\mathit{comb}}, \overline{\mathit{hole}})$;
           \State Store $ot$ in $L$; ~~~ $\setminus \setminus$ when $\mathit{Matching}(ot)$ succeeds
	\EndFor
\State \Return $L$;
\end{algorithmic}  
\end{algorithm}
...
cartesian product:  
$\overline{comb} = (\inactive\cup \overline{ot}_1) \times (\inactive\cup \overline{ot}_2)\times \dots \times (\inactive\cup \overline{ot}_n)$, where, for each $i \in [1, n]$, $\overline{ot}_i$ is the set of open transitions obtained from the $i$-th subnet in line~3 of the algorithm.
...
\begin{mathpar}
  ...
  \end{mathpar}
\caption{\TODO{Xudong revert to the previous example}One of the unsatisfiable open transitions in the Failure Monitor pNet}  \label{schema:unsat-ot}
\end{figure}
...
\begin{figure}[t]
  ...
  \caption{\TODO{Xudong: revert to the previous example}The input of the Z3 solver in SMT-LIB language and the output result}  \label{schema:smt-lib}
\end{figure}
