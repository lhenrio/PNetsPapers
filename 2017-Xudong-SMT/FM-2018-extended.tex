...
irrelevant.  $\uplus$ is the disjoint union on
indexed sets.
...
\TODO{I have pushed the rules in the appendix, we need to add a short high
  level desciption here.}




\TODO{ERIC: I choose here to remove the expample of construction of an
  OT as a deduction tree. First because it was already published in
  the Forte'16 paper, but also because it is not the way the algorithm
  is working.}
...
\TODO{Change...} In Fig.~\ref{schema:unsat-ot}, we display an example of an
...
that were already published in \cite{Forte}.
