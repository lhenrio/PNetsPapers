...
%%     to XFIG/macros.tex and added a com and to input this latter one
...
\item[$\bullet$] $\symb{SV}_k^{k\in K}$, with $K\in\I_\P$, is a set of
  ...
  $\forall i\!\in\! I_k.\,\alpha_{i}\!\in\!\Sort(\pNet_i)$\noteSB{What is the sort of a pNet? Should we define $\Sort(\pNet_i) \triangleq \{\Label(\symb{SV}_k\,|\,k \in K\}$?}, 
  $\forall j\!\in\!
...
automaton has a set of \emph{state variables} that can be assigned
...
\begin{example}\emph{An open transition.}
  This transition is generated by application of vector \texttt{SV0},
  synchronizing the initial actions of pLTSs \texttt{Failure} and
  \texttt{Timer}, with an action of the hole \texttt{B} equal to
  \texttt{fail}. The local variable \texttt{t} of the \texttt{Timer}
  is assigned to its initial value \texttt{Max}.
In fact this OT is displayed in a simplied, human readable form. The
...
the use-case is provided in appendice~\ref{appendix:fullOutput}.
  ...
  \label{OT:example-1}
	...
	{\{s0~{\xrightarrow{fail(true)}} ~s1, t0~{\xrightarrow{start(true)}} ~t1\}
	  \{\xrightarrow{hb}\}, hb=fail(true) \land v=\underline{fail}, \{t := Max\}
        }
...
appendix \ref{appendix:semRules}. 
...
\State Initialize sets $U=InitSate(P)$, $E=\emptyset$ for unexplored/explored global states, $L$ for result OTs;
\While{!isEmpty(U)}
	\State Chose $S$ in $U$;
...
\State \Return $OA=Reachable(InitState(P),L)$;
...
For pLTS at leaves, we simply take the pLTS transitions of the
...
\State Initialize a list $l$, $L$ for sub-transitions/transitions.
	\For{each Subnet in Pn}
	~~~ $\setminus \setminus$ Recursively applying Tr1 or Tr2 on the sub-nodes.
                \State Store MakeTransitions(Subnet, S) in $l$;
	\EndFor
	\For{each $sv \in SV$}
		\State $\overline{comb} = Combining(l)$;
                 \State $ot = Matching(sv, \overline{comb}, hole, v)$;
                 \State Store $ot$ in $L$; ~~~ $\setminus \setminus$ if Filter(ot) succeeds
	\EndFor
\State \Return $L$;
\end{algorithmic}  
\end{algorithm}
...
then the combination $\overline{comb}$, a set of n-tuples, is the
cartesian product of sets :  
$\overline{comb} : (\inactive\cup \overline{ot}_1) \times (\inactive\cup \overline{ot}_2)\times ... \times (\inactive\cup \overline{ot}_n).$
...
$C_n\in \overline{comb} = {(v_i)}^{i\in I}$, the hole behaviors $Hole$,
...
Each of these possible filters is more precise than the previous
...
the failure controller performs a ``$fail$'' action, while the timer
executes a ``$resume$''. The chosen synchronization
vector (SV0 from Fig.~\ref{schema:ArchFailure:pNet}) does not match
with these actions, as shown by the predicate
fragment ``resume(true)=start($b_2$)''. 
...
In appendix \ref{appendix:mapsmt} we define formally a condition of
...
appendix~\ref{appendix:translating-assignments}). This appears in
Fig.~\ref{schema:smt-lib} in line 10, that encodes possible values of
...
In appendix \ref{appendix:mapsmt} we formally define the translation of each
...
checking method using Z3. For example, if the $\mathit{functions}$ and the
