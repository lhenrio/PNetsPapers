...
\begin{example}[An open transition]
 ...
 A full output of 
the use-case is provided in ~\cite{RR}.
...
pLTSs, one for pNet nodes. These rules are improved, adding guards in the synchronisation vectors and quantifier in the guards, in ~\cite{RR}.
...
by the algorithm, allowing tracability of the result~\cite{RR}.
...
In~\cite{RR}, we  define the condition of
...
In~\cite{RR} we formally define all steps of the translation of each
...
slighly bigger example, assembling 2 Failure controllers. In~\cite{RR}, we show that Z3 can check the
