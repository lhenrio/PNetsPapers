...
Xudong \MakeUppercase{Qin}\thanks[inria-sam]{Universit\'e C\^ote d'Azur, INRIA, CNRS, 06902 Sophia Antipolis, France}\thanks[ecnu]{Shanghai Key Laboratory for Trustworthy Computing, East China Normal University, Shanghai, China}
\and
Simon \MakeUppercase{Bliudze}\thanks{INRIA Lille-Nord Europe, 40 avenue Halley, 59650 Villeneuve d'Asq, France}
\and
Eric \MakeUppercase{Madelaine}\thanksref{inria-sam}
\and
Min \MakeUppercase{Zhang}\thanksref{ecnu}
%\thanks{MoE Engineering Research Center for Software Hardware Co-design Technology and Application, East China Normal University, 200062, Shanghai, China}
}

%% Ceci apparait sur chaque page paire.
\authorhead{Qin \& Bliudze \& Madelaine \& Zhang}

\RRetitle{Using SMT engine to generate Symbolic Automata - Extended version}
\RRtitle{Utilisation d'un Moteur SMT pour g\'en\'erer des Automates Symboliques- Version \'etendue}
\titlehead{Using SMT engine to generate Symbolic Automata}

\RRnote{KAIROS is a common team between the I3S Lab (Univ. of Nice
  Sophia-Antipolis and CNRS) and INRIA Sophia Antipolis M\'editerran\'ee.\\
This work was partially funded by the Associated Team FM4CPS between Inria and ECNU Shanghai.}

\RRabstract{Open pNets are used to model the behaviour of open systems, both
...
\RRresume{Les pNets ouverts sont utilisés pour modéliser le comportement des systèmes ouverts, synchrones ou asynchrones, exprimée dans divers calculs ou
langages de programmation. Ils sont dotés d'une sémantique opérationnelle symbolique en termes d'«Automata Ouverts». Cela nous permet de vérifier les propriétés de ces systèmes d'une manière compositionnelle.
Nous avons implémenté un algorithme calculant ces sémantiques,
en construisant des prédicats exprimant les conditions de synchronisation
entre les actions des composants du pNet. La vérification de tels
prédicats nécessite un raisonnement symbolique sur les logiques de premier ordre, mais également sur des données spécifiques à l'application. Nous utilisons le moteur SMT Z3 pour vérifier la
satisfiabilité des prédicats, et ne conserver dans l'automate ouvert que les
transitions satisfiables.

Nous illustrons notre approche par un exemple d'inspiration industrielle.
Pour cela nous partons d'«architectures» de systèmes BIP, qui ont été utilisés dans le cadre d'un projet de l'Agence Spatiale Européenne pour spécifier le logiciel de contrôle d'un nanosatellite au Centre d'ingénierie spatiale de l'EPFL. Nous utilisons les pNets pour encoder une architecture BIP
étendu avec des données explicites, et calculer sa
sémantique en termes d'automates ouverts. Cet automate peut être utilisé pour prouver des 
propriétés comportementales; nous donnons 2 exemples, une propriete de sureté et une de vivacité.}
 
\RRmotcle{S\'emantique comportementale symbolique, Analyse compositionelle de logiciels, R\'eseaux d'automates synchronis\'es}
\RRkeyword{Symbolic behavioral semantics, 
Compositional analysis of software systems, 
Networks of synchronized automata}
\RRprojet{Kairos}
%\RRprojetcommun{Scale}
...
\section{Introduction}\footnote{This Research Report is the extended version of the eponym paper presented at the 18th Int. Workshop on Automated Verification of Critical Systems (AVOCS'18) in Oxford, July 2018.}
...
\begin{definition}[Open transitions]
	...
	An \emph{open transition} $OT$ over a set  $\langle S_i,s_{0 i}, \rightarrow_i\rangle^{i\in
	I}$ of LTSs, a
	set $J$ of holes, %with sorts $Sort_j^{j\in J}$
	...
	\end{mathpar}
	where $s, s'\in\mathcal{S}$, the $a_i, b_j, \alpha$ are
        action expressions, $s_i{\xrightarrow{a_i}}_i s_i^{\prime}$ is
        a transition of the 
	LTS $\langle S_i,s_{0 i}, \rightarrow_i\rangle$, $b_j$
	...
	over the variables of the terms, labels, and states $s_i$,
        $b_j$, $s$, $\alpha$. \Post\ is a set of equations that  
...
pLTSs, one for pNet nodes.  For convenience we also include these rules in
App.~\ref{appendix:semRules}.

In \cite{henrio:Forte2016} we also
proved that a pNet with finitely many pLTSs, holes, and
synchronisation vectors, and with finite pLTSs, has a finite
semantics. Remark that all the elements of such pNets and open automata being
...
unexplored set, until this set is empty. As the set of
global states is finite (under the conditions above), this terminates.
...
$$
  \mathit{MkPred}(sv, C, \overline{\mathit{hole}})=(\forall i\in I, a_i=a'_i) \land
...
from synchronisation vectors and from pLTS transitions, using the
satisfiability check in the SMT engine.


\subsection{Management of state variable assignments}
\label{section:state-variables}
...
%% If the result is ``sat'', and more details are needed, Z3 can exhibit a
%% model (i.e. an instanciation of the variables) showing the satisfiability.
...
In Appendix~\ref{appendix:well-formedness} and~\ref{appendix:mapsmt}, we  define the condition of
...
and predicates of the algebra data-types.
...
Appendix~\ref{appendix:fullOutput}, involve $\OTvar{resume(false)}$ from
...
$\NameCtrl$.  Therefore, under reasonable scheduling assumptions,
...
\emph{Scaling up.} \label{paragraph:scalingup} One important motivation of this work is to
...
slighly bigger example, assembling 2 Failure controllers. In
appendix~\ref{appendix:scalingup}, we show that Z3 can check the
...
\appendix
%\section{Appendix}
These appendices include details that were kept out of the original paper by lack of space.
...
that were already published in \cite{henrio:Forte2016}. These rules are referenced from section \ref{section:implementation}, \pageref{section:implementation}, and implemented by the algorithms \ref{alg1} \& \ref{alg2}.
Compared to the rules in \cite{henrio:Forte2016}, they were slightly extended to take into account guards in the synchronisation vectors.
...
Given an open pNet $\mylangle \overline{\pNet},\overline{S}, \symb{SV}_k^{k\in K}\myrangle$ with leaves $pLTS_i^{i\in L}$ and holes $Hole_j^{j\in J}$, if the sets $L$ and $J$ are finite, if the synchronization vectors of all pNets 
...
management of state variable assignments. These were described shortly
in Section \ref{section:state-variables},\ref{section:translation}.
...
\begin{itemize}
  \item $SVars(gs)$ the state variables of all states in the global
    state $gs$
  \item $Assigns(svar)$ the set of possible assignments (= expressions) of variable
      $svar$.
\end{itemize}
...
pNet Algebra presentation and predicates. This is the first part of the
translation introduced in section \ref{section:translation}.
...
\begin{minipage}{\textwidth}
	\caption{\label{Table:predefinedSorts}\leftline{Algebra Presentation: predefined Sorts and Operators}}
	\begin{tabular}{p{3cm}p{3cm}p{6cm}}
		\hline\specialrule{0em}{1pt}{1pt}
		Sort & Constructors & Auxiliary Operators
                \\\specialrule{0em}{1pt}{1pt}
...
\end{minipage}
\begin{minipage}{\textwidth}
\caption{\label{table:BIPoperators}\leftline{Additional operator for the BIP algebra}}
	\begin{tabular}{p{3cm}p{3cm}p{6cm}}
		\hline\specialrule{0em}{1pt}{1pt}
		Sort & Constructors & Auxiliary Operators
                \\\hline\specialrule{0em}{1pt}{1pt} 
...
\end{minipage} 
\begin{minipage}{\textwidth}
	\caption{\label{table:CCSalgebra}\leftline{Additional sorts and operators for the CCS algebra}}
	\begin{tabular}{p{3cm}p{3cm}p{6cm}}
		\hline\specialrule{0em}{1pt}{1pt}
		Sort & Constructors & Auxiliary Operators
                \\\specialrule{0em}{1pt}{1pt}
...
\label{appendix:well-formedness}
...
In this section we define separately (as introduced in Section~\ref{section:translation}):
\begin{itemize}
...
%\begin{table}\caption{\leftline{Mapping}}
	\begin{tabular}{p{3cm}p{9cm}}
		...
		&$Other operators \lhook\joinrel\longrightarrow$	declare-fun		\\\specialrule{0em}{1pt}{1pt}
		(Open Transition)							
		&								\\\specialrule{0em}{1pt}{1pt}
...
\subsection{Presentation Translation} %\TODO{Duplicate ? no,
...
use-case, as summarized in section~\ref{section:resultOA} page~\pageref{section:resultOA}. It is displayed in full and raw format, each OT containing
...
In our discussion~\pageref{paragraph:scalingup}, we mentionned an experiment done with a slightly larger example.
