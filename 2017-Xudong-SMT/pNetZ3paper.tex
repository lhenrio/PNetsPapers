...
% Tr1 :
% 1. Get the start state of the pLTS through the *TrackIndexImpl( )*.
% 2. For all the transitions start with the start state.
%       1. Get the target state of the transition. Generate the target global state according to the target state.
%       2. Get the guard of the transition, add it into the predicate of the open transition. If there is no guard, add *true*.
%       3. Store the generated open transition in the open transition list.
% Tr2 :
% It is more complicated, the SV (synchronization vector) need to be cloned, the variable actions need a fresh variable (renaming), and it need deal with the parameters come from the subnet.
...
%       - If it is a pLTS, use the Tr1 recursively, store the open transitions list of the pLTS in the list of keeping subnets' open transitions.
%       - If it is a hole, generate a new hole behavior for the open transition. 
%           1. Count the number. The hole behaviour counter increase one.
%           2. Build a fresh hole behavior through the method *fresh( )*.
%           3. Generate the hole behavior with the type *ActionExpression*. Store it in the hole behaviors list of the open transition.
%       - If it is a pNet, use the Tr2 recursively, store the open transitions list of the subnet in the list of keeping subnets' open transitions.
% 3. Generate a new result action for the open transition.
%       1. Count the number. The result action counter increase one.
%       2. Build a fresh result action through the method *fresh( )*.
%       3. Generate the result action with the type *ActionExpression*.
% 4. For all SVs of the pNet.
%       1. Invoke the *mixing( )* method. Match result action and the combination of the subnets' open transitions and hole behaviors with the SV to generate the predicates of the open transition. Then a possible open transition of the pNet is generated.
%           - Every time the SV is chosen it need be cloned by the method *clone( )* and the cloned one is used for matching. 
%           - Similarly, the variable elements of the combination are also need fresh variables.
%       2. Store the open transition of the pNet in the opentransition list.
