...
\ERIC{  The rule uses TR1 for the $l$ transition of $C_1$ and for the $b$ transition of $C_2$, uses an action $b_P$ of hole $P$, then combines the result using the second vector of the PN2 sub-pNet, and the third vector of the top node. This yield a final $\tau$ transition. The deduction tree in the figure shows how the predicate are generated in this process.
}
...
The proof can be found in \cite{FORTE16}.
...
\QIN{
We have used a brute-force method to generate all the possible open transitions in the open automaton. It obviously contains some of unsatisfiability in the open transitions. In Fig~\ref{schema:unsat-ot}, we show an unsatisfiable open transition from the result of the CCS running example. It presents the case that the top level transmits out the action from the PN2, formula $a.P$, as this time only PN2 is working and C1 is performing action $a$ when hole $P$ is also working. In this case, the second synchronization vector of PN2, transmitting the behavior from hole $P$ to the higher level, is used while it is obviously violate the guard of the vector. So we can easily find the contradiction between the generated predicate term $ra:152:1=l \!\land\! a=:ra:152:1$ and the guard composed into the predicate $a \neq l$. To get a correct open automaton of the pNet, such unsatisfiable open transitions need to be eliminated. We choose a SMT solver Z3 as the checker for the open transitions considering the predicates may contain the expression using uninterpreted functions declared by users.
}
\begin{figure}[t]
$\begin{array}{@{}r@{}l@{}}
...
$\begin{array}{@{}r@{}l@{}}
    ot_1  = &\openrule{\{0 \xrightarrow{a} 1, 0 \xrightarrow{b} 1\}, ~~a=ich(v_{11}) \!\land\! b=ch(v_{11}) \!\land\! a\neq l \!\land\! b\neq r\!\land\! v_1=\tau}
    {\ostate{00} \xrightarrow{v_1} \ostate{11}}\\
  \end{array}$
  
  \begin{minipage}{6cm}
$\begin{array}{@{}r@{}l@{}}
    ot_3  = &\openrule{\{0 \xrightarrow{a} 1\}, ~~a\neq l \!\land\! v_3=a}
    {\ostate{00} \xrightarrow{v_3} \ostate{10}}\\
    ot_5  = &\openrule{\{0 \xrightarrow{b} 1\}, ~~b\neq r \!\land\! v_5=b}
  ...
  \begin{minipage}{6cm}
$\begin{array}{@{}r@{}l@{}}
    ot_7  = &\openrule{\{1 \xrightarrow{l} 1\}, ~~\{\xrightarrow{hb_{17}}\}, ~~v_7=hb_{17}}
    {\ostate{10} \xrightarrow{v_7} \ostate{10}}\\
  \end{array}$
  \end{minipage}
    
$\begin{array}{@{}r@{}l@{}}
    ot_9  = &\openrule{\{0 \xrightarrow{b} 1, 1 \xrightarrow{l} 1\}, ~~\{\xrightarrow{hb_{19}}\}, ~~b=ich(v_{19}) \!\land\! b\neq r \!\land\! hb_{19}= ch(v_9)\!\land\! v_9=\tau}
    {\ostate{10} \xrightarrow{v_9} \ostate{11}}\\
  \end{array}$

$\begin{array}{@{}r@{}l@{}}
    ot_{13}  = &\openrule{\{1 \xrightarrow{l} 1, 1 \xrightarrow{r} 1\}, ~~\{\xrightarrow{hb_{113}}, \xrightarrow{hb_{213}}\}, ~~hb_{113}= ich(v_{13}) \!\land\! hb_{213}= ch(v_{13}) \!\land\! v_{13}=\tau}
    {\ostate{11} \xrightarrow{v_{13}} \ostate{11}}\\
  \end{array}$

$\begin{array}{@{}r@{}l@{}}
    ot_{15}  = &\openrule{\{1 \xrightarrow{l} 1\}, ~~\{\xrightarrow{hb_{115}}\}, ~~v_{15}=hb_{115}}
