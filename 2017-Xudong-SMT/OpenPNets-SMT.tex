...
The root node of the tree PN1 is the top level of the pNet
  ...
  one of the subnets is working alone. The vector syntax is typically
  ``<act, \_ > -> act'', in which ``\_'' means that the corresponding
  subnet is not involved in this synchronisation.
  \TODO{The vectors for communication are wrong, we cannot use ?ch(v)
    and !ch(v), the action algebra has changed since PDP'2015, now you
    must remove the ``?'' and ``!'' from the vectors. }
  Remark that these 4
...
\item[$\bullet$]
\QIN{
The $x_j$ here is called the $state\ variable$ of state $s'$. $State\ variables$ can only be modified by the assignment in the transition targets to its owner state. The global state variable of a pLTS is the variable $x$ satisfying $\forall s\!\in\! S .\, x\!\in\! \vars(s)$.
}
...
The semantic of the open pNets is already defined as an open automaton in \cite{FORTE16} using open transitions to present the transitions of its global states.
...
\begin{itemize}
	\item Sorts: Constants sets of the algebra, types of the data
          and actions. 
	\item Operators: Operators comes from other algebras \TODO{why
          others ? there can be action constructors also}
          constructors for the parameterized actions.
...
\paragraph{Implementation of the expressions meta-model}
...
\QIN{
VerCors platform uses pNet as the intermediate language for some
language or graphical formalism to be translated into both input for a
checker and for generating code automatically to ease the
specification from non-expert users\cite{HKM-FASE16}, compiling it
from those language or graphical formalism in the pNet API. We compute
the open automaton of an open pNet also in that pNet API directly
implementing the sketch showed before. However, we made some
modification, merging the step 2a-2b and applying Tr2 with the
premisses from subnets to generate open transitions, application of
Tr1 is still conducted in a simple way. At same time, we apply the
fresh function to rename all the variable to make them unique and
readable. Besides that, we also have a management of assignments
throughout the whole computation. Satisfiability check is done only at
top level of the deduction tree construction. An alternative would be
to submit the satisfiability check to the SMT solver at each level of
the tree construction, potentially reducing the overall number of
...
\TODO{Xudong: Missing a general summary before going into details. You
  can get inspiration from the following (reasonable level of
  abstraction):}
  
\ERIC{
=> Context = use the existing software architecture from VCE (ref to \cite{HKM-FASE16} paper)\\
