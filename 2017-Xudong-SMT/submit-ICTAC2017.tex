...
\State Initialize a set for unexplored global states $U$, a set for explored global states $E$;
\While{!isEmpty(U)}
	\State Extract a global state $S$ from the $U$;
	\State Start at the root pNet node
	\State Recursively applying Tr1 or Tr2 on it and its subnets;
	\If{Subnet is pLTS, apply the Tr1} 
		\For{each transition $TR$ start from the state $s$ in $S$}
			\State Generate open transition $OT$ according to $TR$;
			\State Store $OT$ into the result list $L$;
		\EndFor
	\EndIf
