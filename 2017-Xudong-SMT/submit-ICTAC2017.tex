...
\paragraph{Structure.}
In \ref{section:examples} we present a simple example that will  illustrate all parts of the paper. Then in the
background section
...
We choose the expression \texttt{P>>(accept(v);Q)}, that includes the
Enable and Prefix 
...
In Fig.~\ref{schema:lotos-pnet}, we show the encoding of the formula using our encoding.

\ERIC{
  This pNet is composed of: a toplevel node PN1 encoding the Enable
  operator, with its ``controller'' C1; a ``hole'' representing the
  process variable P; another node PN2 representing the Prefix
  operator, with its controller C2, and its hole Q. Controllers are
  labelled transition systems (pLTS) with state variables and value-passing
  transitions, defining the dynamics of the operator semantics.}

In order to show all features of the pNet model, we choose to use two
styles of encodings used in the controllers.
The most common style used in process calculi is ``state oriented'',
we use it here for C2: it has two states to
indicate the position of the control point in the right side of the
Prefix expression $(acc(v);Q)$. The second one is
``data oriented'': controler C1 has a single state, with a state
variable s0 encoding the change of control point from left to right sub-net.
...
Three synchronization vectors are given for synchronizing the behavior
of the subnets to make the pNet act as the Enable operator. The first
...
to the corresponding transition of the controller C1.
The second vector composes the
action from the subnets and gives out a synchronized action
...
convenient for observing internal events during model-checking. The
second vector constraints that the transition occurs only when process
$P$ performs a $\delta(x)$ action, transferring the control to $Q$,
and sending a value $x$ at the same time. This value must be
``accepted''  through an $acc(x)$ action before $Q$ taking the
control. The last one acts as the first one, but transmitting the
action $a_2$ of Q.  
...
section \ref{section:full-result}. We detail one of its transition here:
...
Remark the structured states in the conclusion of this rule, encoding
the combination of state-spaces of the leaves: both of them start in
state $\ostate{00}$, where both $C_1$ and $C_2$ are in their initial
states. They end in different structured states, because in $ot_2$
...
automaton representing the behavior of an open pNet, and we prove that
...
$\begin{array}{@{}r@{}l@{}}
    ot  = &\openrule{\{0 \xrightarrow{\delta} 0\}, ~~\{\xrightarrow{:hb:12:1}\}, ~~:ra:11:1=l \!\land\! \delta=:ra:11:1 \!\land\! s0=0 \\
     \!\land\! \texttt{:hb:12:1}=\texttt{a1:sva:1:1} \!\land\! :ra:1:1=a1:sva:1:1 \!\land\! a1:sva:1:1 \neq \delta(x), ~~\{s0 \leftarrow 1\}}
