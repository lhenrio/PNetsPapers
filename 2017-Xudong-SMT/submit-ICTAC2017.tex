...
However, more details are needed when doing the implementation. 
Algorithm 1 shows there are much more things added, such as how we apply 
the Tr1 or Tr2.
...
\State Initialize the initial global state $G$ of the open automaton, add it into $U$;
\State Initialize a list $L$ for storing open transitions;
\While{!isEmpty(U)}
	\State Extract a global state $S$ from the $U$;
	\State For the pNet node, recursively applying Tr1 or Tr2 on its subnets;
	\If{Subnet is pLTS, apply the Tr1} 
		\State Generate open transition $OT$ according to the transitions in pLTS;
	\EndIf
	\If{Subnet is pNet, apply the Tr2} 
		\For{each $sv \in SV$}
			...
			\State Generate open transition $OT$ using the predicate from the former steps;
		\EndFor
	\EndIf
	\State Store $OT$ into $L$;
	\State Get the target global state $T$ from $OT$;
	\If{$(U.contains(T))\ ||\ (E.contains(T))$} 
		\State Add $T$ into the $U$;
	\EndIf
\EndWhile
