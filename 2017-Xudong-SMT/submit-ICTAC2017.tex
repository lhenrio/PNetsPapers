...
\item We validate the whole approach on small use-cases covering all
...
We choose here the expression \texttt{P>>(acc(v);Q)}, that includes the
Enable and Prefix 
...
operator, a pLTS C1 acting as the {\sl controller} of the node; a hole
representing the 
...
Controller C2 is ``state oriented'': it has two states to
...
The root pNet node PN1 has 3 sub-pNets: its controller C1,
the hole P and the sub-pNet PN2.
Three synchronization vectors define the possible
synchronizations between the actions of the subnets.
The first vector ``$< l, a_1 , \_ > -> a_1$'' transmits an action
$a_1$ of P to the upper level. Here the ``l'' action is related
to the corresponding transition of the controller C1, ``$\_$'' means
that the corresponding subnet is not involved in this synchronization,
and there is a guard meaning this synchronisation can only occur if
$a_1$ is not a $\delta$.
...
The vector result $\underline{\delta(x)}$ is a ``synchronized''
action, that cannot be further
synchronized at upper levels. Synchronized actions is a useful
...
%% It is parameterized both at the level of
...
%% \QIN{[This CCS example is removed.]}
...
\State Initialize sets $U$, $E$ for unexplored/explores global states
$U$, $L$ for result OTs;
\While{!isEmpty(U)}
	\State Extract a global state $S$ from the $U$;
	\State Recursively applying Tr1 or Tr2 on the root node and its subnets:
	\If{Subnet is pLTS}
                \State $s$ is the local state of the pLTS in the global state $S$;
		\For{each transition $tr$ starting from $s$}
			\State apply Tr1 to build the corresponding
                        $OT$; store $OT$ in $L$;
		\EndFor
	\EndIf
	\If{Subnet is pNet, apply the Tr2} 
		\For{each $sv \in SV$}
			\State Invoke $Combining()$ to generate the combination of sub-transitions;
			\State Invoke $Matching()$ to match the combination with the $sv$;
			\State Store the result set of open transitions into $L$;
		\EndFor
	\EndIf
	...
	\EndFor
\EndWhile
\State Prune the open transitions in $L$ using SMT solver;
\State \Return $L$;
...
\QIN{
For the open pNets, in previous work, we already have a sketch for computing the open automaton.
The computation repeats applying Tr1 and Tr2 to deduct all the possible open transitions and pruning the 
unsatisfiable transitions by SMT solver until all the global states are explored. 
In the end, a reachability check is supposed to potentially prune the state space. 
Algorithm 1 is what we used in the implementation, there are much more details added based on 
the sketch, such as how we apply the Tr1 or Tr2.
}
...
Then it keeps presenting the behavior of $Q$. This can also be seen in $ot_3$. 
...
$\begin{array}{@{}r@{}l@{}}
  ...
  $\begin{array}{@{}r@{}l@{}}
    ot_3  = &\openrule{\{0 \xrightarrow{r}_{C_1} 0, 1 \xrightarrow{q}_{C_2} 1\}, ~~\{\xrightarrow{hb_{3}}_Q\}, ~~[s_0=1 \!\land\! hb_{3} \neq acc(x) \!\land\! v_3=hb_{3}]}
