...
Our implementation does not build explicitely the proof trees for each
open transition, but instead : [Combining] enumerate all possible combinations of
pLTS transition (using TR1) and each synchronisation vector choice
...
\subsection{Management of state variable assignements}
...
that assign potentialy different values to a state variable.
...
synchronisation vectors. Naturally this builds some transtions where
...
vector is chosen that does not match with this behavior, and we can
easily find the contradiction in the 
generated predicate term ``$\OTvar{:ra:11:1}=l \!\land\! \delta=\OTvar{:ra:11:1}$''. 
...
Generaly, some $datatypes$ are declared to input the sorts, while here
...
translations, such as $\{s_0 \leftarrow 1\}$ in
...
Running our algorithm for our LOTOS example yields 30 open
transitions, out of which 25 are detected unsatisfiable by Z3, so we
get only 5 results left; the resulting open automaton is shown in
Fig. ~\ref{schema:lotos-result}, we have drawn manually the open
automaton obtained as the final result generated by algorithm.  
