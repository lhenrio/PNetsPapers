...
\ERIC{The set $L1$ may contain unreachable transitions. To remove those transitions, the last step of the
...
From the structure of the main loop of the algorithms, we can get that algorithm~\ref{alg1} computes less transitions than algorithm~\ref{alg3}, i.e. $L2_{reach}\subseteq  L1$, and as a consequence $States(L2_{reach})\subseteq  States(L1)$, so we have:
...
Suppose that  $\ostate{\tilde{t}}\in States(L2_{reach})$. It is reachable in the result automaton of Algorithm 3, so there exists a sequence of transitions 
\[\ostate{\tilde{t_0}}\xrightarrow{l_1}\ostate{\tilde{t_1}}\xrightarrow{l_2}...\xrightarrow{l_{n}}\ostate{\tilde{t}}\] where in each label of the transition $Pred_{l_i}^{i\in [1,n]}$ is satisfiable. For the same state $\ostate{\tilde{t}}\in States(L1_{reach})$, there exists a sequence of transitions \[\ostate{\tilde{t_0}}\xrightarrow{l'_1}\ostate{\tilde{t_1}}\xrightarrow{l'_2}...\xrightarrow{l'_{n}}\ostate{\tilde{t}}\]where for each transition $Rename(\ostate{\tilde{t_{i-1}}}\xrightarrow{l_{i}}\ostate{\tilde{t_{i}}}) = \ostate{\tilde{t_{i-1}}}\xrightarrow{l'_{i}}\ostate{\tilde{t_{i}}}$ and $Pred_{l'_i}^{i\in [1,n]}$ is satisfiable. 
From the structure of the algorithms, 
\[\forall \ostate{\tilde{t}}\in States(L2_{reach}), \ostate{\tilde{t}}\xrightarrow{l'_{n+1}}\ostate{\tilde{t'}}\in L2_{reach}\]
\[\exists \ostate{\tilde{t}}\xrightarrow{l_{n+1}}\ostate{\tilde{t'}}\in L1 \quad s.t.\quad Rename(\ostate{\tilde{t}}\xrightarrow{l_{n+1}}\ostate{\tilde{t'}}) = \ostate{\tilde{t}}\xrightarrow{l'_{n+1}}\ostate{\tilde{t'}})\]
...
We define the function $States$ as before, and the function $Assignments : \langle OT\rangle\times State\rightarrow\langle Assign\rangle$
computing the set of assignments of a global state from a given set of OTs.

The structure implicitly computed by the algorithm is a triple $ Aut= \langle OTs, States,\\ Assigns\rangle$.

Then the assignments set of $OA_1$ is $\{Assignments(L1, State_{i})\}^{State_{i}\in States(L1)}$ and $OA_2$ is $\{Assignments(L2_{sat}, State_{j})\}^{State_{j}\in States(L2_{sat})}$. The result triples should be 
...
Basing on the notation of the triple, we define the function $Refine : Aut \rightarrow Aut$ implementing $makeReachableSubAutomaton(S0,OTs)$, it uses the assignments of the previous step to refine the computed reachable space.
\begin{align*}
Refine(Aut) &=\langle L', States(L'), \{Assignments(L', State_{i})\}^{State_{i}\in States(L')}\rangle \\
&=Aut'
\end{align*}
where $L' = L - L_{unsat}$, $L_{unsat}$ is the set of unsatisfiable transitions checked in this iteration. So there must have $|L'|<|L|$, and potentially, their target states and corresponding assignments could be reduced: 
...
\noteSB{I guess pNets in this figure have to be replaced by open
  automata}
...
\hl{Application of the Failure Monitor architecture ensures that, whenever
...
units (more details in Section~\ref{section:FailureTimer}).}
...
coordinator component is encoded as a PLTS; dangling ports are encoded as actions of the hole;
...
triggers, we use the classical approach where non-participation of a
port in an interaction is simulated by an additional loop
transition~\cite{milner83-calculi}.  To this end, we consider an
action algebra involving additional Boolean variables: the effective
transitions carry the value $\true$ (\eg $\cstate{0}
