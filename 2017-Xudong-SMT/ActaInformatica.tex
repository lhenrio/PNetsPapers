...
If we use the data-oriented pNet Enable2 from Figure \ref{schema:enable-pnets} to building a similar
``P>>(Q>>R)'' expression, the corresponding open transition would be:
  ...
  \label{OT:example-1}
	...
	{\{\xrightarrow{\delta(x3)}_P ~~
          \xrightarrow{acc(x3)}_Q \} ~~
          [v_1=0 \land v_2 = 0 \land \forall z. acc(x3) \neq
            \delta(z)] ~~ \{v1:=1\}
        }
	{\ostate{0,0} \xrightarrow {\underline{\delta(x3)}} \ostate{0,0}}
	\end{mathpar}
        where $v_1$ and $v_2$ are respectively the state variables of
        the first and second Enable2 pNet. Indeed the global
