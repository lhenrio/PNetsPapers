...
\author{ Xudong Qin\autref{1}\sponsor{This work was partially funded by the Associated Team FM4CPS between INRIA and ECNU, Shanghai},
  Simon Bliudze\autref{2}, Eric Madelaine\autref{3}, Zechen Hou\autref{1}, Yuxin Deng\autref{1}, and Min Zhang\autref{4}\todoMargin{Should we remove Min ?}} 
...
\item We implement the interaction between our algorithm and the Z3
    ...
    \item \ERIC{we define a strategy checking satisfiability of open transitions in the course the residual algorithm, and show that this ensures that we still generate the reachable part of the open automaton, and that what is generated is minimal with respect to the decidability of predicate decidability} 
...
\begin{example}[An open transition]
 ...
 A full output of 
the use-case is provided in~\cite{Avocs-RR}.
...
\TODO{the proof of the finiteness theorem refers to SOS rules Tr1 and
...
\begin{proof}
  \noteEM{I tentatively copied the proof here, from RR-8898. We should
...
reachable global states.  \TODO{Make this more precise : sat and reachability is computed only at the end...}
...
\TODO{add a high level description of the new strategy, discuss decidability/approximation of the SAT checking, express some ``minimality'' property}
...
\section{Use-cases}
\TODO{new section here, showing various features on different examples. Possible: Enable operator in Lotos, with (equivalent) state-based and data-based encodings; Associativity of Enable; simple communication protocol, specification and implementation; BIP FailureTimer architecture, including non-trivial ``Interval'' data-type and operators; without and with encoding of maximal progress.}

\subsection{Result for the running example}
\label{section:resultOA}
\noteEM{This is the original example from the Avocs paper}
...
\subsection{Extended version of the FailureTimer BIP Architecture}
\label{section:resultTimerInterval}
\noteEM{Here we have extended the FailureTimer architecture using time intervals,
  that makes more interesting the timeout/reset temporal behavior}

We present here an extended version of the FailureTimer BIP
architecture, in which the timeout action can occur
non-deterministicaly during some time interval. This behavior is more
realistic, but ensuring safety is a bit more complicated, as it
involves reasonning on the time intervals in the predicates of the
open automaton.


\begin{figure}[t]
  \centering
  \includegraphics[width=0.9\columnwidth]{XFIG/BIPspec-ArchFailureTimerMax-v4}
  \caption{The BIP specification of the Failure Monitor architecture}
  \label{fig:schema:ArchFailure:BIP}
\end{figure}

Figure \ref{fig:schema:ArchFailure:BIP} shows components
...
\TODO{refer to Research Report (not yet avaialble) or add an appendix,
  for a full explanation of this example, 
  and the translation from BIP to pNets}
...
Figure \ref{fig:FailureTimer:pNet} shows the encoding of the Failure Monitor
