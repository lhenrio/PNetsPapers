...
Later on, symbolic semantics were extended to the $\pi$-calculus \cite{Milner99} and decision algorithms for various symbolic bisimulations were proposed \cite{Li99,Deng2001}. However, no
...
In Section~\ref{sec:cases} we give a few use-cases, in particular, a BIP
...
$\P$ fix the term algebra elements $\Talg, \AlgB_{\P}, \AlgA_\P$.
...
Figure \ref{schema:enable-pnets}, we use a controller $C_1$ with two
...
actions do not play any special role for defining
...
composing two occurences of the EnableState pNet. %For simplicity, we only represent one instance of the synchronisation vector set, and of the controller.
In Section~\ref{section:use-case:Enable} we show how these encodings can
...
parametrised actions, but include neither guard nor assignments.
...
\begin{example}[Open transitions]
  ...
  trivially true. The global states shows that controler $CState1$ has
...
The semantics of pNet in terms of open automata has been defined in
...
the proof of the finiteness theorem refers to SOS rules Tr1 and
  ...
  to add an appendix with the semantic rules (constructing open
...
semantics and the interaction with the Z3 SMT solver. Under the hypotheses of Theorem \ref{Th:finiteness}
...
into SMTlib assertions, and checked for satisfiability (function \texttt{filterUnsatTransitions}).
...
approximation, and potentialy more information, that can be used to
...
state space. So the smart algorithm performs in two steps: during the
...
the open automaton need not to collect the set of possible values dealing
...
\subsection{Enable Operator} in Lotos, with (equivalent) state-based
and data-based encodings; Associativity of Enable;
...
The open automata generated for the 2 encodings of Enable in
Figure~\ref{schema:enable-pnets} are displayed in
Figure~\ref{schema:enable-OAs}. We have shown in \ref{Forte} how to
prove that these two automata are bismilar.
...
\bigl(\PortFail \rightarrow \EX \EU[]{\lnot \PortFail}{
  \PortReset}\bigr)$.  Additional properties are provided in \cite{TimerRR}.
...
\noteInEM{Modifications stopped here}
\label{section:use-case:Double}
\TODO{Enhanced FailureTimer, 2 instances composed, with duplicated control and Timer}

In the conception of the nanosatellite software \cite[], the FailureTimer architecture was used to control several processes
...
and then they will tick, and timeout, synchrounously.


\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{ActaXFIG/BIPspec-DoubleArchFailureTimer-v2-2}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{ActaXFIG/Double-OA}
...
In each cell of the following table, we display the number of OTs whose predicates were founSAT/UNSAT by the SMT engine, and the overall time spent for the execution of the whole algorithm.
...
%                \textsl{for any sort} & & $=,\ \ne$
	...
	\end{tabular}
        ...
        This shows a significant benefit of the ``smart'' strategy, in the considered use-cases, between 30\% and 80\% gain depending on the examples.
...
in terms of computing time. In~\cite{Avocs-RR}, we show that, for a slighly bigger example of assembling two failure controllers, Z3 can check the
