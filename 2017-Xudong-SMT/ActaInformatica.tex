...
\State $\mathit{OA_1^0}$=makeReachableSubAutomaton($\mathit{InitState}(P),L1)$;
\Repeat $\mathit{OA_1^n}$=RefineReachableSubAutomaton($\mathit{OA_1^{n-1}}$)
...
iteratively (lines 13-15), in the same way than in algorithm \ref{alg1}. 
Naturally, this step is only useful if there are some assignments.

\medskip
Now we will prove that the smart algorithm computes the same 
open automaton.
...
speak of equality (of transitions, and of open automata) without the phrase ``up to a renaming of fresh variables''.
 ...
 \begin{itemize}
    ...
    \item $L1_{reach}$: the final set of OTs of Algorithm~\ref{alg1}, as computed
      by line 12 (as $OA_1^0$). Without assignments the following
...
\centerline{$L1_{reach} \subseteq L1_{sat} \subseteq L0$}
...
$L2_{sat} \subseteq Rename(L0)$ (up to renaming of fresh variables), and as a consequence $States(L2_{reach})\subseteq  States(L0)$, i.e.,\\
...
\in States(L1_{reach})$, and vice-versa.
  ...
  $S_n \xrightarrow{l_{n}}S$ in $L1_{sat}$, with $l'_n=Rename(l_n)$;
  ...
  $l'_n=Rename(L_n)$ that will have
  been checked on the fly, in line 6 of Algorithm~\ref{alg3}. Then
  the transition $S_n \xrightarrow{l'_{n}}S$ will be added to
  $L2_{sat}$, and thus $S \in States(L2_{reach})$. 
  
Note that $L0$ is finite, so we conclude $L1_{reach}=L2_{reach}$, and
$States(L1_{reach})=States(L2_{reach})$. 
...
\begin{proof}
  ...
  \begin{itemize}
    ...
    \item $Assigns : \overline{OT}, Vars\ \rightarrow \overline{Assign}$: similarily,
      but for assignments concerning one specific state variable.
  \end{itemize}
...
We already know that $L2_{sat}\subseteq L1_{sat}$ (up to renaming),
because every OT that will be checked positively (using assignments) in line 12 of
algorithm \ref{alg3} will also be tested by algorithm \ref{alg1} (as
...
\text{with } Aut_2^0 \subseteq Aut_1^0, \text{ component-wise and up to renaming.}
\end{align*}
...
Based on this triple structure, we define the function $Refine : Aut
...
\smallskip
In each iteration $k$, consider a transition $l \in L^k$, define 

\[Pred(l,L^{k-1}) = Pred_{l}\wedge \bigwedge_{v\in vars(pn)}\{ \bigvee_{Assign\in
  Assigns(v,L^{k-1})} Assign\}\]

Checking satisfiability of $l$ means checking satifiability of
$Pred(l,L^{k-1})$.

%% \noteInEM{Xudong: Previous version looks wrong to me, because the
%%   disjunction is between the various possible assignments of each
%%   state var, but then you need the conjunction of the information for
%%   all considered vars:
%% \[\bigvee_{Assign_i\in Assignments_{L, States_{L}}}(Pred_{l}\wedge
%% Assign_{i})\text{ is true}.\]}

Note that this is equivalent to the predicate defined in section
\ref{section:pruning}, because we consider all states variables of the
...
$Assign(L^{k-1})$, became Unsat; or this OT is at
the end of a sequence starting from a state in $s_0\ \in State(L^k)$,
the next OT became Unsat, and the following states in the sequence are
not reachable using satisfiable OTs in $Aut^k$. More formaly, there is a chain
\[s_0\xrightarrow{ot_1}s_1\xrightarrow{ot_2}...s_{n}\xrightarrow{ot_{n+1}}s_{n+1}\]
with $Pred(ot_1,L^{k-1})$ Unsat, and all states $s_i, i \in [1..n]$
unreachable in $Aut^k$.

\smallskip
In each iteration, the number of triples may decrease, thus there
...
\medskip
Now we prove that the triples at the end of the fixpoint computation
...
preserved by application of $Refine$ (meaning that the refine function
in algorithm 1 cannot go ``faster'' than in algorithm 3), then prove
that when the fix-points are reached, then they are the same. 

Suppose $Hyp^{k-1}$, and prove $Hyp^k$.
...
\smallskip
Now consider the fixpoints of the $Refine$ iterations. From the
initial inclusion $L_2^0\ \subseteq L_1^0$, and our invariant, we get
inclusion of the fixpoints $L_2^*\ \subseteq L_1^*$.

Suppose there exists an OT $l$ in $L_1^*$ that is not in $L_2^*$. This
means that $Pred(l,Assigns(L_1^*))$ is satisfiable, but
$Pred(l,L_2^*)$ is satisfiable, that is a contradiction; or as in the
invariant proof, there is a similar sequence, with an OT $l_1$
bringing the contradiction.

So we conclude $L_2^* = L_1^*$
...
%% Then we come to see what changes have occurred during the iterations. In the first iteration, the inclusion relation between their assignments implies an inclusion relation of unsatisfiable transitions. If there are two transitions $s\xrightarrow{l}s' \in L1_{unsat}$ and $s\xrightarrow{l}s' \in L2_{unsat},$ then
%% \begin{align*}
...
%% \end{align*}
%% From the relation $\forall s\xrightarrow{l}s' \in L1_{unsat},\exists s\xrightarrow{l}s' \in L2_{unsat}$, it can be implied that $L1_{unsat}\subseteq L2_{unsat}.$
...
%% Then in the next iteration, the unsatisfiable transition $s\xrightarrow{l}s'\in L1'_{unsat}$ removed from $L1'_{sat}$ has two cases, we use $L1'_{unsat1}$ and $L1'_{unsat2}$ where $L1'_{unsat1}+L1'_{unsat2}=L1'_{unsat}$ to present them. In the first case, it might be the same transition (up to renaming) removed from $L2'_{sat}$,
%% \[s\xrightarrow{l}s'\in L1'_{unsat1}\longrightarrow s\xrightarrow{l}s'\in L2'_{unsat}.\]
...
%% \[s\xrightarrow{l}s'\in L1'_{unsat2}\longrightarrow s\xrightarrow{l}s'
%% \notin L2_{sat}.\]
