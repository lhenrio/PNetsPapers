...
For each of these, define $States: <OT> \rightarrow <State>$ which is the function computing the set of states in a set of OTs. Between two transitions sets of the
...
Suppose that  $\ostate{\tilde{t}}\in States(L1_{reach})$, it is reachable in the result automaton of Algorithm 1, so there exists a sequence of transitions $\ostate{\tilde{t_0}}...\xrightarrow{v_i}...\ostate{\tilde{t}}$ where $Pred_i$ are all satisfiable. And there exists $\ostate{\tilde{u}}\in States(L2_{reach})$ where $\ostate{\tilde{t}}=\ostate{\tilde{u}}$.

Similarly, from the structure of the algorithms, 
\[\forall \ostate{\tilde{u}}\in States(L2_{reach}), \exists\ostate{\tilde{t}}\in States(L1), \ostate{\tilde{t}}=\ostate{\tilde{u}}\]
\[s.t.\quad OT_t = \ostate{\tilde{t}}\xrightarrow{v}\ostate{\tilde{t'}}\in L1, OT_u = \ostate{\tilde{u}}\xrightarrow{v}\ostate{\tilde{u'}}\in L2_{reach}, Rename(OT_t) = OT_u.\]
where $Pred_t$ and $Pred_u$ are satisfiable.

Then we can get a new longer sequence $\ostate{\tilde{t_0}}...\xrightarrow{v_i}...\ostate{\tilde{t'}}$ implying that $\ostate{\tilde{t'}}\in States(L1_{reach}),OT_t\in L1_{reach}$. So 
$States(L2_{reach})\subseteq  States(L1_{reach})$.

Moreover,
\[\forall OT_u\notin L2_{reach}, \exists OT_t\in L1, Rename(OT_t) = OT_u\]
where $Pred_t$ and $Pred_u$ are not satisfiable. Then according to the pruning function in Algorithm 1, $OT_t\notin L1_{reach}$. 
...
In the general case, the smart algorithm computes 
