...
\abstract{Open pNets are formal models that can express the behaviour of open systems, either
	...
	We propose and implement an algorithm computing these semantics,
...
\ERIC{We also propose and implement an optimised
	algorithm that performs part of the pruning on-the-fly, and show 
	this new algorithm is correct with respect to the original one.
	...
	encodings, and prove some basic properties. The second is  industry-oriented and based on the so-called ``BIP architectures'', which have
...
As mentioned above, early attempts were made by Lin and others to define symbolic bisimulations for process calculi such as value-passing CCS and the $\pi$-calculus, and design algorithms to compute the most general condition under which two processes are bisimilar. However, there is no algorithmic treatment of the symbolic systems developed by
...
\begin{example}[Open transitions]
  Transition $OT_1$ of the pNet $EnableStateCompLeft$ is generated by
  combining the first vector of its root node with
  the second vector of its second node. Its generated predicate is
  trivially true. The global states show that controler $CState1$ is
...
\ref{schema:enable-pnets} to build a similar $EnableDataCompLeft$
``P>>(Q>>R)'' expression, the corresponding open transition would be:
...
\State \QIN{$\mathit{OA}$ = \texttt{makeReachableSubAutomaton}($\mathit{InitState}(P)$,$\mathit{L1}$)}
\State \Return $\mathit{OA}$;
...
\QIN{The only assignments for state variables that happen at the PLTS level are as follows: every time the MakeTransitions
...
open automaton from the reachable transitions.}
...
In Figure~\ref{schema:unsat-ot}, we show an 
unsatisfiable open transition 
from the open automaton of our running example.

It shows the case where processes $P$ and $R$ ``try'' to communicate directly, $P$, but this is made impossible by the combination of chosen synchronisation vectors and their guards. This
...
\begin{figure}[h]
	...
	\end{mathpar}
\caption{One of the unsatisfiable open transitions}  \label{schema:unsat-ot}
\end{figure}
...
the solvers can use specific properties of each action algebra.

\subsection{Computing Reachable State Space on the Fly}

\ERIC{The basic algorithm from the previous section potentially explores some
...
the SAT checking of subsequent transitions. But we \emph{cannot} use this
...
state space. So the smart algorithm proceeds in two steps: during the
...
SAT checking of all transitions from step1, but this time using the
assignment information. This may reduce the reachable state space. If
we want to be sure that the obtained automaton is minimal, we should
iterate this step until a fixed point is reached. But in practical cases, very
little will be gained, so the default implementation does only one
step. Leaving some UNSAT transitions in the
symbolic automaton is still logically correct.
...
\IfThenElse {(NonEmptyAssignmentSet($\mathit{L}$))}
        {\State \quad\ \Return $\mathit{OA}$=makeAutomaton($\mathit{InitState}(P),L)$;}
        {\State \quad\ $\mathit{L1}$=filterUnsatTransitions($\mathit{L,Assignments}$);
         \State \quad\ \Return $\mathit{OA}$=makeReachableSubAutomaton($\mathit{InitState}(P),L1)$};
\end{algorithmic}  
\end{algorithm}
...
the same open automaton as that of Algorithm 1.
  \end{theorem}

\TODO{Need to refine theorem, and to check the proof... To answer
  Yuxin question about ``exactly the same'', indeed it is not
  correct... But not for states: state labels will be exactly the
  same. But transitions will not, because of the mechanism for
  generating fresh variables; so we may need to reword as ``bijective''}

\begin{proof}
Let $L$, $L1$, $L_{Smart}$ respectively mean the result open transition set of original algorithm before pruning, original algorithm after pruning and smart algorithm. Similarly, $State(L)$, $State(L1)$, $State(L_{Smart})$ mean the result global state set of them. According to the algorithm, there has $L_{Smart}\in L$ and $State(L_{Smart})\in State(L)$.

Suppose that $f$: $State(L1)\rightarrow State(L_{Smart})$.
\begin{enumerate}
\setlength{\itemindent}{3.5em}
	\item[(Surjective)] If $\ostate{\tilde{s_0}}=Initial(p)$ then $f(\ostate{\tilde{s}})=\ostate{\tilde{t_0}}=Initial(p)$ must exists, the transitions started from them fulfill that
	\[\forall \ostate{\tilde{t_0'}}, \ostate{\tilde{t_0}}\xrightarrow{v_j}\ostate{\tilde{t_0'}}\in L_{Smart},\quad \exists\ostate{\tilde{s'}},\ostate{\tilde{s_0}}\xrightarrow{v_i}\ostate{\tilde{s_0'}}\in L.\]
	The state $\ostate{\tilde{t'}}\in State(L_{Smart})$ and as $Pred_j$ of $\ostate{\tilde{t}}\xrightarrow{v_j}\ostate{\tilde{t_0'}}$ is satisfiable, then if the $Pred_i$ of $\ostate{\tilde{s_0}}\xrightarrow{v_i}\ostate{\tilde{s_0'}}$ is also satisfiable, the state $\ostate{\tilde{s_0'}}$ is reachable in the original open automaton s.t. $\ostate{\tilde{s_0'}}\in State(L1)$, we can construct the mapping
	\[f(\ostate{\tilde{s_0'}})=\ostate{\tilde{t_0'}}.\]
	If the $Pred_i$ of $\ostate{\tilde{s_0}}\xrightarrow{v_i}\ostate{\tilde{s_0'}}$ is not satisfiable then two transitions are not matched and $\ostate{\tilde{s_0'}}\notin State(L1)$.
	
	Let $\textit{Explored}$, $\textit{Explored}_{Smart}$ respectively mean the explored states set of original algorithm and smart algorithm.
	
	Suppose that  $\ostate{\tilde{s}}\in \textit{Explored}$ there is $\ostate{\tilde{t}}\in \textit{Explored}_{Smart}$ where $f(\ostate{\tilde{s}})=\ostate{\tilde{t}}$, there exists a sequence of transitions $\ostate{\tilde{s_0}}...\xrightarrow{v_i}...\ostate{\tilde{s}}$ and $\ostate{\tilde{t_0}}...\xrightarrow{v_j}...\ostate{\tilde{t}}$. 
	
	The transitions $\ostate{\tilde{s}}\xrightarrow{v}\ostate{\tilde{s'}}$ and $\ostate{\tilde{t}}\xrightarrow{v}\ostate{\tilde{t'}}$ have several cases:
	\begin{enumerate}
		\item[(1)] $\ostate{\tilde{t'}}\in \textit{Explored}_{Smart}$
		
		We can find that 
		 \[\exists\ostate{\tilde{s'}}\in \textit{Explored},\quad \ostate{\tilde{s}}\xrightarrow{v}\ostate{\tilde{s'}}\quad where\quad f(\ostate{\tilde{s'}})=\ostate{\tilde{t'}}.\]
		\item[(2)] $\ostate{\tilde{t'}}\notin \textit{Explored}_{Smart}$
		
		There must has 
		\[\exists\ostate{\tilde{s'}}\notin \textit{Explored},\quad \ostate{\tilde{s}}\xrightarrow{v'}\ostate{\tilde{s'}}.\]
		If the $Pred_i$ of $\ostate{\tilde{s}}\xrightarrow{v'}\ostate{\tilde{s'}}$ is satisfiable then there exists a sequence of transitions $\ostate{\tilde{s_0}}...\xrightarrow{v_i}...\ostate{\tilde{s}}\xrightarrow{v'}\ostate{\tilde{s'}}$. So the $\ostate{\tilde{s'}}$ is reachable in the original open automaton, we can construct the mapping
		\[f(\ostate{\tilde{s'}})=\ostate{\tilde{t'}}.\]
		If the $Pred_i$ of $\ostate{\tilde{s}}\xrightarrow{v'}\ostate{\tilde{s'}}$ is not satisfiable then two transitions are not matched and $\ostate{\tilde{s'}}\notin State(L1)$.
	\end{enumerate}	
	Overall, \[\forall \ostate{\tilde{t}}\in State(L_{Smart}),\quad \exists\ostate{\tilde{s}}\in State(L1), f(\ostate{\tilde{s}})=\ostate{\tilde{t}}.\]
	\item[(Injective)] If there is a pair of states $\ostate{\tilde{s}} and \ostate{\tilde{t}}$ constructs the mapping $f(\ostate{\tilde{s}})=\ostate{\tilde{t}}$. We can know that $\ostate{\tilde{s}}\in \textit{Explored},\ostate{\tilde{t}}\in \textit{Explored}_{Smart}$.
	
	As when there is $\ostate{\tilde{t'}}\in \textit{Explored}_{Smart}$, the state is must be $\ostate{\tilde{s'}}\in \textit{Explored}$ where $f(\ostate{\tilde{s'}})=\ostate{\tilde{t'}}$.
	So it is a one-to-one mapping.
\end{enumerate}

Then as the function $f$ is both surjective and injective, it is a bijective function. The theorem is proved.

%Intuitively, $\forall \ostate{\tilde{s}}\in\textit{Explored}=State(L)\text{, there must exist a transition}\ostate{\tilde{s_i}}\xrightarrow{v_i}\ostate{\tilde{s}}\text{ and }\forall \ostate{\tilde{s}}\in\textit{Explored}_{Smart}=State(L_{Smart})\text{, }\exists\ostate{\tilde{pre(s)}}\text{, }\ostate{\tilde{pre(s)}}\xrightarrow{v}\ostate{\tilde{s}}\text{ where \textit{Pred} is satisfiable and the state}\ostate{\tilde{pre(s)}}\text{ has two cases:}$
%\begin{enumerate}
%	\item[(1)] $\ostate{\tilde{pre(s)}}=\ostate{\tilde{s}}$
%	\item[(2)] $\ostate{\tilde{pre(s)}}\in\textit{Explored}\text{-}\{\ostate{\tilde{s}}\}$
%\end{enumerate}
%By induction, there must exists a sequence $\ostate{\tilde{s_0}}\xrightarrow{v_i}\ostate{\tilde{s}}$ where all the $\textit{Pred}_i$ are satisfiable.
%
%Then $\forall\ostate{\tilde{t}}\textit{Explored}=State(L)$, there are also two cases:
%\begin{enumerate}
%	\item[(1)] $\ostate{\tilde{t}}\in State(L1)$
%	
%	By definition of reachability, there exists a sequence $\ostate{\tilde{s_0}}...\xrightarrow{v_j}...\ostate{\tilde{t}}$ where all the $\textit{Pred}_j$ are satisfiable.
%	
%	$\Rightarrow \ostate{\tilde{t}}\in State(L_{Smart})$
%	\item[(2)] $\ostate{\tilde{t}}\in State(L)\text{-}State(L1)$, then $\forall \ostate{\tilde{u}}\in\textit{Explored}-\{\ostate{\tilde{t}}\}$,
%	\begin{enumerate}
%		\item[(1)] $\ostate{\tilde{u}}\in State(L)\text{-}State(L1)$.
%		\item[(2)] $\ostate{\tilde{u}}\in State(L1)$, but there is no transition $\ostate{\tilde{u}}\xrightarrow{v_u}\ostate{\tilde{t}}$.
%		\item[(3)] $\ostate{\tilde{u}}\in State(L1)$, there exists transition $\ostate{\tilde{u}}\xrightarrow{v_u}\ostate{\tilde{t}}$, but the $\textit{Pred}_u$ is unsatisfiable.
%	\end{enumerate}
%	By induction, there is no sequence from $\ostate{\tilde{s_0}}$ to $\ostate{\tilde{t}}$.
%	
%	$\Rightarrow \ostate{\tilde{t}}\notin State(L_{Smart})$
%\end{enumerate}
%Overall, $State(L1)$ is equal to $State(L_{Smart})$.
\end{proof}
...
But in the presence of assignments, we need to collect all of them
  ...
  space, not necessarily the minimal one (discussion later).
  
\begin{theorem}[Correctness with assignments]
  \label{Th:CorrectnessWithAssignments}
In the general case, the smart
algorithm, with a final reachability computation, computes exactly
the same open automaton as that of Algorithm 1.
\end{theorem}

\TODO{Xudong: proof - to be checked}

\begin{proof}
In Theorem 2, we have already proved the result set of global states of two algorithm are the same without the assignments.

If there exists assignments in the open transitions. 

In Algorithm 3, $\forall\ostate{\tilde{t}}\notin State(L_{Smart})$, if there is a transition $\ostate{\tilde{t}}\xrightarrow{v_t}\ostate{\tilde{t'}}$ contains an assignment $\textit{Post}_t$, and in following transitions $\ostate{\tilde{u}}\xrightarrow{v_u}\ostate{\tilde{u'}}$, we need to check $\textit{Pred}\and\bigwedge\textit{Post}_i$ where $\textit{Post}_i \notin \{\textit{Post}_t\}$.

In Algorithm 1, the same state $\ostate{\tilde{t}}\notin State(L1)$, also has a transition $\ostate{\tilde{t}}\xrightarrow{v_t}\ostate{\tilde{t'}}$ contains an assignment $\textit{Post}_t$, and in following transitions $\ostate{\tilde{u}}\xrightarrow{v_u}\ostate{\tilde{u'}}$, what need to check is $\textit{Pred}\and\bigwedge\textit{Post}_i$.
As $b_1\and b_2\rightarrow b_1$, if $\ostate{\tilde{u}}\xrightarrow{v_u}\ostate{\tilde{u'}}$ exists in the result of Algorithm 3, it is also contained in the original result.

After the final reachability computation, $\{\textit{Post}_t\}$ is removed from $\{\textit{Post}_i\}$ then the assertions to check become the same, then the transitions of the result are also the same. So the result open automaton of Algorithm 3 is exactly the same as that of  Algorithm 1.
\end{proof}
...
\TODO{Change the text: DONE}It contains the declaration
...
\begin{figure}[ht]
\centerline{ 
  \includegraphics[width=0.30\linewidth]{ActaXFIG/Enable1}
  \hspace{25mm}
  \includegraphics[width=0.25\linewidth]{ActaXFIG/Enable2}}
...
Figure~\ref{schema:enable-OAs}. We have shown in \cite{henrio:Forte2016} how to
prove that these two automata are bisimilar.
...
and then they will tick, and timeout, synchronously.

\TODO{Eric: try reorganise the figures. Add properties ?}
...
%% \begin{figure}[t]
%%   \centering
%%   \includegraphics[width=0.9\columnwidth]{ActaXFIG/DoubleFailureTimerPNET-v2-2}
%%   \caption{The open pNet encoding the Failure Monitor architecture (\ref{fig:schema:ArchFailure:BIP})}
%%   \label{fig:FailureTimer:pNet}
%% \end{figure}

%% \begin{figure}[t]
%%   \centering
%%   \includegraphics[width=0.9\columnwidth]{XFIG/DoubleTimerOA-v2-2}
...
All tests are run on HP xxx laptop under Windows 10 x64, with an Intel quad-Core i7-6600U
...
% correct in a separate paper\mdash 
also opens the way for an extension
...
will be to model-check logical properties, and to check equivalences of pNets. While
...
different structures. For bisimulation, we foresee that SMT methods
...
Given an open pNet $\mylangle \overline{\pNet},\overline{S}, \symb{SV}_k^{k\in K}\myrangle$ with leaves $pLTS_i^{i\in I}$ and holes $Hole_j^{j\in
  J}$, if the sets $I$ and $J$ are finite, if the synchronization vectors of all pNets 
  included in  $\mylangle \overline{\pNet},\overline{S}, \symb{SV}_k^{k\in K}\myrangle$ 
  are finite, and if
$\forall i \in I.\, finite{(states(pLTS_i))} \text{ and } pLTS_i$
...
\begin{proof}
  ...
  product of the states of the pNet $pLTS_i^{i\in I}$, that is finite by
  ...
  at each node the number of global open transitions is finite. Similarily rule Tr1 is 
