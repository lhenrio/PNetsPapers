...
\begin{definition}[pNets]\label{def-pnets}
	...
	\begin{itemize}
		...
		synchronisation vectors ($K\in\I_\P$).  It is required that \linebreak $\forall k\!\in\! K,
...
For any pNet $p$, let $\overline{\mathit{Leaves}} = \langle S_i,{s_i}_0, \to_i\rangle^{i \in L}$ be the set of PLTSs at its leaves,
...
\Repeat $\mathit{OA_n}$=RefineReachableSubAutomaton($\mathit{OA_{n-1}}$)
...
incorrect, but is naturally not an optimal representation. So we prune
...
Function \texttt{filterUnsatTransitions}, in line 11 of
Algorithm\ref{alg1}, and function \\
\texttt{RefineReachableSubAutomaton}
in line 13, make use of the Z3 engine to implement the
...
The set $L1$ may contain unreachable transitions. To remove those transitions, the last step of the
algorithm builds the final open automaton via a traversal of $L1$,
...
%    I'm not sure if parts of it can be reused}
...
speak of equality (of transitions, and of open automata) without the phrase ``up to a renaming of fresh variables''.
 ...
 \begin{itemize}
    ...
    \item $L1_{reach}$: the final set of OTs of Algorithm~\ref{alg1}, as computed
      by line 12 (as $OA_0$). Without assignments the following
    ...
    \item $L2_{reach}$: the final set of OTs of Algorithm~\ref{alg3}, as computed
          by line 13. Here also $OA_n = OA_0$.
   \item $States: \overline{OT}\rightarrow\overline{State}$:  the function that
computes the set of target states of a set of OTs.  
  \item $Rename:  L1_{reach} \rightarrow L2_{reach}$:  a function that keeps state names of transitions but renames fresh variables in the transition labels.
\end{itemize}
...
\smallskip
We now prove that $L1_{reach}=L2_{reach}$, which will imply
the equality of the two open automata.
...
Algorithm~\ref{alg1}. Formally, we have that $L2_{reach}\subseteq  L2_{sat} \subseteq L0$, and as a consequence $States(L2_{reach})\subseteq  States(L0)$, i.e.,\\
...
that $\forall S \in L0. \forall n \in \mathbb{N}. S \in States(L2_{reach}) \land |S|_{L2_{reach}}=n \Rightarrow S
\in States(L1_{reach})$, and vice-versa.
  ...
  with all $l_k \in L2_{reach}$. In particular, since
  $L2_{reach}\subseteq L2_{sat}$, we have that $l_{n+1} \in L2_{sat}$ and
  ...
  $S_n \xrightarrow{l_{n}}S$ will be checked and found SAT, so
  $S \in States(L1_{reach})$. 

  Conversely, if $|S|_{L1_{reach}}=n+1$, we get a similar chain in
  $L1_{reach}$, with the transition $S_n \xrightarrow{l_{n}}S$.  Then $ S_n \in
  States(L1_{reach})$ and $l_n$ is satisfiable. By induction, we have
  $S_n \in States(L2_{reach})=States(L2_{sat})$, so $l_n$ will have
  been checked ``on-the-fly'', in line 6 of Algorithm~\ref{alg3}. Then
  the transition $S_n \xrightarrow{l_{n}}S$ will be added to $L2_{sat}$, and thus $S \in States(L2_{reach})$.
  
Note that $L0$ is finite, so we conclude $L1_{reach}=L2_{reach}$, and $States(L1_{reach})=States(L2_{reach})$.

%  If there is an initial state $\ostate{\overline{t_0}}\in States(L2_{reach})$ then the initial state of the original automaton $\ostate{\overline{t_0}}\in States(L1_{reach})$ must exists.

%% \noteInEM{What you are doing below is a proof by recursion on the
%%   length of the path reaching a given OT, isn't it ? I suggest you
%%   explicite this and conclude $L2_{reach}\subseteq  L1_{reach}$.}

%% Suppose that  $\ostate{\overline{t}}\in States(L2_{reach})$. It is reachable in the result automaton of Algorithm 3, so there exists a sequence of transitions 
%% \[\ostate{\overline{t_0}}\xrightarrow{l_1}\ostate{\overline{t_1}}\xrightarrow{l_2}...\xrightarrow{l_{n}}\ostate{\overline{t}}\] where in each label of the transition $Pred_{l_i}^{i\in [1,n]}$ is satisfiable. For the same state $\ostate{\overline{t}}\in States(L1_{reach})$, there exists a sequence of transitions \[\ostate{\overline{t_0}}\xrightarrow{l'_1}\ostate{\overline{t_1}}\xrightarrow{l'_2}...\xrightarrow{l'_{n}}\ostate{\overline{t}}\]where for each transition $Rename(\ostate{\overline{t_{i-1}}}\xrightarrow{l_{i}}\ostate{\overline{t_{i}}}) = \ostate{\overline{t_{i-1}}}\xrightarrow{l'_{i}}\ostate{\overline{t_{i}}}$ and $Pred_{l'_i}^{i\in [1,n]}$ is satisfiable. 
%% From the structure of the algorithms, 
%% \[\forall \ostate{\overline{t}}\in States(L2_{reach}), \ostate{\overline{t}}\xrightarrow{l'_{n+1}}\ostate{\overline{t'}}\in L2_{reach}\]
%% \[\exists \ostate{\overline{t}}\xrightarrow{l_{n+1}}\ostate{\overline{t'}}\in L1 \quad s.t.\quad Rename(\ostate{\overline{t}}\xrightarrow{l_{n+1}}\ostate{\overline{t'}}) = \ostate{\overline{t}}\xrightarrow{l'_{n+1}}\ostate{\overline{t'}})\]
%% where $Pred_{l_{n+1}}$ and $Pred_{l'_{n+1}}$ are satisfiable.

%% Then we can get a new longer sequence \[\ostate{\overline{t_0}}\xrightarrow{l_1}\ostate{\overline{t_1}}\xrightarrow{l_2}...\xrightarrow{l_{n}}\ostate{\overline{t}}\xrightarrow{l_{n+1}}\ostate{\overline{t}}\] \[\Rightarrow\ostate{\overline{t'}}\in States(L1_{reach})\wedge\ostate{\overline{t}}\xrightarrow{l_{n+1}}\ostate{\overline{t'}}\in L1_{reach}\in L1.\] So for arbitrary transition in $L2_{reach}$, there is a same transition in $L1_{reach}$ after renaming its label, then we get
%% $L2_{reach}\subseteq  L1_{reach}$.

%% Conversely,
%% \[\forall \ostate{\overline{t'}}\notin States(L2_{reach}), \ostate{\overline{t}}\xrightarrow{l'}\ostate{\overline{t'}}\notin L2_{reach},\quad\exists \ostate{\overline{t'}}\in States(L1), \ostate{\overline{t}}\xrightarrow{l}\ostate{\overline{t'}}\in L1\]
%% where $Pred_{l}$ and $Pred_{l'}$ are not satisfiable. As pruning function in Algorithm 1 will remove all the unsatisfiable transitions, then $\ostate{\overline{t}}\xrightarrow{l}\ostate{\overline{t'}}\notin L1_{reach}$. 

%As a result, we get that $L1_{reach}=L2_{reach}$, 
%We will construct a mapping $f$: $State(L1-reach)\rightarrow
%State(L2-reach)$.
%\ERIC{I do not understand this, as states are identical. You would
%  rather need mapping betwen transitions ?}
%\begin{enumerate}
%\setlength{\itemindent}{3.5em}
%	\item[(Surjective)] If $\ostate{\overline{s_0}}=Initial(p)$ then $f(\ostate{\tilde{s}})=\ostate{\tilde{t_0}}=Initial(p)$ must exists, the transitions started from them fulfill that
...
\begin{proof}
  Define the OT sets $L0, L1_{sat}, L2_{sat}, L2_{reach}$ and the function $States$ as in the proof of Theorem~\ref{Th:CorrectnessWithoutAssignments}, and
  ...
  \begin{itemize}
    \item $L1_{reach*}$: the final set of OTs of Algorithm~\ref{alg1}, as computed
      ...
      \item $L2_{reach*}$: the final set of OTs of Algorithm~\ref{alg3}, as computed
        by an iterative application of $makeReachableSubAutomaton$ (line
        16) up to a fixed point.
        \item $Assignments : \overline{OT}\ \times State\rightarrow \overline{Assign}$: the function of
...
The assignments set of $OA_1$ is $\{Assignments(L1, S_i)\}^{S_i\in States(L1)}$ and that of $OA_2$ is $\{Assignments(L2_{sat}, S_j)\}^{S_j\in States(L2_{sat})}$. The result triples are:  
\noindent
\begin{align*}
...
Based on this triple structure, we define the function $Refine : Aut \rightarrow Aut$ modelling $makeReachableSubAutomaton(S0,OTs)$. It uses the assignments of the previous step to refine the computed reachable space:
\begin{align*}
...
\[States_{L'}=\{S_0\}\cup \bigcup States(L'),\]
...
In one single iteration, the satisfiability of a transition $\ostate{\tilde{t}}\xrightarrow{l}\ostate{\tilde{t'}}\in L$ is checked through whether 
\[\bigvee_{Assign_i\in Assignments_{L, States_{L}}}(Pred_{l}\wedge Assign_{i})\text{ is true}.\]
More transitions could become unsatisfiable after the reduction of the assignments set, so the function needs to be iterated several times. In each iteration, the number of triples decreases, thus there exists a sequence \[Aut^n = Refine(Aut^{n-1})\quad s.t. |Aut^n| < |Aut^{n-1}|.\]
The iteration terminates if $Aut''$ contains no more unsatisfiable transition, then there is a fixed point $Aut'' = Refine(Aut'')$ and $|Aut''| = |Refine(Aut'')|$.

Let $Aut''_1$ and $Aut''_2$ be two fixed points with $Refine(Aut''_1) = Aut''_1$ and $Refine(Aut''_2) = Aut''_2$. Focusing on the transition sets in $Aut''_1$ and $Aut''_2$, $L1''_{sat}=L1_{reach}$ and $L2''_{sat}=L2_{reach}$, we need to check their equivalence.
...
\text{implies }&\bigvee_{Assign_j\in Assignments_{L2_{sat}, States_{L2_{sat}}}} (Pred_{l}\wedge Assign_j)\text{ is false}.
\end{align*}
From the relation $\forall\ostate{\tilde{t}}\xrightarrow{l}\ostate{\tilde{t'}} \in L1_{unsat},\exists\ostate{\tilde{t}}\xrightarrow{l}\ostate{\tilde{t'}} \in L2_{unsat}$, it can be implied that $L1_{unsat}\subseteq L2_{unsat}.$
...
Then in the next iteration, the unsatisfiable transition $\ostate{\tilde{t}}\xrightarrow{l}\ostate{\tilde{t'}}\in L1'_{unsat}$ removed from $L1'_{sat}$ has two cases, we use $L1'_{unsat1}$ and $L1'_{unsat2}$ where $L1'_{unsat1}+L1'_{unsat2}=L1'_{unsat}$ to present them. In the first case, it might be the same transition (up to renaming) removed from $L2'_{sat}$,
\[\ostate{\tilde{t}}\xrightarrow{l}\ostate{\tilde{t'}}\in L1'_{unsat1}\longrightarrow\ostate{\tilde{t}}\xrightarrow{l}\ostate{\tilde{t'}}\in L2'_{unsat}.\]

Or it is the transition not removed in the last iteration because of the inclusion
...
\[\ostate{\tilde{t}}\xrightarrow{l}\ostate{\tilde{t'}}\in L1'_{unsat2}\longrightarrow\ostate{\tilde{t}}\xrightarrow{l}\ostate{\tilde{t'}}\notin L2_{sat}.\]

By induction, if there is no more unsatisfiable transitions in $L2''_{sat}$, so $L2''_{unsat}$ is an empty set and then $L1'_{unsat1}$ is also empty as $L1'_{unsat1}\subseteq L2''_{unsat}$. The transitions to remove should only belong to $L1'_{unsat2}\nsubseteq L2''_{sat}$. So the iteration continues removing all the transitions not in $L2''_{sat}$, until $L1_{reach}=L2_{reach}$. It implies that $States_{L1_{reach}}=States_{L2_{reach}}$ and
...
\begin{figure}[ht]
\centerline{ 
  \includegraphics[width=0.30\linewidth]{ActaXFIG/Enable1-aut}
  \hspace{25mm}
  \includegraphics[width=0.25\linewidth]{ActaXFIG/Enable2-aut}}
...
%
  ...
  results were provided to check whether two are architectures are
  interfering or not.
%
