...
Algorithm\ref{alg1} makes use of the Z3 engine to implement the
satisfiability checking, for all the OTs computed that far, and
returns the set $L1$ of satisfiable OTs. 

\subsection{Building the Open Automaton}

\ERIC{The set $L1$ may contain unreachable transitions, the last step of the
algorithm builds the final Open Automaton as a traversal of $L1$,
starting at the initial state $InitState(p)$ as defined in
page~\pageref{def-states}. More precisely, this is a residual
algorithm building a graph from transitions in $L1$, and states are
the set of target states of transitions in $L1$, plus the initial
states. Each state in the graph is decorated with a set of its local
...
would be the fix point of this iteration. In practice this would be
expensive, and it is not incorrect to leave some Unsat OTs in the
result; in practical cases, very little will be gained computing the
fixpoint, so we only perform one step. Further algorithms 
...
%  product of the pLTS states, as defined in \pageref{def-states}. In a
%  similar way, the set of state variables are also equal. Comparing
...
%  renaming of these fresh variables\footnote{I think this does not
%    need any specific proof, given that it is the same function,
%    called with the same arguments}.
%
%  Let us define the following sets of OTs, as computed by the 2
%  algorithms:
%  \ERIC{not sure whether all these sets are useful, but it's good to
%    have them precisely defined for the moment}
%  \begin{itemize}
%    \item $L1$ = original set of transitions by Algorithm 1
%      (i.e. before line 11), 
%    \item $L1-sat$ = original set of transitions by Algorithm 1, after
%      satisfiability check (i.e. after line 11),
%    \item $L1-reach$ = final set of OTs of Algorithm 1, as computed
%      by line 12.
%    \item $L2-sat$ = original set of transitions by Algorithm 2
%      (i.e. before line 13), 
%    \item $L2-reach$ = final set of OTs of Algorithm 2, as computed
%          by line 13.
%  \end{itemize}
%
%  For each of these, define $States : OT^* -> State^*$ the function
%  computing the set of target states of a set of OTs.
%
%  We want to prove that $L1-reach = L2-reach$ (upto renaming of fresh
%  variables) and $States(L1-reach) = States(L2-reach)$.
%
%  From the structure of the main loops of the algorithms, we know that
%  $L1-sat \subset L2-sat$ and subsequently $States(L1-sat) \subset       
%  States(L2-sat)$.
%  \qed
%\end{proof}
%  
...
Let
 \begin{itemize}
    \item $L1$ = original set of transitions by Algorithm 1
      (i.e. before line 11), 
    \item $L1_{sat}$ = original set of transitions by Algorithm 1, after
      satisfiability check (i.e. after line 11),
    \item $L1_{reach}$ = final set of OTs of Algorithm 1, as computed
      by line 12.
    \item $L2_{sat}$ = original set of transitions by Algorithm 2
      (i.e. before line 13), 
    \item $L2_{reach}$ = final set of OTs of Algorithm 2, as computed
          by line 13.
\end{itemize}
When the pNets contains no assignment, there is no need for the last filtering step, so $L1_{sat}=L1_{reach}$ and $L2_{sat}=L2_{reach}$.

For each of these, define $States: \langle OT\rangle\rightarrow\langle State\rangle$ which is the function computing the set of states in a set of OTs. Between two transitions sets of the
algorithms, define $Rename:  L1_{reach} \rightarrow L2_{reach}$ which is the function renaming fresh variables.

According to the rules Tr1 and Tr2 in \pageref{def-states}, the global state $\ostate{\tilde{s}}=\ostate{\tilde{s_0...s_n}}$ is a composition of n pLTS states $s_{i}^{i\in [0,n]}$.

From the structure of the main loop of the algorithms, we can get $States(L2_{reach})\subseteq  States(L1)$, then
\[\forall \ostate{\tilde{u}}\in States(L2_{reach}), \exists\ostate{\tilde{t}}\in States(L1), \ostate{\tilde{t}}=\ostate{\tilde{a_0...a_n}}=\ostate{\tilde{u}}.\]

If there is an initial state $\ostate{\tilde{t_0}}\in States(L2_{reach})$ then the initial state of the original automaton $\ostate{\tilde{t_0}}\in States(L1_{reach})$ must exists.

Suppose that  $\ostate{\tilde{t}}\in States(L2_{reach})$, it is reachable in the result automaton of Algorithm 2, so there exists a sequence of transitions $\ostate{\tilde{t_0}}...\xrightarrow{v_j}...\ostate{\tilde{t}}$ where $Pred_j$ are all satisfiable. Then there exists the same state $\ostate{\tilde{t}}\in States(L1_{reach})$ and $\ostate{\tilde{t_0}}...\xrightarrow{v_i}...\ostate{\tilde{t}}$ where $Pred_i$ are all satisfiable.
From the structure of the algorithms, 
\[\forall \ostate{\tilde{t}}\in States(L2_{reach}), OT = \ostate{\tilde{t}}\xrightarrow{v}\ostate{\tilde{t'}}\in L2_{reach}\]
\[\exists OT' = \ostate{\tilde{t}}\xrightarrow{v'}\ostate{\tilde{t'}}\in L1 \quad s.t.\quad Rename(OT') = OT.\]
where $Pred$ and $Pred'$ are satisfiable.

Then we can get a new longer sequence $\ostate{\tilde{t_0}}...\xrightarrow{v_i}...\ostate{\tilde{t'}}\xrightarrow{v'}\ostate{\tilde{t'}}$ implying that $\ostate{\tilde{t'}}\in States(L1_{reach}),OT'\in L1_{reach}$. So 
...
\begin{proof}
In Theorem 2, we have already proved the result automaton $OA_1$ and $OA_2$ of two algorithm are the same without the assignments.
...
the structure of an Open Automaton) is a triple $ Aut= \langle OTs, States, Assigns\rangle$.

Then the assignments set of $OA_1$ is $Assignments(L1, States(L1))$ and $OA_2$ is $Assignments(L2_{reach}, States(L2_{reach}))$. The computed triples are 
\[Aut_1 = \langle L1_{reach}, States(L1_{reach}), Assignments(L1, States(L1))\rangle\] and \[Aut_2 = \langle L2_{reach}, States(L2_{reach}), Assignments(L2_{reach}, States(L2_{reach}))\rangle\]

Based on the notation of the triple, define the function $Refine : Aut \rightarrow Aut$ refining the result automaton together with the assignments,
\begin{align*}
...
Given that every iteration will remove some unsatisfiable transitions, $Refine$ is a monotone function. There exists a sequence $Aut^n = Refine(Aut^{n-1})$ s.t. $|Aut^n|\le |Aut^{n-1}|$.
Let $Aut''$ contains the $OT''$ which has no more unsatisfiable transition, then there is $Aut'' = Refine(Aut'')$ and $|Aut''| = |Refine(Aut'')|$.

Let the fixed point of $OA_1$ to be $Aut''_1$ and the fixed point of $OA_2$ to be $Aut''_2$.

For the transitions set in $Aut''_1$ and $Aut''_2$, $L1_{reach}''$ and $L2''_{reach}$, we need to check their equivalence.
At the first iteration, the unsatisfiable transitions in $OA_1$ has only one case.
\begin{align*}
&\quad Pred\and \bigwedge_i Assignments_i(L1, States(L1) \text{ is false}\\
&\quad\rightarrow Pred \and \bigwedge_j Assignments_j(L2_{reach}, States(L2_{reach})\text{ is false}\\
&\Rightarrow \forall t\in L1_{unsat} \rightarrow t\in L2_{unsat}\\
&\Rightarrow L1_{unsat}\subseteq L2_{unsat}.
\end{align*}
However, its reverse direction is not fulfilled. Recall that $L1_{reach}=L2_{reach}$, these relation can imply that,
\begin{align*}
Assignments(L1, &States(L1)\supseteq Assignments(L2_{reach}, States(L2_{reach})\\
&\supseteq Assignments(L1_{reach}-L1_{unsat}, States(L1_{reach}-L1_{unsat}))\\
&\supseteq Assignments(L2_{reach}-L2_{unsat}, States(L2_{reach}-L2_{unsat}))
\end{align*}
So after the first iteration, the unsatisfiable transitions in $OA_1$ has two cases.
\begin{align*}
&\forall t\in L1'_{unsat} \rightarrow t\in L2'_{unsat}\\
\text{ or }&\forall t\in L1'_{unsat} \rightarrow t\notin L2'_{reach}.
\end{align*}
By induction, after removing all the transitions not in the $L2''_{reach}$, we can get that $L1''_{reach}=L2''_{reach}$, subsequently, $States(L1''_{reach})=States(L2''_{reach})$,
$Assignments\\(L1''_{reach},States''_{L1_{reach}})=Assignments(L2''_{reach},States''_{L2_{reach}})$.
\end{proof}
...
pNets.
We also compare the practical performances of the 2 algorithms and
