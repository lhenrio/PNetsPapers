...
\begin{table}[t]\caption{\label{table:BIPalgebra}\leftline{Algebra
	...
	\begin{tabular}{p{3cm}p{3cm}p{6cm}}
		...
		%                \textsl{for any sort} & & $=,\ \ne$
		\\\hline\specialrule{0em}{1pt}{1pt}
		\multicolumn{3}{l}{\sl Extension for the BIP use-case of Figure~\ref{schema:ArchFailure:pNet}}
		\\\hline\specialrule{0em}{1pt}{1pt}
...
PLTSs, and the other for pNet nodes. These rules are slightly improved, adding guards in the synchronisation vectors and syntax for universal quantifier in the guards (see~\cite{Avocs-RR}).
...
the proof of the finiteness theorem refers to SOS rules Tr1 and
  Tr2. Yuxin, I agree this proof is NOT required in this paper. But for the
  sake of reviewing process, (and if I find enough time), I will try
  to add an appendix with the semantic rules (Tr1 and Tr2, constructing open
  transitions from pNets) and the proof of finiteness below }
  
  The possible set of states of the open-automaton is the cartesian
  product of the states of the pNet $pLTS_i^{i\in L}$, that is finite by
...
from the open automaton of our running example. \TODO{change the text}It shows the case where
the failure controller performs a ``$\PortFail$'' action, while the timer
executes a ``$\PortResume$''. The chosen synchronisation
vector (SV0 from Figure~\ref{schema:ArchFailure:pNet}) does not match
...
\begin{figure}[h]
	...
	\end{mathpar}
\caption{\TODO{Eric: check, it looks wrong}One of the unsatisfiable open transitions}  \label{schema:unsat-ot}
\end{figure}
...
\TODO{Yuxin: I can add an example showing in detail why we cannot use
  the assignment information on the fly. Do you think this is useful,
  or id the explanation above sufficient ?}
{\color{green} Yuxin: I think the explanation above is sufficient. What's more important is to give formal proofs for the three theorems.}
...
\TODO{Eric : Eventually provide counter-example for smart with unsing incomplete
  assignment sets}
...
\TODO{Change the text}It contains the declaration
