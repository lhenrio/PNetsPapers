...
\begin{itemize}
	...
	\item \QIN{to separate clearly the
	transitions of a generated open automaton from the
	combination of all possible (symbolic) behaviours, and from
	the symbolic reasoning part, specifically here, to check the
	satisfiability of the predicates in the transitions generated by our algorithm using an SMT
	engine;}
...
Among these works, several have shown the capability of the SMT engines
...
\begin{definition}[pNets]\label{def-pnets}
	...
	\begin{itemize}
		...
		synchronisation vectors ($K\in\I_\P$ where $\I_\P$ is the set of indexed
		sets with a range depending on variables of $\P$).  It is required that each
