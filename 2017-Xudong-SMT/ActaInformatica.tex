...
\end{itemize}
Consider the first  vector of $EnableState$, written as $\langle a_1, \text{-}, l\rangle\rightarrow a_1[\forall y_1. a_1\neq \delta(y_1)]$. It denotes a possible behavior of $P>>Q$, for any possible action of the hole $P$ that respects the vector predicate, that is any action that does not match $\delta(y_1)$, for any value of the variable $y_1$.
Remark that the data-oriented encoding, in pNet $EnableData$ has the same synchronisation vectors, up to alpha conversion. Only the controller $CState$ has been replaced by $CData$.
}
\end{example}
...
It has been obtained by replacing the second hole in pNet $EnableState$ by another pNet representing $Q>>R$. Technically, the action constants in the PLTS $CState1$ and $CState2$ have been renamed to avoid clashes, and the variables in the vectors also given different names.
