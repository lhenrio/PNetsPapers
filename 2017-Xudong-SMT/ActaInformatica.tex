...
\ERIC{To illustrate our definitions and results, we will use a small running example, coming from the specification language LOTOS \cite{LotosISO89}.
  Suppose we have two unspecified processes $P$ and $Q$. In the Enable expression ``$P>>Q$'', an \texttt{exit} statement within $P$ terminates the current process and pass the control to $Q$, carrying a value $x$ that is captured by the \texttt{accept} statement of $Q$.
...
\ERIC{The $EnableData$ pNet will use the same algebra presentation.}
%\QIN{
...
\begin{itemize}
	\item[$\bullet$] $S=\{0, 1\}$, $s_0=0$, $vars(s_0)=\{v\}$, $\texttt{init}\ \{v:=0\}$;
	\item[$\bullet$] $\to=\{0\xrightarrow{l, [v=0]}0, 0\xrightarrow{\delta,  [v=0], \{v\leftarrow1\}}1, 1\xrightarrow{r, [v=1]}1\}$.
\end{itemize}
}
...
However, while building the open automaton, that is a symbolic representation of the pNet behavior, it is not logically incorrect to keep potentialy unsatisfiable transitions, so we will treat undecidable results, and time bounds failures, as SAT. So the open automaton, may have more transitions and states that the theoretical minimum.}
