...
More transitions could be unsatisfiable after the reduction of the assignments set, so the function needs to be iterated several times. In each iteration, the number of triples decreases, thus there exists a sequence \[Aut^n = Refine(Aut^{n-1})\quad s.t. |Aut^n| < |Aut^{n-1}|.\]
...
Figure~\ref{schema:enable3bis}. Building the symmetric system \noteSB{Should this be typeset in math mode?}``(P>>Q)>>R'' in a similar
