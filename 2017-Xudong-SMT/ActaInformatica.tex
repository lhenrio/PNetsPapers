...
As mentioned above, early attempts were made by Lin and others to define symbolic bisimulations for process calculi such as value-passing CCS and the $\pi$-calculus, and design algorithms to compute the most general condition under which two processes are bisimilar. The idea of symbolic semantics was also generalised to the setting of quantum CCS \cite{FDY14}. 
...
the same open automaton as that of Algorithm 1 up to a renaming of  variables in the transitioin labels.
  \end{theorem}
...
algorithm, with a final reachability computation, computes 
the same open automaton as that of Algorithm 1 up to a renaming of  variables in the transitioin labels.
\end{theorem}
...
\medskip
\begin{theorem*}
\noindent\textbf{Theorem 1 (Finiteness) }\cite{henrio:Forte2016}
	Let $pnet=\mylangle \overline{\pNet},\overline{S}, \symb{SV}_k^{k\in K}\myrangle$ be an open pNet  with leaves $l_i^{i\in I}$ and holes $h_j^{j\in
		J}$, where the sets $I$ and $J$ are finite. Suppose the synchronisation vectors of all pNets 
	included in  $pnet$   are finite, and 
	% $\forall i \in L.\, finite{(states(l_i))} \text{ and } l_i$ 
