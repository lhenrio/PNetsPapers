...
\newcommand{\QIN}[1]{{#1}}
\newcommand{\YX}[1]{{#1}}
\newcommand{\QINX}[1]{\textcolor{orange}{#1}}
...
\institute{Xudong Qin(\email{52174501019@stu.ecnu.edu.cn}) \and
  Zechen Hou (\email{51174500089@stu.ecnu.edu.cn}) \and
...
\begin{itemize}
	...
	\item We define a strategy checking satisfiability of open transitions \QINX{during the execution of} the residual algorithm, and show the correctness of the optimised algorithm. \QINX{Meanwhile its result is equivalent to the original one.}
...
\ERIC{In Fig. \ref{schema:enable-pnets} \QINX{we give the pNet encoding of the ``Enable'' operator in two different styles,} \YX{which are  state-oriented and data-oriented, respectively.} 
...
\QINX{It shows an example that the generic $\texttt{FUN}$ constructor can be used to construct expressions (of the Action sort, or of any data sort) in any algebra presentations.
  Here \texttt{FUN} will be \QINX{instantiated} e.g. as $\delta(x)$ in the form of $\texttt{FUN}_{Action, Data}(\delta, x)$ where $x$ is a variable of type $Data$.
...
variable names. Action terms (parameterised actions) are given by the following grammar:
\[
...
incorrect, but is \QINX{clearly} not an optimal representation. So (line 11) we prune
...
\QINX{Then starting at the initial state, we compute (lines 12) the set of reachable transitions and states taking the assignments into account. We also iteratively prune (lines 13-14) the unreachable states by refining the possible set of values of states variables (more details in Section~\ref{section:BuildingTheOpenAutomaton}).} 
...
\State Initialize an empty list $l$ and a set $L$ for sub-transitions and transitions, respectively;
...
\QINX{When building the open automaton, which is a symbolic representation of the pNet behaviour, a symbolic transition can be pruned if its predicate is checked unsatisfiable. However in some cases SMT finds it undecidable, so we may still have some spurious symbolic transitions in the result. We cannot pretend to get minimality, but still it is better to prune as many unsatisfiable transitions as possible, together with some states which become unreachable. So we will treat undecidable results, and time bounds failures, as SAT.} So the open automaton may have more transitions and states than the theoretical minimum.}
...
\QIN{As the removal of the unsatisfiable transitions \YX{may entail some transitions without predecessor} transitions, }
...
starting at the initial state $InitState(p)$.
%as defined in page~\pageref{def-states}
More precisely, this is a residual algorithm building a graph from the reachable transitions in $L1$.
...
\QINX{A further reduction may now be gained by using the information from
...
result in more UNSAT transitions. Furthermore, if there are assignments of state variables inside some loops of the automaton, and the set of their possible values may influence some predicates, we will need more iterations. Because in that case more predicates of transitions will be unsatisfiable after pruning some assignments in the previous iteration.} The set of possible
...
expensive as it requires several traversals of the whole set of open transitions, and it is not incorrect to leave some UNSAT OTs in the
result. Empirically, on real life systems, the unreachable transitions constitute  a small part of the ultimate open transitions, very little will be gained to compute the fixed point. So we would only perform one step of iteration. Further algorithms 
...
%  \caption{\QINX{makeReachableSubAutomaton()}}
%  \label{alg:makeReachableSubAutomaton}
%\begin{algorithmic}[1]
%\Require \QINX{The initial state of the pNet $\textit{InitState}(P)$. A set of satisfiable transitions $L$.}
%\State Initialize a set $A$;
%\State Store $(\textit{InitState}(P), \textit{InitState}(P).Assignments)$ into $S$;  \hfill $\setminus \setminus$$S.Assignments$ is the assignment of the variable in the state $S$.
%\While{!$\mathit{isEmpty}(A)$}
%	\For {each $(S, Assignments) \in A$}
%          \For {each $OT$ from $S$}
%          \State Check the predicate of $OT$ taking $Assignments$ in to account;
%	  \IfThenElse{$\textit{false}$}{\textbf{skip};}{Add target of $OT$ and its assignments ($S'$, $S'.Assignments$) into $A$;}
%	\EndFor	\EndFor
%\EndWhile
%\State Build $\mathit{OA}$ with the set $L$;
%\State \QINX{\Return $\mathit{OA}$;}
%\end{algorithmic}  
%\end{algorithm}
%
%\begin{algorithm}[htbp]
%  \caption{\QINX{refineReachableSubAutomaton()}}
%  \label{alg:refineReachableSubAutomaton}
%\begin{algorithmic}[1]
%\Require \QINX{An open automaton $\mathit{OA}$.}
%\Repeat
%\For {\QINX{each global state $S$ in $\mathit{OA}$}}
%          \State \QINX{$b=$ whether $S$ has preceeding transitions.}
%	  \IfThenElse{$b$}{\textbf{skip};}{\QINX{remove $S$ and the transitions from $S$;}}
%\EndFor
%\Until  \QINX{$\mathit{OA}$ does not change;}
%\State \QINX{\Return $\mathit{OA}$;}
%\end{algorithmic}  
%\end{algorithm}
...
with the values of local variables, so the final automaton is $OA_2^0$ as
computed in line 13.
...
\begin{proof} %[Theorem \ref{Th:CorrectnessWithoutAssignments}]
 ...
 product of the PLTS states.
 %, as defined in page \pageref{def-states}. 
 In a similar way, the set of state variables attached to each state are
...
\end{itemize}
%
This translation ensures that \QINX{the error occurred will not be caused by the incorrect inputs.}
%no runtime error will occur in the SMT engine.
...
%
  ...
  modification enforces similar properties, while
...
are detected unsatisfiable by Z3. \QINX{According to the unsatisfiable transitions, there are 3 states which are not reachable.} The resulting open automaton, with
...
the predicates\QINX{, and then we submit them to the Z3 solver.}
%
%\hl{%
%
...
handling of assignments makes \QINX{the computation of the reachable states} more
...
show that they do impose corresponding properties. Safety of the system is obtained compositionally, whereas liveness is verified by checking \emph{pair-wise} non-interference \cite{AttieBBJS16-architectures-faoc}. In both cases, the main characteristic of the approach is compositionality, so we can compute the behavior and analyse the properties on \emph{small open systems}, and use these properties independently when composing bigger systems.
%
}
