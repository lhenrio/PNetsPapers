...
\YX{The PLTS of the controller $CData$ is $\langle S',s'_0, \to\rangle$} where:
\begin{itemize}
	\item[$\bullet$] \YX{$S'=\{0\}$, $s'_0=0$, $vars(s'_0)=\{v\}$;} % $\texttt{init}\ \{v:=0\}$;
...
\QIN{As the removal of the unsatisfiable transitions \YX{may entail some transitions without proceeding} transitions, }
...
result. \QIN{\YX{Empirically, the unreachable transitions constitute  a small part of the ultimate} open transitions, very little will be gained to compute the fixed point. So we would only perform one step of iteration.} Further algorithms 
...
\QIN{The initial transition of $EnableData$ is transformed to the initialisation of the state variables, which is $\{v\leftarrow1\}$, for its open automata.}
