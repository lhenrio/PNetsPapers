...
\begin{proof}
  ...
  similar way, the set of state variables are also the same. Comparing
  ...
  $MakeTransitions$ will always returns the same set of OTs, up to
  renaming of these fresh variables.

  Let us define the following sets of OTs, as computed by the 2
  algorithms:
  \begin{itemize}
    \item $L$ = original set of transitions by Algorithm 1, before pruning
      (i.e. before line 11),
  ...
  Now remark that without assignments,
  Theorem~\ref{Th:CorrectnessWithoutAssignments} boils to proving 
...
controlers from Figure~\ref{schema:enable-pnets}, their nature is very
...
implementation of the controlers, but only encode the logical
relations between the actions of the holes.

We have shown in \cite{henrio:Forte2016} how we can
...
\medskip
Now if we look at the Enable composition from
Figure~\ref{schema:enable-composed}, its open automaton is shown in
Figure~\ref{schema:enable3bis}. Building the symetric system ``(P>>Q)>>R'' in a similar
...
many classical process algebra or parallel languages operators, even
