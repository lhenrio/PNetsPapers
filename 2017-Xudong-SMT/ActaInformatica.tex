...
unsatisfiable transitions. \ERIC{We define and implement an optimized
algorithm that performs part of the pruning on-the-fly, and show how
...
encodings, and prove basic properties. The second is an industrial
oriented use-case, based on so-called ``BIP architectures'', that have
been used in the framework of an ESA project and to specify the
...
properties, then show how our algorithms scale up, using a
composition of two such architectures.}

\keywords{Networks of Synchronised Automata, Symbolic Behavioral
  Semantics, Compositional Analysis of Software Components, SMT} 
}
...
the pNet model, as found in previous publications, and present a
running example based on Lotos ``Enable'' operator.
...
shows the full result of the semantic computation on the running
example.
In section \ref{section:use-cases} we show the result of the study of
our running example, then explain a bigger use-case based on a BIP
architecture from the nano-satellite case-study. We show the benefits
drawn from the smart algorithm.
...
\end{figure}
	...
	Fig. \ref{schema:enable-pnets}, we use a controller $CState$ with two
	...
	composing 2 occurences of the EnableState pNet. In section
        \ref{section:use-case:Enable} we will show how these encodings can
        be use to prove properties of the Enable operator.
...
\begin{example}[Open transitions]
  ...
  trivialy true. The global states shows that controler $CState1$ is
...
\ref{schema:enable-pnets} to building a similar $EnableDataCompLeft$
...
pLTSs, one for pNet nodes. These rules are slightly improved, adding
guards in the synchronisation vectors and syntax for universal
quantifier in the guards (see~\cite{Avocs-RR}). 
  
In \cite{henrio:Forte2016} we also proved the following result, that
ensure that the termination of our semantic construction algorithm:
...
holes of each node, so \emph{MakeTransitions} is never called on a hole.
...
\QIN{The only assignments happen on state variables, at pLTS level: every time the MakeTransitions
...
\ERIC{The basic algorithm from the previous section potentialy explores some
...
Algorithm~\ref{alg3} implement this new stategy: in line 5, it runs
...
Then at the end of the residual loop, we do another traversal of the
automaton, this time using the assignment information collected that
far, and compute the final reachable sub-automaton. This step,
naturaly, is only useful if there are some assignments.


The easy case is when the system does not contain assignments to state
variables. This does not mean ``data-independancy'': we can still have
...
the open automaton need not collect the set of possible values deal
...
But in presence of assignments, we need to collect all of them
...
In this section we present the result of open automata construction
for several examples, starting with 3 constructions from
section~\ref{section:running-example} using the Enable operator.
Then we give detailed results for the Failure Controler that was
...
units (more details in Sect.~\ref{section:FailureTimer}).
...
\subsection{Composing 2 FailureTimer BIP Architectures}
\label{section:use-case:Double}

In the conception of the Nano satellite Software \cite{CubETH-case-study}, the Failure
Timer architecture was used to control several processes
...
open automaton. Remark that it shows that th etwo timers are forced to
...
In each cell of the following table, we display the number of OTs
whose predicates were found SAT/UNSAT by the SMT engine, and the overall
time spent for the whole algorithm execution.

All tests are run on HP xxx laptop under Windows 10 x64, with a Intel quad-Core i7-6600U
processor, and 32Gb of RAM. The software platform is VerCors
(https://team.inria.fr/scale/software/vercors/), running on top of
...
But as soon as there is more that on pLTS in the system and not all
states are reachable, these tests show a significant benefit of the ``smart'' strategy, in
the considered use-cases, 50\% for the FailureTimer,
...
slighly bigger example, assembling 2 Failure
