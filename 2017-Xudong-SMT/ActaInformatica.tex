...
\begin{proof of Theorem \ref{Th:CorrectnessWithoutAssignments}}
...
For each of these, define $States: \langle OT\rangle\rightarrow\langle
State\rangle$ which is the function computing the set of states in a
set of OTs. Between two transitions sets of the algorithms, define
...
According to the definition of global states in
page~\pageref{def-states}, the global state
$\ostate{\tilde{s}}=\ostate{\tilde{s_0...s_n}}$ is a composition of n
pLTS states $s_{i}^{i\in [0,n]}$. 
...
\noteInEric{I don't agree with the previous statement: the main loop
  constructs L1 (resp $L2_{sat}$), note the sets of states, so this
  argument would give you 
  
The reachability function $computeReachableSubAutomaton$ being called
with the same initial state $\ostate{\tilde{t_0}}$, this state is the
initial state of both $L1_{reach})$ and $L2_{reach})$.
...
Then the assignments set of $OA_1$ is $Assignments(L1, States(L1))$ and $OA_2$ is $Assignments(L2_{sat}, States(L2_{sat}))$. The computed triples are 
...
Let the fixed points $Refine(Aut''_1) = Aut''_1$, $Refine(Aut''_2) = Aut''_2$.

For the transitions set in $Aut''_1$ and $Aut''_2$, $L1''_{sat}=L1_{reach}$ and $L2''_{sat}=L2_{reach}$, we need to check their equivalence.
...
\end{align*}
By induction, after removing all the transitions not in the $L2''_{sat}$, we can get that $L1_{reach}=L2_{reach}$, subsequently, $States_{L1_{reach}}=States_{L2_{reach}}$,
