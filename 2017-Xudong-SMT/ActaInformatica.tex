...
\QIN{
Let us first introduce a notation in LOTOS to give an intuition how we construct the algebra presentation.
}

\begin{example}[The ``Enable'' Operator]\label{example:EnableOperator}
\QIN{
Suppose there are two processes $P$, $Q$. In the Enable expression ``$P>>Q$'', an \texttt{exit} statement within $P$ terminates the current process, carrying a value $x$ that is captured by the \texttt{accept} statement of $Q$. 
}

\QIN{
A possible encoding of such ``Enable'' operator, called $EnableState$, uses a controller $CState$ with three simple control actions $l, r, \delta$ in it to synchronise the behaviour of $P$ and $Q$. Here we use a simple action algebra, containing two constructors $\delta(x)$ and $acc(x)$, for any possible data type of the variable $x$, corresponding to the statements \texttt{exit} and \texttt{accept}. To construct the algebra presentation, we extend the triple $\APres=\langle\mathit{Sorts},\mathit{Constrs},\mathit{Ops}\rangle$ shown in Table \ref{table:BIPalgebra} with four new constructors.} 

\QIN{Using the \texttt{FUN} constructor, we express the custom constructors such as $\delta(x)$ in the form of $\texttt{FUN}_{Action, Data}(\delta, x)$ where $x$ is a variable of type $Data$. Here we have $arity(\texttt{FUN}_{Action, Data})=2$ and $\texttt{FUN}_{Action, Data}: Action, Data\to Action$. Note that the control action $\delta$ is present by the constructor $\delta$ of arity 0 here which is a different application of the constructor.
}
...
\QIN{
The PLTS of the controller $CState$ of the encoding $EnableState$ defined in Example \ref{example:EnableOperator} is given as a triple $\langle S,s_0, \to\rangle$ where:
\begin{itemize}
	\item[$\bullet$] $S=\{0, 1\}$;
	\item[$\bullet$] $s_0=0$;
	\item[$\bullet$] $\to=\{0\xrightarrow{l}0, 0\xrightarrow{\delta}1, 1\xrightarrow{r}1\}$.
\end{itemize}
Add the guard and assignment to the label of the transition, the PLTS of the controller $CData$ is given as a triple $\langle S,s_0, \to\rangle$ where:
\begin{itemize}
	\item[$\bullet$] $S=\{0, 1\}$;
	\item[$\bullet$] $s_0=0$;
...
\end{example}
\QIN{The PLTSs contained in $EnableStateCompLeft$ are similar to the PLTS $CState$.}
...
\QIN{
Consider the encoding $EnableState$ defined in Example \ref{example:EnableOperator}, the processes $P$ and $Q$ are holes of the pNet.} In this example we use \emph{synchronised actions}, to express actions that cannot be further synchronised, similar to the silent action $\tau$ of CCS. We denote them as any action expression with the text underlined, as e.g. $\underline{\delta(x_1)}$. Such synchronised actions do not play any special role for defining the semantics of pNets, but as one can expect, will be crucial for defining weak equivalences. \QIN{The pNet can be given as follow:
\begin{itemize}
	\item[$\bullet$] $\pNet=\{CState\}$;
	\item[$\bullet$] $J=\{P, Q\}$;
	\item[$\bullet$] $\symb{SV}=\{\langle a_1, \text{-}, l\rangle\rightarrow a_1[\forall y1.a_1\neq \delta(y1)], 
	\langle \delta(x_1), acc(x_1), \delta\rangle\rightarrow\underline{\delta(x_1)},\\
	\langle \text{-}, a_2, r\rangle\rightarrow a_2[\forall y2.a_2\neq acc(y2)]\}$.
\end{itemize}
Replace $CState$ with $CData$ the pNet of $EnableData$ can be given with similar $\symb{SV}$.
}
\end{example}
...
\QIN{
Consider the encoding $EnableStateCompLeft$ defined in Example \ref{example:ComposeEnableOperators}, the processes $P$, $Q$ and $R$ are holes of the pNet. Let $pNet_1$ be the sub-pNet that presents $Q>>R$. The pNet can be given as follow:
\begin{itemize}
