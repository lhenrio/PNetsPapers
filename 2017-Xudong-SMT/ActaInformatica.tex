...
that $\forall S \in L0. S \in States(L2_{reach}) \land |S|_{l2_{reach}}=n => S
\in States(L1_{reach})$, and vice-versa.
  ...
  $L2_{reach}\subseteq L2_{sat}$, $l_{n+1} \in L2_{sat}$, meaning it
  ...
  been checked ``on-the-fly'', in line 6 of algorithm~\ref{alg3}, and
  the OT $S_n \xrightarrow{l_{n}}S$ added to $L2_{sat})$, and we
  conclude $S \in States(L2_{reach})$.
  
Remember that $L0$ is finite, so we conclude $L1_{reach}=L2_{reach}$, and $States(L1_{reach})=States(L2_{reach})$.
...
\noteSB{Eric, please check whether this is ok.}\hl{with ports
