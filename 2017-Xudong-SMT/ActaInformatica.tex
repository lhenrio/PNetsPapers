...
Clearly all these $\{l_i\}_{i \in [1..n]}$ were also satisfiable and reachable in $L_2^0$ during the ``smart'' traversal, which did not use assignments. Now consider the chain of assignment sets $\{Assigns(L_2^i)\}_{i \in [0..k]}$, used during the iteration of $Refine$ by Algorithm \ref{alg3}; we argue that the specific instantiation of variables that makes $Pred(l,L_1^*)$ satisfiable was contained in $Assigns(L_2^0)$, and that if it were removed during one of the $Refine$ step in Algorithm \ref{alg1}, then it would be removed similarly in Algorithm \ref{alg3}. So we get a contradiction.
